...
import { executeE04MultiRealm } from './e04-multi-realm.evidence.mjs';
import { finalizeE04Evidence } from './e04-multi-realm.formal.mjs';

const identifier = randomUUID().replaceAll('-', '').slice(0, 16);
const runToken = identifier.slice(0, 8);
const container = `zhudatuan-e04-${identifier}`;
const databaseName = 'zhudatuan_registration';
const password = `MultiRealmEvidence${identifier}A`;
const criteriaPath = join(repositoryRoot, '05_docs_ziliao', 'docs_wendang', 'architecture', 'evidence', 'e2.0',
  'E2.0-E04-正式重测判据-2026-09-14.json');
...
if (Date.parse(criteria.locked_at) >= Date.parse(startedAt)) throw new Error('E04_CRITERIA_NOT_LOCKED_BEFORE_EXECUTION');
await verifyCriteriaSources(criteria);
if (formalExecution && sourceControl.tree_state !== 'CLEAN') throw new Error('E04_EVIDENCE_REQUIRES_CLEAN_TREE');

const evidenceDirectory = formalExecution
  ? resolve(repositoryRoot, evidenceArgument)
  : await mkdtemp(join(tmpdir(), 'zhudatuan-e04-evidence-'));
if (formalExecution) {
...
try {
  ...
  try {
    ...
    try {
      executionSummary = await executeE04MultiRealm(database, criteria, runToken, evidenceDirectory);
    ...
    await run('node', ['04_tools/scripts/audit/e04-multi-realm.oracle.mjs', '--run-directory', evidenceDirectory,
      '--criteria', criteriaPath, '--output', join(evidenceDirectory, 'multi-realm-independent-recount.json')]);
  } finally {
  ...
  if (formalExecution) {
    const result = await finalizeE04Evidence({
      evidenceDirectory, criteriaPath, environment, sourceControl, startedAt, completedAt, container, executionSummary,
    });
    console.log(`SFL E04 PostgreSQL 17 formal evidence completed: ${repositoryPath(evidenceDirectory)} outcome=${result.oracle.claim_outcome}`);
  } else {
    const oracle = await readJson(join(evidenceDirectory, 'multi-realm-independent-recount.json'));
    if (oracle.claim_outcome !== 'MET') {
      throw new Error(`E04_ACCEPTANCE_${oracle.claim_outcome.replaceAll(' ', '_')}:${JSON.stringify({
        missing: oracle.missing_items,
        contradictions: oracle.contradictions,
        thresholds: oracle.thresholds?.filter((entry) => !entry.met),
      })}`);
    }
    console.log(`SFL E04 PostgreSQL 17 acceptance passed: realms=${executionSummary.realm_count} transitions=${executionSummary.transition_count} negatives=${executionSummary.negative_observation_count}`);
  }
} finally {
...
async function environmentEvidence(input) {
  ...
  return Object.freeze({
    schema_version: 'e04-environment-v1',
    captured_at: input.capturedAt,
    kind: 'DEV',
    environment_id: `local-disposable-postgresql17-e04-${identifier}`,
    description: 'A newly created PostgreSQL 17 Alpine container replayed the complete production migration chain and executed the locked three-Realm credential, asset, transition and 48-negative E04 matrix.',
    owner: 'Ethan-controlled local Codex workspace',
    isolation: `Disposable Docker container ${container}; synthetic E04 identities, Sessions and orders only; every negative write was savepoint-rolled back and the container was removed after execution.`,
    real_customer_data: false,
    production_impact: 'None. No production service, database, domain, payment provider, deployment channel or customer record was contacted.',
    differences_from_production: [
      'Local disposable Docker PostgreSQL rather than managed Alibaba Cloud PostgreSQL.',
      'Synthetic Realm, identity, Session and order fixtures rather than customer traffic.',
      'Credential lookup and node-bound authorization executed production source; Session rows and the final order read/write adapter were driven directly by the evidence workload rather than public HTTP ingress.',
    ...
    configuration_digest: `sha256:${sha256(Buffer.from([
      ...
      JSON.stringify(criteria.realm_matrix),
      JSON.stringify(criteria.negative_matrix),
    ].join('\n')))}`,
  });
}
...
async function verifyCriteriaSources(criteriaValue) {
  for (const source of [...criteriaValue.authoritative_basis, ...criteriaValue.implementation_basis,
    ...
    if (actual !== source.sha256) throw new Error(`E04_BASIS_DIGEST_MISMATCH:${source.path}:${actual}`);
  }
}
...
async function waitForPostgres() {
  ...
  throw new Error('E04_POSTGRES_NOT_READY');
}
...
async function postgresPort() {
  ...
  if (!port || !/^\d+$/.test(port)) throw new Error(`E04_POSTGRES_PORT_INVALID:${output.trim()}`);
  return port;
}
...
function option(name) {
  ...
  if (!value || value.startsWith('--')) throw new Error(`E04_OPTION_VALUE_REQUIRED:${name}`);
  return value;
}
...
function assertRepositoryEvidencePath(path) {
  ...
  if (local.startsWith('..') || local === evidenceRoot || !local.startsWith(`${evidenceRoot}/`)) {
    throw new Error(`E04_EVIDENCE_DIRECTORY_INVALID:${local}`);
  }
}

async function createEvidenceDirectory(path) {
  const exists = await stat(path).then(() => true, () => false);
  if (exists) throw new Error(`E04_EVIDENCE_DIRECTORY_EXISTS:${repositoryPath(path)}`);
