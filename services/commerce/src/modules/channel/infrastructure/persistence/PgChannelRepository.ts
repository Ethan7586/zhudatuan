...
export class PgChannelRepository implements ChannelRepository {
  ...
  async enqueue(kind: 'catalogsync' | 'pricesync' | 'inventorysync' | 'statementsync' | 'fulfillment' | 'paymentrefund', scope: string, payload: Readonly<Record<string, unknown>>, priority: number, stableId?: string): Promise<void> {
