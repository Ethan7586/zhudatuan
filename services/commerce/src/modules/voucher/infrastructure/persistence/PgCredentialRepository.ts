...
export class PgCredentialRepository implements CredentialRepository {
  ...
  async generate(call: Parameters<CredentialRepository['generate']>[0]) {
    const database = this.transactions.database(call.context.transaction); const pool = path(call, 'poolid'); const count = Number(body(call).count); const idempotency = requiredIdempotency(call);
    ...
    const selected = await database.query<{ mode: string; state: string; generated: number; capacity: number }>(`select mode,state,generated::integer,capacity::integer from voucher.credentialpool where id=$1 and scope_id=$2 for update`, [pool, call.scope]);
    const row = selected.rows[0]; if (!row) throw new DomainError('RESOURCE_NOT_FOUND');
  ...
  async import(call: Parameters<CredentialRepository['import']>[0]) {
    const database = this.transactions.database(call.context.transaction); const pool = path(call, 'poolid'); const value = body(call);
    ...
    const imported = await this.imports.create(write(call), { id: `import:${randomUUID()}`, scope: call.scope, owner: 'voucher', kind: 'credential',
      reference: object.reference, sha256: object.sha256, name: object.name, mediaType: object.mediaType,
      size: object.size, actor: call.actor, authorization: call.authorization, metadata: { pool } });
  ...
  async list(call: Parameters<CredentialRepository['list']>[0]) {
    ...
    const rows = await database.query(`${CREDENTIAL} where credential.scope_id=$1 and ($2::text is null or credential.pool_id=$2)
  ...
  async export(call: Parameters<CredentialRepository['export']>[0]) {
    ...
    const source = await this.transactions.database(call.context.transaction).query<{ version: number; state: string }>(
      `select version::integer,state from voucher.credentialpool where id=$1 and scope_id=$2 for update`, [pool, call.scope]
    );
    ...
    const result = await createVoucherExport(call, { kind: 'credential', snapshot: { pool, watermark }, poolVersion: source.rows[0].version }, {
      database: this.transactions.database(call.context.transaction), exports: this.exports, jobs: this.jobs });
