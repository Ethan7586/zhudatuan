...
export class PgVoucherExport implements VoucherExport {
  constructor(private readonly exports: ExportPort, private readonly jobs: JobPort, private readonly transactions = new PgTransactionAccess()) {}
  async search(call: Parameters<VoucherExport['search']>[0]) {
    const value = body(call); const snapshot = text(value.snapshot, 'snapshot');
    ...
    const result = await createVoucherExport(call, { kind: 'search', snapshot: projection }, {
      database: this.transactions.database(call.context.transaction), exports: this.exports, jobs: this.jobs });
    return { status: 202, body: result };
  }
}

export async function createVoucherExport(call: VoucherCall<OperationId>, input: Readonly<{
  kind: 'credential' | 'issueorder' | 'action' | 'search'; snapshot: Readonly<Record<string, unknown>>; poolVersion?: number;
}>, ports: Readonly<{ database: SqlExecutor; exports: ExportPort; jobs: JobPort }>) {
  const idempotency = requiredIdempotency(call);
  const result = await ports.exports.create(write(call), { scope: call.scope, owner: 'voucher', kind: input.kind, snapshot: input.snapshot,
  ...
  await ports.jobs.create(write(call), { scope: call.scope, owner: 'voucher', kind: 'voucherexport', queue: 'export', payload: { export: result.id },
    idempotency: `${result.id}:${idempotency}`, actor: call.actor });
  return result;
}
