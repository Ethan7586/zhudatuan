...
describe('capability inheritance', () => {
  it('publishes Approval, Voucher, Import and all six surface capabilities from the canonical catalog', () => {
    expect(PublishableCapabilities).toEqual([
      'surface.auth',
      'surface.console',
      'surface.storefront',
      'surface.miniapp',
  ...
  it('rejects enabling a child capability when its parent is not enabled', () => {
    ...
    expect(() => policy.assertChange(
      { state: 'enabled', quota: 10, expiresAt: null },
      { parent: { scope: 'enterprise:one', state: 'disabled', quota: 100 }, dependencies: [] },
      now
    )).toThrow('CAPABILITY_PARENT_REQUIRED');
  });
  ...
  it('rejects enabling when any declared dependency is unhealthy', () => {
    expect(() => new CapabilityPolicy().assertChange(
      { state: 'enabled', quota: null, expiresAt: null },
      { parent: null, dependencies: [{ capability: 'surface.console', healthy: false }] },
      now
    )).toThrow('CAPABILITY_DEPENDENCY_UNHEALTHY');
  });
});
