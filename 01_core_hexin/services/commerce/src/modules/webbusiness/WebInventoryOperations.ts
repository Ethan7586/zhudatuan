...
import { WEB_INVENTORY_OPERATION_IDS } from './WebBusinessOperationIds';

export function webInventoryOperations(context: ModuleContext): ModuleOperations {
  ...
  return new ModuleOperations('inventory', pool, context.container.get(AUDIT_SINK), {
    'inventory.availability.read': async (request, database) => {
      ...
      const result = await database.query(`select stock.id,stock.sku_id,stock.location_id,stock.onhand,stock.safety,
        ...
        group by stock.id order by stock.updated_at desc,stock.id desc limit $5`,
      [access.scope.id, skus.length === 0 ? null : skus, page.sort, page.id, page.fetch]);
