...
import { NAVIGATION_CATALOG } from '../../infrastructure/catalog/NavigationCatalog';
import { NavigationProjector } from './NavigationProjector';
...
describe('NavigationProjector', () => {
  it('reads each bounded context once and produces a filtered immutable tree', async () => {
    ...
    const projection = await projector.project({} as DatabasePool, accessContext(), 'enterprise:1');
  ...
  it('rejects a stale access version before returning any nodes', async () => {
    ...
    await expect(projector.project({} as DatabasePool, accessContext(), 'enterprise:1')).rejects.toThrow('ACCESS_VERSION_STALE');
  });
});
