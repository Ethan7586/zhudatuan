  ...
  return (
    <section className="financeworkspace governanceworkspace">
      ...
      <nav className="governanceswitch" aria-label="财务治理分类">
        <button type="button" aria-label="政策规则" aria-current={model.view === 'policies' ? 'page' : undefined} onClick={() => model.actions.view('policies')}>
          <strong>政策规则</strong>
          <span>定义何时、如何生成平衡分录</span>
        </button>
        <button type="button" aria-label="对账修复" aria-current={model.view === 'repairs' ? 'page' : undefined} onClick={() => model.actions.view('repairs')}>
          <strong>对账修复</strong>
          <span>按经办、复核分离处理账单差异</span>
        </button>
      </nav>
      {model.receipt ? (
        <div className="governancereceipt">
          <ActionReceipt
            state={{ kind: 'success', receipt: model.receipt, objectLabel: model.view === 'policies' ? '财务政策' : '对账修复', impact: '结果已从权威服务回读；历史账本与审批证据保持可追溯。' }}
      ...
      <ResourcePanel
        ...
        actions={
          <div className="governanceactions">
            {model.view === 'policies' && model.can.policyManage ? (
              <Button tone="primary" onPress={model.actions.createPolicy}>
                新建政策
              </Button>
            ) : null}
            {model.view === 'repairs' && model.can.repairSubmit ? (
              <Button tone="primary" onPress={model.actions.createRepair}>
                创建修复建议
              </Button>
            ) : null}
            <Button onPress={model.actions.refresh} isDisabled={model.fetching}>
              {model.fetching ? '刷新中…' : '刷新'}
            </Button>
          </div>
        }
        notice={
          <section className="governancenotice">
            <strong>复式记账与经办复核分离</strong>
            <p>借贷必须平衡、币种不可混算、关账期间不可直接写入；人工修复必须由另一名复核人审批。</p>
          </section>
        }
      >
        {model.view === 'policies' ? <PolicyTable model={model} /> : <RepairTable model={model} />}
        <footer className="governancepagination">
          <span>服务端共 {page?.count ?? 0} 条</span>
          <div>
  ...
  const columns: readonly DataColumn<FinancePolicy>[] = [
    {
      ...
      label: '政策',
      render: (row) => (
        <div className="governanceprimary">
    ...
    ...(model.can.policyManage
      ? [
          {
            key: 'actions',
            label: '操作',
            render: (row: FinancePolicy) => (
              <div className="governanceactions">
                <Button onPress={() => model.actions.editPolicy(row)}>修订</Button>
                {row.status !== 'retired' ? (
                  <Button tone="danger" onPress={() => model.actions.retirePolicy(row)}>
                    停用
                  </Button>
                ) : null}
              </div>
            ),
          } satisfies DataColumn<FinancePolicy>,
        ]
      : []),
  ];
  ...
  const columns: readonly DataColumn<FinanceRepair>[] = [
    {
      ...
      render: (row) => (
        <div className="governanceprimary">
          <strong>{chineseReference('账单', row.statementId)}</strong>
    ...
    },
    {
      ...
      render: (row) =>
        row.differences.length === 0
          ? '未返回明细'
          : `${row.differences.length} 项 · ${formatMinor(
              row.differences.reduce((total, item) => total + item.deltaMinor, 0),
              row.differences[0]?.currency ?? 'CNY'
            )}`,
    },
    { key: 'state', label: '状态', render: (row) => <Status tone={row.status === 'approved' ? 'success' : row.status === 'rejected' || row.status === 'reversed' ? 'danger' : 'warning'}>{chineseDomainLabel(row.status)}</Status> },
    {
      ...
      render: (row) => (
        <div className="governanceprimary">
          <span>{chineseReference('经办人', row.makerId)}</span>
          <span>{row.checkerId ? chineseReference('复核人', row.checkerId) : '等待独立复核'}</span>
        </div>
      ),
    },
    ...
    { key: 'updated', label: '更新时间', render: (row) => formatDate(row.updatedAt) },
    {
      ...
      render: (row) => (
        <div className="governanceactions">
          <Button onPress={() => model.actions.reviewRepair(row)}>{row.status === 'submitted' ? '复核详情' : '查看详情'}</Button>
          {row.status === 'approved' && model.can.repairReverse ? (
            <Button tone="danger" onPress={() => model.actions.reverseRepair(row)}>
              追加回滚
            </Button>
          ) : null}
        </div>
