  ...
  return (
    <Dialog open={invitation !== undefined} title="撤销邀请" {...(invitation === undefined ? {} : { eyebrow: invitation.id })} onClose={onClose} dismissable={!busy}>
      <Form label="撤销邀请" className="invitationform" onSubmit={(event) => void submit(event)}>
        ...
        <footer className="invitationactions"><Button onPress={onClose} isDisabled={busy}>取消</Button><Button type="submit" tone="danger" isDisabled={busy || reason.trim().length < 4}>{busy ? '正在撤销…' : '确认撤销'}</Button></footer>
      </Form>
    </Dialog>
  );
}
