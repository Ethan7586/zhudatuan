...
describe('FulfillmentEventJob', () => {
  it.each([
    ...
    ['verification.completed', 'verification:one', { verification: 'verification:one', subjectType: 'fulfillment', subject: 'fulfillment:one' }, { eventType: 'verification.completed', resourceId: 'fulfillment:one', subjectType: 'fulfillment' }],
...
function claimed(event: string, aggregateId: string, payload: Readonly<Record<string, unknown>>): ClaimedJob {
  return { id: 'job:one', kind: 'fulfillmentevent', scope: 'mall:one', attempts: 1, token: 1,
