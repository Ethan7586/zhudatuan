...
function previewCopy(mode: CommerceWorkspaceMode): PreviewCopy {
  if (mode === 'management') return {
    title: '创建商城 · 原子执行流程',
    notice: '商城创建发动机已经接通。提交后由平台控制面一次建立完整的商城起点，并在成功后刷新商城清单。',
    boundaryTitle: '一笔事务，失败整体回滚',
    boundary: '商城身份、组织关系、初始商品池、商城应用和开店草稿属于同一次提交；任何一步失败都不会留下半个商城。',
    steps: [
      ['填写信息', '所属集团、商城名称、商城代码和访问标识'],
      ['冲突预检', '确认集团有效，代码和访问标识没有被占用'],
      ['商城身份', '生成独立 mall_id 并建立组织归属'],
      ['初始商品池', '建立本商城独立的默认商品池'],
      ['开店草稿', '建立商城应用、首个版本和绑定关系'],
      ['清单回显', '创建成功后刷新商城清单，继续店铺装修'],
    ],
  };
