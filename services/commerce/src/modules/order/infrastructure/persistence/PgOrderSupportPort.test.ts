...
describe('PgOrderSupportPort', () => {
  it('exposes a minimal summary and creates an idempotent Order-owned collaboration action', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.includes('insert into ordering.supportcollaboration'))
        return result([{ id: 'supportcollaboration:case:one', order_id: 'order:one', aftersale_id: 'aftersale:one', support_case_id: 'case:one', kind: 'caseopened', created_at: '2026-09-06T00:00:00.000Z' }]);
      return result([]);
    });
    const action = await withWriteTransaction(query, (context) =>
      new PgOrderSupportPort().collaborate(context, { id: 'supportcollaboration:case:one', order: 'order:one', supportCase: 'case:one', scopes: ['mall:one'], member: 'member:one', memberOnly: true, actor: 'actor:one', trace: 'trace:one' })
    );
  ...
  it('does not disclose or mutate an order outside the supplied member and scope boundary', async () => {
    ...
    await expect(
      withWriteTransaction(query, (context) =>
        new PgOrderSupportPort().collaborate(context, {
          id: 'supportcollaboration:case:one',
          order: 'order:foreign',
          supportCase: 'case:one',
          scopes: ['mall:one'],
          member: 'member:one',
          memberOnly: true,
          actor: 'actor:one',
