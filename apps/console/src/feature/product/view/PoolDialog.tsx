      ...
      >
        ...
        <div className="productflowbody">
          <section className="productpoolsummary" aria-label="当前商品池">
            {viewmodel.loading ? <p>正在读取商品池…</p> : viewmodel.pools.map((pool) => <PoolCard key={pool.id} pool={pool} selected={viewmodel.selected?.id === pool.id} onSelect={viewmodel.select} />)}
          </section>
          <label>
            操作
        ...
        <footer>
          ...
          <button className="productactionprimary" type="submit" disabled={viewmodel.selected === undefined || viewmodel.submitting}>
