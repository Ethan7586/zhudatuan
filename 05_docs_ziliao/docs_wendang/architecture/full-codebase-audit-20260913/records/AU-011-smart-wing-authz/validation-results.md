...
| 本包直接workspace test | Missing script: test | 本包没有自有test script | 根聚合测试执行结果 |
| 根test静态拓扑 | Storefront `vitest.config.ts`明确include本包`src/**/*.test.ts` | 根`test:unit`可间接发现13用例 | 依赖未安装，本次未取得真实通过/失败结果 |
| 本包直接workspace typecheck | Missing script: typecheck | 本包没有自有typecheck script；根typecheck也不执行其独立tsconfig | 源码类型通过/失败 |
