...
export async function importStock(database: SqlExecutor, catalog: CatalogSku, scope: string, importid: string, rowNumber: number, row: Readonly<Record<string, string>>): Promise<void> {
...
async function movement(database: SqlExecutor, stock: string, delta: number, importid: string, row: number): Promise<boolean> {
