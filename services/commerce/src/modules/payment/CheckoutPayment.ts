...
import { PaymentPort, type PaymentTenderPlan } from './PaymentPort';
...
export class CheckoutPayment implements CheckoutPaymentPort {
  constructor(
    ...
    private readonly intents: PreparePayment
  ) {}

  prepare(database: OperationDatabase, input: Readonly<PrepareInput>): Promise<PreparedPayment> {
    return this.payments.prepare(database, input);
  }
  ...
  async continue(request: OperationRequest, input: Readonly<{ payment: PreparedPayment; order: string; scene: WechatScene }>): Promise<CheckoutPaymentResult> {
