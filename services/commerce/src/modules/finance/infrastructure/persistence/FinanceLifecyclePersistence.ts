...
export function financeLifecyclePersistence(
  organization: OrganizationReadPort
): Pick<FinancePersistence, 'reconciliationsRead' | 'settlementsRead' | 'withdrawalsRead' | 'holdsRead' | 'periodsRead' | 'periodsManage' | 'backfillsRead' | 'backfillsDecide'> {
  return {
    reconciliationsRead: async (request, database) => {
      ...
      const result = await database.query(
        ...
        and ($2::text is null or reconciliation.id>$2) order by reconciliation.id limit $3`,
        [scopes, page.id, page.fetch]
      );
    ...
    settlementsRead: async (request, database) => {
      ...
      const result = await database.query(
        ...
        settlement.gross_minor,settlement.fee_minor,settlement.invoice_basis,
    ...
    withdrawalsRead: async (request, database) => {
      ...
      const result = await database.query(
        ...
        withdrawal.reason,withdrawal.evidence,withdrawal.provider_reference,withdrawal.created_at,withdrawal.updated_at,
    ...
    periodsManage: async (request, database) => {
      ...
      if (action === 'request') {
        const result = await database.query(
          `with current as(select encode(public.digest(coalesce(string_agg(journal.id||':'||entry.id||':'||entry.amount_minor,
          ...
          created as(insert into finance.periodclose(id,scope_id,period,state,source_hash,requested_by,reason,evidence,requested_at,version)
            ...
            on conflict(scope_id,period) do update set state='pending',source_hash=excluded.source_hash,requested_by=excluded.requested_by,
              ...
              where finance.periodclose.state='rejected' returning *)
      ...
      const result = await database.query(
        ...
        [access.scope.id, period, decision, access.actor.id, textField(body, 'reason', 1000), JSON.stringify({ decisionEvidence: record(body.evidence), trace: access.trace })]
      );
      ...
      if (decision === 'approved') {
        ...
        const statement = await database.query<{ id: string; period_start: string; period_end: string; currency: string; opening_minor: number; debit_minor: number; credit_minor: number; closing_minor: number; state: string }>(
          `update finance.statement set state='final',
        ...
        await event(database, 'finance.period.closed', 'periodclose', close.id, access.scope.id, {
          period,
          close: close.id,
          sourceHash: (close as { source_hash?: string }).source_hash,
          statementSnapshot: {
            statement: snapshot.id,
            periodStart: snapshot.period_start,
            periodEnd: snapshot.period_end,
            currency: snapshot.currency,
            openingMinor: snapshot.opening_minor,
            debitMinor: snapshot.debit_minor,
            creditMinor: snapshot.credit_minor,
            closingMinor: snapshot.closing_minor,
    ...
    backfillsDecide: async (request, database) => {
      ...
      const result = await database.query(
        ...
        [request.input.path.backfillid!, decision, access.actor.id, JSON.stringify({ reason: textField(body, 'reason', 1000), evidence: record(body.evidence) })]
      );
...
async function event(database: SqlExecutor, type: string, aggregateType: string, aggregate: string, scope: string, payload: unknown) {
  const id = `event:${randomUUID()}`;
  await new PgRuntimeWriter(database).append({ id, type, aggregateType, aggregate, scope, payload: record(payload), trace: id });
}
