...
import { JOB_CATALOG, PROVIDER_JOB_IDS } from '../pipeline/JobCatalog';
...
export function defineModuleManifest(input: ModuleManifestInput): ModuleManifest {
  ...
  const api = workloadDefinition(input.id, 'api', { dependencies, bindings: input.bindings ?? dependencies, services, ...(input.ports === undefined ? {} : { ports: input.ports }) });
...
function ownedOperations(id: string): readonly string[] {
  return Object.freeze(
    COMMERCE_OPERATIONS.filter(({ module }) => module === id)
      .map(({ id: operation }) => operation)
      .sort()
  );
}
...
function ownedEvents(id: string): readonly string[] {
  return Object.freeze(
    COMMERCE_EVENTS.filter(({ module }) => module === id)
      .map(({ type }) => type)
      .sort()
  );
}
...
function ownedJobs(id: string, workload: ModuleWorkload): readonly string[] {
  ...
  return Object.freeze(
    JOB_CATALOG.filter(({ owner, id: job }) => owner === id && provider.has(job) === (workload === 'provider'))
      .map(({ id: job }) => job)
      .sort()
  );
}
