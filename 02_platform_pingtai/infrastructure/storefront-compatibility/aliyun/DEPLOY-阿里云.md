...
cd /opt/smart-wing
cp 01_core_hexin/services/commerce-api/.env.example .env.production
# 编辑 .env.production，填写仅服务器持有的密钥
npm ci --include=dev
npm run build
cp 02_platform_pingtai/infrastructure/aliyun/Caddyfile /etc/caddy/Caddyfile
caddy validate --config /etc/caddy/Caddyfile
systemctl reload caddy
pm2 start 02_platform_pingtai/infrastructure/aliyun/ecosystem.config.cjs
pm2 save
```
...
```bash
cd /opt/smart-wing
bash 02_platform_pingtai/infrastructure/aliyun/deploy.sh
```
...
2. 创建只允许该 Bucket `02_platform_pingtai/database/postgresql/*` 读写的 RAM 身份，不使用账号主 AccessKey。
...
```bash
cd /opt/smart-wing
cp 02_platform_pingtai/infrastructure/aliyun/backup.env.example /opt/smart-wing/.env.backup
...
cp 02_platform_pingtai/infrastructure/aliyun/smart-wing-postgres-backup.service /etc/systemd/system/
cp 02_platform_pingtai/infrastructure/aliyun/smart-wing-postgres-backup.timer /etc/systemd/system/
...
`.env.production` 不进 Git，也不复制到 `01_core_hexin/apps/`。Cookie 必须保持 host-only，不设置 `.hbbtzn.com` 的共享 Domain。生产必须分别配置 `SESSION_SIGNING_KEY` 与 `ADMIN_SESSION_SIGNING_KEY`（两者不得相同），否则后台域不会签发或接受会话。
...
- 修改环境变量后运行 `pm2 startOrReload 02_platform_pingtai/infrastructure/aliyun/ecosystem.config.cjs --update-env` 并 `pm2 save`。
