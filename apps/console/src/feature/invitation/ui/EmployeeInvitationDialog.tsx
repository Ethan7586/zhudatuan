  ...
  return (
    <Dialog open={open} title="邀请员工注册" eyebrow="普通员工 · 员工商城" onClose={onClose} dismissable={!busy}>
      <Form label="员工邀请" className="invitationform" onSubmit={(event) => void submit(event)} validationErrors={errors}>
        ...
        <select id="employeeOrganization" value={draft.organizationId} onChange={(event) => update('organizationId', event.target.value)} disabled={busy} required aria-describedby={errors.organizationId ? 'employeeOrganizationError' : undefined}>
          <option value="">请选择员工可以进入的商城</option>
          {targets.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}
        </select>
        {errors.organizationId ? <span id="employeeOrganizationError" className="invitationfielderror">{errors.organizationId}</span> : null}
        <label htmlFor="employeeName">姓名</label>
        <input ref={first} id="employeeName" value={draft.displayName} onChange={(event) => update('displayName', event.target.value)} autoComplete="name" maxLength={80} disabled={busy} aria-describedby={errors.displayName ? 'employeeNameError' : undefined} />
        {errors.displayName ? <span id="employeeNameError" className="invitationfielderror">{errors.displayName}</span> : null}
        <label htmlFor="employeeMobile">手机号</label>
        <input id="employeeMobile" value={draft.mobile} onChange={(event) => update('mobile', event.target.value)} inputMode="tel" autoComplete="tel" maxLength={32} disabled={busy} aria-describedby={errors.mobile ? 'employeeMobileError' : undefined} />
        {errors.mobile ? <span id="employeeMobileError" className="invitationfielderror">{errors.mobile}</span> : null}
        ...
        {errors.reason ? <span id="employeeReasonError" className="invitationfielderror">{errors.reason}</span> : null}
        ...
        <footer className="invitationactions">
          ...
          <Button type="submit" tone="primary" isDisabled={busy}>{busy ? '正在创建…' : '创建员工邀请'}</Button>
        </footer>
      </Form>
    </Dialog>
  );
