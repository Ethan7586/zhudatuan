  ...
  it('sends proof, expected version, csrf and stable identity through the write boundary', async () => {
    ...
    server.use(
      http.put('https://shop.test/api/v1/notifications/templates/template%3Aone', async ({ request }) => {
        ...
        expect(await request.json()).toEqual({ channel: 'sms', eventType: 'identity.challenge', version: 1, variables: { code: 'string' }, providerTemplate: 'SMS_100', subject: null, body: '验证码 {{code}}', status: 'active' });
