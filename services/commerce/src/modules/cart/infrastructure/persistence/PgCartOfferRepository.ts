...
export class PgCartOfferRepository implements CartOfferRepository {
  ...
  async resolve(context: ReadTransactionContext, mall: string, references: readonly CartOfferReference[]): Promise<ReadonlyMap<string, CartOffer>> {
    ...
    const listings = await this.catalog.inspect(
      context,
      unique.map(({ listing }) => listing),
      mall
    );
    const skus = [
      ...new Set(
        unique.flatMap((reference) => {
          const current = listings.get(reference.listing)?.sku;
          return current ? [current] : reference.sku ? [reference.sku] : [];
        })
      ),
    ];
    const [prices, stocks] = await allParallel([() => this.pricing.currentMany(context, mall, skus), () => this.inventory.availability(context, mall, skus)] as const, { concurrency: 2, expiresAt: context.deadline, signal: context.signal });
