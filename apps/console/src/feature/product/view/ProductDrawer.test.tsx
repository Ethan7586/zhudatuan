...
describe('ProductDrawer', () => {
  it('keeps quick inspection in the drawer and offers a direct full-detail action', async () => {
    ...
    render(<ProductDrawer listing={listing} tab="overview" detail={detail} sections={readySections()} onTab={() => undefined} onClose={() => undefined} onDetail={openDetail} onAction={() => undefined} onPool={() => undefined} canUse={allow} />);

    expect(screen.getByRole('heading', { name: '商品概览' })).toBeTruthy();
    await userEvent.setup().click(screen.getByRole('button', { name: '打开完整详情' }));
  ...
  it('isolates a pricing failure to its panel and retries only the failed section', async () => {
    ...
    view.rerender(<ProductDrawer listing={listing} tab="malls" detail={detail} sections={sections} onTab={() => undefined} onClose={() => undefined} onDetail={() => undefined} onAction={() => undefined} onPool={() => undefined} canUse={allow} />);
  ...
  it('offers contextual pool management only after a published listing is taken down', async () => {
    ...
    const view = render(<ProductDrawer listing={listing} tab="source" detail={detail} sections={readySections()} onTab={() => undefined} onClose={() => undefined} onDetail={() => undefined} onAction={() => undefined} onPool={openPool} canUse={allow} />);
    expect(screen.getByRole('button', { name: '管理商品投池' }).hasAttribute('disabled')).toBe(true);
    view.rerender(<ProductDrawer listing={unpublished} tab="source" detail={detail} sections={readySections()} onTab={() => undefined} onClose={() => undefined} onDetail={() => undefined} onAction={() => undefined} onPool={openPool} canUse={allow} />);
  ...
  it('keeps read actions available while every denied write clearly explains why it is disabled', () => {
    ...
    expect(screen.getByRole('button', { name: '打开完整详情' }).getAttribute('title')).toBe('当前账号没有查看完整商品详情的权限。');
...
const detail = {
  ...
  listings: [{ id: 'listing:one', scope: 'mall:one', pool: null, sku: 'sku:one', title: '办公福利礼盒', status: 'published', version: 3 }],
  prices: [{ sku: 'sku:one', scope: 'mall:one', currency: 'CNY', amountMinor: 9900, bookStatus: 'active', bookVersion: 'offer:four', priceVersion: 4 }],
  inventory: [{ sku: 'sku:one', scope: 'mall:one', location: 'warehouse:one', onhand: 14, safety: 2 }],
