      ...
      if (completed) {
        if (await orderPort.completeFulfillment(client, loaded.order_id)) {
          await client.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,available_at)
            values($1,'order.received',1,'order',$2,$3,jsonb_build_object('mall',$3,'order',$2,'member',$4),$5,clock_timestamp(),clock_timestamp())
            on conflict(id) do nothing`, [`event:order:received:${digest(loaded.order_id)}`, loaded.order_id, loaded.mall_id, loaded.member_id, job.id]);
        }
