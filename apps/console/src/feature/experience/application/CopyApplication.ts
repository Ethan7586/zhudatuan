...
export class CopyApplication {
  ...
  execute(context: ConsoleContext, application: string, draft: ApplicationDraft, identity: string, signal?: AbortSignal) {
    assertOperationAccess(context, experienceOperations.copy);
    assertApplicationDraft(draft);
    return this.port.copy(context, application, draft, '控制台复制商城应用', identity, signal);
  }
}
