...
const container = `zhudatuan-identity-concurrency-${identifier}`;
const database = 'zhudatuan_identity_concurrency';
const password = `IdentityConcurrency${identifier}A`;
...
try {
  ...
  await run('docker', [
    ...
    '-p', '127.0.0.1::5432',
    'postgres:17-alpine', '-c', 'max_connections=220',
  ], { quiet: true });
  ...
  if (!port || !/^\d+$/.test(port)) throw new Error(`IDENTITY_CONCURRENCY_POSTGRES_PORT_INVALID:${portOutput.trim()}`);

  const adminUrl = `postgresql://postgres:${password}@127.0.0.1:${port}/${database}`;
  const runtimeUrl = `postgresql://zhudatuanidentityapi:${password}@127.0.0.1:${port}/${database}`;
  const appUrl = `postgresql://shopapp:${password}@127.0.0.1:${port}/${database}`;
  ...
  await run('docker', ['exec', container, 'psql', '-X', '-v', 'ON_ERROR_STOP=1', '-U', 'postgres', '-d', database,
    '-c', `alter role zhudatuanidentityapi login password '${password}'; alter role shopapp login password '${password}';`], { quiet: true });
  await run('npm', ['--workspace', '@shop/commerce', 'run', 'test:integration', '--', 'IdentityConcurrency.test.ts'], {
    environment: { SHOP_TEST_ADMIN_DATABASE_URL: adminUrl, SHOP_TEST_DATABASE_URL: runtimeUrl,
      SHOP_TEST_APP_DATABASE_URL: appUrl },
  });
  console.log('SFL identity PostgreSQL 17 concurrency acceptance passed: users=100 realms=L0+L1 races=4');
} finally {
...
async function waitForPostgres() {
  ...
  throw new Error('IDENTITY_CONCURRENCY_POSTGRES_NOT_READY');
}
