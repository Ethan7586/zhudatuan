...
describe('MemberPort identity boundary', () => {
  ...
  it('persists encrypted and masked mobile profile fields instead of synthesizing only a response', async () => {
    ...
    await port.createPending(database, { member: 'member:one', principal: 'principal:one', display: 'One', mobileCiphertext: 'ciphertext', mobileFingerprint: 'f'.repeat(64), mobileMasked: '+86****8000' });
