      ...
      {!ticket || !context ? (
        ...
        <div className="supportcontextbody">
          ...
          <ContextSection title="处理与服务时限">
            ...
            <Row label="当前客服" value={ticket.assigned_agent_id ? chineseReference('客服', ticket.assigned_agent_id) : '待分配'} />
            <Time label="响应期限" value={ticket.response_due_at} />
            <Time label="解决期限" value={ticket.resolution_due_at} />
          </ContextSection>
                ...
                {agents
                  ...
                  .map((item) => (
                    <option key={item.id} value={item.id}>
                      {chineseReference('客服', item.membership_id)} · {chineseDomainList(item.skills, '专属服务') || '通用服务'}
                    </option>
                  ))}
              </select>
