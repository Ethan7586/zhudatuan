...
function application(handler = vi.fn(async () => ({ status: 200, body: { accepted: true } }))) {
  ...
  routes.freeze();
  return { app: new HttpApp(routes, ['https://console.example'], csrf), handler };
}
...
describe('HTTP boundary contract', () => {
  it('rejects an untrusted origin before a handler can observe the request', async () => {
    ...
    const response = await app.handle(
      new Request('https://api.example/api/v1/identity/session', {
    ...
    const response = await app.handle(
      new Request('https://api.example/api/v1/identity/sessions', {
        ...
        headers: {
          ...
          'x-request-id': 'request-contract',
          'x-client-target': 'console',
          cookie: '__Host-auth-csrf=expected',
  ...
  it('requires the anonymous bootstrap CSRF token for authentication writes', async () => {
    const { app, handler } = application();
    const response = await app.handle(
      new Request('https://api.example/api/v1/identity/challenges', {
        method: 'POST',
        headers: {
          cookie: '__Host-storefront-session=session; __Host-storefront-csrf=expected; __Host-auth-csrf=bootstrap',
          origin: 'https://console.example',
          'content-type': 'application/json',
          'x-contract-version': CONTRACT_VERSION,
  ...
  it('returns the security and contract headers on an accepted request', async () => {
    ...
    const response = await app.handle(
      new Request('https://api.example/api/v1/identity/session', {
        headers: { origin: 'https://console.example', 'x-contract-version': CONTRACT_VERSION, 'x-client-target': 'storefront' },
      })
    );
