  ...
  lifecycle(): OperationLifecycle<EnrollmentDraft, EnrollmentInvitationScope> {
    return operationLifecycle({
      ...
      prepare: async (request, loaded) => {
        ...
        if (authorization.stateHash !== preauth.authorization.stateHash || authorization.nonceHash !== preauth.authorization.nonceHash || authorization.challenge !== preauth.authorization.challenge) {
          throw new DomainError('PREAUTH_REQUIRED');
        }
        const principal = loaded.principal ?? `principal:${randomUUID()}`;
        const subject = body.mode === 'bound' ? canonicalMobile(await this.kms.decrypt('pii', 'identity/mobile', requiredCiphertext(loaded.mobileCiphertext), { principal })) : canonicalMobile(textField(body, 'subject', 32));
