...
export function useCartCommand() {
  ...
  const showToast = session.showToast;
  ...
  useEffect(() => {
    if (cart.data?.merge === 'completed') showToast('登录前选择的商品已合并到购物车', 'success');
    if (cart.data?.merge === 'blocked') showToast('登录前购物车商品过多，暂未合并；请先整理当前购物车', 'error');
  }, [cart.data?.merge, showToast]);
  ...
  const add = (product: Product, quantity = 1, _selectedSpec: Readonly<Record<string, string>> = {}) => {
    ...
    if (!availability.canPurchase || product.stock < nextQuantity || !Number.isSafeInteger(quantity) || quantity < 1) {
      showToast(availability.canPurchase ? '购买数量超过当前可售库存，请减少数量后重试' : availability.availabilityText, 'error');
      return;
    }
    void change(product.listingId, nextQuantity, current ? Number(current.version) : null)
      .then(() => showToast('已加入购物车', 'success'))
      .catch(() => showToast('购物车更新失败，请刷新后重试', 'error'));
  };
