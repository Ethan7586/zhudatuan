...
describe('LoginGateway', () => {
  it('submits a trimmed password in the body and preserves a membership outcome', async () => {
    ...
    await expect(gateway.authenticate({ kind: 'password', subject: ' employee ', password: 'Secret-12345!', target: 'storefront', returns: {} })).resolves.toMatchObject({ kind: 'membership' });
