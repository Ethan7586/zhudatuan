...
export class CredentialGenerateJob implements JobProcessor {
  ...
  process(job: ClaimedJob, signal: AbortSignal, deadline = Date.now() + 120_000): Promise<void> {
    ...
    return this.task.execute(job.id, text(payload.pool, 'VOUCHER_POOL_REQUIRED'), integer(payload.count, 'VOUCHER_COUNT_INVALID'),
      integer(payload.start, 'VOUCHER_START_INVALID'), text(job.scope, 'VOUCHER_SCOPE_REQUIRED'), signal, deadline);
  }
}
function record(value: unknown): Readonly<Record<string, unknown>> { if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('JOB_PAYLOAD_INVALID'); return value as Readonly<Record<string, unknown>>; }
