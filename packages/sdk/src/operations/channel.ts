...
function bindConnectionsTest(client: OperationExecutor): OperationMethod<"channel.connections.test"> { return bindOperation(client, defineOperation({ ...{"id":"channel.connections.test","method":"POST","path":"/api/v1/channels/connections/{connectionid}/tests","audience":"console","targets":["console"],"responseMode":"json","idempotent":false,"timeout":800,"errorUnion":["AUTHENTICATION_REQUIRED","AUTHORIZATION_DENIED","CAPABILITY_DENIED","CONTENT_TYPE_UNSUPPORTED","CONTRACT_VERSION_UNSUPPORTED","CSRF_TOKEN_INVALID","DEADLINE_EXCEEDED","IDEMPOTENCY_CONFLICT","IDEMPOTENCY_KEY_REQUIRED","INTERNAL_ERROR","ORIGIN_REQUIRED","PERMISSION_DENIED","RATE_LIMITED","REQUEST_BODY_TOO_LARGE","REQUEST_JSON_INVALID","SCOPE_DENIED","STEPUP_REQUIRED","URL_SENSITIVE_DATA_FORBIDDEN","VALIDATION_FAILED"]}, input: exactOperationInput("ChannelConnectionsTestInput", ["connectionid"] as const, true), output: exactOperationOutput("ChannelConnectionsTestOutput") })); }
