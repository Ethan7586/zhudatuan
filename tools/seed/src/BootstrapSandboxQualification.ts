...
import { SANDBOX_QUALIFICATION_SCHEMA_CHECKSUM, SANDBOX_QUALIFICATION_SCHEMA_VERSION, sandboxQualificationBootstrapEnvironment, sandboxQualificationBootstrapSummary } from './SandboxQualificationBootstrapPlan';
...
try {
  ...
  const result = await database.query<{ qualification: Readonly<Record<string, unknown>> }>('select deployment.sandbox_member_qualification_bootstrap($1,$2) qualification', [environment.sentinel, environment.membership]);
  const qualification = result.rows[0]?.qualification;
  if (!qualification || qualification.membership !== environment.membership || qualification.status !== 'active' || qualification.scope !== 'mall-zhudatuan' || qualification.benefitAmountGranted !== false) {
...
async function assertDatabaseBoundary(database: Client): Promise<void> {
  ...
  if (
    ...
    row.schema_version !== SANDBOX_QUALIFICATION_SCHEMA_VERSION ||
