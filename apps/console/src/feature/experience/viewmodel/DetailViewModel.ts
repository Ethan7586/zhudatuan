...
export function useExperienceDetailViewModel(context: ConsoleContext, dependencies: ExperienceDependencies, application?: string) {
  const query = useQuery({ queryKey: applicationDetailKey(context, application ?? 'closed'), queryFn: ({ signal }) => dependencies.readDetail.execute(context, application ?? '', signal), enabled: application !== undefined, staleTime: 30_000 });
