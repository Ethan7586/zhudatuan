...
import { canChangeListingPublication, isPublishedListing, productStatus } from '../model/ProductAction';
...
export function ProductDrawer({ listing, tab, detail, pending, error, onTab, onClose, onAction }: ProductDrawerProps) {
  return (
    <ModalOverlay
...
function ProductDrawerContent({
  ...
  const canChangePublication = canChangeListingPublication(listing.status, detail?.status);
  return (
    <>
      ...
      <div id="productdrawerpanel" className="productdrawerbody" role="tabpanel" aria-labelledby={`producttab-${tab}`}>
        <ProductDrawerPanels tab={tab} listing={listing} detail={detail} pending={pending} {...(error === undefined ? {} : { error })} />
      </div>
      <footer className="productdrawerfooter">
        ...
        <button
          ...
          onClick={() => {
            onClose();
            onAction({ kind: 'edit', listing, status: productStatus(detail?.status) });
          }}
        >
