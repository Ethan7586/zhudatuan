...
export class PgVoucherImport implements BatchImportPort {
  ...
  async process(target: ImportTarget, signal: AbortSignal): Promise<boolean> {
    ...
    try {
      await client.query('begin');
      await configureWorker(client, target.scope);
      const job = await client.query<{ cursor_value: number; total_count: number; cardpool_id: string }>(
        `select cursor_value,total_count,cardpool_id
        from voucher.importjob where id=$1 and state in('ready','running') for update`,
        [target.id]
      );
      if (!job.rows[0]) {
        await client.query('commit');
  ...
  async failures(target: ImportTarget): Promise<readonly ImportFailure[]> {
    const result = await this.pool.query<{ row_number: number; reason_code: string; field: string | null; detail: string }>('select row_number,reason_code,field,detail from voucher.importerror where job_id=$1 order by row_number', [
      target.id,
    ]);
  ...
  private async reset(target: ImportTarget): Promise<void> {
    ...
    try {
      ...
      await client.query('delete from voucher.importrow where job_id=$1', [target.id]);
      await client.query('delete from voucher.importerror where job_id=$1', [target.id]);
      await client.query('commit');
    } catch (cause) {
      await client.query('rollback');
      throw cause;
