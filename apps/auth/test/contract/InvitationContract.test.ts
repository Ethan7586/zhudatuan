...
describe('invitation contract', () => {
  it('accepts only explicit invitation outcome variants', () => {
    ...
    expect(
      mapInvitation({
        kind: 'proofRequired',
        proof: { reference: 'proof-1', purpose: 'invitation_acceptance', expiresAt: '2099-01-01T00:10:00.000Z', retryAt: '2099-01-01T00:00:30.000Z', attemptsRemaining: 10, method: 'otp', target: 'storefront' },
      })
    ).toMatchObject({ kind: 'proof', challenge: { purpose: 'invitation_acceptance', attemptsRemaining: 10 } });
  });
});
