...
export class PgTenderRepository implements TenderRepository {
  ...
  async create(call: Parameters<TenderRepository['create']>[0]) {
    ...
    const result = await this.tenders.reserve({
      context: write(call),
      scope: call.scope,
      actor: call.actor,
      now: call.now,
  ...
  async consume(call: Parameters<TenderRepository['consume']>[0]) {
    ...
    const id = path(call, 'holdid');
    const selected = await database.query<HoldRow>(`select ${HOLD_FIELDS} from voucher.tenderhold hold where hold.id=$1 and hold.scope_id=$2`, [id, call.scope]);
    const hold = selected.rows[0];
    if (!hold) throw new DomainError('RESOURCE_NOT_FOUND');
    const value = body(call);
    const redeemed = await this.redemptions.redeem({
      context: write(call),
      scope: call.scope,
      voucher: hold.voucher_id,
      hold: id,
      expectedHoldVersion: expected(call),
  ...
  async release(call: Parameters<TenderRepository['release']>[0]) {
    return {
      status: 200,
      body: await this.tenders.release({ context: write(call), scope: call.scope, hold: path(call, 'holdid'), reason: text(body(call).reason, 'reason'), expectedVersion: expected(call), actor: call.actor, now: call.now }),
    };
  }
}
interface HoldRow {
...
function ownOnly(target: string) {
