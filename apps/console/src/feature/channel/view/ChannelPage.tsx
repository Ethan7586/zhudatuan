import { Button, FilterBar, PageHeader, Pagination, ResourceState } from '@shop/design';
...
export function ChannelPage({ title, model }: Readonly<{ title: string; model: ChannelViewModel }>) {
  return (
    <section className="channelpage">
      ...
      />
      <FilterBar label="渠道视图">
        <label>
          查看
          <select value={model.view} onChange={(event) => model.actions.selectView(event.target.value as ChannelView)}>
      ...
      {model.receipt ? (
        <div className="channelreceipt">
          <ActionReceipt state={{ kind: 'success', receipt: model.receipt, objectLabel: '渠道业务', impact: '连接、同步游标或外部操作记录已按服务端结果更新。' }} dismiss={{ label: '关闭回执', onPress: model.actions.dismissReceipt }} />
        </div>
      ) : null}
      <ConnectionDialog model={model.action} onClose={model.actions.closeAction} />
      <ChannelActionDialog model={model.action} onClose={model.actions.closeAction} />
    </section>
  );
}
