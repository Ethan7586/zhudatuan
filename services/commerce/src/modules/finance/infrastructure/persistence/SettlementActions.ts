...
/** Finance persistence action assembly. */
...
export function settlementPersistence(repository: FinanceRepositoryFactory): Pick<FinancePersistence, 'settlementsDecide' | 'settlementsAdjust'> {
  return {
    settlementsDecide: (request, database) => decide(request, database, repository),
    settlementsAdjust: (request, database) => adjust(request, database, repository),
  };
}

async function decide(request: OperationRequest, database: SqlExecutor, repository: FinanceRepositoryFactory) {
  ...
  const result = await database.query(
    ...
    [selected.id, approved ? 'payable' : 'cancelled', access.actor.id, JSON.stringify({ reason: textField(body, 'reason', 1000), evidence: record(body.evidence) })]
  );
  if (approved) {
    ...
    await repository(database).event('finance.settlement.approved', 'settlement', selected.id, selected.scope_id, {
...
async function adjust(request: OperationRequest, database: SqlExecutor, repository: FinanceRepositoryFactory) {
  const access = requireAccess(request);
  const body = bodyRecord(request.input);
  ...
  if (action === 'request') {
    ...
    const result = await database.query(
      ...
      [id, request.input.path.settlementid!, line.id, access.scope.id, direction, amount, tax, access.actor.id, textField(body, 'reason', 1000), JSON.stringify(record(body.evidence))]
    );
    return rowResult(result, 201);
  }
  if (action !== 'approve' && action !== 'reject') throw new Error('FINANCE_SETTLEMENT_ADJUSTMENT_ACTION_INVALID');
  return decideAdjustment(database, request.input.path.settlementid!, textField(body, 'adjustment'), action, access.actor.id, textField(body, 'reason', 1000), record(body.evidence), access.scope.id, repository);
}
...
async function decideAdjustment(
  ...
  await database.query(
    `update finance.settlement set gross_minor=$2,fee_minor=$3,amount_minor=$4,version=version+1,
    evidence=evidence||jsonb_build_object('lastAdjustment',$5) where id=$1`,
    [settlement, gross, split.feeMinor, split.netMinor, adjustment]
  );
...
async function postSettlement(database: SqlExecutor, selected: Settlement): Promise<void> {
  await finance.post(requireWriteTransaction(database.transaction), {
    ...
    referenceId: `${selected.id}:partner`,
    currency: selected.currency,
  ...
  if (selected.fee_minor > 0)
    await finance.post(requireWriteTransaction(database.transaction), {
      scope: selected.scope_id,
      referenceType: 'finance.settlement.approved',
      referenceId: `${selected.id}:platform`,
      currency: selected.currency,
