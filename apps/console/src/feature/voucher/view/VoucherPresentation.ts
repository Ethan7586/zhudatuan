...
export const voucherViewMeta: Readonly<Record<VoucherView, VoucherViewMeta>> = Object.freeze({
  ...
  statusbatches: Object.freeze({ label: '操作批次', short: '操作', description: '批量激活、禁用、延期和作废的执行进度' }),
  bindings: Object.freeze({ label: '卡券中心', short: '卡券', description: '卡券余额、绑定记录与有效期' }),
  redemptions: Object.freeze({ label: '消费明细', short: '消费', description: '核销金额、关联订单与冲正状态' }),
  history: Object.freeze({ label: '状态历史', short: '历史', description: '卡券状态变更、操作原因与审计时间' }),
});
...
function businessMetric(view: VoucherView, rows: readonly VoucherRecord[]): VoucherSummaryMetric {
  ...
  const labels: Readonly<Record<Exclude<VoucherView, 'programs'>, readonly [string, string]>> = Object.freeze({
    ...
    statusbatches: ['本页处理数量', '来自批量操作权威任务'],
    bindings: ['本页卡券数量', '来自范围内卡券读模型'],
    redemptions: ['本页消费记录', '来自服务端消费回执'],
    history: ['本页历史记录', '来自不可变状态事件'],
  });
