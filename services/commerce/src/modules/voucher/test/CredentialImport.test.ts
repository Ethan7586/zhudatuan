...
describe('credential import strategy', () => {
  it('persists only protected values and never exposes source credentials to the staged payload', async () => {
    const query = vi.fn(async (sql: string) => result(sql.startsWith('select product_id') ? [{ product_id: 'product:one', state: 'open', mode: 'imported' }]
      : sql.startsWith('update voucher.credentialpool') ? [{ product_id: 'product:one' }] : []));
    ...
    await process.stage(target, [{ number: 'VC001234', secret: 'Secret9876' }, { number: 'invalid number', secret: 'DontPersist123' }], execution);
  ...
  it('keeps invalid source values out of persisted chunks', async () => {
    const harness = batchHarness();
    const process = createCredentialImportProcess(harness.factory, transactionManager(async () => result([])), {} as ImportPort, {} as JobPort,
      { protect: vi.fn(async () => { throw new Error('SHOULD_NOT_RUN'); }) }, authorization);
  ...
  it('propagates database outages as infrastructure failures', async () => {
    ...
    const harness = batchHarness();
    const process = createCredentialImportProcess(harness.factory, transactionManager(async () => { throw failure; }), {} as ImportPort, {} as JobPort,
      { protect: vi.fn(async (_value, purpose) => ({ ciphertext: `encrypted-${purpose}`, fingerprint: (purpose === 'number' ? 'b' : 'c').repeat(64), masked: '****1234', keyVersion: 'key:one' })) }, authorization);
    const execution = runContext();
  ...
  it('does not stage a row when envelope encryption fails', async () => {
    const harness = batchHarness();
    const process = createCredentialImportProcess(harness.factory, transactionManager(async () => result([])), {} as ImportPort, {} as JobPort,
      { protect: vi.fn(async () => { throw new Error('KMS_ENCRYPT_FAILED'); }) }, authorization);
...
function batchHarness() {
  ...
  const factory: ImportBatchFactoryPort = { create(configuration) { return process(configuration, rows); } };
  return { factory, rows };
}
...
function process(configuration: ImportBatchConfiguration, staged: { row: number; value: Readonly<Record<string, string>> }[]): BatchImportProcessPort {
  return {
    ...
    stage: async (candidate, source, execution) => {
      ...
      const prepared = configuration.prepare ? await configuration.prepare(candidate, rows, execution)
        : { rows: rows.map((item) => ({ row: item.row, payload: item.value })), failures: [] };
      staged.push(...prepared.rows.map((item) => ({ row: item.row, value: item.payload })));
    },
    process: async (candidate, signal, deadline) => {
      for (const item of staged) {
        try {
          await configuration.transactions.write({ tenant: candidate.scope, membership: '', scope: candidate.scope, actor: 'test', trace: candidate.id, operation: 'test', workload: 'jobs', signal, deadline },
            (context) => configuration.write(context, candidate, item.row, item.value));
        } catch (cause) {
    ...
    failures: async () => [], report: async () => undefined, complete: async () => undefined, reject: async () => undefined, fault: async () => undefined,
  };
}
