...
export interface VoucherDrawerAction { readonly label: string; readonly tone?: 'default' | 'primary' | 'danger'; readonly run: () => void }
      ...
      {record ? (
        <div className="voucherdrawerbody">
          ...
          <dl className="voucherfacts">
            ...
            <div><dt>版本</dt><dd>{record.version === null ? '—' : `第 ${record.version} 版`}</dd></div>
          </dl>
          {actions.length ? <footer>{actions.map((action) => <Button key={action.label} tone={action.tone ?? 'default'} onPress={action.run}>{action.label}</Button>)}</footer> : null}
        </div>
      ) : null}
    </Drawer>
  );
