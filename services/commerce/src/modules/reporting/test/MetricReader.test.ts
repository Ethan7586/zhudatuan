...
describe('report metric reader policy', () => {
  it('authorizes before reading and never trusts a caller supplied scope', async () => {
    ...
    await expect(reader.read('reporting.sales.read', { query: { limit: 20 } } as never, {
      operation: 'reporting.sales.read', transaction: {},
      security: { kind: 'anonymous', channel: 'public', target: 'console', trace: 'trace:one' },
    } as never, 'sales')).rejects.toThrow('AUTHENTICATION_REQUIRED');
  ...
  it('returns governed values, preserves null comparisons and uses the frozen watermark as the summary time', async () => {
    ...
    expect(reports.cockpit).toHaveBeenCalledWith(expect.anything(), {
      scope: 'mall:one', period: '30days', application: null,
      watermarkAt: watermark.occurredAt, watermarkVersion: 7,
...
function summary(): CockpitSummary {
  return {
    catalogCount: 2.2, availableStock: -1.4, orderCount: 1.2, afterSaleCount: 0,
    sales: {
      asOf: '2026-09-06T12:00:00.000Z', cumulativeSalesCents: 100.4, paidOrderCount: 2.2,
      averageOrderValueCents: 50.6, periodSalesCents: 100.4, periodPaidOrderCount: 2.2, refundedCents: 0,
      activeProductCount: 2.2, soldProductCount: 1.2, unsoldActiveProductCount: 1.2,
      period: { from: '2026-08-07T00:00:00.000Z', to: '2026-09-06T00:00:00.000Z' }, conclusion: '经营稳定',
      deltas: { netSalesRatio: null, paidOrdersRatio: 0.1, averageOrderRatio: -0.2, refundRate: 0, refundRateDeltaPoints: null },
      trend: [{ date: '2026-09-05', salesCents: 100.4, orderCount: 2.2 }], weeklyTrend: [],
