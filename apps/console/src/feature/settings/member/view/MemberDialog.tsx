  ...
  return (
    <Dialog open title="编辑成员" eyebrow="成员资料 · 服务端状态 · 版本校验" onClose={model.actions.close} dismissable={!model.manage.busy}>
      ...
      >
        ...
        {model.conflict ? <section className="membererror" role="alert" aria-label="成员并发冲突"><strong>{model.conflict.title}</strong><ul>{model.conflict.details.map((detail) => <li key={detail}>{detail}</li>)}</ul><Button onPress={model.actions.resolveConflict}>{model.conflict.member ? '使用最新状态继续编辑' : '关闭并重新选择'}</Button></section> : null}
