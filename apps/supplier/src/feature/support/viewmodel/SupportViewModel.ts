...
export const supportViewModel = defineSupplierViewModel({
  ...
  read: (client, context, route) => route.id === 'suppliercase'
    ? client.support.messagesRead({ path: { caseid: route.parameters.caseId }, query: { limit: 50 } }, context)
    : client.support.casesRead({ query: { limit: 50, ownership: 'all' } }, context),
  project: (value, route) => route.id === 'suppliercase' ? messageProjection(value) : caseProjection(value),
  actions: (value, route, selectedKey) => route.id === 'suppliercase'
    ? conversationActions(value)
    : selectedOperatorRecord(value, selectedKey) ? Object.freeze([Object.freeze({ id: 'open', label: '打开会话', description: '查看订单上下文并参与协同。', tone: 'primary', requiresSelection: true, fields: Object.freeze([]) })]) : Object.freeze([]),
  execute: async (client, context, route, value, selectedKey, action, input) => {
    ...
    if (action.id === 'send') {
      ...
      await client.support.messagesSend({ path: { caseid: route.parameters.caseId }, body: {
        message: requiredText(input, 'message', 4000), clientMessageId: supplierMessageId(), visibility,
      } }, context);
...
function conversationActions(value: unknown) {
  ...
  const send = Object.freeze({
    id: 'send', label: '发送消息', description: '回复顾客，或发送仅供应商和平台可见的内部消息。', tone: 'primary' as const, expectedVersion: state.version, identityScope: true,
    fields: Object.freeze([
      actionField('visibility', '消息类型', { kind: 'choice', value: 'internal', choices: Object.freeze([{ value: 'internal', label: '内部协同' }, { value: 'external', label: '回复顾客' }]) }),
...
function caseProjection(value: unknown) {
  return operatorCollection(value, operatorItems(value).map((item) => operatorRow({
    key: operatorText(item, 'id'), title: operatorText(item, 'subject'),
    detail: `${priorityLabel(operatorText(item, 'priority'))} · 未读 ${operatorText(item, 'unread_count')} · ${operatorText(item, 'skill')}`,
    statusLabel: supportCaseStatus(operatorText(item, 'state'), operatorText(item, 'sla_risk')), timestamp: operatorText(item, 'updated_at'),
  })));
}

function messageProjection(value: unknown) {
  return operatorCollection(value, operatorItems(value).map((item) => operatorRow({
    key: operatorText(item, 'id'), title: operatorText(item, 'authorType') === 'member' ? '顾客' : operatorText(item, 'visibility') === 'internal' ? '内部协同' : '客服回复',
    detail: operatorText(item, 'body') || '附件消息', statusLabel: operatorText(item, 'visibility') === 'internal' ? '仅内部可见' : '顾客可见', timestamp: operatorText(item, 'createdAt'),
  })));
}
...
function conversationState(value: unknown) {
  ...
  return { conversation: first ? operatorText(first, 'conversationId') : '', latest: operatorNumber(root ?? {}, 'latestSequence') ?? 0, lastRead: operatorNumber(root ?? {}, 'lastReadSequence') ?? 0, version: operatorNumber(root ?? {}, 'conversationVersion') ?? 0 };
}

function priorityLabel(value: string): string { return value === 'urgent' ? '非常紧急' : value === 'high' ? '高优先级' : value === 'low' ? '低优先级' : '普通优先级'; }
