...
export class VoucherDeadletter implements JobDeadletter {
  ...
  private async issue(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, batchid: string, error: string): Promise<void> {
    ...
    const target = await sql.query<{ scope_id: string }>('select program.scope_id from voucher.issuebatch batch join voucher.program program on program.id=batch.program_id where batch.id=$1', [batchid]);
    if (target.rows[0]) {
      const event = `event:${randomUUID()}`;
      await new PgRuntimeWriter(sql).append({ id: event, type: 'voucher.issue.failed', aggregateType: 'issuebatch', aggregate: batchid, scope: target.rows[0].scope_id, payload: { batch: batchid, error }, trace: event });
    }
  }
  ...
  private async import(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, importid: string, error: string): Promise<void> {
    ...
    const sql = database as unknown as RuntimeSql;
    const target = await sql.query<{ scope_id: string }>('select scope_id from voucher.importjob where id=$1', [importid]);
    if (target.rows[0]) {
      const event = `event:${randomUUID()}`;
      await new PgRuntimeWriter(sql).append({ id: event, type: 'voucher.import.failed', aggregateType: 'importjob', aggregate: importid, scope: target.rows[0].scope_id, payload: { import: importid, error }, trace: event });
    }
  }
  ...
  private async status(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, batchid: string, error: string): Promise<void> {
    ...
    const sql = database as unknown as RuntimeSql;
    const target = await sql.query<{ scope_id: string }>('select scope_id from voucher.statusbatch where id=$1', [batchid]);
    if (target.rows[0]) {
      const event = `event:${randomUUID()}`;
      await new PgRuntimeWriter(sql).append({ id: event, type: 'voucher.status.failed', aggregateType: 'statusbatch', aggregate: batchid, scope: target.rows[0].scope_id, payload: { batch: batchid, error }, trace: event });
    }
  }
}
