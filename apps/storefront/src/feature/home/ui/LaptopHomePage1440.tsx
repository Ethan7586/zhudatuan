...
import { Sparkles, CreditCard, Gift, ChevronRight, ShieldCheck, Zap, Tag, ArrowRight, FileText, Truck, CheckCircle2, Building, Coffee, Ticket, Smartphone, Flame, Award, ShoppingCart } from 'lucide-react';
...
export const LaptopHomePage1440: React.FC<LaptopHomePage1440Props> = ({ onSelectTab, surface = 'standard' }) => {
  ...
  const categories = presentationCategories.slice(0, 7).map((category) => ({ name: category.name, sub: category.description || '当前商城已发布分类', badge: '已发布' }));

  const visibleProducts = products.slice(0, surfaceCopy.wideHomeProductCount);
  ...
  return (
    <div className="w-full bg-[var(--sw-background)] pb-8 font-sans">
          ...
          <div className="sw-web-category-sidebar w-[210px] flex-shrink-0 bg-white border border-gray-200 rounded-lg shadow-2xs overflow-hidden">
            ...
            <div className="divide-y divide-gray-100 text-xs">
              {categories.map((cat, idx) => (
                <button type="button" key={idx} onClick={() => onSelectTab('category')} className="w-full text-left p-2.5 hover:bg-blue-50/70 transition-colors cursor-pointer group flex items-center justify-between">
            ...
            <div className="p-2.5 bg-gradient-to-br from-blue-50 to-indigo-50 border-t border-blue-100">
              <button type="button" onClick={() => onSelectTab('category')} className="w-full bg-[var(--sw-brand)] hover:bg-blue-700 text-white rounded-md p-2.5 cursor-pointer transition-colors text-center shadow-2xs">
                <div className="text-xs font-black flex items-center justify-center gap-1">
                  <Zap className="w-4 h-4 text-yellow-300" />
                  <span>企业专享补贴通道</span>
                </div>
                <div className="text-[10px] text-blue-100 mt-0.5">开票资格与文件以订单发票记录为准</div>
              </button>
            </div>
          </div>
            ...
            <div className="sw-web-wide-hero relative rounded-lg overflow-hidden bg-gradient-to-r from-[var(--sw-brand-dark)] via-[var(--sw-brand)] to-indigo-900 text-white p-5 h-[250px] flex flex-col justify-between shadow-xs border border-blue-900">
              ...
              <div className="relative z-10">
                <div className="inline-flex items-center gap-1.5 bg-yellow-400 text-gray-900 font-black text-xs px-2.5 py-0.5 rounded-full mb-2 shadow-2xs">
                  ...
                  <span>{currentMall.enterpriseName} · 当前员工福利专场</span>
                </div>

                <h1 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight text-white drop-shadow-xs">企业采购与福利兑换 · 权威数据实时校验</h1>
                <p className="text-xs text-blue-100 mt-1.5 max-w-[550px]">展示当前商城已发布的实物、卡券与服务商品；价格、库存、账户资格及履约承诺以服务端记录为准。</p>
              </div>
              ...
              <div className="relative z-10 flex items-center justify-between border-t border-white/20 pt-3">
                <div className="flex items-center gap-4 text-xs text-blue-100">
                  <span className="flex items-center gap-1">
                    ...
                    <span>已发布商品来源可追溯</span>
                  </span>
                  <span className="flex items-center gap-1">
                    <Truck className="w-4 h-4 text-yellow-300" />
                    <span>物流状态按订单时间线更新</span>
                  </span>
                  <span className="flex items-center gap-1">
                    <FileText className="w-4 h-4 text-cyan-300" />
                    <span>发票签发后可验证下载</span>
                  </span>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-3.5 shadow-2xs">
              <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-100">
                ...
                <button onClick={() => onSelectTab('category')} className="text-xs text-[var(--sw-brand)] hover:underline font-bold flex items-center gap-0.5 cursor-pointer">
                  <span>查看全部 {products.length} 件商品</span>
                ...
                {visibleProducts.map((product) => (
                  ...
                  >
                    <div className="absolute top-2 left-2 z-10 flex flex-col gap-1">
                      <span className="bg-[var(--sw-promotion)] text-white text-[9px] font-bold px-1.5 py-0.2 rounded shadow-2xs">企采价</span>
