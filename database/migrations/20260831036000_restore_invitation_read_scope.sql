...
do $precondition$ begin
  if not exists(select 1 from runtime.schemaversion where version='20260831035000') then
    raise exception 'INVITATION_READ_SCOPE_PREVIOUS_HEAD_MISSING';
  end if;
  if exists(select 1 from runtime.schemaversion where version='20260831036000') then
    raise exception 'INVITATION_READ_SCOPE_ALREADY_APPLIED';
  end if;
end $precondition$;
...
begin
  ...
  if position('identity.invitations.create' in definition)=0
      or position('identity.invitations.read' in definition)=0
      or position('identity.members.manage' in definition)=0
      or position('payment.intents.create' in definition)=0
      or position('sku.product_id=p_resource' in definition)=0
      or position('versionrecord.application_id' in definition)=0 then
    raise exception 'IDENTITY_RESOURCE_SCOPE_BRANCH_MISSING';
  end if;
  for candidate in
    select membership.id,membership.organization_id from access.membership membership
  ...
  if exists(select 1 from access.membership target
    where access.resource_scope('identity.members.manage',target.id,target.id)<>target.organization_id) then
    raise exception 'MANAGED_MEMBER_SCOPE_RESOLUTION_INVALID';
  end if;
end $verify$;

select runtime.record_migration_evidence('20260831036000',1,1,0,0,
  'select membership.id,access.resource_scope(''identity.invitations.read'',null,membership.id) from access.membership membership where membership.client<>''storefront'' order by membership.id;',
  'select pg_get_functiondef(''access.resource_scope(text,text,text)''::regprocedure);');
insert into runtime.schemaversion(version,checksum)
values('20260831036000',encode(public.digest('20260831036000_restore_invitation_read_scope','sha256'),'hex'));
