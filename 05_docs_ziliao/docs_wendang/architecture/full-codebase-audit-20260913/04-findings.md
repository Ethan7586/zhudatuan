...
| 文件和精确位置 | `05_docs_ziliao/docs_wendang/architecture/full-codebase-audit-20260913/05-dead-code-candidates.md:22-2328,2562`；`records/AU-906-independent-review-index/index.md`；`10-coverage-manifest.csv:3306,3325`。 |
| 当前行为 | AU-906/RV-0074 已将 12 个可定位对象补为 GX-0042 至 GX-0053；累计 GX 52 中现有 50 个唯一 ID，仍有两个无对象标识的历史计数单位。全部已定位 P1 与 GX 已有第二轮记录；manifest 仍有两个明确“需要专项专家复核”文件。 |
| 预期行为 | 所有 P1、G3 与 GX 必须具有唯一 ID、当前状态、证据位置、复核 Owner/方法和第二轮结果；汇总数应可由该清单机械重算。 |
| 直接证据 | [FACT][E-AU-905-001] manifest 4,238 条目且无暂未审阅；[FACT][RV-0075/RV-0076] 已定位 P1 均已独立复核；[FACT][RV-0074] 50 个可枚举 GX ID 与历史累计 GX 52 的两个无对象标识差额。 |
| 调用链或运行入口 | 首轮 findings/candidates/manifest → 独立复核调度 → 最终问题/候选计数与治理排序。 |
| 用户影响 | 若把历史累计强行当成逐项清单，可能虚构两个高风险对象，或错误宣称最后两个计数差额已有对象级复核。 |
| 数据影响 | 无直接数据写入。 |
| 安全影响 | GX 中含身份、支付、迁移与历史登录材料；范围不完整会削弱安全/数据风险复核的覆盖证明。 |
| 根因 | 首轮多批次累计使用了增量数字，早期没有同步维护统一 GX/RV authority；后续索引已恢复所有可定位对象，但无法从当前仓库历史找回最后两个计数单位的对象身份。 |
| 建议方向 | 保留 independent-review-index 作为唯一 authority；未来仅在获得原始工作记录或 Owner 受控历史证据后追溯两个计数单位，不得按文件数、名称或累计算术补造 GX。 |
| 预计修改范围 | 审计报告/复核索引；不改生产实现。 |
| 验证方式 | ID 唯一、计数可机械重算、每项可追溯首审与第二轮证据；G3/P1/GX 没有复核记录时不能在最终汇总标为完成。 |
| 回滚方式 | 回退单一审计索引/报告提交，保留原始首审记录。 |
| 是否需要独立复核 | 已完成可定位范围：RV-0001 至 RV-0076。两个无对象标识差额无法实施对象级复核，保持未确认。 |
