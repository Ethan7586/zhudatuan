...
- 生产禁止加载`tests/browser/MvpServer.mjs`、历史payment simulation RPC或任何“支付成功开关”。
...
`tests/browser/MvpServer.mjs`是确定性的、仅本地的MVP编排服务。它验证页面发出的API路径、顺序和输入，覆盖：
...
```bash
MVP_API_PORT=4310 node tests/browser/MvpServer.mjs
```
