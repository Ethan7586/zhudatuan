...
export interface AlertRuleDefinition {
  ...
  readonly measure: 'count' | 'percent' | 'burnrate';
...
export class AlertRule {
  ...
  constructor(definition: AlertRuleDefinition) {
    if (!/^[a-z][a-z0-9]*$/.test(definition.id)
      ...
      || !['count', 'percent', 'burnrate'].includes(definition.measure)
      || !Number.isFinite(definition.threshold) || definition.threshold < 0
      || !Number.isSafeInteger(definition.windowSeconds) || definition.windowSeconds < 60
      || !['warning', 'critical'].includes(definition.severity)
      || !/^[a-z][a-z0-9]*$/.test(definition.owner)
      || !/^docs\/operations\/[a-z0-9]+\.md$/.test(definition.runbook)) throw new Error(`ALERT_RULE_INVALID:${definition.id}`);
