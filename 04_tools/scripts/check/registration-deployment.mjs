...
const [deliverySource, buildSource, packageSource, serviceSource, environmentSource, bootstrapSource, predecessorMigrationSource, runnerSource, postgresInitSource, reconciliationSource, databaseAuditSource,postgresInitFixtureSource] = await Promise.all([
  ...
  read('package.json'),
  read('02_platform_pingtai/infrastructure/zhudatuan/aliyun/systemd/zhudatuan-registration-bootstrap.service'),
  read('02_platform_pingtai/infrastructure/zhudatuan/aliyun/registration-bootstrap.env.example'),
  read('04_tools/tools/seed/src/BootstrapRegistration.ts'),
  ...
  read('02_platform_pingtai/infrastructure/zhudatuan/aliyun/postgres-init-registration.sh'),
  read('02_platform_pingtai/infrastructure/zhudatuan/aliyun/postgres-reconcile-registration-boundary.sql'),
  read('04_tools/scripts/audit/database-contracts.mjs'),
  read('04_tools/scripts/audit/postgres-init-registration.pg16-fixture.mjs'),
]);
...
if (!buildSource.includes("BootstrapRegistration: '04_tools/tools/seed/src/BootstrapRegistration.ts'")) {
  throw new Error('REGISTRATION_BOOTSTRAP_BUILD_ENTRY_MISSING');
}
const packageJson = JSON.parse(packageSource);
if (packageJson.scripts?.['bootstrap:registration:production'] !== 'node 01_core_hexin/services/commerce/dist/BootstrapRegistration.js') {
  throw new Error('REGISTRATION_BOOTSTRAP_PRODUCTION_COMMAND_INVALID');
}

for (const token of [
  'ConditionPathExists=/opt/zhudatuan/current/01_core_hexin/services/commerce/dist/BootstrapRegistration.js',
  ...
  'ExecStart=/usr/bin/node 01_core_hexin/services/commerce/dist/BootstrapRegistration.js',
  'User=zhudatuan',
  'Group=zhudatuan',
  'UMask=0077',
...
if (serviceSource.includes('--import tsx') || serviceSource.includes('04_tools/tools/seed/src/BootstrapRegistration.ts')) {
...
const migrationSource = targetFile ? await read(`02_platform_pingtai/database/supabase/migrations/${targetFile}`) : '';
