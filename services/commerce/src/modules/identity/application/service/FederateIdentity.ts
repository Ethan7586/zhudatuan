...
export class FederateIdentity {
  ...
  async prepareCallback(input: FederationCallbackInput, loaded: LoadedFederationCallback, context: FederationRequestContext): Promise<PreparedFederationCallback> {
    ...
    const protectedSubject = JSON.stringify({ subject: subject.subject, claims: subject.claims });
    const envelope =
      accepted.transaction.purpose === 'link'
        ? await this.kms.encrypt('pii', 'identity/federatedsubject', protectedSubject, {
            principal: accepted.transaction.principal!,
            provider: instance.id,
          })
        : await this.kms.encrypt('pii', 'identity/federation', protectedSubject, { provider: instance.id });
