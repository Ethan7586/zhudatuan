...
describe('PgProductRepository detail projection', () => {
  it('returns media, channel, pool and ordered timeline facts from one catalog query', async () => {
    const query = vi.fn(async (_sql: string, _values?: readonly unknown[]) => ({
      rows: [{
        id: 'product:one', title: '早餐', product_type: 'physical', status: 'active', version: '7', category_id: 'category:food', brand_id: null,
        owner_partner_id: null, cover_url: 'https://assets.example/cover.jpg', subtitle: null, description: '工作日早餐',
        createdAt: '2026-09-01T00:00:00.000Z', updatedAt: '2026-09-07T00:00:00.000Z',
        attributes: { regionIds: ['region:east'], media: [{ id: 'media:manual', kind: 'document', url: 'https://assets.example/manual.pdf', alt: '说明书', sort: 2 }, { id: 'media:unsafe', kind: 'image', url: 'javascript:alert(1)', sort: 1 }] },
        skus: [{ id: 'sku:one', code: 'MEAL-1', status: 'active', specifications: [], version: '2' }],
        listings: [{ id: 'listing:one', scope: 'mall:one', pool: 'pool:one', sku: 'sku:one', title: '早餐', status: 'published', effectiveAt: null, expiresAt: null, createdAt: '2026-09-02T00:00:00.000Z', updatedAt: '2026-09-06T00:00:00.000Z', version: '3' }],
