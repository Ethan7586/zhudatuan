...
export function ConversationPage({ caseId }: Readonly<{ caseId: string }>) {
  ...
  useEffect(() => {
    const next = conversationDrafts.read(caseId);
    setDraft(next);
    setFailed(null);
    setNotice('');
    lastRead.current = 0;
  ...
  const markRead = useCallback(
    (sequence: number) => {
      ...
      void readUpdater.execute(session, conversationId, sequence).catch(() => {
  ...
  useLayoutEffect(() => {
    ...
    if (!element) return;
    const first = merged.items[0]?.sequence ?? 0;
    const last = merged.items.at(-1)?.sequence ?? 0;
  ...
  return (
    <main className="storesupportconversation">
      <header className="storesupportconversationheader">
        <button type="button" aria-label="返回客服中心" onClick={() => void navigate('/support')}>
          <ArrowLeft size={18} />
        </button>
        <div>
          <p>智慧翼 · 客户服务</p>
          <h1>{ticket.data?.subject ?? '工单会话'}</h1>
          <span>
      ...
      <section
        ...
        onScroll={(event) => {
          ...
          if (bottom) {
            setNewMessage(false);
            markRead(merged.items.at(-1)?.sequence ?? 0);
          }
        }}
      >
        {conversation.hasNextPage ? (
          <button className="storesupportearlier" type="button" disabled={conversation.isFetchingNextPage} onClick={() => void conversation.fetchNextPage()}>
            {conversation.isFetchingNextPage ? '加载中…' : '加载更早消息'}
          </button>
        ) : null}
        ...
        {merged.items.map((message, index) => (
          <MessageBubble key={message.id} message={message} attachments={merged.attachments.filter((item) => item.messageId === message.id)} showDate={index === 0 || day(message.createdAt) !== day(merged.items[index - 1]!.createdAt)} />
        ))}
      ...
      <SupportComposer
        ...
        onChange={(message) => updateDraft({ ...draft, message })}
        onSend={sendCurrent}
        onRetry={() => {
          if (failed) send.mutate(failed);
        }}
        onFile={(file) => upload.mutate({ file, localId: `upload:${crypto.randomUUID()}` })}
      />
    </main>
  );
}
...
function required(value: Session | null): Session {
