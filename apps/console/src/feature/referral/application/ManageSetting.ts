...
export class ManageSetting {
  constructor(private readonly port: Pick<ReferralPort, 'execute'>) {}
  execute(context: ConsoleContext, values: ReferralActionValues & Readonly<{ action: Extract<ReferralActionValues['action'], { kind: 'setting' }> }>, proof: string, identity: string, signal?: AbortSignal) {
