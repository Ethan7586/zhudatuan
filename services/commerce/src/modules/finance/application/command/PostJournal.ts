...
export class PostJournal {
  ...
  async execute(envelope: Readonly<Record<string, unknown>>): Promise<void> {
    ...
    const scope = await this.pool.query<{ scope_id: string; received_at: string }>(
      `select job.scope_id,inbox.received_at from runtime.job job
      join runtime.inbox inbox on inbox.event_id=$1 and inbox.consumer='job:reconciliation' where job.payload->>'eventId'=$1 limit 1`,
      [eventid]
    );
    ...
    try {
      ...
      if (event === 'payment.captured' || event === 'refund.completed') await this.payment(client, eventid, event, payload, target);
  ...
  private async payment(database: OperationDatabase, eventid: string, event: string, payload: Readonly<Record<string, unknown>>, target: Readonly<{ scope_id: string; received_at: string }>): Promise<void> {
    ...
    const amount = await this.payments.externalAmount(database, event === 'refund.completed' ? 'refund' : 'payment', reference);
    if (amount === 0) return;
