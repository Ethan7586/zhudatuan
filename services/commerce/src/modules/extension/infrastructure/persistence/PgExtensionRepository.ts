...
export class PgExtensionRepository implements ExtensionRepository {
  ...
  async manifest(provider: string): Promise<RegisteredManifest | null> {
    const result = await this.database.query<RegisteredManifest>(
  ...
  async reconfigure(
    ...
    const current = await this.lock(id, scope);
  ...
  async latestHealth(id: string, version: number): Promise<Readonly<{ state: 'healthy' | 'degraded' | 'unavailable'; checkedAt: string; latency: number }> | null> {
    const result = await this.database.query<{ state: 'healthy' | 'degraded' | 'unavailable'; checkedAt: string; latency: number }>(
  ...
  async enqueueHealth(id: string, scope: string, delaySeconds = 0): Promise<void> {
    await this.database.query(
      ...
      values('job:extensionhealth:'||$1||':'||to_char(date_trunc('minute',clock_timestamp()+make_interval(secs=>$3)),'YYYYMMDDHH24MI'),
        ...
        clock_timestamp()+make_interval(secs=>$3),clock_timestamp(),clock_timestamp()) on conflict(id) do nothing`,
      [id, scope, delaySeconds]
    );
  }
  ...
  async list(scopes: readonly string[], cursor: Readonly<{ sort: string | null; id: string | null }>, fetch: number): Promise<readonly ExtensionListRow[]> {
    const result = await this.database.query<ExtensionListRow>(
  ...
  async targets(limit: number) {
    const result = await this.database.query<{ id: string; scope_id: string; extension_id: string; status: InstallationState }>(
      `select id,scope_id,
  ...
  async summaries(ids: readonly string[]) {
    ...
    const result = await this.database.query<import('../../application/port/ExtensionLoader').ExtensionSummary>(
