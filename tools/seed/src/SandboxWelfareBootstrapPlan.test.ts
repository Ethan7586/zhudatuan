...
import { SANDBOX_WELFARE_CONFIRMATION, SANDBOX_WELFARE_SCHEMA_CHECKSUM, SANDBOX_WELFARE_SCHEMA_VERSION, sandboxWelfareBootstrapEnvironment, sandboxWelfareBootstrapSummary } from './SandboxWelfareBootstrapPlan';
...
const base = Object.freeze({
  ...
  ZHUDATUAN_SANDBOX_WELFARE_DATABASE_URL: `postgresql://zhudatuansandboxbootstrap:${databasePassword}@127.0.0.1:55432/zhudatuan_registration`,
...
describe('sandbox welfare bootstrap plan', () => {
  it('requires Owner confirmation, one explicit member, amount and CNY on the independent test endpoint', () => {
    assert.deepEqual(sandboxWelfareBootstrapEnvironment(base), {
      amountMinor: 100,
      confirmation: SANDBOX_WELFARE_CONFIRMATION,
      connectionString: base.ZHUDATUAN_SANDBOX_WELFARE_DATABASE_URL,
      currency: 'CNY',
      expectedDatabase: 'zhudatuan_registration',
      membership: 'membership:sandbox:one',
  ...
  it('pins 180000 and reports only the explicit non-secret grant', () => {
    ...
    assert.equal(sandboxWelfareBootstrapSummary('membership:one', 100), 'ZHUDATUAN_SANDBOX_WELFARE_READY membership=membership:one amountMinor=100 currency=CNY sandbox=true');
  });
});
