import { chineseReference, queryCondition, hasFailureCode, presentError, safeQueryError } from '@shop/presentation';
...
export function SupportWorkbench() {
  ...
  const direct = useQuery({
    queryKey: ['console', context.scope.id, 'support.ticket', caseId],
    queryFn: ({ signal }) => gateway.queue(context, { limit: 1, keyword: caseId! }, signal),
  ...
  const history = useInfiniteQuery({
    queryKey: ['console', context.scope.id, caseId, 'support.history'],
    initialPageParam: undefined as string | undefined,
    queryFn: ({ pageParam, signal }) => gateway.history(context, caseId!, pageParam, signal),
    getNextPageParam: (page) => page.nextCursor,
  ...
  const reconcile = useCallback(
    (event: SupportEvent) => {
      ...
      cache.setQueriesData<InfiniteData<TicketPage, string | undefined>>({ queryKey: queuePrefix }, (current) =>
        current
          ? {
              ...current,
              pages: current.pages.map((page) => ({
                ...
                items: page.items.map((item) =>
                  item.id === event.ticketId
                    ? {
                        ...item,
                        updated_at: event.occurredAt,
                        version: Math.max(item.version, event.version ?? item.version),
                        unread_count: event.type === 'support.message.sent' && event.ticketId !== caseId ? item.unread_count + 1 : item.unread_count,
                      }
                    : item
                ),
              })),
      ...
      if (event.evidenceId && (event.type === 'support.attachment.ready' || event.type === 'support.attachment.rejected')) {
        setUploads((current) =>
          Object.fromEntries(Object.entries(current).map(([key, values]) => [key, values.map((item) => (item.id === event.evidenceId ? { ...item, state: event.type === 'support.attachment.ready' ? 'clean' : 'rejected' } : item))]))
        );
      }
    },
  ...
  useEffect(() => {
    ...
    setConnected(true);
    void eventSource.listen(context, reconcile, controller.signal).catch(() => {
      if (!controller.signal.aborted) {
        setConnected(false);
        void queue.refetch();
        if (caseId) void conversation.refetch();
      }
    });
  ...
  const upload = useMutation({
    mutationFn: async (file: File) => {
      if (!caseId) throw new Error('请先选择工单。');
      return attachmentUploader.execute(context, caseId, file);
    },
    onMutate: (file) => {
      if (caseId) setUploads((current) => ({ ...current, [caseId]: [...(current[caseId] ?? []), { id: `upload:${crypto.randomUUID()}`, name: file.name, state: 'uploading' }] }));
    },
    onSuccess: (value) => {
      if (caseId) setUploads((current) => ({ ...current, [caseId]: [...(current[caseId] ?? []).filter((item) => item.state !== 'uploading'), value] }));
    },
    onError: (cause) => {
      if (caseId) setUploads((current) => ({ ...current, [caseId]: (current[caseId] ?? []).map((item) => (item.state === 'uploading' ? { ...item, state: 'failed', error: errorText(cause) } : item)) }));
    },
  });
  ...
  const read = useCallback(
    (sequence: number) => {
      ...
      void readUpdater.execute(context, ticket.conversation_id, sequence).catch(() => {
        lastRead.current = without(lastRead.current, ticket.conversation_id);
      });
    },
  ...
  return (
    <section className="supportworkspace">
      ...
      />
      {settings ? (
        <SupportSettings gateway={gateway} />
      ) : (
        <>
          <div className="supportdesk" data-selected={ticket ? 'true' : 'false'} data-connected={connected}>
            <SupportQueue
              tickets={tickets}
              filter={filter}
              {...(caseId === undefined ? {} : { selected: caseId })}
              condition={queryCondition({ pending: queue.isPending, fetching: queue.isFetching, error: queue.error, hasData: queue.data !== undefined, empty: tickets.length === 0 })}
              {...(safeQueryError(queue.error) ? { error: safeQueryError(queue.error)! } : {})}
              {...(queue.hasNextPage && queue.data?.pages.at(-1)?.nextCursor ? { nextCursor: queue.data.pages.at(-1)!.nextCursor } : {})}
              path={path}
            ...
            <SupportConversation
              ...
              messages={messages}
              condition={
                caseId === undefined ? 'empty' : queryCondition({ pending: conversation.isPending, fetching: conversation.isFetching, error: conversation.error, hasData: conversation.data !== undefined, empty: messages.length === 0 })
              }
              {...(safeQueryError(conversation.error) ? { error: safeQueryError(conversation.error)! } : {})}
              {...(conversation.data?.pages.at(-1)?.nextCursor ? { nextCursor: conversation.data.pages.at(-1)!.nextCursor } : {})}
              backPath={path}
              draft={caseId ? (drafts[caseId] ?? '') : ''}
              unavailable={unavailable}
              sending={send.isPending && send.variables?.ticketId === caseId}
              {...(send.isPending && send.variables?.ticketId === caseId ? { pending: send.variables } : {})}
              {...(caseId && failed[caseId] ? { failed: failed[caseId] } : {})}
              messageAttachments={latestConversation?.attachments ?? []}
              uploads={caseId ? (uploads[caseId] ?? []) : []}
              onDraft={(value) => {
                if (caseId) setDrafts((current) => ({ ...current, [caseId]: value }));
              }}
              onSend={sendCurrent}
              onRetrySend={() => {
                if (caseId && failed[caseId]) send.mutate(failed[caseId]);
              }}
              onFile={(file) => upload.mutate(file)}
              onEarlier={() => void conversation.fetchNextPage()}
            ...
            <SupportContextPanel
              open={contextOpen}
              {...(ticket === undefined || latestConversation?.context === undefined ? {} : { ticket, context: latestConversation.context })}
              agents={agents.data?.items ?? []}
              busy={ticketAction.isPending}
              {...(safeQueryError(ticketAction.error) ? { error: conflictText(ticketAction.error) } : {})}
              onDismiss={() => setContextOpen(false)}
              onClose={() => ticketAction.mutate({ kind: 'close' })}
              onReopen={() => ticketAction.mutate({ kind: 'reopen' })}
              onAssign={(agent, reason) => ticketAction.mutate({ kind: 'assign', agent, reason })}
          ...
          <HistoryPanel
            open={historyOpen}
            items={historyItems}
            condition={queryCondition({ pending: history.isPending, fetching: history.isFetching, error: history.error, hasData: history.data !== undefined, empty: historyItems.length === 0 })}
            {...(safeQueryError(history.error) ? { error: safeQueryError(history.error)! } : {})}
            {...(history.hasNextPage && history.data?.pages.at(-1)?.nextCursor ? { nextCursor: history.data.pages.at(-1)!.nextCursor } : {})}
...
function conflictText(cause: unknown): string {
  return hasFailureCode(cause, 'VERSION_CONFLICT') ? '工单已被其他客服更新，已刷新最新状态；请确认后重新操作。' : errorText(cause);
}
