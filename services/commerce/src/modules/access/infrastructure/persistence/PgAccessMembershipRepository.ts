...
export class PgAccessMembershipRepository {
  async activeMemberships(database: OperationDatabase, member: string, target: 'console' | 'storefront'): Promise<readonly ActiveMembershipReference[]> {
    const result = await database.query<{ id: string; client: string; organization_id: string; access_version: number }>(
  ...
  async lockSession(database: OperationDatabase, membership: string, target: 'console' | 'storefront'): Promise<number | null> {
    const result = await database.query<{ access_version: number }>(
  ...
  async directoryMemberships(database: OperationDatabase, memberships: readonly string[]): Promise<readonly DirectoryMembershipReference[]> {
    const result = await database.query<{ id: string; principal_id: string; client: string }>(
  ...
  async ensureImported(database: OperationDatabase, input: Readonly<{ membership: string; member: string; principal: string; organization: string; client: 'operator' | 'storefront'; employee: string | null }>): Promise<void> {
  ...
  async activeMember(database: OperationDatabase, membership: string): Promise<string | null> {
    const result = await database.query<{ member_id: string }>(`select member_id from access.membership where id=$1 and status='active'`, [membership]);
    return result.rows[0]?.member_id ?? null;
  }

  async activeMemberIn(database: OperationDatabase, member: string, organizations: readonly string[]): Promise<boolean> {
  ...
  async memberPage(database: OperationDatabase, organization: string, after: string | null, limit: number): Promise<readonly MemberRecord[]> {
  ...
  async memberProfile(database: OperationDatabase, membership: string): Promise<MemberRecord | null> {
    const result = await database.query<MemberRow>(
      `select id,member_id,organization_id,employee_no,status,access_version,joined_at
  ...
  async setEmployeeNumber(database: OperationDatabase, membership: string, employee: string | null): Promise<boolean> {
  ...
  async managementMember(database: OperationDatabase, membership: string): Promise<Readonly<{ member: string; accessVersion: number }> | null> {
    const result = await database.query<{ member_id: string; access_version: number }>(
  ...
  async setMembershipStatus(database: OperationDatabase, membership: string, status: 'active' | 'suspended' | 'left'): Promise<boolean> {
  ...
  async applyDirectoryState(database: OperationDatabase, input: Readonly<{ membership: string; status: 'active' | 'suspended' | 'left' }>): Promise<number | null> {
    const changed = await database.query<{ access_version: number }>(
      `update access.membership set status=$2,
    ...
    const current = changed.rows[0] ?? (await database.query<{ access_version: number }>('select access_version from access.membership where id=$1 for update', [input.membership])).rows[0];
    return current ? Number(current.access_version) : null;
  }

  async replaceDirectoryDepartment(database: OperationDatabase, input: Readonly<{ membership: string; department: string; grant: string; accessVersion: number }>): Promise<void> {
    await database.query(`delete from access.scopegrant where membership_id=$1 and scope_kind='department'`, [input.membership]);
    await database.query(
      `insert into access.scopegrant(id,membership_id,scope_kind,scope_id,scope_path,effect,effective_at,
