  ...
  return {
    'inventory.imports.create': operationLifecycle({
      ...
      execute: async (_request, database, prepared) => {
        ...
        const result = await database.query(`insert into inventory.importjob(id,scope_id,object_ref,sha256,state,created_at,updated_at)
    ...
    'inventory.imports.read': operationLifecycle({
      execute: async (request, database) => {
        ...
        return rowResult(await database.query(`select job.id,job.state,job.total_count,job.cursor_value,job.success_count,job.failure_count,
          ...
          from inventory.importjob job where job.id=$1 and job.scope_id=$2`, [id, access.scope.id]));
      },
