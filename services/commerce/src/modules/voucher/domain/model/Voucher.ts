...
export class Voucher {
  constructor(readonly value: VoucherValue) {
    if (
      !Number.isSafeInteger(value.initialMinor) ||
      !Number.isSafeInteger(value.remainingMinor) ||
      value.initialMinor <= 0 ||
      value.remainingMinor < 0 ||
      value.remainingMinor > value.initialMinor ||
      !Number.isFinite(value.startsAt.getTime()) ||
      !Number.isFinite(value.expiresAt.getTime()) ||
      value.expiresAt <= value.startsAt
    )
      throw new DomainError('VOUCHER_STATE_INVALID');
  }
  bind(holder: string): Voucher {
    new BindingPolicy().bind(this.value, holder);
    return this.move('bound', { holder });
  }
  unbind(): Voucher {
    new BindingPolicy().unbind(this.value);
    return this.move('available', { holder: null });
  }
  ...
  hold(): Voucher {
    if (this.value.state !== 'active') throw new DomainError('VOUCHER_NOT_USABLE');
    return this.move('held');
  }
  release(now: Date): Voucher {
    if (this.value.state !== 'held') throw new DomainError('VOUCHER_HOLD_CONFLICT');
    return this.move(now >= this.value.expiresAt ? 'expired' : 'active');
  }
  ...
  refund(amount: number): Voucher {
    if (!['redeemed', 'active', 'reversed'].includes(this.value.state) || !Number.isSafeInteger(amount) || amount <= 0 || this.value.remainingMinor + amount > this.value.initialMinor)
      throw new DomainError('VOUCHER_REFUND_EXCEEDS_REDEMPTION');
  ...
  private move(state: VoucherState, change: Partial<VoucherValue> = {}): Voucher {
    return new Voucher(Object.freeze({ ...this.value, ...change, state, version: this.value.version + 1 }));
  }
}
