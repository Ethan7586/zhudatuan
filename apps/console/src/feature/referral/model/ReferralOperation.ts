...
export function createReferralCommand(values: ReferralActionValues): ReferralCommand {
  ...
  if (action.kind === 'product') {
    return Object.freeze({
      ...
      expectedVersion: action.item.version,
      input: Object.freeze({ path: Object.freeze({ productid: action.item.productId }), body: Object.freeze({ enabled: values.enabled, rateBasisPoints: values.rateBasisPoints, expectedVersion: action.item.version, reason }) }),
    });
  }
  return Object.freeze({
