...
describe('frozen reporting query', () => {
  it('anchors a realtime day to the snapshot IANA timezone across the spring DST boundary', async () => {
    ...
    try {
      ...
      `);
      const rows = await withReadTransaction(async (sql, values) => {
        const selected = await database.query<Record<string, unknown>>(sql, values ? [...values] : []);
        return result(selected.rows);
      }, (context) => new PgReportRepository(new PgTransactionAccess()).metrics(context, {
        scope: 'mall:one',
        dimension: 'sales',
        period: 'realtime',
        application: null,
      ...
      expect(rows[0]).toMatchObject({ value: 100, currency: 'USD', period: {
