...
describe('runtime itemized import receipts', () => {
  it('persists domain preflight failures with the normalized chunk before marking the import ready', async () => {
    ...
    const process = new RuntimeBatchImportProcess({
      owner: 'catalog',
      failure: 'CATALOG_ROW_INVALID',
      transactions: transactionManager(async () => result([])),
      runtime: runtime as unknown as ImportRuntimeCoordinator,
      authorization: { assert: vi.fn(async () => undefined) },
      ...
      [
        {
          sequence: 0,
          rows: [
            { row: 2, payload: { sku: 'GOOD' } },
            { row: 3, payload: { invalid: 'CATALOG_SKU_INVALID' } },
          ],
        },
      ],
...
function target(): ImportTarget {
  return Object.freeze({ id: 'import:one', scope: 'scope:one', reference: 'object:one', sha256: 'a'.repeat(64), state: 'running', authorization: Object.freeze({ actor: 'actor:one' }), confirmed: true });
}
