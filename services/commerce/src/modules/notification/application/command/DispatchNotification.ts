...
export class DispatchNotification {
  ...
  async dispatch(id: string): Promise<void> {
    const selected = (await this.repository.claim(id)).rows[0];
    if (!selected) return;
    ...
    try {
      ...
      await this.repository.fail(selected, selected.channel, deliveryError(cause));
      throw cause;
    }
  }

  async challenge(id: string): Promise<void> {
    const selected = (await this.repository.challenge(id)).rows[0];
    if (!selected) return;
    ...
    try {
      [code, recipient] = await Promise.all([
        this.kms.decrypt('pii', 'identity/challenge', selected.code_ciphertext, { challenge: id, purpose: selected.purpose }),
        this.kms.decrypt('pii', 'identity/destination', selected.destination_ciphertext, { challenge: id, purpose: selected.purpose }),
      ]);
    } catch (cause) {
