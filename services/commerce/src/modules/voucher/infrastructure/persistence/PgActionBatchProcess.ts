...
export class PgActionBatchProcess implements ActionBatchWork {
  ...
  async maintain(input: Parameters<ActionBatchWork['maintain']>[0]): Promise<void> {
    for (;;) {
      ...
      const changed = await this.manager.write(
        {
          tenant: 'organization-platform-root',
          membership: '',
          scope: 'organization-platform-root',
          actor: 'system:voucher',
          trace: input.job,
          operation: 'job.voucher.maintenance',
          workload: 'jobs',
          signal: input.signal,
        ...
        async (context) => {
          ...
          for (const hold of holds.rows) {
            await this.tenders.release({ context, scope: hold.scope_id, hold: hold.id, ifActive: true, expiredOnly: true, reason: 'holdexpiry', actor: context.actor, now: new Date() });
          }
...
async function applyItem(database: SqlExecutor, input: Parameters<ActionBatchWork['execute']>[0], batch: BatchRow, row: VoucherRow): Promise<void> {
  ...
  try {
    ...
    await database.query(`update voucher.voucher set holder_id=$3,state=$4,expires_at=$5,version=$6 where id=$1 and scope_id=$2 and version=$7`, [
      row.id,
      input.scope,
      batch.action === 'void' ? null : changed.value.holder,
      changed.value.state,
      changed.value.expiresAt,
    ...
    await database.query(
      `update voucher.actionitem set state='succeeded',previous_state=$3,next_state=$4,error_code=null,retryable=false,updated_at=clock_timestamp()
      where batch_id=$1 and voucher_id=$2 and state='queued'`,
      [batch.id, row.id, row.state, changed.value.state]
    );
    ...
    await database.query(
      `update voucher.actionitem set state='failed',previous_state=$3,next_state=null,error_code=$4,retryable=$5,updated_at=clock_timestamp()
      where batch_id=$1 and voucher_id=$2 and state='queued'`,
      [batch.id, row.id, row.state, cause.code, retryable(cause.code)]
    );
    await database.query('release savepoint voucheractionitem');
  }
}
...
async function settle(database: SqlExecutor, input: Parameters<ActionBatchWork['execute']>[0], batch: BatchRow): Promise<boolean> {
  ...
  await database.query(
    `update voucher.actionbatch set state=case when $3>0 then 'failed' else 'completed' end,processed=$4,succeeded=$5,failed=$3,retryable=$6,version=version+1
    where id=$1 and scope_id=$2 and state='running'`,
    [batch.id, input.scope, progress.failed, progress.processed, progress.succeeded, progress.retryable]
  );
  return true;
}
...
async function progressOf(database: SqlExecutor, batch: string): Promise<Progress> {
  return (
    (
     ...
     from voucher.actionitem where batch_id=$1`,
        [batch]
      )
...
function requiredExpiry(value: Date | null): Date {
  if (!value) throw new DomainError('VOUCHER_STATE_INVALID');
  return new Date(value);
}
function retryable(code: string): boolean {
  return ['VERSION_CONFLICT', 'VOUCHER_HOLD_CONFLICT'].includes(code);
}
