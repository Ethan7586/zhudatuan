...
describe('notification outbox delivery', () => {
  it('uses the frozen dispatch snapshot and records permanent failures without retrying the runtime job', async () => {
    ...
    const send = vi.fn(async () => { throw new Error('EMAIL_DELIVERY_REQUEST_INVALID'); });
    ...
    expect(fail).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ template_version: 4 }), 'email.1', 1,
  ...
  it('separates retryable provider failures so Runtime can apply its durable retry policy', async () => {
    ...
    const send = vi.fn(async () => { throw new Error('EMAIL_PROVIDER_UNAVAILABLE'); });
    const command = deliveryCommand({ claim: vi.fn(async () => dispatchRecord()), fail } as unknown as DeliveryRepository, send);

    await expect(command.dispatch('dispatch:one', execution())).rejects.toThrow('EMAIL_PROVIDER_UNAVAILABLE');
    expect(fail).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'email.1', 1,
      expect.objectContaining({ kind: 'retryable', code: 'EMAIL_PROVIDER_UNAVAILABLE', message: expect.stringMatching(/[\u3400-\u9fff]/u) }));
  });

  it('degrades to the next registered strategy after recording a retryable primary failure', async () => {
    const primary = vi.fn(async () => { throw new Error('PRIMARY_UNAVAILABLE'); });
    ...
    const command = new NotificationDeliveryProcess(transactionManager(async () => result([])), repository,
      { decrypt: vi.fn(async () => 'member@example.test') } as unknown as KmsClient, registry);

    await command.dispatch('dispatch:one', execution());

    expect(recordFailure).toHaveBeenCalledWith(expect.anything(), expect.anything(), 'mail.primary', 1,
      expect.objectContaining({ kind: 'retryable', code: 'PRIMARY_UNAVAILABLE', message: expect.stringMatching(/[\u3400-\u9fff]/u) }));
  ...
  it('does not queue unsubscribed marketing and defers enabled notifications through quiet hours', async () => {
    ...
    try {
      ...
      await eventCommand(eventRepository(templateRecord({ purpose: 'marketing', preference_enabled: false }), blockedQueue))
        .event({ job: 'job:event', id: 'event:blocked', type: 'order.paid', payload: { member: 'member:one', order: 'O1' } }, execution());
      expect(blockedQueue).not.toHaveBeenCalled();

      const deferredQueue = queueMock();
      await eventCommand(eventRepository(templateRecord({ quiet_start: '22:00:00', quiet_end: '07:00:00', quiet_timezone: 'Asia/Shanghai' }), deferredQueue))
        .event({ job: 'job:event', id: 'event:quiet', type: 'order.paid', payload: { member: 'member:one', order: 'O1' } }, execution());
  ...
  it('redacts arbitrary provider text before persistence and retry propagation', async () => {
    const fail = vi.fn(async () => false);
    const send = vi.fn(async () => { throw new Error('send failed for member@example.test'); });
    const command = deliveryCommand({ claim: vi.fn(async () => dispatchRecord()), fail } as unknown as DeliveryRepository, send);
...
function dispatchRecord() {
  return Object.freeze({
    id: 'dispatch:one', scope_id: 'mall:one', member_id: 'member:one', template_id: 'template:one', channel: 'email' as const,
    event_type: 'order.paid', template_version: 4, provider_template: 'paid-v4', variable_schema: { order: 'string' as const },
    subject: '支付成功', body: '订单已支付', payload: { order: 'O1' }, recipient_ciphertext: 'ciphertext', recipient_ref: null,
    purpose: 'transactional' as const, mandatory: false, attempt_sequence: 1, max_attempts: 5, preference_enabled: true,
    authorization_state: 'accepted' as const, consent_source: 'member' as const, quiet_start: null, quiet_end: null,
    quiet_timezone: null, preference_version: 1,
  });
}

function eventCommand(repository: DeliveryRepository) {
  return new NotificationDeliveryProcess(transactionManager(async () => result([])), repository, {} as KmsClient, new DeliveryRegistry([]));
}
...
function templateRecord(overrides: Partial<Readonly<{
  purpose: 'transactional' | 'marketing'; preference_enabled: boolean; quiet_start: string | null; quiet_end: string | null; quiet_timezone: string | null;
}>> = {}) {
  return Object.freeze({
    id: 'template:one', scope_id: 'mall:one', channel: 'inapp' as const, event_type: 'order.paid', version: 4,
    variable_schema: { order: 'string' as const }, provider_template: null, subject: '订单 {{order}}', body: '已支付', status: 'active' as const,
    created_at: '2026-09-05T00:00:00Z', purpose: 'transactional' as const, mandatory: false, preference_enabled: true,
    authorization_state: 'accepted' as const, consent_source: 'member' as const, quiet_start: null, quiet_end: null,
    quiet_timezone: null, preference_version: 1, ...overrides,
  });
}
