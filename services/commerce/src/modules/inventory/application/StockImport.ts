...
export async function importStock(database: OperationDatabase, catalog: CatalogSku, scope: string, importid: string, rowNumber: number,
  ...
  const reserved = await database.query<{ quantity: string }>(`select coalesce(sum(quantity),0)::text quantity from inventory.reservation
    where stockitem_id=$1 and state='active' and expires_at>clock_timestamp()`, [existing.rows[0].id]);
...
async function movement(database: OperationDatabase, stock: string, delta: number, importid: string, row: number): Promise<boolean> {
  ...
  const result = await database.query(`insert into inventory.movement(id,stockitem_id,kind,quantity_delta,reference_type,reference_id,occurred_at)
