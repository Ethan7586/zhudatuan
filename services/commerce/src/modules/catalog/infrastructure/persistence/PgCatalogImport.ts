...
import { importProduct } from '../../application/CatalogProductImport';
...
export class PgCatalogImport implements BatchImportPort {
  ...
  async process(target: ImportTarget, signal: AbortSignal): Promise<boolean> {
    ...
    try {
      ...
      const job = await client.query<{ cursor_value: number; total_count: number }>(
        `select cursor_value,total_count from catalog.importjob
        where id=$1 and state in('ready','running') for update`,
        [target.id]
      );
      if (!job.rows[0]) {
      ...
      for (const row of staged.rows) {
        ...
        try {
          await importProduct(client, target.scope, row.payload);
          successes += 1;
        } catch (cause) {
          await client.query('rollback to savepoint importrow');
          failures += 1;
          await client.query(
  ...
  async failures(target: ImportTarget): Promise<readonly ImportFailure[]> {
    const result = await this.pool.query<{ row_number: number; reason_code: string; field: string | null; detail: string }>(
      'select row_number,reason_code,field,detail from catalog.importerror where job_id=$1 order by row_number,reason_code',
      [target.id]
    );
