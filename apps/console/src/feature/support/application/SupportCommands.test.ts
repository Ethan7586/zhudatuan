...
describe('support command access', () => {
  ...
  it('requires the catalog assurance and CSRF before assignment, close and reopen', async () => {
    ...
    expect(gateway.assign).toHaveBeenCalledOnce();
    expect(gateway.close).toHaveBeenCalledOnce();
    expect(gateway.reopen).toHaveBeenCalledOnce();
  });
  ...
  it('uses MFA for agent management and step-up for channel account management', async () => {
    ...
    expect(gateway.manageAgent).toHaveBeenCalledOnce();
    expect(gateway.manageAccount).toHaveBeenCalledOnce();
  });
});

function port() {
  return {
    assign: vi.fn().mockResolvedValue(undefined), close: vi.fn().mockResolvedValue(undefined), reopen: vi.fn().mockResolvedValue(undefined),
    manageAgent: vi.fn().mockResolvedValue(undefined), manageAccount: vi.fn().mockResolvedValue(undefined),
  } as unknown as SupportPort & Record<'assign' | 'close' | 'reopen' | 'manageAgent' | 'manageAccount', ReturnType<typeof vi.fn>>;
}
...
function context(operations: readonly OperationId[], level = 3, csrf = true): ConsoleContext {
  ...
  return { session: { actor: 'actor:support', membership: 'membership:support', accessVersion: 7, permissions: [...new Set(policies.flatMap(({ permission }) => permission ? [permission] : []))], capabilities: [...new Set(policies.map(({ capability }) => capability))], assurance: { level }, security: { hasLocalCredential: true, phoneMasked: null, passwordChangedAt: null }, ...(csrf ? { csrf: 'csrf:support:1234' } : {}), target: 'console', syncedAt: '2026-09-04T00:00:00.000Z', scope, scopes: [scope] }, profile: { display_name: '测试客服', employee_no: null }, scope, scopes: [scope] };
}

function ticket(): Ticket {
  return { id: 'case:one', priority: 'high', state: 'assigned', assignedAgentId: 'agent:one', responseDueAt: '2026-09-04T01:00:00.000Z', resolutionDueAt: '2026-09-04T08:00:00.000Z', createdAt: '2026-09-04T00:00:00.000Z', updatedAt: '2026-09-04T00:30:00.000Z', version: 3, conversationId: 'conversation:one', skill: 'refund', memberId: 'member:one', orderId: 'order:one', channel: 'inapp', subject: '订单退款咨询', referenceType: 'order', referenceId: 'order:one', unreadCount: 1, slaRisk: 'risk' };
}
