...
export class PgCredentialPoolRepository implements CredentialPoolRepository {
  ...
  async create(call: Parameters<CredentialPoolRepository['create']>[0]) {
    const database = this.transactions.database(call.context.transaction); const value = body(call);
    const product = text(value.product, 'product'); const mode = value.mode === 'imported' ? 'imported' : value.mode === 'generated' ? 'generated' : null;
    if (!mode) throw new DomainError('VALIDATION_FAILED', { field: 'mode' });
    const aggregate = new CredentialPool({ id: entity('credentialpool'), scope: call.scope, product, name: text(value.name, 'name'), mode,
      prefix: text(value.prefix, 'prefix').toUpperCase(), capacity: Number(value.capacity), generated: 0, state: 'open', version: 1 });
    ...
    await database.query(`insert into voucher.credentialpool(id,number,scope_id,product_id,name,mode,prefix,capacity,generated,state,version,created_at,updated_at)
      values($1,$2,$3,$4,$5,$6,$7,$8,0,'open',1,$9,$9)`, [aggregate.value.id, number('CP'), call.scope, product, aggregate.value.name, mode, aggregate.value.prefix, aggregate.value.capacity, call.now]);
    const changed = await database.query(`update voucher.product set pool_id=$3,version=version+1 where id=$1 and scope_id=$2 returning version::integer`,
      [product, call.scope, aggregate.value.id]);
  ...
  async close(call: Parameters<CredentialPoolRepository['close']>[0]) {
    const database = this.transactions.database(call.context.transaction); const id = path(call, 'poolid');
  ...
  async get(call: Parameters<CredentialPoolRepository['get']>[0]) { return this.read(call, path(call, 'poolid'), 200, 'voucher.credentialpools.get'); }
  async list(call: Parameters<CredentialPoolRepository['list']>[0]) {
    ...
    const rows = await database.query(`${POOL} where pool.scope_id=$1 and ($2::text is null or pool.product_id=$2) and ($3::text is null or pool.state=$3)
      and ($4::text is null or pool.id>$4) group by pool.id order by pool.id limit $5`, [call.scope, optionalText(filter.product), optionalText(filter.state), cursor(filter.cursor), fetch + 1]);
