...
import { executeE03Authorization } from './e03-authorization.evidence.ts';
import { finalizeE03Evidence } from './e03-authorization.formal.mjs';

const identifier = randomUUID().replaceAll('-', '').slice(0, 16);
const runToken = identifier.slice(0, 8);
const container = `zhudatuan-e03-${identifier}`;
const databaseName = 'zhudatuan_registration';
const password = `AuthorizationEvidence${identifier}A`;
const criteriaPath = join(repositoryRoot, '05_docs_ziliao', 'docs_wendang', 'architecture', 'evidence', 'e2.0',
  'E2.0-E03-正式重测判据-2026-09-14.json');
...
if (Date.parse(criteria.locked_at) >= Date.parse(startedAt)) throw new Error('E03_CRITERIA_NOT_LOCKED_BEFORE_EXECUTION');
await verifyCriteriaSources(criteria);
if (formalExecution && sourceControl.tree_state !== 'CLEAN') throw new Error('E03_EVIDENCE_REQUIRES_CLEAN_TREE');

const evidenceDirectory = formalExecution
  ? resolve(repositoryRoot, evidenceArgument)
  : await mkdtemp(join(tmpdir(), 'zhudatuan-e03-evidence-'));
if (formalExecution) {
...
try {
  ...
  await run('docker', ['run', '-d', '--rm', '--name', container, '-e', `POSTGRES_PASSWORD=${password}`,
    '-e', `POSTGRES_DB=${databaseName}`, '-p', '127.0.0.1::5432', 'postgres:17-alpine'], { quiet: true });
  try {
    await waitForPostgres();
    const port = await postgresPort();
    const databaseUrl = `postgresql://postgres:${password}@127.0.0.1:${port}/${databaseName}`;
    const migrationOutput = await captureText(join(repositoryRoot, 'node_modules', '.bin', 'tsx'), [
    ...
    try {
      executionSummary = await executeE03Authorization(database, databaseUrl, criteria, runToken, evidenceDirectory);
      ...
      environment = await environmentEvidence({
        capturedAt: new Date().toISOString(), postgresVersion, dockerVersion, imageId, migrationCount, port,
      });
    ...
    await run('node', ['04_tools/scripts/audit/e03-authorization.oracle.mjs', '--run-directory', evidenceDirectory,
      '--criteria', criteriaPath, '--output', join(evidenceDirectory, 'authorization-independent-recount.json')]);
  } finally {
  ...
  if (formalExecution) {
    const result = await finalizeE03Evidence({
      evidenceDirectory, criteriaPath, environment, sourceControl, startedAt, completedAt, container, executionSummary,
    });
    console.log(`SFL E03 PostgreSQL 17 formal evidence completed: ${repositoryPath(evidenceDirectory)} outcome=${result.oracle.claim_outcome}`);
  } else {
    const oracle = await readJson(join(evidenceDirectory, 'authorization-independent-recount.json'));
    if (oracle.claim_outcome !== 'MET') {
      throw new Error(`E03_ACCEPTANCE_${oracle.claim_outcome.replaceAll(' ', '_')}:${JSON.stringify({
        missing: oracle.missing_items,
        contradictions: oracle.contradictions,
        thresholds: oracle.thresholds?.filter((entry) => !entry.met),
      })}`);
    }
    console.log(`SFL E03 PostgreSQL 17 acceptance passed: truth_rows=${executionSummary.truth_table_row_count} surface_rows=${executionSummary.surface_observation_count} raw_cases=${executionSummary.raw_case_decision_count}`);
  }
} finally {
...
async function environmentEvidence(input) {
  ...
  return Object.freeze({
    schema_version: 'e03-environment-v1',
    captured_at: input.capturedAt,
    kind: 'DEV',
    formal_execution: formalExecution,
    environment_id: `local-disposable-postgresql17-e03-${identifier}`,
    description: 'A newly created PostgreSQL 17 Alpine container replayed the complete production migration chain and executed the locked 32-row E03 matrix through four loopback HTTP authorization surfaces.',
    owner: 'Ethan-controlled local Codex workspace',
    isolation: `Disposable Docker container ${container}; synthetic E03 Principal, Membership, NodeContext, order, Outbox and task rows only; the container was removed after execution.`,
    real_customer_data: false,
    production_impact: 'None. No production service, database, domain, deployment channel, customer record or external Provider was contacted.',
    differences_from_production: [
      'Local disposable Docker PostgreSQL rather than managed Alibaba Cloud PostgreSQL.',
      'Synthetic resolver-backed Principal, Membership and NodeContext fixtures rather than public Session and Membership ingress.',
      'Page, API, task and service were isolated loopback HTTP probe routes around the production AccessPipeline, not deployed customer-facing adapters or real traffic.',
      'The service Provider was deterministic and in-process; no real external Provider was contacted.',
    ...
    configuration_digest: `sha256:${sha256(Buffer.from([
      ...
      JSON.stringify(criteria.dimensions),
      JSON.stringify(criteria.node_membership_matrix),
      JSON.stringify(criteria.operation_matrix),
      JSON.stringify(criteria.surfaces),
    ].join('\n')))}`,
  });
}
...
async function verifyCriteriaSources(criteriaValue) {
  for (const source of [...criteriaValue.authoritative_basis, ...criteriaValue.implementation_basis,
    ...
    if (actual !== source.sha256) throw new Error(`E03_BASIS_DIGEST_MISMATCH:${source.path}:${actual}`);
  }
}
...
async function waitForPostgres() {
  ...
  throw new Error('E03_POSTGRES_NOT_READY');
}
...
async function postgresPort() {
  ...
  if (!port || !/^\d+$/.test(port)) throw new Error(`E03_POSTGRES_PORT_INVALID:${output.trim()}`);
  return port;
}
...
function assertRepositoryEvidencePath(path) {
  const local = repositoryPath(path);
  ...
  if (local.startsWith('..') || local === evidenceRoot || !local.startsWith(`${evidenceRoot}/`)) {
    throw new Error(`E03_EVIDENCE_DIRECTORY_INVALID:${local}`);
  }
}

async function createEvidenceDirectory(path) {
  const exists = await stat(path).then(() => true, () => false);
  if (exists) throw new Error(`E03_EVIDENCE_DIRECTORY_EXISTS:${repositoryPath(path)}`);
