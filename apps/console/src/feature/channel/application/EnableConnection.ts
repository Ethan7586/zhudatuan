...
export class EnableConnection {
  ...
  execute(context: ConsoleContext, connection: string, version: number, proof: string, identity: string, signal?: AbortSignal) {
    assertOperationAccess(context, channelOperations.enable, proof);
    return this.port.enable(context, connection, version, proof, identity, signal);
  }
}
