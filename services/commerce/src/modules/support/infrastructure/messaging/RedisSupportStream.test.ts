...
const event = { id: 'outbox:one', type: 'support.message.sent', scopeId: 'mall:one', ticketId: 'ticket:one', conversationId: 'conversation:one', memberId: 'member:one', messageId: 'message:one', sequence: 2, version: 3, occurredAt: '2026-09-02T00:00:00.000Z' } as const;
...
describe('RedisSupportStream', () => {
  ...
  it('filters storefront events by member and conversation', async () => {
    const stream = new RedisSupportStream({ read: async function* () { yield { id: '1-0', stream: 'mall:one', value: JSON.stringify({ ...event, memberId: 'member:other' }) }; yield { id: '2-0', stream: 'mall:one', value: JSON.stringify(event) }; } } as never);
    const received = [];
  ...
  it('fails closed if a stream adapter yields an event outside the authorized scopes', async () => {
    const stream = new RedisSupportStream({ read: async function* () { yield { id: '1-0', stream: 'mall:foreign', value: JSON.stringify({ ...event, scopeId: 'mall:foreign' }) }; yield { id: '2-0', stream: 'mall:one', value: JSON.stringify(event) }; } } as never);
    const received = [];
  ...
  it('maps expired cursors to the public resync error', async () => {
    const stream = new RedisSupportStream({ read: async function* () { throw new Error('SUPPORT_EVENT_CURSOR_EXPIRED'); } } as never);
    const consume = async () => { for await (const _value of stream.read({ scopes: ['mall:one'], member: 'member:one', storefront: false, conversation: null, cursor: 'old', signal: new AbortController().signal })) void _value; };
  ...
  it('validates an expired cursor before the HTTP stream commits', async () => {
    const validate = vi.fn(async () => { throw new Error('SUPPORT_EVENT_CURSOR_EXPIRED'); });
