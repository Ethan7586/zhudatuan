...
export function createImportProcess(factory: ImportBatchFactoryPort, manager: TransactionManager, runtime: ImportPort, jobs: JobPort,
  inventory: InventoryImportRepository, authorization: TaskAuthorizationPort): ImportProcessPort {
  return factory.create({
    owner: 'inventory', failure: 'INVENTORY_IMPORT_ROW_FAILED', transactions: manager, runtime, authorization,
    ...
    continue: (context, target, sequence) => jobs.create(context, {
      idempotency: `${target.id}:${sequence}`, kind: 'inventoryimport', owner: 'inventory', scope: target.scope, queue: 'import',
      payload: Object.freeze({ import: target.id }), actor: 'system:inventory',
    }).then(() => undefined),
  });
}
