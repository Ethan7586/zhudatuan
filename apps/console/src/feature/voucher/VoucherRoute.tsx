...
import { readVouchers, voucherKey, voucherViews } from './VoucherQuery';
import type { VoucherRecord, VoucherView } from './VoucherSchema';
import { VoucherTable } from './VoucherTable';
import './voucher-workspace.css';
import './voucher-table.css';
import './voucher-dialogs.css';
import './voucher-responsive.css';
...
export function Component() {
  ...
  const rows = useMemo(() => (data?.items ?? []).filter((record) => {
  ...
  const selectView = (nextView: VoucherView) => updateSearch((next) => {
  ...
  const updateFilter = (key: 'q' | 'status', value: string) => updateSearch((next) => {
    if (value === '' || value === 'all') next.delete(key);
    else next.set(key, value);
    next.delete('cursor');
    next.delete('selected');
  });
  const openRecord = useCallback((record: VoucherRecord) => {
    ...
    <div className="voucherworkspace" data-view={view}>
      <ResourcePanel
        ...
        actions={<>
          ...
          <Button tone="primary" onPress={() => { closeRecord(); setCreatorOpen(true); }}>新建卡券</Button>
        </>}
      >
        <div className="vouchercontent">
          <section className="voucherownership" role="note">
            <span aria-hidden="true">域</span>
            <div><strong>当前网站归属：{scopeName}</strong><p>卡券定义、卡号与发行记录按当前 Scope 隔离；网站会员仅领取和使用，不进入本治理台。</p></div>
          </section>
          ...
          <section className="voucherboundarybanner" role="note">
            ...
            <div><strong>高风险写操作继续关闭</strong><p>创建、审批、发行、暂停、作废与冲正需完整 Preview / proof 旅程；当前预览不会改动会员资产。</p></div>
            <button type="button" onClick={() => setCreatorOpen(true)}>查看流程</button>
          </section>

          <ol className="voucherlifecycle" aria-label="卡券生命周期">
            {voucherLifecycle.map((item, index) => <li key={item.key} className={`is-${item.key}`}>
              <span>{index + 1}</span><div><strong>{item.label}</strong><small>{item.role}</small></div>
            </li>)}
          </ol>

          <section className="voucherboard" aria-labelledby="voucherviewtitle">
            <nav className="vouchertabs" aria-label="卡券数据视图">
              {voucherViews.map((candidate) => <button key={candidate} type="button" aria-pressed={candidate === view}
            ...
            <div className="voucherfilterbar">
              <div className="voucherfiltercopy"><strong id="voucherviewtitle">{voucherViewMeta[view].label}</strong><span>{voucherViewMeta[view].description}</span></div>
              <label className="vouchersearch"><span className="sr-only">搜索当前卡券视图</span><input type="search" value={search.get('q') ?? ''}
                onChange={(event) => updateFilter('q', event.target.value)} placeholder="搜索名称、编号或详情" /></label>
              <label className="voucherstatusfilter"><span className="sr-only">按状态筛选</span><select value={status}
                onChange={(event) => updateFilter('status', event.target.value)}>
                <option value="all">全部状态</option>
                {states.map((state) => <option key={state} value={state}>{voucherStateLabel(state)}</option>)}
              </select></label>
            </div>
            <p className="voucherfiltermeta">当前页筛选 · 显示 {rows.length} / {data?.items.length ?? 0} 条 · 未并发读取其他卡券模型</p>
            {data !== undefined && data.items.length === 0 ? <section className="voucherfilteredempty" role="status">
              <strong>暂无{voucherViewMeta[view].label}记录</strong><p>当前网站范围尚未返回这一读模型的数据，可切换其他卡券视图。</p>
            </section> : null}
            {data !== undefined && data.items.length > 0 && rows.length === 0 ? <section className="voucherfilteredempty" role="status">
              <strong>当前页没有匹配记录</strong><p>调整搜索词或状态后即可恢复列表。</p>
              <button type="button" onClick={() => clearFilters(search, setSearch)}>清除筛选</button>
            </section> : null}
            ...
            <footer className="voucherpagination">
              ...
              <Button onPress={() => { if (data?.nextCursor !== undefined) setSearch(pageCursor(search, data.nextCursor)); }}
                isDisabled={data?.nextCursor === undefined}>下一页</Button>
            </footer>
          </section>
        </div>
      </ResourcePanel>
      <VoucherRecordDrawer record={selected} view={view} onClose={closeRecord} />
      <VoucherCreatorPreview open={creatorOpen} onClose={() => setCreatorOpen(false)} />
    </div>
  );
}
