...
export function marketingOperations(context: ModuleContext): ModuleOperations {
  ...
  return new ModuleOperations('marketing', pool, context.container.get(AUDIT_SINK), {
    'marketing.campaigns.read': async (request, database) => {
      const access = requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select id,kind,name,state,budget_minor,spent_minor,currency,effective_at,expires_at,version,updated_at
