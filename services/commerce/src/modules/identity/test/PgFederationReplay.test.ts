  ...
  it('distinguishes a consumed Wechat/OIDC state from an unknown state without reopening the transaction', async () => {
    ...
    await expect(
      withReadTransaction(
        async () => {
          query += 1;
          return (query === 1 ? { rows: [], rowCount: 0 } : { rows: [{ status: 'completed', expires_at: new Date('2099-01-01T00:00:00.000Z'), consumed_at: new Date('2026-09-04T00:00:00.000Z') }], rowCount: 1 }) as unknown as QueryResult;
        },
        (context) => repository.pending(context, 'provider:wechat', Buffer.alloc(32, 7))
      )
    ).rejects.toThrow('FEDERATION_TRANSACTION_CONSUMED');
    expect(query).toBe(2);
  });
});
