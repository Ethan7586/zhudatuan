...
export interface ExportSnapshotEvidence {
...
export async function captureExportSnapshot(database: SqlExecutor, input: Capture): Promise<void> {
  ...
  const previous = await database.query<{ matches: boolean }>(
    `select kind=$3 and source_filter=$4::jsonb and created_by=$5 matches
    from voucher.exportsnapshot where id=$1 and scope_id=$2`,
    values
  );
  if (previous.rows[0]) {
  ...
  const captured = await database.query<{ count: number }>(
    ...
    select count(*)::integer count from items`,
    [...values, source.reference, source.watermark, RUNTIME_LIMITS.voucherExport.snapshotTtlSeconds, CAPACITY_MODEL.voucherBatch + 1]
  );
  if (!captured.rows[0] || captured.rows[0].count > CAPACITY_MODEL.voucherBatch) throw new DomainError('VALIDATION_FAILED', { field: 'filter', reason: 'snapshotlimit' });
}

export async function readExportSnapshot(database: SqlExecutor, work: RuntimeExportWork): Promise<ExportSnapshotEvidence> {
  const row = (
    await database.query<{ count: number; expires: Date }>(
      `select result_count::integer count,expires_at expires
    from voucher.exportsnapshot where id=$1 and scope_id=$2 and kind=$3 and source_filter=$4::jsonb and created_by=$5
      and expires_at>clock_timestamp()`,
      [work.id, work.scope, work.kind, JSON.stringify(work.snapshot), work.authorization.actor]
    )
  ).rows[0];
...
export async function readExportPage(database: SqlExecutor, work: RuntimeExportWork, cursor: string | null, limit: number): Promise<readonly ExportPage[]> {
  ...
  const rows = await database.query<{ ordinal: number; cells: readonly unknown[]; credential: string | null; pool: string | null; number: string | null; secret: string | null }>(
    ...
    [work.id, work.scope, cursor === null ? null : Number(cursor), limit]
  );
...
export async function pruneExportSnapshots(database: SqlExecutor): Promise<number> {
  const items = await database.query(
    `with expired as(select item.snapshot_id,item.ordinal from voucher.exportsnapshotitem item
  ...
  const snapshots = await database.query(
    `with expired as(select snapshot.id from voucher.exportsnapshot snapshot
    ...
    delete from voucher.exportsnapshot snapshot using expired where snapshot.id=expired.id`,
    [RUNTIME_LIMITS.voucherExport.pageRows]
  );
...
function sourceOf(input: Capture) {
  if (input.kind === 'credential')
    return {
      reference: required(input.filter.pool),
      watermark: required(input.filter.watermark),
      sql: `
  ...
  if (input.kind === 'issueorder')
    return {
      reference: required(input.filter.order),
      watermark: null,
      sql: `
  ...
  return {
    reference: required(input.filter.batch),
    watermark: null,
    sql: `
