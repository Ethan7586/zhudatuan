...
export class CreateInvitationGrant {
  ...
  async execute(context: ReadTransactionContext, input: InvitationPlanInput): Promise<BuiltGrant> {
    const issuer = await this.repository.delegationIssuer(context, input.issuer);
    ...
    const target = input.membership === null ? null : await this.repository.delegationTarget(context, input.membership);
    ...
    const organizationScope = await this.organizations.invitationScope(context, organization);
    const partnerScope = organizationScope ? null : await this.partners.invitationScope(context, organization);
    ...
    const roles = await this.roles(context, input, target);
    ...
    const grantsRead = input.membership === null ? Promise.resolve(Object.freeze([])) : this.repository.delegationScopes(context, input.membership);
    const permissionsReadForIssuer = this.authorization.permissions(context, input.issuer);
    const scopesReadForIssuer = this.authorization.scopes(context, input.issuer);
    ...
    const scopeAllowed = await this.organizations.delegationAllowed(context, {
  ...
  private roles(context: ReadTransactionContext, input: InvitationPlanInput, target: DelegationTarget | null): Promise<readonly DelegationRole[]> {
    if (input.kind === 'campaign') return this.repository.campaignRoles(context);
    if (!target) throw new DomainError('MEMBERSHIP_NOT_INVITED');
    return this.repository.delegationRoles(context, target.id);
  }
}
