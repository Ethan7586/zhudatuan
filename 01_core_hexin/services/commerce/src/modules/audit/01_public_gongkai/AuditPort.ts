...
import type { AccessRecord } from '../02_domain_yewu/model/AccessRecord';
import type { AuditRecord } from '../02_domain_yewu/model/AuditRecord';

export interface ArchiveBatch { readonly scope: string; readonly start: string; readonly end: string; readonly firstHash: string;
  readonly lastHash: string; readonly rows: readonly Readonly<Record<string, unknown>>[]; readonly recordIds: readonly string[];
  readonly accessIds: readonly string[]; readonly archiveYears: number }
...
export interface AuditPort {
  ...
  completeArchive(database: AuditDatabase, batch: ArchiveBatch, object: Readonly<{ reference: string; sha256: string; size: number;
