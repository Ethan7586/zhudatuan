...
export function ApprovalDialog({ model, onClose }: Readonly<{ model: VoucherActionViewModel; onClose: () => void }>) {
  ...
  return (
    <Dialog open title="审批备券申请" eyebrow="审批 · 双人复核 · 版本保护" onClose={onClose} dismissable={!model.busy}>
      <form
        onSubmit={(event) => {
      ...
      >
        <div className="vouchercreatorbody">
          <label className="vouchercreatorfield">
            审批结论
            <select value={model.decision} onChange={(event) => model.actions.decision(event.target.value as typeof model.decision)}>
        ...
        <DialogFooter busy={model.busy} disabled={invalid} label="确认审批" onClose={onClose} />
      </form>
    </Dialog>
  );
}
