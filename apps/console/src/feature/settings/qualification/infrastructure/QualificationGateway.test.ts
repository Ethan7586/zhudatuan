...
describe('QualificationGateway', () => {
  it('maps policies and bounded immutable version history', async () => {
    server.use(http.get('https://shop.test/api/v1/qualifications', () => HttpResponse.json({ items: [policy], count: 1 })));
  ...
  it('sends an exact preview command with csrf and stable identity', async () => {
    ...
    await expect(new QualificationGateway('https://shop.test').previewPolicy(context, { kind: 'publish', policy: 'policy:one', name: '员工策略', rule: { effect: 'deny' } }, 'identity:preview')).resolves.toMatchObject({
      nextVersion: 4,
