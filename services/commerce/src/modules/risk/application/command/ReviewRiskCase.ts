...
export function reviewRiskCaseOperations(factory: (database: OperationDatabase) => RiskRepository): OperationActions {
  return {
    'risk.cases.review': async (request, database) => {
      ...
      const current = await repository.riskCase(request.input.path.caseid!, access.scope.id);
      ...
      const reviewed = await repository.reviewCase({ id: current.id, scope: access.scope.id, state, reviewer: access.actor.id, reason: textField(body, 'reason', 1000), evidence, trace: access.trace });
