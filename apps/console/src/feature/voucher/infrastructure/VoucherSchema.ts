...
export const CardLibrarySchema = z
  .object({
    ...
    import_state: z.string().nullable().optional(),
    total_count: NullableInteger.optional(),
    success_count: NullableInteger.optional(),
    failure_count: NullableInteger.optional(),
  })
  .passthrough();
export const VoucherProgramSchema = z
  .object({
    id: z.string().min(1),
    name: z.string().min(1),
    value_minor: DatabaseIntegerSchema,
    currency: z.string().length(3),
    default_valid_days: DatabaseIntegerSchema,
    status: z.string().min(1),
...
export const ReserveSchema = z
  .object({
    ...
    program_version: DatabaseIntegerSchema,
    name: z.string().min(1),
    requested_count: DatabaseIntegerSchema,
  ...
  .passthrough();
export const IssueBatchSchema = z
  .object({
    id: z.string().min(1),
    program_id: z.string().min(1),
    program_version: DatabaseIntegerSchema,
    cardpool_id: z.string().nullable(),
    reserve_request_id: z.string().nullable(),
    name: z.string().min(1),
    state: z.string().min(1),
    requested_count: DatabaseIntegerSchema,
    issued_count: DatabaseIntegerSchema,
    created_at: z.string().min(1),
  })
  .passthrough();
export const StatusBatchSchema = z
  .object({
    ...
    state: z.string().min(1),
    requested_count: DatabaseIntegerSchema,
    succeeded_count: DatabaseIntegerSchema,
    failed_count: DatabaseIntegerSchema,
    created_at: z.string().min(1),
...
export const BindingSchema = z
  .object({
    ...
    initial_minor: DatabaseIntegerSchema,
    remaining_minor: DatabaseIntegerSchema,
    state: z.string().min(1),
