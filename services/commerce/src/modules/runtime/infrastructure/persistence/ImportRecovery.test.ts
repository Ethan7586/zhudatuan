...
describe('runtime import recovery and fencing', () => {
  it('resumes immutable staged chunks without deleting or re-encrypting them', async () => {
    ...
    try {
      ...
      await write(data, (context) =>
        data.importing.stage(context, data.id, 'voucher', [
          {
            sequence: 0,
            rows: [
              { row: 2, payload: { ciphertext: 'encrypted-one' } },
              { row: 3, payload: { ciphertext: 'encrypted-two' } },
            ],
          },
        ])
      );
      ...
      expect(
        (
          await data.database.query(
            `select state,rows_total,checkpoint->>'chunkSize' "chunkSize",checkpoint->>'format' format
        from runtime.imports where id=$1`,
            [data.id]
          )
        ).rows
      ).toEqual([{ state: 'ready', rows_total: 2, chunkSize: '1000', format: 'text/csv' }]);
    } finally {
  ...
  it('binds completion to the active owner, scope and fencing token while preserving counters', async () => {
    ...
    try {
      ...
      expect((await data.database.query(`select state,rows_processed,rows_succeeded,rows_failed from runtime.imports where id=$1`, [data.id])).rows).toEqual([{ state: 'running', rows_processed: 2, rows_succeeded: 1, rows_failed: 1 }]);
      expect((await data.database.query('select row_number,reason_code from runtime.import_errors where import_id=$1', [data.id])).rows).toEqual([{ row_number: 3, reason_code: 'VALIDATION_FAILED' }]);
  ...
  it('releases transient failures for retry and rejects stale workers after the token advances', async () => {
    ...
    try {
      ...
      await expect(
        data.database.exec(`update runtime.import_chunks set payload='[]'::jsonb,version=version+1,updated_at=clock_timestamp()
        where import_id='${data.id}'`)
      ).rejects.toThrow('RUNTIME_IMPORT_CHUNK_IMMUTABLE');
    } finally {
      await data.database.close();
    }
  });
  ...
  it('takes over only an expired lease and increments its fencing token', async () => {
    ...
    try {
      ...
      await data.database.query(
        ...
        'running',1,clock_timestamp()-interval '1 minute','{}'::jsonb,0,1,clock_timestamp()-interval '3 minutes',clock_timestamp()-interval '2 minutes')`,
        [data.id]
      );
      ...
      expect((await data.database.query('select lease_expires_at>clock_timestamp() active from runtime.import_chunks where import_id=$1', [data.id])).rows).toEqual([{ active: true }]);
    } finally {
      await data.database.close();
    }
  });
  ...
  it('allows retention cleanup only after the parent import is terminal and expired', async () => {
    ...
    try {
      ...
      await data.database.query(
        `update runtime.imports set created_at=clock_timestamp()-interval '2 days',
        retention_until=clock_timestamp()-interval '1 day' where id=$1`,
        [data.id]
      );
...
async function fixture() {
  ...
  await withWriteTransaction(query, (context) =>
    importing.create(context, { id, scope: 'scope:test', owner: 'voucher', kind: 'credential', reference: 'object:test', sha256: 'a'.repeat(64), name: 'voucher.csv', mediaType: 'text/csv', size: 10, actor: 'actor:test', authorization: {} })
  );
