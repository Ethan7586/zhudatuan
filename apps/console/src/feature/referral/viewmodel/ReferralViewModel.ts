...
export function useReferralViewModel(context: ConsoleContext, dependencies: ReferralDependencies, requestStepup: () => void) {
  ...
  return Object.freeze({
    ...
    content,
    cursor,
    condition: access.allowed ? queryCondition({ pending: query.isPending, fetching: query.isFetching, error: query.error, hasData: page !== undefined, empty: page?.items.length === 0 }) : 'denied',
