...
import type { DirectorySyncAction, DirectorySyncMode } from '../model/SyncRun';
      ...
      >
        ...
        <footer>
          ...
          <Button type="submit" tone={editor.action === 'cancel' ? 'danger' : 'primary'} isDisabled={model.saving.busy || model.validation !== undefined}>
            {model.saving.busy ? '正在提交…' : editor.action === 'cancel' ? '确认取消任务' : editor.action === 'resume' ? '确认恢复任务' : editor.action === 'preview' ? '生成差异预览' : '确认启动任务'}
          </Button>
        </footer>
      </form>
    </Dialog>
...
function title(action: DirectorySyncAction) {
  return action === 'start' ? '启动通讯录同步' : action === 'preview' ? '预览通讯录差异' : action === 'cancel' ? '取消同步任务' : '恢复同步任务';
}
function impact(action: DirectorySyncAction, mode: DirectorySyncMode) {
  if (action === 'preview') return '读取并计算新增、更新、冻结与恢复数量，但不修改组织、成员、上级关系或供应商游标。';
