...
const runtime = read('services/commerce/src/bootstrap/PaymentJobsRuntime.ts');
const main = read('services/commerce/src/entry/PaymentJobsOnlyMain.ts');
const ready = read('services/commerce/src/entry/PaymentJobsReadyMain.ts');
const environment = read('infrastructure/zhudatuan/aliyun/payment-jobs.env.example');
const service = read('infrastructure/zhudatuan/aliyun/systemd/zhudatuan-payment-jobs.service');
const delivery = read('infrastructure/zhudatuan/aliyun/delivery.yml');
const build = read('scripts/build-commerce.mjs');
...
const requiredService = [
  ...
  'ExecStartPre=/usr/bin/node services/commerce/dist/PaymentJobsReadyMain.js',
  'ExecStart=/usr/bin/node services/commerce/dist/PaymentJobsOnlyMain.js',
  'TimeoutStartSec=30',
