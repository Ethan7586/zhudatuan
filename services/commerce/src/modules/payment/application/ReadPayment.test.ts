...
describe('ReadPayment', () => {
  it('returns only the active controlled action for the owning member', async () => {
    ...
    const read = new ReadPayment({ member: vi.fn(async () => 'member:one') }, { payment: vi.fn(async () => order) });
    await expect(read.execute(database as never, 'membership:one', 'intent:one')).resolves.toEqual({
      intentId: 'intent:one',
      orderId: 'order:one',
  ...
  it('projects an unknown provider outcome as recovery', async () => {
    ...
    const read = new ReadPayment({ member: vi.fn(async () => 'member:one') }, { payment: vi.fn(async () => order) });
    await expect(read.execute(database as never, 'membership:one', 'intent:one')).resolves.toMatchObject({ state: 'recovery', retryAfter: 5 });
  });

  it('hides another member payment as not found', async () => {
    const database = { query: vi.fn(async () => result([])) };
    const read = new ReadPayment({ member: vi.fn(async () => 'member:one') }, { payment: vi.fn() });
    await expect(read.execute(database as never, 'membership:one', 'intent:other')).rejects.toThrow('RESOURCE_NOT_FOUND');
  });
});
