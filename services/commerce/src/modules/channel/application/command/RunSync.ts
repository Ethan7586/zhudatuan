...
export function runSyncOperations(repository: ChannelRepositoryFactory): OperationActions {
  return {
    'channel.syncruns.start': async (request, database) => {
      ...
      const connection = textField(body, 'connection');
      const kind = syncKind(textField(body, 'kind', 64));
      const parameters = kind === 'statement' ? { start: date(body.start), end: date(body.end), timezone: required(body.timezone, 'STATEMENT_TIMEZONE_REQUIRED'), partner: required(body.partner, 'STATEMENT_PARTNER_REQUIRED') } : {};
      const input = JSON.stringify({ connection, kind, cursor: body.cursor ?? null, ...parameters });
      ...
      const accepted = await database.query(
        ...
        [run, connection, kind, body.cursor ?? null, createHash('sha256').update(input).digest('hex'), input, access.scope.id]
      );
    ...
    'channel.syncruns.cancel': async (request, database) => {
      ...
      return rowResult(
        ...
        and run.state in('queued','running') returning run.*`,
          [request.input.path.runid!, access.scope.id]
        )
      );
    },
  };
...
function syncKind(value: string): SyncKind {
  if (!(value in jobs)) throw new Error('SYNC_KIND_INVALID');
