...
export function SyncRunTable({ rows, model }: Readonly<{ rows: readonly DirectorySyncRun[]; model: DirectoryViewModel }>) {
  const columns: readonly DataColumn<DirectorySyncRun>[] = [
    { key: 'mode', label: '模式', render: (row) => `${row.preview ? '差异预览 · ' : ''}${modeLabel(row.mode)}` },
      ...
      render: (row) => (
        <span>
          ...
          <small className="directorysub">
            读取 {row.readCount} · 新增 {row.createCount} · 更新 {row.updateCount} · 冻结 {row.freezeCount} · 恢复 {row.restoreCount} · 冲突 {row.conflictCount} · 忽略 {row.ignoredCount}
          </small>
        </span>
      ),
    },
