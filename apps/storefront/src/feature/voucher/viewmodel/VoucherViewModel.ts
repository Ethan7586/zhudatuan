...
import { voucherQuery } from './VoucherQueryKey';
...
export function useVoucherViewModel() {
  ...
  const query = useQuery({
    queryKey: voucherQuery(session.query.scoped),
  ...
  return Object.freeze({
    state: query.isPending ? ('loading' as const) : query.isError ? (stepup ? ('denied' as const) : ('failed' as const)) : query.data?.vouchers.length === 0 ? ('empty' as const) : ('ready' as const),
    vouchers: query.data?.vouchers ?? Object.freeze([]),
    redemptions: query.data?.redemptions ?? Object.freeze([]),
    verification,
