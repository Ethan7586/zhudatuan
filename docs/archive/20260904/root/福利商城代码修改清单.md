...
> 重新核对日期：2026-08-25
> 唯一需求基线：`docs/福利商城功能清单260821.xlsx`，SHA-256 `78cfc3b350ced633afd6bfc951f2d2a94991780fa1d4b6d571e9f1d3df322942`
> 实施方案：`docs/福利商城根治实施方案.md`
> 强制架构基线：`docs/福利商城架构和补齐修改清单.md`，SHA-256 `48f7d95dfad87aa9cc06705ff52a35561d7dabfb1ef5e75636e8418405a24ad2`
> VI唯一基线：`docs/brand/SMART-WING-UNIFIED-VI-1.0.md`，SHA-256 `6c9a57889534fd1203aa181dbce922af5609c51d7e5ed594964c510b1abb4508`
> 代码标准：`docs/代码与前端开发规范.md`，SHA-256 `ee2cad28f5fac9d57317c1141e9161afb7e995586849dae041bef232e6d3b9fc`
> 当前代码根：`smart-wing` 独立分商城仓库根 `.`；仓库拓扑以 `docs/operations/MallTopology.md` 为最高优先级
...
1. 本文件中“当前路径”允许出现 `apps/console`、`features`、`extensions/channel`、`extensions/vendors`、`database` 等迁移源名称；它们不代表最终结构。
2. 所有“目标路径”必须落到总方案第 16 节的 `Shop`：前端使用 `feature`、`entity`、`route` 单数；渠道扩展使用 `extensions/channel`；数据库使用 `database/migrations|contracts|fixtures|tools`；运维文档使用 `docs/operations`。
...
2. `workspaces`改为：`apps/*`、`services/*`、`packages/*`、`extensions/channel/*`、`extensions/payment/*`、`extensions/notification/*`、`extensions/storage/*`、`extensions/tax/*`、`tools/*`；不再把 `providers` 和 `vendors` 设为两个架构层。
...
6. `quality`只固定为`node tools/quality/bootstrap.mjs`这一入口，不在`package.json`复制线性顺序；唯一`TaskCatalog.ts`恰好执行根方案23.2的21节点DAG：`install→generated`；generated后并发secrets/format/linebudget/architecture/contract/docs；`generated+format→naming`；`naming+linebudget+architecture+contract→typecheck`；`typecheck→unit+component`；`contract→database`；`database+contract→migration`；`typecheck+unit→build`；`build+migration→bundle`；`bundle+database+component→journey→security+performance`；`bundle+secrets→sbom`；最后`docs+security+performance+bundle+sbom+migration→sign`。任何未知节点、漏边、第二顺序或别名脚本为零。
...
| 当前迁移源                             | 唯一目标                                    | 必须修改                                                                                                                    |
| -------------------------------------- | ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| `scripts/audit/workspace.mjs`          | `tools/quality/src/audit/Workspace.ts`      | 只扫描最终Canonical Workspace；归档不在仓库；识别`extensions`和`tools`Workspace                                             |
| `scripts/audit/callgraph.mjs`          | `tools/quality/src/audit/callgraph/Main.ts` | 从Contract Operation Catalog、Route Registry导出、SDK清单和Event Consumer Registry交叉校验，不再用字符串包含判断路由存在    |
| `scripts/audit/duplicate.mjs`          | `tools/quality/src/audit/Duplicate.ts`      | 唯一Owner改为`@shop/authz`、`ErrorPresenter`、`RouteRegistry`、`packages/config`；重复块候选必须有Owner处置结果             |
| `scripts/audit/naming.mjs`             | `tools/quality/src/audit/Naming.ts`         | 删除旧Commit基线豁免；最终生产目录直接达到零连接符违规；仅保留CLI、审计、测试、配置、SQL和工具强制文件豁免                  |
| `scripts/audit/boundary.mjs`           | `tools/quality/src/audit/Boundary.ts`       | 目标路径改为`services/commerce/src/modules`和六端固定分层；检查模块公开Port，不允许跨模块内部导入                           |
| `scripts/audit/regression.mjs`         | `tools/quality/src/audit/Regression.ts`     | 增加旧Route、旧RPC、Mock、Simulation、Fallback、Demo、`@smart-wing`和旧目录零残留检查                                       |
| `scripts/audit/database-contracts.mjs` | `tools/quality/src/audit/Database.ts`       | 校验164个当前迁移及94个历史迁移的不可变Hash、后续Repair迁移、Fresh Replay、生产副本升级、函数签名、Grant、RLS和Schema Owner |
| `scripts/check-line-budget.mjs`        | `tools/quality/src/audit/LineBudget.ts`     | 只对产品源码执行299行门禁；删除Admin/Auth原型豁免；文档、已应用SQL、生成代码和测试使用各自门禁                              |
| `scripts/check-platform-delivery.mjs`  | `tools/quality/src/audit/Delivery.ts`       | 改为读取生成的21项Requirement Matrix和Released证据，不再用源码字符串推断完成                                                |
| `scripts/verify-p0.mjs`                | `tools/quality/src/audit/Pzero.ts`          | 覆盖身份、授权、下单、库存、支付、退款、Outbox、财务借贷和无Mock生产依赖图                                                  |
...
根`package.json`唯一脚本固定为`"quality": "node tools/quality/bootstrap.mjs"`，并把`tsx`以精确版本`4.20.6`锁在根devDependencies和lockfile。普通CI先把不可变Commit检出到本次独占的新目录；本地与CI都只调用该脚本。入口模式固定为：无Claim/State=`localSnapshot`，平台签名Claim=`release`，受签Cutover State=`postApply`。三态共用21节点；Clean本地Snapshot可exit 0但无Publisher、Signature、SignBundle、Attestation或部署资格。
...
| `outerShop:apps/storefront`                                                             | `apps/storefront`                                                                       | 保留核心页面和真实交易调用；删除设备展示副本、Mock和巨型Context                                                                            |
| `outerShop:apps/auth`                                                                   | `apps/auth`                                                                             | 保留视觉、Credential Discovery、注册协议；删除全部浏览器模拟安全逻辑                                                                       |
...
| `nestedCandidate:extensions/channel/*`                                                  | `extensions/channel/*`                                                                  | Provider 是渠道能力实现，保留业务 Mapper/Contract，统一 Manifest、Factory、Health 和 capability 目录                                       |
...
| `outerShop:database/migrations`、`nestedCandidate:database/migrations`                  | `database/migrations`                                                                   | 两份逐文件比较；保持已应用历史文件名与内容Hash，不在移动中改写SQL                                                                          |
| 两根的`database/contracts/sql`与`database/contracts`                                    | `database/contracts`                                                                    | 按Owner合并成唯一SQL合同目录，重复断言只保留一个Owner                                                                                      |
...
| `nestedCandidate:tools/localinfra\|localkms\|localobjects\|localsecrets\|seed`          | `infrastructure/container` 或 `database/tools`                                          | 仅保留可复现本地环境与数据工具；最终`tools`只保留contractgen、requirementgen、tokengen、quality                                            |
...
当前`apps/storefront/worker/index.ts`直接导入`services/commerce-api/src/api/router`。修改为：
...
| `delivery-matrix.json`         | 移到生成需求制品       | `docs/requirements/mapping.json`                                                                       |
...
|    2 | `foundation/application/ModuleOperations.ts`及各`*Operations.ts`把解析、SQL、事务、幂等和审计混在Action Map                                                  | 新增OperationPipeline、TransactionContext、Idempotency；每Operation专属Controller和Handler                                                                                                                                 | 专属Handler覆盖100%、行为/错误/ETag/幂等对照通过；CallGraph与Bundle审计限定`services/**/src`、`apps/**/src`、生产Package Export、Build和Deploy入口中的运行引用为0后删除，docs/evidence/FileDisposition中的历史词不计 |
...
- 当前六端同时存在 `feature` 与 `features`，而 `scripts/audit/boundary.mjs` 只识别单数，现有“0发现”是假绿；先让审计器同时识别迁移源并要求最终 `features` 为零，再移动目录。
...
上列源码每一行都是Shop-relative、FileDisposition无Source、单Target的精确`create`；不得把目录、brace、glob或`route/api/model/ui`占位项当作目标。运行Evidence `docs/operations/evidence/quality/consoleworkbench.json`绝不进入Source create清单；源树只跟踪`docs/operations/evidence/.gitignore`，由Journey Runner在运行时创建并固定其Schema，至少记录五工作台×Sidebar展开/折叠、移动端标题、document.title、aria-current、Deep Link、Back/Forward、loading/empty/error/permission和关键Command Preview结果。
...
现有`apps/storefront`改名为`apps/storefront`，Package Name改为`@shop/storefront`。只保留响应式Web商城；微信小程序交互归`apps/miniapp`，Android原生假页、平板独立复制页、架构演示页不属于MVP生产路径。
...
现有`apps/auth`改名为`apps/auth`，Package Name改为`@shop/auth`。
...
- `00-新任务从这里开始/`、`reports/`、`START-HERE-新任务入口.md`和Windows启动脚本在`FileDisposition.yml`记录Hash与删除理由后删除；目标仓库不新增`docs/archive`，也不让历史交付材料进入生产Artifact。
...
当前`database/migrations/`实际有 164 个迁移，其中 94 个历史迁移、70 个候选根治迁移；`database/contracts/sql/`有33个SQL合同测试，`database/contracts/`另有当前Catalog合同制品。164 个已存在迁移全部视为不可变输入：不改写、不重命名、不删除。先固定 `path + sha256 + order` Manifest，在新建空库中从第1个顺序重放，再在当前 Head 之后追加新的 Repair Migration。
...
```text
database/migrations/20260821010000_assert_source_head.sql
database/migrations/20260821011000_create_domain_schemas.sql
database/migrations/20260821012000_create_runtime_control.sql
database/migrations/20260821013000_create_identity_access.sql
database/migrations/20260821014000_create_organization_partner.sql
database/migrations/20260821015000_create_capability_member.sql
database/migrations/20260821016000_create_qualification.sql
database/migrations/20260821017000_create_catalog_pricing.sql
database/migrations/20260821018000_create_inventory_experience.sql
database/migrations/20260821019000_create_cart_checkout_order.sql
database/migrations/20260821020000_create_fulfillment_verification.sql
database/migrations/20260821021000_create_payment_voucher_benefit.sql
database/migrations/20260821022000_create_finance_channel.sql
database/migrations/20260821023000_create_support_notification.sql
database/migrations/20260821024000_create_reporting_risk_audit.sql
database/migrations/20260821025000_create_extension.sql
database/migrations/20260821026000_backfill_domain_data.sql
database/migrations/20260821027000_reconcile_domain_data.sql
database/migrations/20260821028000_build_domain_indexes.sql
database/migrations/20260821029000_publish_domain_contract.sql
database/migrations/20260821030000_revoke_public_access.sql
database/migrations/20260821031000_drop_legacy_objects.sql
database/migrations/20260821032000_assert_target_head.sql
```
...
- 将旧`scripts/audit/duplicate.mjs`、`boundary.mjs`、`callgraph.mjs`、`naming.mjs`、`check-line-budget.mjs`的有效检查语义按第13节迁入`tools/quality/src/audit`；删历史基线豁免、旧路径硬编码和根`scripts`第二入口。
- 行数预算只检查生产源码；SQL、测试、审计证据、配置、生成文件、脚本和架构文档按用途设置独立预算，不再因文档超299行阻塞代码检查。
- 历史交付说明在`FileDisposition.yml`记录Hash与删除理由后从目标仓库删除；如组织另行保留，必须在仓库和发布制品之外，目标树不新增`docs/archive`。
...
- 任何命名例外必须来自框架强制约定并在`tools/quality/config/naming.yml`精确声明；不使用Git历史自动豁免。
...
| 清单                 | 唯一源                                                                            | 生成/消费者                                                                                                       | CI断言                                                                                                                                                                           |
| -------------------- | --------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Requirement          | `ExternalAuthority ../docs/福利商城功能清单260821.xlsx`（从最终Shop根解析，只读） | `docs/requirements/requirements.yml`、`mvp.yml`、`providers.yml`、Contract Type、追踪报告                         | 296独立项+21 MVP+20 Provider；Workbook Hash等于权威Hash且旧源名为0                                                                                                               |
...
| File Disposition     | `docs/operations/FileDisposition.yml`                                             | 目标Move Plan、Archive Manifest、删除批次、CI文件覆盖报告                                                         | 每个源文件恰一动作；`keep/move/refactor/rewrite`有规范Target，`delete`无Target、有Archive且`replacement XOR noReplacementReason`，`create`无Source且有Target；无未分类或目标碰撞 |
...
```text
tools/quality/src/audit/callgraph/
├─ Main.ts
├─ Projects.ts
├─ Imports.ts
├─ Operations.ts
...
删除基于历史Commit的Ratchet豁免。对目标Workspace全量检查：生产目录/文件无连接符，框架例外仅来自`tools/quality/config/naming.yml`；类/值对象名词化，Command用动词，Query用Get/List/Search，Event用过去式，Port用能力语义，Adapter按技术实现命名。
...
> 历史冻结：本节记录 2026-08-25 的多来源盘点模型。依据 `docs/operations/MallTopology.md`，这些 Source Root 不再是当前实施输入，本节不得触发扫描、移动、删除、归档或权限变更。

迁移输入不是单一目录：外层`Shop`、嵌套候选、分支工作树、`Shop1`以及两个无Git的Smart复制树都含可能独有的应用、服务、包、数据库、扩展、基础设施、脚本、测试或文档。`docs/operations/FileDisposition.yml`是六个Source Root的唯一机器总账；每个Root固定`{rootId,path,baseCommitOrNull,sourceSnapshotHash,statusManifestHash,parentRootIdOrNull,parentDelegationHashOrNull,delegatedRoots[]}`，父Root的`delegatedRoots`按child rootId排序且每项恰为`{rootId,snapshotHash,delegationHash}`，子Root反向保存完全相等的`parentRootId/parentDelegationHash`。缺任一方向、Hash不对称、同child重复，或父Root委托后仍扫描child Entry都失败。Root path只以已验证的Shop Repository Root解析：`outerShop=.`、`nestedCandidate=smart-wing`、`branchReplica=smart-wing-branch-work`、`shopReplica=../Shop1`、`legacySmart=../smart-wing`、`legacySmartCopy=../smart-wing 2`；不得再加`Shop/`前缀。`tools/quality/src/FileDisposition.ts`扫描这些完整Snapshot而非单一Commit，并生成/验证：
...
entries:
  ...
  - action: create
    target: docs/operations/FileDisposition.yml
...
| 指标                            |                  目标 | 代码/测试落点                                              |
| ------------------------------- | --------------------: | ---------------------------------------------------------- |
| 公共目录缓存命中p95             |                ≤150ms | Catalog Reader + CDN/Redis、`tests/performance/catalog.js` |
...
- 记录Web Vitals、Route Load、API Span、Hydration/Render Long Task和JS Error；构建性能预算只在`tools/quality/config/bundles.yml`维护一份。
...
```text
docs/operations/DatabaseFailover.md
docs/operations/MigrationRollback.md
docs/operations/PaymentIncident.md
docs/operations/RefundRepair.md
docs/operations/InventoryRepair.md
docs/operations/LedgerRepair.md
docs/operations/OutboxReplay.md
docs/operations/DeadletterReplay.md
docs/operations/ChannelDegrade.md
docs/operations/CredentialRotation.md
docs/operations/CrossTenantIncident.md
docs/operations/DataBackfill.md
docs/operations/MallRollback.md
docs/operations/ReleaseRollback.md
```
...
tests/integration/                                 # DB/Redis/Object/Queue/Provider Test Double
tests/integration/messaging/MessageBroker.test.ts   # 真RabbitMQ Broker合同
tests/journey/                                     # 六端真实业务旅程
tests/performance/                                 # 阶梯、峰值、浸泡、恢复压测
tests/security/                                    # AuthN/AuthZ/Secret/Upload/Webhook/Dependency
tests/recovery/                                    # PITR、重放、故障注入、RPO/RTO
```

两个Vitest Config都是精确`create`动作：`integration.vitest.ts`的root是Shop，include只有`tests/integration/**/*.test.ts`，exclude是recovery/journey/node_modules/构建输出，Node+isolate+`passWithNoTests=false`，30s Test/120s Hook Timeout，Worker数取生成Test Config且每Worker独立Schema/Bucket/Vhost；`recovery.vitest.ts`的root也是Shop，include只有`tests/recovery/**/*.test.ts`，exclude是integration/journey/node_modules/构建输出，Node+isolate+`passWithNoTests=false`+`fileParallelism=false`+`maxWorkers=1`，120s Test/300s Hook Timeout。它们只导入Vitest API与`packages/config/src/generated/TestConfig.ts`，不启动容器、不读业务Secret、不复制Runtime Config。
...
| 对齐项                |                                                                  本清单覆盖 | 机器门禁                                             |
| --------------------- | --------------------------------------------------------------------------: | ---------------------------------------------------- |
| Excel独立功能         |                                                                         296 | `tools/quality/src/audit/Requirement.ts`             |
...
| 当前SQL Contract Test | `database/contracts/sql` 33个语义基线，另有`database/contracts` Catalog制品 | 按Owner合并，覆盖不变量只增不减                      |
...
- `docs/operations/MallTopology.md`是总商城与分商城仓库拓扑的最高优先级裁决。
- `docs/福利商城架构和补齐修改清单.md`是296条广域需求、28模块内部时序、现状证据和优先级补齐的仓内权威架构基线。
- `docs/福利商城根治实施方案.md`与`docs/福利商城前端代码修改清单.md`分别收束系统实施和前端 Batch 0–7。
