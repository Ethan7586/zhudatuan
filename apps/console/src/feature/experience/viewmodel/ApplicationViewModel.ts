...
export const experienceViews: readonly Readonly<{ key: ExperienceView; label: string }>[] = Object.freeze([
  { key: 'all', label: '全部商城应用' }, { key: 'published', label: '已发布' }, { key: 'drafts', label: '开店与装修草稿' }, { key: 'attention', label: '需要处理' },
]);
...
export function useApplicationViewModel(context: ConsoleContext, dependencies: ExperienceDependencies, requestStepup: () => void) {
  ...
  useEffect(() => {
    ...
    setName(copy?.name ?? record?.name ?? '主打团员工福利商城'); setCode(copy?.code ?? 'ZHUDATUAN_EMPLOYEE'); setSlug(copy?.slug ?? 'zhudatuan-employee'); setStatus(record?.status ?? 'draft'); setConfirmed(false); setIdentity(dependencies.createIdentity()); setReceipt(undefined);
  }, [actionId, dependencies]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (record, input) => { setReceipt(Object.freeze({ requestId: input.identity, reference: record.id, occurredAt: record.updatedAt, message: input.action.kind === 'create' ? '商城应用、初始草稿和商品池绑定已原子创建。' : input.action.kind === 'copy' ? '商城应用已复制并建立独立版本链。' : '商城经营设置已保存并完成权威重读。' })); await refresh(); },
  });
  ...
  const change = (setter: (value: string) => void) => (value: string) => { setter(value); setConfirmed(false); setIdentity(dependencies.createIdentity()); setReceipt(undefined); };
  ...
  const selectView = (nextView: ExperienceView) => updateSearch((next) => { nextView === 'all' ? next.delete('view') : next.set('view', nextView); next.delete('cursor'); next.delete('selected'); });
  const updateQuery = (value: string) => updateSearch((next) => { value === '' ? next.delete('q') : next.set('q', value); next.delete('cursor'); next.delete('selected'); });
  ...
  const clearFilters = () => updateSearch((next) => { next.delete('q'); next.delete('view'); next.delete('cursor'); next.delete('selected'); });
  const nextPage = () => { if (query.data?.nextCursor) setSearch(pageCursor(search, query.data.nextCursor)); };
  ...
  const permissions = Object.freeze({ create: canUseOperation(context, experienceOperations.create), copy: canUseOperation(context, experienceOperations.copy), update: canUseOperation(context, experienceOperations.update), publish: canUseOperation(context, experienceOperations.publish) });
  return Object.freeze({ presentation, scopeName: context.scope.name ?? chineseReference('组织范围', context.scope.id), condition, error: safeQueryError(query.error), data: query.data, rows, summary, flow: commerceFlow(presentation.mode), view, views: experienceViews, search: searchText, selected, detail, entry, action: Object.freeze({ value: action, name, code, slug, status, confirmed, validation, required: action && action.kind !== 'design' ? requiredAssurance(operation(action)) : 0, assurance: context.session.assurance.level, busy: mutation.isPending, error: mutation.error ? presentError(mutation.error).message : undefined, receipt, actions: Object.freeze({ open: openAction, close: closeAction, name: change(setName), code: change(setCode), slug: change(setSlug), status: (value: ExperienceStatus) => { setStatus(value); setConfirmed(false); setIdentity(dependencies.createIdentity()); setReceipt(undefined); }, confirmed: setConfirmed, submit, stepup: requestStepup }) }), version, permissions, fetching: query.isFetching, actions: Object.freeze({ refresh: () => void query.refetch(), selectView, search: updateQuery, openRecord, closeRecord, clearFilters, nextPage }) });
}

function readView(value: string | null): ExperienceView { return experienceViews.some((candidate) => candidate.key === value) ? value as ExperienceView : 'all'; }
function identify(action: ExperienceAction | null): string { return !action ? 'closed' : action.kind === 'create' ? action.kind : `${action.kind}:${action.record.id}`; }
function operation(action: Exclude<ExperienceAction, { kind: 'design' }>) { return action.kind === 'create' ? experienceOperations.create : action.kind === 'copy' ? experienceOperations.copy : experienceOperations.update; }
function validateAction(action: ExperienceAction | null, name: string, code: string, slug: string, confirmed: boolean): string | undefined { if (!action || action.kind === 'design') return undefined; if (!name.trim() || name.trim().length > 120) return '商城名称须为 1 至 120 个字符。'; if (action.kind !== 'manage' && !/^[A-Z][A-Z0-9_]{2,31}$/.test(code)) return '应用代码须以字母开头，只能包含大写字母、数字和下划线。'; if (action.kind !== 'manage' && !/^[a-z0-9][a-z0-9-]{2,47}$/.test(slug)) return '公开路径须为 3 至 48 个小写字母、数字或连字符。'; if (!confirmed) return '请先核对商城范围与本次变更。'; return undefined; }
