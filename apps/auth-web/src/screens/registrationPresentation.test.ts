...
describe('invitation registration presentation', () => {
  it('describes a Console invitation as an ordinary administrator with zero initial business permissions', () => {
    ...
    expect(presentation.successNotice).toContain('登录后进入运营后台');
  });

  it('uses ordinary employee language only after resolving a storefront invitation', () => {
    ...
    expect(pending.description).toContain('验证企业邀请码后');
