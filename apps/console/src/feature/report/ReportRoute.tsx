...
const columns: readonly DataColumn<ReportMetric>[] = [
  ...
  { key: 'dimensions', label: '维度', render: (row) => Object.entries(row.dimensions).map(([key, value]) => `${key}: ${value}`).join(' · ') || '全部' },
  { key: 'value', label: '服务端值', render: (row) => row.unit === 'minor' ? formatMinor(row.value)
...
export function Component() {
  ...
  return <PagedResource title="数据报表" eyebrow="ZHUDATUAN REPORTING" description="指标、口径、范围、截至时间和投影版本全部由 reporting 读模型返回。"
    ...
    actions={<><label className="inlinefield">报表<select value={view} onChange={(event) => setFilter('view', event.target.value)}>
      <option value="sales">销售</option><option value="products">商品</option><option value="malls">商城</option>
      <option value="categories">分类</option><option value="channels">渠道</option><option value="powderclass">粉类</option>
      <option value="voucher">卡券消费</option></select></label>
