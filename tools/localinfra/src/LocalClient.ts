...
export async function localSecret(endpoint: string, bearerToken: string, reference: string): Promise<string> {
  if (!endpoint.startsWith('https://') || !/^[A-Za-z0-9_-]{43,512}$/.test(bearerToken) || bearerToken.length % 4 === 1 || !/^[a-z0-9][a-z0-9/.-]{2,255}$/.test(reference)) throw new Error('LOCAL_SECRET_REQUEST_INVALID');
