-- Make every credential discovery response disclose the number of active
-- entrances to the trusted API. Until a server-bound selection token exists,
-- commerce-api fails closed when this array contains more than one entry.
...
as $$
  select jsonb_build_object(
    ...
    'entrances', coalesce((
      select jsonb_agg(
        ...
        order by case when entrance.target = 'storefront' then 0 else 1 end,
          entrance.created_at,
          entrance.id
      )
  ...
  cross join lateral (
    ...
    order by case when membership.target = 'storefront' then 0 else 1 end,
      membership.created_at,
      membership.id
    limit 1
  ) selected
  where alias.provider = p_provider
    and alias.subject = p_subject;
$$;
