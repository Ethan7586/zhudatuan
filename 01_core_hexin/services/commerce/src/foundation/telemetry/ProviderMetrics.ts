...
export class ProviderMetrics {
  ...
  observe(provider: string, operation: string, context: TelemetryContext, milliseconds: number, result: 'success' | 'failure',
    errorCode?: string): void {
    const measured = { ...context, provider, operation, result, dependency: provider, dependencyDurationMs: milliseconds,
