...
export class ApplyRiskDecision {
  async execute(database: OperationDatabase, command: Readonly<{ decision: string; scope: string; listing: string }>): Promise<void> {
    ...
    const result = await database.query(`update catalog.listing set status='unpublished',expires_at=clock_timestamp(),version=version+1,
      updated_at=clock_timestamp() where id=$1 and scope_id=$2 and status='published' returning id`, [command.listing, command.scope]);
    ...
    await database.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,available_at)
      ...
      clock_timestamp(),clock_timestamp()) on conflict(id) do nothing`, [`event:catalog:risk:${command.decision}`, command.listing, command.scope, command.decision]);
  }
}
