...
import type { Bootstrap, LoginMethod as Method } from '../../bootstrap/model/Bootstrap';
import type { Provider } from '../../federation/model/Provider';
import { ProviderList } from '../../federation/view/ProviderList';
import { InvitationForm } from '../../invitation/view/InvitationForm';
...
export function LoginPage(props: Readonly<LoginPageProps>) {
  ...
  return <AuthShell><AuthCard stage={first ? 1 : 2} onBack={props.onBack}><section className="authpanel" aria-busy={props.busy}>
    <header><h2>{first ? '统一账号认证' : '完成身份验证'}</h2><p>{first ? '选择目标系统，并使用企业授予的身份安全登录。' : '完成本次验证后即可继续。'}</p></header>
    <Alert {...(props.failure ? { failure: props.failure } : {})} {...(props.notice ? { notice: props.notice } : {})} />
    {first ? <div className="authformstack">
      ...
      {props.bootstrap.methods.includes('federation') ? <div className="authproviders"><ProviderList providers={props.providers} busy={props.busy} loading={props.providersLoading} failed={props.providerFailure !== undefined} onSelect={props.onProvider} />{props.providerFailure ? <Alert failure={props.providerFailure} onAction={props.onProviderRetry} /> : null}</div> : null}
      <label className="authagreement"><input type="checkbox" checked={props.accepted} onChange={(event) => props.onAccepted(event.target.checked)} /><span>我已阅读并同意<button type="button" onClick={() => policy.open('terms')}>《{props.bootstrap.legal.termsTitle}》</button>和<button type="button" onClick={() => policy.open('privacy')}>《{props.bootstrap.legal.privacyTitle}》</button></span></label>
      {props.fields.agreement ? <p className="authfieldissue" role="alert">{props.fields.agreement}</p> : null}
    </div> : props.stage}
  </section>{policy.policy === undefined ? null : <PolicyDialog policy={props.bootstrap.legal} kind={policy.policy} onClose={policy.close} onAccept={() => { props.onAccepted(true); policy.close(); }} />}</AuthCard></AuthShell>;
}
