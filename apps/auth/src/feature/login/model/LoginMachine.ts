import type { LoginEvent, LoginState } from './LoginState';
...
export function loginMachine(state: LoginState, event: LoginEvent): LoginState {
  ...
  switch (event.type) {
    case 'BOOTSTRAP_REQUESTED':
      return bootstrap(state, event.target);
    case 'TARGET_CHANGED':
      return event.target === state.target ? state : bootstrap(state, event.target);
    case 'BOOTSTRAP_SUCCEEDED':
      return state.phase === 'bootstrapping' ? Object.freeze({ ...base(state), phase: 'ready', target: event.bootstrap.target, method: event.bootstrap.preferredMethod, bootstrap: event.bootstrap }) : state;
    case 'BOOTSTRAP_FAILED':
      return state.phase === 'bootstrapping' ? Object.freeze({ ...base(state), phase: 'bootstrapfailure', failure: event.failure }) : state;
    case 'METHOD_CHANGED':
      return (state.phase === 'ready' || state.phase === 'recoverablefailure') && state.bootstrap.methods.includes(event.method) ? Object.freeze({ ...base(state), phase: 'ready', method: event.method, bootstrap: state.bootstrap }) : state;
    case 'ACCEPTANCE_CHANGED':
      return isRunning(state) ? state : Object.freeze({ ...state, accepted: event.accepted });
    case 'NOTICE_CHANGED':
      return isRunning(state) ? state : Object.freeze({ ...state, notice: event.notice });
    case 'CHALLENGE_REQUESTED':
      return state.phase === 'ready' ? Object.freeze({ ...base(state), phase: 'challengepending', command: state.command + 1, bootstrap: state.bootstrap }) : state;
    case 'CHALLENGE_SUCCEEDED':
      return state.phase === 'challengepending' ? Object.freeze({ ...base(state), phase: 'ready', bootstrap: state.bootstrap, notice: event.notice }) : state;
    case 'CHALLENGE_FAILED':
      return state.phase === 'challengepending' ? Object.freeze({ ...base(state), phase: 'recoverablefailure', bootstrap: state.bootstrap, failure: event.failure }) : state;
    ...
    case 'AUTHENTICATED':
      return isRunning(state) || state.phase === 'membershipselection' || state.phase === 'proof' || state.phase === 'enrollment'
        ...
        : state;
    case 'TICKET_EXCHANGED':
      return state.phase === 'exchangingticket' ? Object.freeze({ ...base(state), phase: 'redirecting', redirectUrl: state.redirectUrl, bootstrap: state.bootstrap }) : state;
    case 'ENROLLMENT_REQUIRED':
      return isRunning(state) ? Object.freeze({ ...base(state), phase: 'enrollment', enrollment: event.enrollment, submitting: false, bootstrap: state.bootstrap }) : state;
    case 'ENROLLMENT_SUBMIT_REQUESTED':
      return state.phase === 'enrollment' && !state.submitting ? Object.freeze({ ...state, submitting: true, command: state.command + 1 }) : state;
    case 'ENROLLMENT_FAILED':
      return state.phase === 'enrollment' && state.submitting ? Object.freeze({ ...state, submitting: false }) : state;
    case 'PROOF_REQUIRED':
      return isRunning(state) ? Object.freeze({ ...base(state), phase: 'proof', reference: event.reference, methodKind: event.method, expiresAt: event.expiresAt, bootstrap: state.bootstrap }) : state;
    ...
    case 'CANCELLED':
      return isRunning(state) || state.phase === 'bootstrapping' ? Object.freeze({ ...base(state), phase: 'cancelled' }) : state;
