...
begin
  if not exists(select 1 from runtime.schemaversion where version='20260902013000') then
    raise exception 'OWNER_GOVERNANCE_READS_PREVIOUS_HEAD_MISSING';
  end if;
  if exists(select 1 from runtime.schemaversion where version='20260902014000') then
    raise exception 'OWNER_GOVERNANCE_READS_ALREADY_APPLIED';
  end if;
  if exists(
    select 1
    ...
    raise exception 'OWNER_GOVERNANCE_READS_CONTRACT_MISSING';
  end if;
end
$precondition$;
...
select runtime.record_migration_evidence(
  '20260902014000',3,3,0,0,
  'select role.id,permission.code,mapping.effect from access.role role join access.rolepermission mapping on mapping.role_id=role.id join access.permission permission on permission.id=mapping.permission_id where role.kind=''owner'' and permission.code in(''notification.template.read'',''notification.announcement.read'',''risk.read'') order by role.id,permission.code;',
  'select membership.id,available.operation_id from access.membership membership join lateral capability.membership_operations(membership.id) available on available.operation_id in(''notification.templates.read'',''notification.announcements.read'',''risk.center.read'') where membership.status=''active'' order by membership.id,available.operation_id;'
);

insert into runtime.schemaversion(version,checksum)
values(
  '20260902014000',
  encode(public.digest('20260902014000_enable_owner_governance_reads','sha256'),'hex')
);
...
begin
  if exists(
    ...
    raise exception 'OWNER_GOVERNANCE_READS_CLOSURE_INVALID';
  end if;
end
$assert$;
