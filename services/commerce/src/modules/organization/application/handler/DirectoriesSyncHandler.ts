...
export class DirectoriesSyncHandler implements OperationHandler<'organization.directories.sync', 'write'> {
  ...
  async execute(input: OperationInputFor<'organization.directories.sync'>, context: WriteHandlerContext<'organization.directories.sync'>): Promise<OperationReply<OperationOutputFor<'organization.directories.sync'>>> {
    ...
    if (action === 'cancel') {
      ...
      return { status: 200, body: { id: String(run.id), state: 'cancelled', mode: syncMode(run.mode), preview: run.preview === true } };
    }
    const run =
      action === 'resume'
        ? await this.organizations.resumeRun(context.transaction, id, textField(body, 'run', 64), context.idempotencyKey ?? context.requestId)
        : await this.organizations.createRun(context.transaction, id, syncMode(body.mode), context.idempotencyKey ?? context.requestId, action === 'preview');
    if (!run) throw new DomainError('VALIDATION_FAILED');
    ...
    return { status: 202, body: { id: run.id, state: run.state, mode: syncMode(run.mode), preview: run.preview } };
  }
}
function syncAction(value: unknown): 'start' | 'preview' | 'cancel' | 'resume' {
  if (value !== 'start' && value !== 'preview' && value !== 'cancel' && value !== 'resume') throw new DomainError('VALIDATION_FAILED');
  return value;
}
