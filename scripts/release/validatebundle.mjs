...
if (release.buildProvenance?.sha256 !== fileHash(join(root, release.buildProvenance.path))) throw new Error('RELEASE_BUILD_PROVENANCE_MISMATCH');
...
if (pointer.schema !== 'shop.pointer.v1' || pointer.releaseId !== release.releaseId || pointer.commit !== release.commit || JSON.stringify(pointer.clients) !== JSON.stringify(release.clients)) throw new Error('RELEASE_POINTER_MISMATCH');
