...
export class FulfillmentAccess {
  constructor(
    readonly orders: FulfillmentOrderPort,
    private readonly organizations: OrganizationReadPort
  ) {}

  async member(database: OperationDatabase, order: string, member: string): Promise<void> {
    const snapshot = await this.orders.snapshot(database, order);
    if (!snapshot || snapshot.member !== member) throw new DomainError('RESOURCE_NOT_FOUND');
  }

  async fulfillment(database: OperationDatabase, id: string, scope: string): Promise<ManagedFulfillment> {
  ...
  async returned(database: OperationDatabase, id: string, scope: string): Promise<ManagedReturn> {
    const result = await database.query<{
      id: string;
      order: string;
      partner: string | null;
  ...
  private async authorize(database: OperationDatabase, fulfillment: ManagedFulfillment, scope: string): Promise<void> {
    if (fulfillment.partner === scope || fulfillment.store === scope) return;
    const order = await this.orders.snapshot(database, fulfillment.order);
    if (!order) throw new DomainError('RESOURCE_NOT_FOUND');
    const descendants = await this.organizations.descendants(database, scope);
