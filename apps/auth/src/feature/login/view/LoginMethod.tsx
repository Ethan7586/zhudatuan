...
import type { LoginMethod as Method } from '../../bootstrap/model/Bootstrap';
...
export function LoginMethod({ method, methods, busy, onChange }: Readonly<{ method: Method; methods: readonly string[]; busy: boolean; onChange: (method: Method) => void }>) {
  ...
  return <div className="authmethods" role="tablist" aria-label="登录方式">{available.map((value, index) => <button id={`auth-tab-${value}`} aria-controls={`auth-panel-${value}`} key={value} type="button" role="tab" aria-selected={method === value} tabIndex={method === value ? 0 : -1} disabled={busy} onClick={() => onChange(value)} onKeyDown={(event) => move(event, index, available, onChange)}>{LABELS[value]}</button>)}</div>;
}
...
function move(event: KeyboardEvent<HTMLButtonElement>, index: number, available: readonly Method[], onChange: (method: Method) => void): void {
  ...
  const next = event.key === 'ArrowRight' || event.key === 'ArrowDown' ? (index + 1) % available.length : event.key === 'ArrowLeft' || event.key === 'ArrowUp' ? (index + last) % available.length : event.key === 'Home' ? 0 : event.key === 'End' ? last : undefined;
