...
1. 安全增强不得成为独立目标，也不得以“提高安全分”“最佳实践”“行业标准”或假设风险为由进入代码、流程或发布主路径。
2. 默认禁止新增安全检查、审批、封板、证明链、权限收窄、阻断式门禁或额外权威状态。已有功能正确、可用、可恢复、简单和高效之后，才允许评估安全增强。
3. 只有存在可复现的真实漏洞，或能够明确对应资金、数据、隐私、权限或合规损失时，安全改动才有资格被提议；提议必须同时说明威胁证据、预期损失、正常路径成本、失败模式、恢复方式、删除方式和更简单的替代方案。
4. Ethan 对“提高安全”“做到高分”或整批方案的笼统同意不构成实施授权。安全改动必须由 Ethan 针对明确威胁和明确改动范围单独主动授权；AI 不得通过诱导确认、默认选项或拆批实施取得事实授权。
5. 即使取得专项授权，若安全改动会降低主路径可用性、显著增加复杂度、制造第二份权威状态，或缺少自动恢复与一键移除能力，仍不得实施；应返回更简单方案或保持现状。
6. 能用日志、告警、审计或事后检查处理的问题，不得升级为阻断式门禁。任何获准的阻断机制必须单一权威、失败可恢复、成本可量化，并先证明不会妨碍正常业务。
7. 本节约束的是新增安全增强，不授权删除身份认证、租户隔离、支付保护、备份恢复等现有基础保护；删除或削弱现有保护仍须由 Ethan 针对明确范围单独决定。
