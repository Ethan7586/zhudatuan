#!/usr/bin/env bash
# Install one of the two identical Aliyun build Runner slots.
set -euo pipefail
...
case "${ZDT_BUILD_SLOT:-}" in
  1)
    readonly RUNNER_NAME='aliyun-staging-zdt-build'
    readonly RUNNER_USER='zdt-build'
    readonly RUNNER_ROOT='/opt/actions-runner-build'
    readonly RUNNER_LABELS='zdt-aliyun-build,zdt-aliyun-build-1'
    ;;
  2)
    ...
    readonly RUNNER_LABELS='zdt-aliyun-build,zdt-aliyun-build-2'
    ;;
  *)
    echo 'ZDT_BUILD_SLOT must be 1 or 2.' >&2
    exit 64
    ;;
esac
...
[ "$(uname -s)" = 'Linux' ] && [ "$(uname -m)" = 'x86_64' ] || {
  echo 'The Aliyun Runner requires Linux x64.' >&2; exit 64;
}
[[ "${ZDT_GITHUB_PROXY_URL:-}" =~ ^http://127\.0\.0\.1:[0-9]{2,5}$ ]] || {
  echo 'ZDT_GITHUB_PROXY_URL must be a loopback HTTP proxy.' >&2; exit 64;
}
[ -f "$NETWORK_ENV" ] || { echo "Install the Runner network file first: ${NETWORK_ENV}" >&2; exit 64; }
...
if [ ! -f "$RUNNER_ROOT/.runner" ]; then
  ...
  install_root="$(mktemp -d "/var/tmp/zdt-build-${ZDT_BUILD_SLOT}.XXXXXX")"
...
service_path="$(find /etc/systemd/system -maxdepth 1 -type f -name "actions.runner.*.${RUNNER_NAME}.service" -print -quit)"
[ -n "$service_path" ] || { echo "Build slot ${ZDT_BUILD_SLOT} service was not installed." >&2; exit 1; }
...
rm -f -- "$env_tmp"
printf '%s\n' '[Service]' "EnvironmentFile=${NETWORK_ENV}" 'UMask=0077' 'TasksMax=2048' \
  > "$drop_in/20-zdt-runner.conf"
chmod 0700 "$RUNNER_ROOT" "$RUNNER_ROOT/_work" 2>/dev/null || true
systemctl daemon-reload
systemctl disable "$service_name" >/dev/null 2>&1 || true
systemctl stop "$service_name" >/dev/null 2>&1 || true
printf 'Build slot %s installed offline: %s\n' "$ZDT_BUILD_SLOT" "$service_name"
