...
export class TenderPolicy {
  validate(value: TenderHoldValue): void {
    if (!value.id || !value.voucher || !value.owner || !value.idempotency || !Number.isSafeInteger(value.amountMinor) || value.amountMinor <= 0 || !Number.isFinite(value.expiresAt.getTime()) || value.version < 0)
  ...
  release(value: TenderHoldValue, _now: Date): void {
    if (value.state !== 'active') throw new DomainError('VOUCHER_HOLD_CONFLICT');
  }
}
