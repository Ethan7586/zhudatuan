...
describe('finance reconciliation source', () => {
  it('consumes already-published immutable StatementLine rows without reading or parsing the object again', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.includes('line_count')) return result([{
        scope_id: 'mall:one', provider: 'provider', partner_id: 'partner:one', period: '2026-09', created_by: 'maker',
        statement_hash: 'a'.repeat(64), statement_ref: 'object:statement', statement_id: 'statement:one',
        reconciliation_debit: 0, reconciliation_credit: 0, reconciliation_difference: 0, version: 0,
        local_hash: 'a'.repeat(64),
        local_debit: 500, local_credit: 200, threshold_rule: { amountMinor: 0 }, line_count: 2,
      }]);
    ...
    const process = new ReconcileStatement(
      ...
      { batches },
      { statement } as unknown as FinanceChannelPort,
      { reconciliation: payments } as unknown as FinancePaymentPort,
      { reconciliation: fulfillments }
    );
  ...
  it('streams a Channel statement through the Runtime tabular port in bounded batches', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.includes('line_count')) return result([{
        scope_id: 'mall:one', provider: 'provider', partner_id: 'partner:one', period: '2026-09', created_by: 'maker',
        statement_hash: 'a'.repeat(64), statement_ref: 'channel:statement', statement_id: null,
        reconciliation_debit: 0, reconciliation_credit: 0, reconciliation_difference: 0, version: 0,
        local_hash: null, local_debit: null, local_credit: null, threshold_rule: { amountMinor: 0 }, line_count: 0,
      }]);
      if (sql.includes("set state='matching'")) return result([{ id: 'reconciliation:one' }]);
    ...
    const batches = vi.fn(async function* () {
      yield [{ reference: 'pay:one', type: 'payment', amountMinor: '500', currency: 'CNY' }, { reference: 'refund:one', type: 'refund', amountMinor: '200', currency: 'CNY' }];
    });
    const process = new ReconcileStatement(
      transactionManager(query),
      { batches },
      { statement: vi.fn(async () => ({ id: 'channel:statement', scope: 'mall:one', objectRef: 'object:channel', sha256: 'a'.repeat(64), period: { start: '2026-08-01', end: '2026-08-31', timezone: 'Asia/Shanghai' } })) } as unknown as FinanceChannelPort,
