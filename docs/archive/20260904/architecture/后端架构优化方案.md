...
本文档是本次架构硬切的唯一终态设计依据；实施基线记录在 `evidence/architecture/cutover.json`。
...
| 模块根待迁移生产文件                                   |                    95 | 不含 21 个错放测试文件                           |
| 全仓 `*Operations.ts`                                  |       31 个、3,419 行 | 必须全部拆除                                     |
| Contract Operation                                     |                   269 | 最终必须 269:269 映射真实 Handler                |
| Handler 文件                                           |                   269 | 268 个是三行绑定壳，只有 1 个有真实编排          |
| 使用 `OperationDatabase` 的生产文件                    |    218 个、999 次引用 | 数据库能力扩散严重                               |
| public 层直接使用 `OperationDatabase`                  | 46 个文件、225 次引用 | 直接违反目标公共端口约束                         |
| public 层泄露任意 DB/PG 类型                           |             61 个文件 | 包含 `DatabasePool`、`QueryResult`、`query()` 等 |
| 当前 boundary 检查                                     |              错误通过 | 现有规则尚未覆盖本次终态约束                     |
