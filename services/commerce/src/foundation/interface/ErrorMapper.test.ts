...
describe('ErrorMapper', () => {
  it('maps a reused idempotency key to the canonical conflict', async () => {
    const response = new ErrorMapper().map(new ApplicationError('IDEMPOTENCY_CONFLICT'), 'request:one', operationFor('IDEMPOTENCY_CONFLICT'));
  ...
  it('hides typed errors not declared by the current operation', () => {
    const response = new ErrorMapper().map(new ApplicationError('CREDENTIAL_INVALID'), 'request:wrong-union', operationFor('CART_EMPTY'));
  ...
  it('maps a catalogued HTTP boundary failure only through its operation union', () => {
    const response = new ErrorMapper().map(new ApplicationError('REQUEST_JSON_INVALID'), 'request:boundary', 'identity.sessions.create');
    expect(response.status).toBe(400);
    expect(response.body).toMatchObject({ code: 'REQUEST_JSON_INVALID', requestId: 'request:boundary' });

    const withoutOperation = new ErrorMapper().map(new ApplicationError('REQUEST_JSON_INVALID'), 'request:startup');
    expect(withoutOperation.body).toMatchObject({ code: 'INTERNAL_ERROR' });

    const impossibleForRead = new ErrorMapper().map(new ApplicationError('REQUEST_JSON_INVALID'), 'request:read', 'identity.providers.read');
