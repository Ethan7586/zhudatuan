...
export class ChannelOperationPort {
  ...
  async record(context: WriteTransactionContext, input: ProviderOperationInput): Promise<void> {
    ...
    const changed = await database.query(
      ...
      [input.id, input.provider, input.scope, input.kind, input.idempotency, input.reference, result?.externalReference ?? null, input.state, input.requestHash, JSON.stringify(response.summary), response.hash]
    );
  ...
  async update(context: WriteTransactionContext, input: ProviderOperationUpdate): Promise<void> {
    const database = this.transactions.database(context);
    ...
    const changed = await database.query(
      ...
      [input.provider, context.scope, input.kind, input.idempotency, result?.externalReference ?? null, input.state, JSON.stringify(response.summary), response.hash, current.value.version]
    );
