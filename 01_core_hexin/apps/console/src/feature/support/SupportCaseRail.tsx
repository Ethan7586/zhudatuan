...
export function SupportCaseRail(props: SupportCaseRailProps) {
  ...
  return (
    <aside className="supportcaserail" aria-label="客服工单队列">
      <div className="supportpanelheading">
        <div><span>会话队列</span><strong>{props.count}</strong></div>
      ...
      <div className="supportcasefilters">
        <label>
          ...
          <input type="search" value={query} onChange={(event) => setQuery(event.target.value)}
            placeholder="搜索当前页工单" autoComplete="off" />
        </label>
        <label>
      ...
      <div className="supportcasepagehint">仅筛选当前已读取的 {props.cases.length} 条记录</div>
      <div className="supportcaseviewport">
        <ResourceState condition={props.condition} {...(props.error === undefined ? {} : { error: props.error })} retry={props.onRetry}>
          {visibleCases.length === 0 ? <div className="supportlocalempty"><strong>没有匹配的工单</strong><span>换个关键词或状态试试。</span></div> :
            <nav className="supportcaselist" aria-label="客服工单">
...
function SupportCaseLink({ item, selected, supportPath }: Readonly<{
  ...
  return (
    <Link to={`${supportPath}/${encodeURIComponent(item.id)}`} aria-current={selected ? 'page' : undefined}>
      <span className="supportcaseicon" data-tone={supportTone(item.priority)} aria-hidden="true">
        {item.subject.trim().slice(0, 1) || '工'}
      </span>
      <span className="supportcasecopy">
        <span className="supportcasetopline"><strong>{item.subject}</strong><time dateTime={item.updated_at}>{supportTime(item.updated_at)}</time></span>
        <span className="supportcasepreview">{supportChannelLabel(item.channel)} · {shortIdentifier(item.id)}</span>
        <span className="supportcasebadges">
          {item.state.toLowerCase() === 'open' ? <em data-tone="danger" aria-label="有客户新消息">新消息</em> : null}
          <em data-tone={supportTone(item.priority)}>{supportPriorityLabel(item.priority)}</em>
