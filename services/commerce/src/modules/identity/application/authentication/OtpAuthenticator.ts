...
import { identitySubjectVariants } from '../../IdentitySubject';
...
export class OtpAuthenticator implements AuthenticationStrategy {
  ...
  async authenticate(request: OperationRequest, database: OperationDatabase, body: AuthenticationBody): Promise<AuthenticationReply> {
    ...
    const verified = await this.challenges.verify(database, challenge, code, (id, value) => this.code(id, value), { purpose: 'login', destinationHash: subject });
    ...
    await this.challenges.consume(database, challenge, code, (id, value) => this.code(id, value), verified.principal_id, { purpose: 'login', destinationHash: subject });
    await this.assurances.record(database, { principal: verified.principal_id, method: 'phone_otp', level: 2, evidenceHash: this.digest(challenge), expiresIn: '15minutes' });
    ...
    const session = await this.issuer.issue(database, {
    ...
    const ticket = await this.tickets.issue(database, session.session, target, authorization);
