...
export class PgVoucherSearch implements VoucherSearch {
  ...
  async read(call: Parameters<VoucherSearch['read']>[0], prepared: SearchFilter) {
    ...
    const rows = await database.query(`${VOUCHER} join voucher.credential searchcredential on searchcredential.id=voucher.credential_id
  ...
  async facets(call: Parameters<VoucherSearch['facets']>[0], prepared: SearchFilter) {
    const database = this.transactions.database(call.context.transaction); const built = this.filter(call, prepared);
    const rows = await database.query<{ states: unknown; products: unknown; pools: unknown }>(`with filtered as materialized(
      ...
      coalesce((select jsonb_agg(jsonb_build_object('value',pool_id,'count',count) order by pool_id) from(select pool_id,count(*)::integer count from filtered group by pool_id) item),'[]') pools`, built.values);
  ...
  async snapshot(call: Parameters<VoucherSearch['snapshot']>[0], prepared: SearchFilter) {
    ...
    const built = this.filter(call, prepared); const id = `searchsnapshot:${randomUUID()}`; const filterHash = createHash('sha256').update(canonical(filter)).digest('hex');
    const expiresAt = new Date(call.now.getTime() + RUNTIME_LIMITS.voucherExport.snapshotTtlSeconds * 1000);
    await database.query(`insert into voucher.searchsnapshot(id,scope_id,filter,filter_hash,watermark,result_count,expires_at,created_by,created_at) values($1,$2,$3::jsonb,$4,$5,0,$6,$7,$5)`, [id, call.scope, JSON.stringify(filter), filterHash, call.now, expiresAt, call.actor]);
    ...
    const selected = await database.query(`insert into voucher.searchsnapshotitem(snapshot_id,scope_id,voucher_id,ordinal,
  ...
  private filter(call: SearchCall, prepared: SearchFilter) {
    ...
    const values: unknown[] = [call.scope]; const terms = ['voucher.scope_id=$1'];
    if (ownOnly(call.target)) { values.push(call.member); terms.push(`holder.member_id=$${values.length}`); }
    const add = (column: string, value: unknown) => { if (typeof value === 'string' && value !== '') { values.push(value); terms.push(`${column}=$${values.length}`); } };
    add('voucher.product_id', source.product); add('searchcredential.pool_id', source.pool); add('holder.member_id', source.holder); add('voucher.state', source.state);
    if (typeof source.customer === 'string' && source.customer !== '') { values.push(source.customer); terms.push(`exists(select 1 from voucher.product product where product.id=voucher.product_id and product.customer_id=$${values.length})`); }
    if (typeof source.expiresBefore === 'string') { values.push(source.expiresBefore); terms.push(`voucher.expires_at<$${values.length}::timestamptz`); }
    if (typeof source.expiresAfter === 'string') { values.push(source.expiresAfter); terms.push(`voucher.expires_at>$${values.length}::timestamptz`); }
    if (prepared.query) {
...
function canonical(value: RecordValue): string { return JSON.stringify(Object.fromEntries(Object.entries(value).sort(([left], [right]) => left.localeCompare(right)))); }
function ownOnly(target: string): boolean { return target === 'storefront' || target === 'miniapp'; }
