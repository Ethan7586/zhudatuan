...
export class HealthProbe {
  ...
  constructor(private readonly baseUrl: string, private readonly release: string, fetcher: typeof fetch = fetch) { this.http = new HttpClient(fetcher); }

  async verify(probe: 'live' | 'startup' | 'ready'): Promise<void> {
    const response = await this.http.send(`${this.baseUrl}/health/${probe}`, {
