...
export class ManageStepup {
    ...
    return operationLifecycle<PreparedStepup, LoadedStepup>({
      ...
      prepare: async (request, loaded) => {
        ...
        return {
          ...
          envelope,
          recipient,
          binding: actionBinding(bodyRecord(request)),
      ...
      execute: async (request, database, value) => {
        ...
        const started = await this.challenges.issue(database, {
          id: value.id,
        ...
        if (approval) await this.stepups.save(database, value.id, approval);
  ...
  complete(): OperationAction {
    return async (request, database) => {
      ...
      await this.challenges.consume(database, challenge, textField(body, 'code'), (id, code) => this.code(id, code), actor.actor.id, { purpose: 'stepup' });
      await this.assurances.record(database, { principal: actor.actor.id, method: 'otp', level: 3, evidenceHash: this.digest(challenge), expiresIn: '15minutes' });
      if (!(await this.sessions.elevate(database, actor.actor.id, actor.actor.session, 3))) reject('AUTHENTICATION_REQUIRED');
      const binding = await this.stepups.consume(database, challenge, actor.actor.membership);
      const issued = binding ? await this.actionProofs.issue(database, binding, checker(actor)) : null;
