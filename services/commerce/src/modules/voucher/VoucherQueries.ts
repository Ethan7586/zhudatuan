...
export function voucherQueries(organizations: Pick<OrganizationReadPort, 'descendants'>): OperationActions {
  return {
    ...
    'voucher.bindings.read': async (request, database) => {
      ...
      const visibleScopes = personal ? [] : await organizations.descendants(database, organizationScope(access.scope));
      const result = await database.query(
