...
describe('AfterSalePersistence read scope', () => {
  it('uses the organization closure for console lists and does not require an order id', async () => {
    ...
    const service = new AfterSalePersistence({ evaluate: vi.fn() } as never, { descendants } as never);
    ...
    expect(read?.[0]).toContain('orders.mall_id=$13');
    expect(read?.[1]).toEqual([false, 'enterprise:one', false, false, ['enterprise:one', 'mall:one'], '', 'all', '', 'UTC', '', '', '', '', null, null, 51]);
  });
  ...
  it('keeps storefront member lists within the current member id without organization traversal', async () => {
    ...
    expect(query.mock.calls[0]?.[1]).toEqual([true, 'member:one', false, false, [], '', 'all', '', 'UTC', '', '', '', '', null, null, 51]);
  });
  ...
  it('uses the configured organization timezone and every bounded filter', async () => {
    ...
    const query = vi.fn(async (_sql: string, _values: readonly unknown[]) => ({ rows: [], rowCount: 0 }));
    const service = new AfterSalePersistence({ evaluate: vi.fn() } as never, { descendants, scope } as never);
    ...
    expect(query.mock.calls[0]?.[1]).toEqual([false, 'enterprise:one', false, false, ['enterprise:one', 'mall:one'], 'order:one', 'all', 'today', 'Asia/Shanghai', 'paid', 'paid', 'allocated', 'mall:one', null, null, 51]);
  });
});

function request(target: 'console' | 'storefront', kind: 'enterprise' | 'owner', scope: string): MutableRequest {
  return {
