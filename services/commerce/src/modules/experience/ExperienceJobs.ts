...
export class ExperienceJobProcessor implements JobProcessor {
  ...
  constructor(
    ...
    this.publisher = new CdnPublisher(objects);
  }
  ...
  async process(job: ClaimedJob, signal: AbortSignal): Promise<void> {
    ...
    const selected = await this.pool.query<PublicationRow>(
  ...
  private async activate(event: string, target: PublicationRow, path: string, stored: StoredObject): Promise<void> {
    ...
    try {
      ...
      const release = await database.query<{ state: string; superseded: boolean }>(
        `select state,exists(select 1 from experience.release current
      ...
      await database.query(
        ...
        [target.release_id, target.application_id, target.version_id, target.configuration_hash, path, stored.reference, stored.sha256, stored.size, release.rows[0].superseded ? 'staged' : 'active']
      );
      if (release.rows[0].superseded) {
        ...
        await database.query(
          ...
          [target.application_id, target.version_id]
        );
      }
  ...
  private async publishCache(malls: readonly string[], target: PublicationRow, path: string): Promise<void> {
    const value: PublishedExperience = Object.freeze({ release: target.release_id, version: target.version_id, hash: target.configuration_hash, document: target.configuration, effective_at: target.effective_at, object_key: path });
