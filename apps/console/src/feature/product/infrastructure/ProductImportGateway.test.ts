...
interface RequestLog { readonly method: string; readonly path: string; readonly headers: Headers; readonly body: unknown }
const requests: RequestLog[] = [];
const server = setupServer(http.all('*/api/v1/**', async ({ request }) => {
...
describe('ProductGateway import workflow', () => {
  it('uploads once, creates the catalog preflight and confirms the exact preview version', async () => {
    const upload = vi.fn(async () => ({ objectRef: 'object:one', sha256: 'a'.repeat(64), fileName: 'products.csv', mediaType: 'text/csv' as const, size: 24 }));
    ...
    expect(requests.map(({ method, path }) => `${method} ${path}`)).toEqual([
      'POST /api/v1/catalog/imports', 'GET /api/v1/runtime/imports/import%3Aone', 'POST /api/v1/runtime/imports/import%3Aone/confirm',
    ]);
...
function command(kind: string): ProductCommand { return { scope: { kind: 'mall', id: 'mall:one' }, accessVersion: 7, identity: `command:${kind}`, csrf: 'csrf:one' }; }
...
const ready = { id: 'import:one', type: 'import', owner: 'catalog', kind: 'product', title: '商品导入', state: 'ready', processed: 0, total: 1, succeeded: 0, failed: 0, retryableItems: 0, cancellable: true, retryable: false, version: 2, createdAt: '2026-09-07T08:00:00.000Z', updatedAt: '2026-09-07T08:01:00.000Z', expiresAt: '2026-09-08T08:00:00.000Z', fileName: 'products.csv', downloadAvailable: false, confirmationRequired: true, previewHash: 'b'.repeat(64), columns: ['sku', 'title'], validationErrors: 0 };
