...
import { ReadNavigationTree } from './ReadNavigationTree';

describe('ReadNavigationTree', () => {
  it('returns the authoritative projection when Redis is degraded', async () => {
    const cache: NavigationCache = { get: async () => null, put: async () => false, accept: async () => false, invalidate: async () => false, state: () => ({ available: false, reason: 'REDIS_DOWN' }) };
    ...
    const query = new ReadNavigationTree({} as DatabasePool, cache, projector as never, { run: async (_key: string, action: () => Promise<unknown>) => action() } as never, 's'.repeat(32), 'catalog');
    const result = await query.execute(accessContext());
