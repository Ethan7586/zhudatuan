...
type ReviewAction = Extract<ReferralActionValues['action'], { kind: 'approve' | 'disqualify' }>;

export class ReviewMember {
  constructor(private readonly port: Pick<ReferralPort, 'execute'>) {}
  execute(context: ConsoleContext, values: ReferralActionValues & Readonly<{ action: ReviewAction }>, proof: string, identity: string, signal?: AbortSignal) {
