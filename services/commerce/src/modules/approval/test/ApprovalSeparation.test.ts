...
describe('principal-bound action proofs', () => {
  it('rejects a historical same-account proof on consumption without erasing the old evidence', async () => {
    ...
    try {
      ...
      expect((await data.database.query('select checker_membership_id,consumed_at from access.actionproof')).rows)
        .toEqual([{ checker_membership_id: 'membership:makerother', consumed_at: null }]);
    } finally { await data.database.close(); }
  });
  ...
  it('blocks same-account action proofs and step-up requests through database enforcement', async () => {
    ...
    try {
      ...
      await expect(data.database.exec(`insert into identity.stepuprequest(challenge_id,operation_id,resource_id,request_hash,expected_version,maker_membership_id,checker_membership_id,target,scope_id,checker_access_version,created_at)
...
async function fixture(legacyProof = false) {
  ...
  if (legacyProof) await database.exec(`insert into access.actionproof(id,token_hash,operation_id,resource_id,request_hash,expected_version,target,scope_id,
  ...
  const write = <T>(member: Member, work: (context: WriteTransactionContext) => Promise<T>, scope = 'scope:test', actor = members[member] as string) =>
    manager.write({ tenant: 'tenant:test', membership: `membership:${member}`, scope, actor, trace: 'trace:approval', operation: 'approval.tasks.approve',
      deadline: Date.now() + 10_000, signal: new AbortController().signal }, work);
  await write('maker', async context => {
    await repository.createTemplate(context, { id: 'approvaltemplate:one', scopeId: context.scope, code: 'voucher.issue', name: '发放双人审批', subjectKind: 'voucherissue',
      actorId: context.membership, expectedVersion: null, escalations: [], steps: [{ sequence: 1, name: '独立复核', dueHours: 4,
        approvers: [{ kind: 'permission', value: 'approval.task.decide', minimumApprovals: 2 }] }] });
    ...
    await repository.createInstance(context, { id: 'approvalinstance:one', scopeId: context.scope, requesterId: context.membership, subjectKind: 'voucherissue',
      subjectId: 'issue:one', subjectVersion: 1, subjectSnapshot: { quantity: 1 }, action: 'voucher.issueorders.issue', evidenceHash: 'a'.repeat(64),
      amountMinor: 1000, currency: 'CNY', constraints: {}, expiresAt: new Date(Date.now() + 3600_000).toISOString() });
  });
  ...
  const approve = async (member: Member, options: { scope?: string; version?: number; actor?: string } = {}) => {
    ...
    return write(member, transaction => {
      const access = { actor: { id: members[member] }, membership: { id: transaction.membership, permissions: { allows: new Set(['approval.task.decide']) } },
        roles: [], scope: { id: transaction.scope } } as unknown as AccessContext;
      return application.decide({ path: { taskid: task.id }, body: { expectedVersion, reason: '已复核发放证据', evidence: {} } },
        { operation: 'approval.tasks.approve', requestId: 'request:one', traceId: transaction.trace, transaction, security: { kind: 'session', access },
          expectedVersion, headers: {}, rawBody: '', deadline: transaction.deadline, signal: transaction.signal }, 'approved');
    }, options.scope, options.actor);
  };
  const consume = (token: string) => write('maker', context => repository.consumeProof(context, { tokenHash: createHash('sha256').update(token).digest(),
    scopeId: context.scope, subjectKind: 'voucherissue', subjectId: 'issue:one', subjectVersion: 1, action: 'voucher.issueorders.issue', evidenceHash: 'a'.repeat(64),
    amountMinor: 1000, currency: 'CNY', constraints: {}, consumerOperation: 'voucher.issueorders.issue', requestHash: 'b'.repeat(64), consumerId: context.membership }));
  const issue = (member: Member) => database.query(`select * from access.issue_action_proof('00000000-0000-4000-8000-000000000001',decode(repeat('12',32),'hex'),
    'voucher.credentialexports.create','voucher:one',repeat('a',64),1,'console','scope:test','membership:maker',$1,'voucher.credential.export')`, [`membership:${member}`]);
  const consumeAction = () => database.query(`select * from access.consume_action_proof(decode(repeat('12',32),'hex'),'voucher.credentialexports.create',
