...
import type { AuditAppender } from '../../../pipeline/AuditAppender';
...
describe('voucher search preparation and audit', () => {
  ...
  it('normalizes direct number lookup before opening the transaction and masks its audit resource', async () => {
    const data = fixture();
    await data.executor.execute(new VouchersGetByNumberHandler(data.application), { path: { number: 'vc001234' }, query: {} }, readHandlerContext('voucher.vouchers.getbynumber', {} as ReadTransactionContext));
    expect(data.fingerprint).toHaveBeenCalledExactlyOnceWith('VC001234', 'number', 'mall:one');
  ...
  it.each([{ query: 'a'.repeat(129) }, { query: 'VC00\u00001234' }, { expiresBefore: 'not a date' }, { expiresAfter: '2026-09-06T00:00:00.000Z', expiresBefore: '2026-09-05T00:00:00.000Z' }])(
    'rejects invalid filters before performing remote work',
    async (query) => {
  ...
  it('normalizes the same filter identically for list, facets and snapshot preparation', async () => {
    ...
    expect(await service.prepare({ query: { ...filter, limit: 20, cursor: 'voucher:last' } } as never, 'mall:one')).toEqual(await service.prepare({ body: { filter } } as never, 'mall:one'));
  });
});
...
function fixture() {
  ...
  const audit = vi.fn<AuditAppender['append']>(async () => {
    expect(pgTransactionState.getStore()?.open).toBe(true);
  });
  ...
  const application = new VoucherApplication(
    ...
    {} as never,
    {} as never,
    new PgVoucherRepository({} as never, {} as never, {} as never, {} as never),
    {} as never,
    search,
    {} as never,
    {} as never
  );
  const executor = new OperationExecutor(transactionManager(query), { claim: vi.fn(), checkpoint: vi.fn(), complete: vi.fn() }, { verify: vi.fn() }, new AuditDecorator({ append: audit }), { append: vi.fn() });
