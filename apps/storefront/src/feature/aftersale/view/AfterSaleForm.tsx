  ...
  return (
    <>
      {page?.availableLines.map((line) => (
        <article key={line.lineId} className={`rounded-lg border p-3 ${line.available ? 'border-edge' : 'border-edge bg-subtle text-muted'}`}>
          <div className="flex items-start justify-between gap-4">
            <div>
              <b className="text-sm text-content">{line.title}</b>
              <p className="mt-1">
          ...
          {line.available ? (
            <label className="mt-3 flex items-center gap-2">
              ...
              <input
                ...
                onChange={(event) => actions.changeQuantity(line.lineId, Math.min(line.maximumQuantity, Math.max(0, Number(event.target.value) || 0)))}
                className="w-20 rounded border px-2 py-1"
              />
              {line.requiresReturn ? '需退回商品并通过验收' : '无需退回商品'}
            </label>
          ) : (
            <p className="mt-2 font-bold text-warning-strong">不可申请：{unavailable(line.unavailableReason)}</p>
          )}
        </article>
      ))}
      <label className="block font-bold">
        售后原因
        <select value={reason} onChange={(event) => actions.changeReason(event.target.value)} className="mt-1 w-full rounded-lg border p-2 font-normal">
          {afterSaleReasonOptions.map((option) => (
      ...
      <label className="block font-bold">
        ...
        <textarea
          ...
          className="mt-1 min-h-24 w-full rounded-lg border p-2 font-normal"
          placeholder="请描述问题、发生时间和期望处理方式"
        />
      </label>
      <div>
        <b>图片/附件</b>
        ...
        <label className="mt-2 inline-flex min-h-11 cursor-pointer items-center rounded-lg border px-3 py-2 font-bold focus-within:ring-2 focus-within:ring-[var(--sw-brand)]">
          ...
          <input
            aria-label="选择售后附件"
            type="file"
            accept={ORDER_AFTERSALE_ATTACHMENT_TYPES.join(',')}
            multiple
            className="sr-only"
            onChange={(event) => {
              actions.upload(event.target.files);
              event.target.value = '';
            }}
          />
        </label>
      ...
      <button
        type="button"
        disabled={busy || selected.length === 0 || attachments.some(({ state }) => state !== 'ready')}
        onClick={actions.submit}
        className="w-full min-h-11 rounded-lg bg-[var(--sw-brand)] py-2.5 font-black text-inverse disabled:cursor-not-allowed disabled:opacity-40"
      >
        {busy ? '正在安全提交…' : '提交售后申请'}
      </button>
    </>
