...
export function VoucherPage({ model }: Readonly<{ model: VoucherViewModel }>) {
  ...
  return (
    <div className="voucherworkspace" data-view={model.view}>
      <ResourcePanel title="卡券中心" eyebrow={chineseSectionLabel('卡券治理')} description="管理卡券方案、卡号资产、备券审批、发行、状态批次、绑定与消费回执。" condition={model.condition} {...(model.error ? { error: model.error } : {})} retry={model.actions.refresh} actions={<PrimaryActions model={model} />}>
        <div className="vouchercontent">
          {model.receipt ? <section className="voucherreceipt" role="status"><strong>操作已提交</strong><p>{model.receipt}</p><Button onPress={model.actions.dismissReceipt}>知道了</Button></section> : null}
          <section className="voucherownership" role="note"><span aria-hidden="true">域</span><div><strong>当前网站归属：{model.scopeName}</strong><p>所有列表和写操作按当前管理范围隔离；待确认的客户管理与产品档案不在本页面创建。</p></div></section>
          ...
          <section className="voucherboard" aria-labelledby="voucherviewtitle">
            ...
            <div className="voucherfilterbar"><div className="voucherfiltercopy"><strong id="voucherviewtitle">{voucherViewMeta[model.view].label}</strong><span>{voucherViewMeta[model.view].description}</span></div><label className="vouchersearch"><span className="sr-only">搜索当前卡券视图</span><input type="search" value={model.queryText} onChange={(event) => model.actions.filter('q', event.target.value)} placeholder="搜索名称、编号或详情" /></label><label className="voucherstatusfilter"><span className="sr-only">按状态筛选</span><select value={model.status} onChange={(event) => model.actions.filter('status', event.target.value)}><option value="all">全部状态</option>{model.states.map((state) => <option key={state} value={state}>{voucherStateLabel(state)}</option>)}</select></label></div>
            <p className="voucherfiltermeta">当前页筛选 · 显示 {model.rows.length} / {model.data?.items.length ?? 0} 条 · 服务端游标分页</p>
            {model.data && model.data.items.length === 0 ? <section className="voucherfilteredempty" role="status"><strong>暂无{voucherViewMeta[model.view].label}记录</strong><p>可使用页面上方已授权操作创建第一条记录，或切换其他视图。</p></section> : null}
            ...
            <footer className="voucherpagination"><span>本页 {model.data?.count ?? 0} 条 · 游标分页</span><Button onPress={model.actions.next} isDisabled={!model.data?.nextCursor}>下一页</Button></footer>
          </section>
        </div>
      </ResourcePanel>
...
function PrimaryActions({ model }: Readonly<{ model: VoucherViewModel }>) {
  return <>{model.permissions.saveProgram ? <Button tone="primary" onPress={() => model.actions.start({ kind: 'createprogram' })}>新建卡券</Button> : null}{model.permissions.createLibrary ? <Button onPress={() => model.actions.start({ kind: 'createlibrary' })}>新建卡号库</Button> : null}{model.permissions.requestReserve ? <Button onPress={() => model.actions.start({ kind: 'requestreserve' })}>申请备券</Button> : null}{model.permissions.issueBatch ? <Button onPress={() => model.actions.start({ kind: 'issuebatch' })}>发行卡券</Button> : null}{model.permissions.changeStatus && model.view === 'bindings' ? <Button onPress={model.actions.choosePage}>选择本页</Button> : null}{model.permissions.changeStatus && model.view === 'bindings' && model.selectedRows.length ? <Button tone="danger" onPress={model.actions.changeSelected}>批量操作（{model.selectedRows.length}）</Button> : null}<Button onPress={model.actions.refresh}>刷新数据</Button></>;
}
