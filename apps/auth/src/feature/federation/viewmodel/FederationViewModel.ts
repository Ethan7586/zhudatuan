...
export class FederationViewModel {
  ...
  read(target: AuthTarget, signal: AbortSignal) {
    return this.providers.execute(target, signal);
  }
  start(provider: string, target: AuthTarget, returns: Omit<AuthRequest, 'target'>, signal: AbortSignal) {
    return this.federation.execute(provider, target, returns, signal);
  }
}
