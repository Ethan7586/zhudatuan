...
import { enqueue } from './VoucherJobSupport';

export class VoucherIssueJobs {
  constructor(
    private readonly pool: DatabasePool,
    private readonly kms: KmsClient,
    private readonly finance: VoucherAccountingPort
  ) {}
  async issue(batchid: string): Promise<void> {
    ...
    try {
      ...
      const batch = await reserved.query<{
        program_id: string;
    ...
    const envelopes = await mapParallel(values, 16, async (item) => ({ item, envelope: await this.kms.encrypt('pii', 'voucher/code', item.code, { voucher: item.id, batch: batchid }) }));
  ...
  private async completeChunk(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<import('pg').QueryResult> }>, batchid: string, scope: string, count: number): Promise<boolean> {
