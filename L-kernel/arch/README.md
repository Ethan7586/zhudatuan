...
生产宿主使用 `/var/lib/l-arch/state.json` 保存版本化状态，API 进程监听同一文件并在 500 毫秒轮询周期内收敛。唯一修改入口位于与 Arch 并列的本机节点控制服务：`GET /v1/arch` 查看期望状态，`PUT /v1/arch` 修改单个“节点 + 接口”。状态文件只含通用标识、状态和版本，不含任何 L 的业务资料。
