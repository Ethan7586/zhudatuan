...
export function journeyFor(prefix: string, module: string): string {
  if (prefix === 'PLAT' || prefix === 'STORE') return '03_quality_ceshi/tests/journeys/mvp03_platform.spec.ts';
  if (prefix === 'DIST') return '03_quality_ceshi/tests/journeys/mvp04_distribution.spec.ts';
  if (prefix === 'INTEG' || prefix === 'CHAIN' || prefix === 'SUPPLY') return '03_quality_ceshi/tests/journeys/mvp23_providers.spec.ts';
  ...
  return '03_quality_ceshi/tests/journeys/mvp' + number + '_' + name + '.spec.ts';
}
