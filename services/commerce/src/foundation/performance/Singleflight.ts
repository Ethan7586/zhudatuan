...
export class Singleflight {
  ...
  private async wait<T>(key: string, flight: Flight<T>, shared: boolean, options: Readonly<{ signal?: AbortSignal | undefined; deadline?: number | undefined }>): Promise<T> {
    ...
    const controller = new AbortController();
    const timer = options.deadline === undefined ? undefined : setTimeout(() => controller.abort(new DomainError('DEADLINE_EXCEEDED')), Math.max(0, options.deadline - Date.now()));
    timer?.unref?.();
    ...
    try {
      ...
      const outcome = controller.signal.aborted ? 'cancelled' : 'failure';
      this.observe?.({ key, shared, outcome, milliseconds: performance.now() - started });
      throw cause;
    } finally {
