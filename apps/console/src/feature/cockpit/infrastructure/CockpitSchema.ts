...
const metric = z
  .object({
    ...
    period: z.object({ from: z.string().min(1), to: z.string().min(1), timezone: z.string().min(1) }).strict(),
...
const sales = z
  .object({
    ...
    trend: z.array(trend),
    weeklyTrend: z.array(trend),
    categories: z.array(category),
    topProducts: z.array(z.object({ productId: z.string().min(1), name: z.string().min(1), salesCents: z.number().finite(), quantity: z.number().int().nonnegative(), orderCount: z.number().int().nonnegative() }).strict()),
...
export const CockpitSchema = z
  .object({
    ...
    summary: z.object({ catalogCount: z.number().finite(), availableStock: z.number().finite(), orderCount: z.number().finite(), afterSaleCount: z.number().finite(), sales }).strict(),
  })
  .strict();
