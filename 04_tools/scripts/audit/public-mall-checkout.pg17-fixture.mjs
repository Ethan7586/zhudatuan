...
try {
  ...
  await run('docker', ['exec', container, 'psql', '-X', '-v', 'ON_ERROR_STOP=1', '-U', 'postgres', '-d', database,
    '-c', `alter role zhudatuanroot superuser; alter role zhudatuanwebapi login password '${password}'; alter role zhudatuanpurchaseapi login password '${password}'; alter role shopapp login password '${password}'; alter role shopjob login password '${password}';`],
  { quiet: true });
  ...
  console.log('public Mall Core PostgreSQL 17 acceptance passed: catalog -> cart -> quote -> order -> payment -> shipment -> refund');
} finally {
