...
describe('signed object upload', () => {
  ...
  it('rejects insecure non-loopback destinations and failed writes', async () => {
    ...
    await expect(uploadObject({ url: 'https://objects.example.test/upload', headers: {}, body: new Blob() }, vi.fn(async () => new Response(null, { status: 403 })))).rejects.toThrow('SDK_OBJECT_UPLOAD_FAILED');
  });
});
