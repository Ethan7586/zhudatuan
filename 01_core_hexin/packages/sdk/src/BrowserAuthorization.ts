...
type Digest = (value: ArrayBuffer) => Promise<ArrayBuffer>;
...
export async function beginBrowserAuthorization(): Promise<BrowserAuthorization> {
  const state = randomToken(32);
  const nonce = randomToken(32);
  const verifier = randomToken(64);
  return Object.freeze({
    request: Object.freeze({ state, nonce, challenge: await pkceS256Challenge(verifier) }),
    secret: Object.freeze({ nonce, verifier }),
  });
}

export async function pkceS256Challenge(verifier: string, digest: Digest | null = browserDigest()): Promise<string> {
  const input = new TextEncoder().encode(verifier);
  const hashed = digest === null ? sha256(input) : new Uint8Array(await digest(arrayBuffer(input)));
...
function browserDigest(): Digest | null {
  try {
    const source = globalThis.crypto as Crypto & { readonly webkitSubtle?: SubtleCrypto };
    const subtle = source?.subtle ?? source?.webkitSubtle;
    return subtle ? (value) => subtle.digest('SHA-256', value) : null;
  } catch {
    return null;
  }
}
...
function randomToken(bytes: number): string {
  ...
  if (!source?.getRandomValues) throw new Error('BROWSER_RANDOM_UNAVAILABLE');
...
function sha256(input: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma0 = rotate(previous15, 7) ^ rotate(previous15, 18) ^ (previous15 >>> 3);
      const sigma1 = rotate(previous2, 17) ^ rotate(previous2, 19) ^ (previous2 >>> 10);
    ...
    round.forEach((word, index) => { hash[index] = (hash[index]! + word) >>> 0; });
  }
