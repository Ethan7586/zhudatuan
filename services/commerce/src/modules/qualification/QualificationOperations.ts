...
export function qualificationOperations(context: ModuleContext): ModuleOperations {
  ...
  return new ModuleOperations('qualification', pool, context.service(AUDIT_SINK), {
    ...
    'qualification.decisions.preview': async (request, database) => {
      ...
      const result = await database.query(
        ...
        [access.scope.id, textField(body, 'member'), textField(body, 'resource')]
      );
    ...
    'qualification.policies.manage': async (request, database) => {
      ...
      if (rule === null || typeof rule !== 'object' || Array.isArray(rule)) throw new DomainError('VALIDATION_FAILED', { field: 'rule' });
