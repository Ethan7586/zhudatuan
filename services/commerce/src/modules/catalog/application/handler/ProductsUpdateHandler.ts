...
export class ProductsUpdateHandler implements OperationHandler<'catalog.products.update', 'write'> {
  ...
  async execute(input: OperationInputFor<'catalog.products.update'>, context: WriteHandlerContext<'catalog.products.update'>): Promise<OperationReply<OperationOutputFor<'catalog.products.update'>>> {
    ...
    const coverObject = await this.media.verify(body.image === undefined ? undefined : body.image === null ? null : productImage(body.image));
    ...
    return { status: 200, body: productOutput(updated) as unknown as OperationOutputFor<'catalog.products.update'>, events: [productRecordEvent(event, updated, { actor: access.actor.id, trace: context.traceId })] };
  }
}
