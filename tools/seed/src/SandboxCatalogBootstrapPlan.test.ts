...
const source = Object.freeze({
  ...
  ZHUDATUAN_SANDBOX_CATALOG_DATABASE_URL:
    `postgresql://zhudatuansandboxbootstrap:${secret}@127.0.0.1:55432/zhudatuan_registration`,
...
test('accepts only the dedicated loopback sandbox database boundary', () => {
  ...
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_CONFIRM: 'yes' }), /CONFIRMATION_REQUIRED/);
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_DATABASE_NAME: 'postgres' }), /NAME_MISMATCH/);
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_DATABASE_URL:
      `postgresql://zhudatuansandboxbootstrap:${secret}@localhost:55432/zhudatuan_registration` }), /ENDPOINT_INVALID/);
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_DATABASE_URL:
      `postgresql://shopmigration:${secret}@127.0.0.1:55432/zhudatuan_registration` }), /ENDPOINT_INVALID/);
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_DATABASE_URL:
      'postgresql://zhudatuansandboxbootstrap:short@127.0.0.1:55432/zhudatuan_registration' }), /ENDPOINT_INVALID/);
  assert.throws(() => sandboxCatalogBootstrapEnvironment({ ...source,
    ZHUDATUAN_SANDBOX_CATALOG_SENTINEL: 'short' }), /SENTINEL_INVALID/);
