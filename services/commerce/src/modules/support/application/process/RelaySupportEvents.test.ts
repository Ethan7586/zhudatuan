...
const realtimeEvent = { id: 'event:one', type: 'support.message.sent', scopeId: 'mall:one', ticketId: 'ticket:one', conversationId: 'conversation:one', memberId: 'member:one', messageId: 'message:one', sequence: 7, version: 4, occurredAt: '2026-09-02T00:00:00.000Z' } as const;
...
describe('RelaySupportEvents', () => {
  ...
  it('records a retryable failure without marking the outbox event complete', async () => {
    const outbox = { claim: vi.fn(async () => event), complete: vi.fn(), fail: vi.fn() };
    const relay = new RelaySupportEvents(transactions() as never, outbox as never, { publish: vi.fn(async () => { throw new Error('REDIS_UNAVAILABLE'); }) } as never, authority() as never);
...
function authority() { return { authoritative: vi.fn(async () => realtimeEvent) }; }
