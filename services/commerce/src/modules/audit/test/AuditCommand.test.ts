...
describe('canonical audit command', () => {
  it('redacts secrets from reason, before/after and evidence before repository append', async () => {
    ...
    const repository = {
      ...
      appendRecord: vi.fn(async (_context, record: AuditRecord) => { appended = record; }),
    };
    const audit = new RecordAudit(repository as never);
    await withWriteTransaction(async () => result([]), (context) => audit.record(context, {
      actor: 'actor:one', actorType: 'console', scope: 'mall:one', request: 'request:one', operation: 'catalog.listings.publish',
      subject: { type: 'actor', id: 'actor:one' }, object: { type: 'catalog', id: 'listing:one' }, outcome: 'succeeded',
      reason: 'Bearer reason-secret', before: { password: 'before-secret' }, after: { authorization: 'after-secret' },
      evidence: { token: 'evidence-secret', safe: 'approved' }, trace: 'trace:one',
