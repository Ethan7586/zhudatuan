...
export class OwnershipAcceptPreviewHandler implements OperationHandler<'access.ownership.transfers.accept.preview', 'write'> {
  ...
  async execute(input: OperationInputFor<'access.ownership.transfers.accept.preview'>, context: WriteHandlerContext<'access.ownership.transfers.accept.preview'>): Promise<OperationReply<OperationOutputFor<'access.ownership.transfers.accept.preview'>>> {
