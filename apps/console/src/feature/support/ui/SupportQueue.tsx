    ...
    <aside className="supportqueue" aria-label="客服工单队列">
      ...
      <div className="supportqueueviewport">
        <ResourceState condition={condition} {...(error === undefined ? {} : { error })} retry={onRetry}>
          <nav className="supportticketlist" aria-label="客服工单">
            {tickets.map((ticket) => (
              <Link key={ticket.id} to={`${path}/${encodeURIComponent(ticket.id)}`} aria-current={ticket.id === selected ? 'page' : undefined}>
                ...
                <span className="supportticketcopy">
                  ...
                  <span className="supportticketbadges">
                    {ticket.unread_count > 0 ? <em data-tone="danger">{ticket.unread_count} 条未读</em> : null}
                    ...
                    {ticket.sla_risk !== 'normal' ? <em data-tone="danger">{ticket.sla_risk === 'overdue' ? '服务时限已超时' : '服务时限临近'}</em> : null}
                  </span>
                </span>
              </Link>
            ))}
      ...
      {nextCursor ? (
        <footer className="supportqueuefooter">
          <Button onPress={() => onNext(nextCursor)}>加载更多工单</Button>
        </footer>
      ) : null}
    </aside>
  );
