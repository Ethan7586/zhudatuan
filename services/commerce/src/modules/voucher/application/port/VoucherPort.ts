...
export class VoucherPort {
  ...
  async preview(database: VoucherDatabase, vouchers: readonly string[], member: string, scope: string): Promise<readonly VoucherChoice[]> {
    ...
    return (
      await database.query<VoucherChoice>(
        `select voucher.id,voucher.remaining_minor::float8 remaining_minor,
  ...
  async available(database: VoucherDatabase, member: string, scope: string): Promise<readonly VoucherChoice[]> {
