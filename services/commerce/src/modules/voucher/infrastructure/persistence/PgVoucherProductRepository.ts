...
export class PgVoucherProductRepository implements VoucherProductRepository {
  constructor(private readonly references: ProductReference, private readonly transactions = new PgTransactionAccess()) {}
  ...
  async revise(call: Parameters<VoucherProductRepository['revise']>[0]) {
    ...
    await this.references.validate(call.context.transaction, call.scope, value.customer, value.qualification);
    await database.query(`update voucher.product set customer_id=$3,name=$4,face_minor=$5,currency=$6,qualification_id=$7,pool_id=$8,
  ...
  async enable(call: Parameters<VoucherProductRepository['enable']>[0]) { return this.state(call, 'enabled'); }
  async disable(call: Parameters<VoucherProductRepository['disable']>[0]) { return this.state(call, 'disabled'); }
  ...
  async get(call: Parameters<VoucherProductRepository['get']>[0]) {
    ...
    const supply = await database.query<{ available: number; allocated: number }>(`select count(*) filter(where state='available')::integer available,count(*) filter(where state='allocated')::integer allocated from voucher.credential where product_id=$1 and scope_id=$2`, [id, call.scope]);
  ...
  async list(call: Parameters<VoucherProductRepository['list']>[0]) {
    const database = this.transactions.database(call.context.transaction);
    const filter = query(call); const fetch = limit(filter.limit);
    const rows = await database.query(`${PRODUCT} where product.scope_id=$1 and ($2::text is null or product.state=$2) and ($3::text is null or product.customer_id=$3)
      and ($4::text is null or product.id>$4) order by product.id limit $5`, [call.scope, optionalText(filter.state), optionalText(filter.customer), cursor(filter.cursor), fetch + 1]);
  ...
  async options(call: Parameters<VoucherProductRepository['options']>[0]) {
    ...
    const rows = await database.query(`select product.id,product.number,product.name,product.face_minor::integer as "faceMinor",product.currency,
      ...
      and ($2::text is null or product.id>$2) order by product.id limit $3`, [call.scope, cursor(filter.cursor), fetch + 1]);
  ...
  private async state(call: Parameters<VoucherProductRepository['enable']>[0] | Parameters<VoucherProductRepository['disable']>[0], target: 'enabled' | 'disabled') {
    const database = this.transactions.database(call.context.transaction); const id = path(call, 'productid');
    const locked = await database.query<ProductRow>(`select ${PRODUCT_FIELDS} from voucher.product product where product.id=$1 and product.scope_id=$2 for update`, [id, call.scope]);
    const row = locked.rows[0]; if (!row) throw new DomainError('RESOURCE_NOT_FOUND'); if (row.version !== expected(call)) throw new DomainError('VERSION_CONFLICT');
    ...
    const aggregate = new VoucherProduct(model(row)); const changed = target === 'enabled' ? aggregate.enable(call.now) : aggregate.disable();
    ...
    return one<typeof call extends Parameters<VoucherProductRepository['enable']>[0] ? 'voucher.products.enable' : 'voucher.products.disable'>(200,
      (await database.query(`${PRODUCT} where product.id=$1 and product.scope_id=$2`, [id, call.scope])).rows[0]);
  }
}

interface ProductRow { readonly id: string; readonly scope_id: string; readonly customer_id: string; readonly name: string; readonly face_minor: number; readonly currency: string; readonly qualification_id: string; readonly pool_id: string | null; readonly starts_at: Date; readonly expires_at: Date; readonly activation: 'automatic' | 'secret' | 'numbersecret'; readonly approval_required: boolean; readonly state: 'draft' | 'enabled' | 'disabled' | 'retired'; readonly version: number; }
function model(row: ProductRow) { return { id: row.id, scope: row.scope_id, customer: row.customer_id, name: row.name, faceMinor: Number(row.face_minor), currency: row.currency, qualification: row.qualification_id, pool: row.pool_id, startsAt: new Date(row.starts_at), expiresAt: new Date(row.expires_at), activation: row.activation, approvalRequired: row.approval_required, state: row.state, version: Number(row.version) } as const; }
function configuration(value: Readonly<Record<string, unknown>>) {
  ...
  return { customer: text(value.customer, 'customer'), name: text(value.name, 'name'), faceMinor: Number(value.faceMinor), currency: text(value.currency, 'currency'), qualification: text(value.qualification, 'qualification'), pool: optionalText(value.pool), startsAt: new Date(text(validity.startsAt, 'validity.startsAt')), expiresAt: new Date(text(validity.expiresAt, 'validity.expiresAt')), activation: activation as 'automatic' | 'secret' | 'numbersecret', approvalRequired: value.approvalRequired === true };
}
