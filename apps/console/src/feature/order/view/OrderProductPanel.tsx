...
export function OrderProductSnapshot({ order, onRetry }: Readonly<{ order: OrderDetail; onRetry: (() => void) | undefined }>) {
  return (
      <OrderDetailSection title="商品快照">
        ...
        {order.sections.products.state === 'ready' ? <div className="orderlinelist">
          {[...order.lines]
...
export function OrderFulfillmentSnapshot({ order, onRetry, canShip, onShip }: Readonly<{ order: OrderDetail; onRetry: (() => void) | undefined; canShip?: (target: OrderDetail['fulfillments'][number]) => boolean; onShip?: (target: OrderDetail['fulfillments'][number]) => void }>) {
  return (
        ...
        {order.sections.fulfillment.state !== 'ready' ? null : order.fulfillments.length === 0 ? (
          ...
          order.fulfillments.map((item) => (
            <article className="orderreadcard" key={item.id}>
              ...
              {canShip?.(item) && onShip ? <button type="button" onClick={() => onShip(item)}>登记发货</button> : null}
            </article>
          ))
        )}
      </OrderDetailSection>
