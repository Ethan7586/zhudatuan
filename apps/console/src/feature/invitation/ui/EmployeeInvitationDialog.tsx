...
export function EmployeeInvitationDialog({
  ...
  const [draft, setDraft] = useState<EmployeeDraft>(() => initialEmployee());
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    await onSubmit(employeeInvitation(scope, draft));
  };
  return (
...
function initialEmployee(): EmployeeDraft {
  return { displayName: '', mobile: '', employeeNo: '', departmentId: '', expiresAt: new Date(Date.now() + 72 * 60 * 60 * 1_000).toISOString(), reason: '' };
}
