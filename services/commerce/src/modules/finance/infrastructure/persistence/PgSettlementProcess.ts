...
export class PgSettlementProcess implements SettlementJobProcess {
  ...
  settle(reconciliation: string, execution: FinanceJobExecution): Promise<void> {
    return this.transactions.write(options(execution), async (context) => {
      ...
      const source = await client.query<{ scope_id: string; partner_id: string; period: string; credit_minor: number; created_by: string; statement_hash: string }>(
        `select
        ...
        and not exists(select 1 from finance.reconciliationitem item where item.reconciliation_id=reconciliation.id
          and item.state not in('matched','resolved')) for update`,
        [reconciliation]
      );
      const basis = source.rows[0];
      if (!basis) throw new Error('SETTLEMENT_NOT_RUNNABLE');
      const rule = (await client.query<{ rule: unknown }>(`select rule from finance.policy where scope_id=$1 and kind='settlement' and state='active'`, [basis.scope_id])).rows[0]?.rule;
      ...
      const result = await client.query<{ id: string; scope_id: string }>(
        ...
        [`settlement:${reconciliation}`, basis.partner_id, basis.period, reconciliation, split.netMinor, basis.scope_id, basis.created_by, basis.statement_hash, split.basisPoints, split.grossMinor, split.feeMinor, split.invoiceBasis]
      );
      ...
      await client.query(
        ...
        [settlement.id, settlement.scope_id, reconciliation]
      );
      if (split.invoiceBasis === 'net' && split.feeMinor > 0)
        await client.query(
          `with eligible as(
  ...
  async withdraw(id: string, execution: FinanceJobExecution): Promise<void> {
    const withdrawal = await this.transactions.write(options(execution), async (context) => {
      const database = this.access.database(context);
      const selected = await database.query<{ id: string; destination_ref: string; amount_minor: number; currency: string }>(
      ...
      const complete = await database.query(`select 1 from finance.withdrawal where id=$1 and state in('paid','failed')`, [id]);
    ...
    if (payout.state === 'failed') {
      await this.transactions.write(options(execution), async (context) =>
        this.access.database(context).query(
    ...
    await this.transactions.write(options(execution), async (context) => {
      const client = this.access.database(context);
      const locked = await client.query<{ scope_id: string; settlement_id: string; partner_id: string; amount_minor: number; currency: string }>(
        `select
        ...
        where withdrawal.id=$1 and withdrawal.state='processing' for update`,
        [id]
      );
      ...
      await this.finance.post(context, {
        scope: row.scope_id,
        referenceType: 'finance.withdrawal.paid',
        referenceId: id,
