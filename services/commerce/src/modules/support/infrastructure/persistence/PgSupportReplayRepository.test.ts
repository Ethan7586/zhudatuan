...
const durable = {
  ...
  version: 1,
  scope: 'mall:one',
  aggregate: 'conversation:one',
  payload: { messageId: 'message:two', sequence: 99, version: 5, memberId: 'member:one' },
...
describe('PgSupportReplayRepository', () => {
  it('uses the persisted message sequence and hides internal system traffic from members', async () => {
    const query = vi.fn(async (sql: string) =>
      sql.includes('where message.id=$1') ? result([{ message_id: 'message:two', ticket_id: 'ticket:one', conversation_id: 'conversation:one', member_id: 'member:one', sequence: 2, visibility: 'internal' }]) : result([])
    );
  ...
  it('replays persisted events in message order and advances the Redis resume cursor across filtered rows', async () => {
    ...
    const query = vi.fn(async () => result([truth('message:2', 2, 'external'), truth('message:1', 1, 'external'), truth('message:3', 3, 'internal')]));
    const batch = await withReadTransaction(query, (context) =>
      new PgSupportReplayRepository(runtime(events)).replay(context, { scopes: ['mall:one'], member: 'member:one', storefront: true, conversation: 'conversation:one', cursor: '10-0' })
    );
