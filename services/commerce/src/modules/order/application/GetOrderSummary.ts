...
export class GetOrderSummary {
  async execute(database: OperationDatabase, order: string, scopes: readonly string[], member: string, memberOnly: boolean): Promise<OrderSummary | null> {
    const result = await database.query<{ id: string; scope_id: string; member_id: string; order_number: string; lifecycle_state: string; total_minor: number }>(
