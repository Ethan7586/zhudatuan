...
export class PgStockRequestRepository implements StockRequestRepository {
  ...
  async create(call: Parameters<StockRequestRepository['create']>[0]) {
    ...
    await database.query(
      ...
      values($1,$2,$3,$4,$5,$6,$7,$8,'draft',null,$9,1,$10,$10)`,
      [request.value.id, number('SR'), call.scope, value.customer, value.product, value.pool, value.quantity, value.reason, call.actor, call.now]
    );
  ...
  async update(call: Parameters<StockRequestRepository['update']>[0]) {
    ...
    await database.query(`update voucher.stockrequest set customer_id=$3,product_id=$4,pool_id=$5,quantity=$6,reason=$7,version=$8 where id=$1 and scope_id=$2`, [
      id,
      call.scope,
      value.customer,
      value.product,
  ...
  async submit(call: Parameters<StockRequestRepository['submit']>[0]) {
    ...
    const receipt = await this.approvals.request(write(call), {
      subject: { kind: 'voucherstock', id, version: row.version, snapshot },
      action: 'voucher.stock.allocate',
      constraints: { pool: row.pool_id, product: row.product_id, quantity: Number(row.quantity) },
      expiresAt: null,
    });
  ...
  async get(call: Parameters<StockRequestRepository['get']>[0]) {
    return this.read(call, path(call, 'requestid'), 200, 'voucher.stockrequests.get');
  }
  async list(call: Parameters<StockRequestRepository['list']>[0]) {
    ...
    const rows = await database.query(
      `${STOCK} where request.scope_id=$1 and ($2::text is null or request.state=$2) and ($3::text is null or request.customer_id=$3)
      and ($4::text is null or request.id>$4) order by request.id limit $5`,
  ...
  async options(call: Parameters<StockRequestRepository['options']>[0]) {
    ...
    const fetch = limit(filter.limit);
    const rows = await database.query(
      `${STOCK_CAPACITY} where request.scope_id=$1 and request.state='approved'
      and ($3::text is null or request.id>$3) order by request.id limit $4`,
      [call.scope, null, cursor(filter.cursor), fetch + 1]
    );
    return page<'voucher.stockrequestoptions.list'>(rows.rows, fetch, 'request');
  }
  private async read(
    call: Parameters<StockRequestRepository[keyof StockRequestRepository]>[0],
    id: string,
    status: number,
    operation: 'voucher.stockrequests.create' | 'voucher.stockrequests.update' | 'voucher.stockrequests.submit' | 'voucher.stockrequests.cancel' | 'voucher.stockrequests.get'
  ) {
...
interface StockRow {
  ...
  readonly state: 'draft' | 'submitted' | 'approved' | 'rejected' | 'cancelled' | 'fulfilled';
  readonly version: number;
}
function command(value: Readonly<Record<string, unknown>>) {
  return { customer: text(value.customer, 'customer'), product: text(value.product, 'product'), pool: text(value.pool, 'pool'), quantity: Number(value.quantity), reason: text(value.reason, 'reason') };
}
function model(row: StockRow) {
  return {
    id: row.id,
    scope: row.scope_id,
...
async function lock(database: ReturnType<PgTransactionAccess['database']>, id: string, scope: string): Promise<StockRow> {
  const row = (await database.query<StockRow>(`select ${STOCK_REQUEST_FIELDS} from voucher.stockrequest request where request.id=$1 and request.scope_id=$2 for update`, [id, scope])).rows[0];
  if (!row) throw new DomainError('RESOURCE_NOT_FOUND');
  return row;
}
async function assertReferences(database: ReturnType<PgTransactionAccess['database']>, scope: string, customer: string, product: string, pool: string) {
  const row = await database.query(
    `select 1 from voucher.product product join voucher.credentialpool pool on pool.id=$4 and pool.product_id=product.id and pool.scope_id=product.scope_id where product.id=$3 and product.scope_id=$1 and product.customer_id=$2 and product.state='enabled' and product.pool_id=pool.id and pool.state='open'`,
    [scope, customer, product, pool]
  );
  if (!row.rows[0]) throw new DomainError('VOUCHER_PRODUCT_INCOMPLETE');
}
function snapshotOf(row: StockRow) {
  return { customer: row.customer_id, product: row.product_id, pool: row.pool_id, quantity: Number(row.quantity), reason: row.reason };
}
