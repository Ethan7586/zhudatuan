...
export const statementViewModel = defineSupplierViewModel({
  routes: ['supplierstatements'], title: '对账账单', description: '查看当前供应商范围的账期、收支和期末金额；平台总账不可修改。',
  ...
  project: (value) => operatorCollection(value, operatorItems(value).map((item) => operatorRow({
    key: operatorText(item, 'id'), title: `${operatorText(item, 'period_start')} 至 ${operatorText(item, 'period_end')}`,
    ...
    statusLabel: operatorText(item, 'state') === 'final' ? '账单已确认' : '账单草稿', timestamp: operatorText(item, 'generated_at'),
  }))),
});

function money(value: number | undefined): string { return value === undefined ? '待同步' : `¥${(value / 100).toFixed(2)}`; }
