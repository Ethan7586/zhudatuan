...
export const financeKey = (context: ConsoleContext) => Object.freeze(['console', context.scope.kind, context.scope.id, context.session.accessVersion, 'finance.overview.read'] as const);

export async function readFinance(context: ConsoleContext, signal: AbortSignal) {
  const value = await overviewRead({}, consoleRequest(context.scope, signal, context.session.accessVersion));
