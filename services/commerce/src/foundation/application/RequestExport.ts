...
import { requireAccess, type OperationDatabase } from './ModuleOperations';
import type { OperationRequest } from './OperationHandler';

export type ProjectionExportReport = 'orders' | 'finance.statement';

export async function requestProjectionExport(request: OperationRequest, database: OperationDatabase, report: ProjectionExportReport, filter: Readonly<Record<string, unknown>>) {
