...
export class RiskEngine {
  evaluate(rule: RiskRule, input: RiskEvaluation): RiskResult {
    ...
    let outcome: RiskOutcome = rule.reviewOperations.includes(input.operation) ? 'review' : rule.challengeOperations.includes(input.operation) ? 'challenge' : 'allow';
    ...
    if (rule.velocity && input.velocity > rule.velocity.maximum) {
      outcome = higher(outcome, rule.velocity.outcome);
      reason = 'velocity';
    }
    ...
    const scored: RiskOutcome = score >= rule.thresholds.deny ? 'deny' : score >= rule.thresholds.review ? 'review' : score >= rule.thresholds.challenge ? 'challenge' : 'allow';
