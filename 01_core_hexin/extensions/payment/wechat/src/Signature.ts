import { findWechatPayPlatformKey, type WechatPayConfig } from './Config';
import { epochSeconds, notificationSignatureMessage, verifyRsaSha256 } from './Crypto';
...
export async function verifyWechatPaySignedBody(config: WechatPayConfig, headers: Headers | WechatPaySignatureHeaders, body: string, options: { nowMs?: number; toleranceSeconds?: number } = {}): Promise<WechatPaySignatureHeaders> {
  ...
  const key = findWechatPayPlatformKey(config, values.serial);
  if (!key) throw new WechatPayProtocolError('WECHAT_PAY_PLATFORM_KEY_ID_UNKNOWN');
  ...
  const valid = await verifyRsaSha256(key.publicKeyPem, notificationSignatureMessage(values.timestamp, values.nonce, body), values.signature);
