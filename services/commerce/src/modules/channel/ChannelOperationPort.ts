...
export class ChannelOperationPort {
  ...
  async replayReference(database: OperationDatabase, operation: string, kind: 'order' | 'return' | 'refund'): Promise<string> {
    const result = await database.query<{ internal_reference: string }>(
