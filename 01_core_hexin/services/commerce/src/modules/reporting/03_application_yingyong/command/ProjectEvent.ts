...
import { projectedMetric, type ProjectionEvent } from '../../02_domain_yewu/model/Projection';
import type { Metric } from '../../02_domain_yewu/model/Metric';
...
export class ProjectEvent {
  ...
  private async cancelled(event: ProjectionEvent): Promise<void> {
    await this.repository.cancelOrder(text(event.payload.order, 'REPORT_ORDER_REQUIRED'), event.occurredAt);
  }

  private async shipped(event: ProjectionEvent): Promise<void> {
    await this.repository.shipOrder(text(event.payload.order, 'REPORT_ORDER_REQUIRED'), text(event.payload.state, 'REPORT_SHIPMENT_STATE_REQUIRED'), event.occurredAt);
  }
  ...
  private async voucher(event: ProjectionEvent, affected: Set<string>): Promise<void> {
    const payload = event.payload;
    const mall = text(payload.mall, 'REPORT_MALL_REQUIRED');
    const store = text(payload.store, 'REPORT_STORE_REQUIRED');
    const timezone = text(payload.timezone, 'REPORT_TIMEZONE_REQUIRED');
    const period = await this.repository.period(event.occurredAt, timezone);
    const range = { ...period, timezone };
  ...
  private async statement(event: ProjectionEvent): Promise<void> {
    ...
    await this.repository.saveStatement(event.scope, statement, event.occurredAt);
  }
}
