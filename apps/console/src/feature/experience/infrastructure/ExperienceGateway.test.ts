...
const summary = { id: 'application:one', mallId: 'mall:one', code: 'ONE', publicSlug: 'one', name: '一号商城', status: 'active', version: 4, headSequence: 2, publishedSequence: 1, entry: { handle: 'one', url: 'http://127.0.0.1:3000/s/one', state: 'ready', releaseId: 'release:one', releaseVersion: 'version:one', contentHash: 'a'.repeat(64) }, updatedAt: '2026-09-03T00:00:00.000Z' } as const;
const version = { id: 'version:two', application_id: 'application:one', sequence: 2, schema_version: '2', configuration: createExperienceDocument('application:one', '员工福利', '欢迎选购'), configuration_hash: 'b'.repeat(64), validation_state: 'valid', reason: '测试', created_by: 'actor:one', created_at: '2026-09-03T00:01:00.000Z' } as const;
...
const server = setupServer(http.all('*/api/v1/experiences/**', async ({ request }) => {
...
function context(): ConsoleContext {
  ...
  return { session: { actor: 'actor:one', membership: 'membership:one', accessVersion: 7, permissions: [], capabilities: [], target: 'console', scope, scopes: [scope], assurance: { level: 3 }, security: { hasLocalCredential: true, phoneMasked: null, passwordChangedAt: null }, csrf: 'csrf:one', syncedAt: '2026-09-03T00:00:00.000Z' }, profile: { display_name: '测试', employee_no: null }, scope, scopes: [scope] };
}
