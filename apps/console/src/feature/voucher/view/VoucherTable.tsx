...
function columnsFor(view: VoucherView, selected: ReadonlySet<string>, onOpen: (record: VoucherRecord) => void, onSelect: (id: string) => void): readonly DataColumn<VoucherRecord>[] {
  ...
  if (view === 'reserves')
    return Object.freeze([identity, { key: 'detail', label: '申请单号', render: (row) => <code className="vouchercode">{row.detail}</code> }, quantityColumn('申请数量'), amountColumn('申请金额（最小单位）'), state, timeColumn(), action]);
...
function identityLabel(view: VoucherView): string {
  return ({ programs: '卡券方案', libraries: '卡号库 / 前缀', reserves: '申请名称', batches: '发行批次', statusbatches: '批量任务', bindings: '卡券方案', redemptions: '消费回执', history: '状态记录' } as const)[view];
}
function amountColumn(label: string): DataColumn<VoucherRecord> {
  return { key: 'amount', label, render: formatRecordAmount };
}
function quantityColumn(label: string): DataColumn<VoucherRecord> {
  return { key: 'quantity', label, render: (row) => formatQuantity(row.quantity) };
}
function timeColumn(label = '创建时间'): DataColumn<VoucherRecord> {
  return { key: 'time', label, render: (row) => formatDate(row.occurredAt) };
}
function versionColumn(): DataColumn<VoucherRecord> {
  return { key: 'version', label: '版本', render: (row) => (row.version === null ? '—' : `第 ${row.version} 版`) };
}
