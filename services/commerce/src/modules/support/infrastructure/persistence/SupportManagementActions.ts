import type { SupportPersistence } from './SupportAction';
...
export function supportManagementPersistence(ports: SupportPersistenceFactory): Pick<SupportPersistence, 'manageAssignment' | 'manageAgent' | 'manageAccount' | 'manageRule' | 'manageSla'> {
  const assignment = new AssignmentPolicy();
  return {
    manageAssignment: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const ticket = textField(body, 'case');
      const agent = textField(body, 'agent');
      const repository = ports(database);
      const scopes = await repository.descendants(access.scope.id);
      const locked = await database.query<{ scope_id: string; conversation_id: string; member_id: string | null }>(
        `select ticket.scope_id,
        ...
        on agent.id=$3 and agent.scope_id=ticket.scope_id where ticket.id=$1 and ticket.state<>'closed'
        and ticket.scope_id=any($2::text[]) for update of ticket`,
        [ticket, scopes, agent]
      );
      const target = locked.rows[0];
      if (!target) throw new Error('SUPPORT_ASSIGNMENT_INVALID');
      ...
      const result = await database.query(
        ...
        values($1,$2,$3,$4,clock_timestamp(),$5) returning *`,
        [request.input.path.assignmentid!, ticket, agent, textField(body, 'reason'), target.scope_id]
      );
      ...
      if (target.member_id)
        await new PgRuntimeWriter(database).append({
          ...
          payload: { ticket, conversation: target.conversation_id, agent, member: target.member_id },
          trace: access.trace,
        });
      await repository.history(ticket, target.scope_id, 'assigned', access.actor.id, { agent, reason: body.reason });
      return rowResult(result);
    },
    manageAgent: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const capacity = integerField(body, 'capacity', 1);
      if (capacity > 100) throw new Error('SUPPORT_AGENT_CAPACITY_EXCEEDED');
      const agentState = choice(body.state, ['offline', 'available', 'busy', 'disabled'], 'SUPPORT_AGENT_STATE_INVALID');
      const result = await database.query(
        `insert into support.agent(id,scope_id,membership_id,skills,capacity,state) values($1,$2,$3,$4::jsonb,$5,$6)
        on conflict(id) do update set skills=excluded.skills,capacity=excluded.capacity,state=excluded.state
        where support.agent.scope_id=$2 returning *`,
        [request.input.path.agentid!, access.scope.id, textField(body, 'membership'), JSON.stringify(strings(body.skills)), capacity, agentState]
      );
      if (agentState !== 'available') {
        const stranded = await database.query<{ id: string; scope_id: string; priority: TicketPriority; skill: string }>(
          `select id,scope_id,
          priority,skill from support.ticket where assigned_agent_id=$1 and scope_id=$2 and state in('assigned','waiting')
          order by id for update limit 101`,
          [request.input.path.agentid!, access.scope.id]
        );
        if (stranded.rows.length > 100) throw new Error('SUPPORT_REASSIGNMENT_BATCH_EXCEEDED');
        const repository = ports(database);
        const agents = await repository.agents(access.scope.id);
        const rules = await repository.rules(access.scope.id);
        for (const ticket of stranded.rows) {
          ...
          if (selected)
            await database.query(
              `insert into support.assignment(id,ticket_id,agent_id,reason,assigned_at,scope_id)
            values($1,$2,$3,'agent-unavailable',clock_timestamp(),$4)`,
              [`assignment:${randomUUID()}`, ticket.id, selected.id, ticket.scope_id]
            );
          await database.query(
            `update support.ticket set assigned_agent_id=$2,state=$3,updated_at=clock_timestamp(),version=version+1
            where id=$1`,
            [ticket.id, selected?.id ?? null, selected ? 'assigned' : 'open']
          );
          await repository.history(ticket.id, ticket.scope_id, selected ? 'reassigned' : 'unassigned', access.actor.id, { unavailableAgent: request.input.path.agentid!, assigned: selected?.id ?? null });
        }
      }
      return rowResult(result);
    },
    manageAccount: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const result = await database.query(
        ...
        and ($7::bigint is null or support.account.version=$7) returning *`,
        [
          request.input.path.accountid!,
          access.scope.id,
          choice(body.channel, ['inapp', 'wechat', 'email', 'sms'], 'SUPPORT_CHANNEL_INVALID'),
          textField(body, 'externalRef'),
          body.secretRef ?? null,
          choice(body.state ?? 'active', ['active', 'disabled'], 'SUPPORT_ACCOUNT_STATE_INVALID'),
    ...
    manageRule: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const priorities = strings(body.priorities) as TicketPriority[];
      const state = choice(body.state, ['active', 'disabled'], 'SUPPORT_RULE_STATE_INVALID');
      ...
      const result = await database.query(
        ...
        [request.input.path.ruleid!, access.scope.id, textField(body, 'name'), textField(body, 'skill'), priorities, weight, state, request.input.expectedVersion ?? null]
      );
      if (!result.rows[0]) throw new DomainError('VERSION_CONFLICT');
      return rowResult(result);
    },
    manageSla: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const priority = choice(body.priority, ['low', 'normal', 'high', 'urgent'], 'SUPPORT_PRIORITY_INVALID') as TicketPriority;
      ...
      new Sla(request.input.path.slaid!, access.scope.id, priority, response, resolution, version);
      const result = await database.query(
        ...
        and support.sla.version<$6 returning *`,
        [request.input.path.slaid!, access.scope.id, priority, response, resolution, version]
      );
      if (!result.rows[0]) throw new Error('SUPPORT_SLA_VERSION_CONFLICT');
      return rowResult(result);
    },
  };
}
