...
export class QueueAdmission {
  ...
  async available(candidate: QueueCandidate): Promise<boolean> {
    if (!candidate.id.startsWith('job:') || !/^[a-z]+$/.test(candidate.queue) || !Number.isSafeInteger(candidate.priority)
      || candidate.priority < 0 || candidate.priority > 1000) throw new Error('JOB_ADMISSION_INPUT_INVALID');
