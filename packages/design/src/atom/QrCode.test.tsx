...
describe('QrCode', () => {
  it('produces a deterministic square matrix with a four-module quiet zone', () => {
    const first = qrMatrix('https://yengze.press/s/mall-one');
    expect(first).toEqual(qrMatrix('https://yengze.press/s/mall-one'));
  ...
  it('allows only HTTPS and the explicit 127.0.0.1 development exception', () => {
    ...
    expect(() => qrMatrix('http://yengze.press/s/mall-one')).toThrow('QRCODE_VALUE_INVALID');
    expect(() => qrMatrix('http://localhost:3000/s/mall-one')).toThrow('QRCODE_VALUE_INVALID');
    expect(() => qrMatrix('https://user@yengze.press/s/mall-one')).toThrow('QRCODE_VALUE_INVALID');
    expect(() => qrMatrix(`https://yengze.press/s/${'a'.repeat(2048)}`)).toThrow('QRCODE_VALUE_INVALID');
  });

  it('renders an accessible image primitive', () => {
    const result = renderToStaticMarkup(<QrCode value="https://yengze.press/s/mall-one" label="主商城二维码" />);
