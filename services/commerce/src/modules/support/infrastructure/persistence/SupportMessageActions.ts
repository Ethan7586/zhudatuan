...
import type { EncryptedMessage, SupportPersistenceFactory } from './SupportPersistencePort';
import { supportBoundary } from '../../application/service/SupportBoundary';

export function supportMessagePersistence(kms: KmsClient, objects: Pick<ObjectStore, 'create'>, ports: SupportPersistenceFactory): Pick<SupportPersistence, 'sendMessage' | 'createAttachment'> {
  return {
    sendMessage: supportLifecycle({
    ...
    createAttachment: supportLifecycle({
      prepare: async (request) => {
        const access = requireAccess(request);
        const body = bodyRecord(request.input);
