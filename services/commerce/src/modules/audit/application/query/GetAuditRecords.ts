...
export function getAuditRecordsOperations(repository: AuditPort): OperationActions {
  return {
    'audit.records.read': async (request, database: OperationDatabase) => {
      ...
      const sort = last?.occurred_at;
      const id = last?.id;
      return { status: 200, body: { items, count: items.length, next: hasMore && typeof sort === 'string' && typeof id === 'string' ? encodeCursor({ sort, id }) : null } };
    },
  };
}
