...
const ADMIN_PRESENTATION: RegistrationPresentation = Object.freeze({
  ...
  successNotice: '普通管理员账号已创建，商城与后台身份已同时开通。后台当前为 0 业务权限；请输入刚才设置的密码，登录后进入运营后台。',
});
...
const STOREFRONT_PRESENTATION: RegistrationPresentation = Object.freeze({
  ...
  successNotice: '普通员工商城账号已创建。请输入刚才设置的密码登录。',
});

export function registrationPresentation(target?: CanonicalInvitation['target']): RegistrationPresentation {
