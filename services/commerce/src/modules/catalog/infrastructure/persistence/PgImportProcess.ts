...
export function createImportProcess(factory: ImportBatchFactoryPort, manager: TransactionManager, runtime: ImportPort, jobs: JobPort, catalog: CatalogImportRepository, authorization: TaskAuthorizationPort): ImportProcessPort {
  return factory.create({
    owner: 'catalog',
    failure: 'CATALOG_IMPORT_ROW_FAILED',
    transactions: manager,
    runtime,
    authorization,
    prepare: (target, rows, execution) => manager.read(importOptions(target, execution), (context) => catalog.prepare(context, target.scope, rows)),
    ...
    continue: (context, target, sequence) =>
      jobs
        .create(context, {
          ...
          actor: 'system:catalog',
        })
        .then(() => undefined),
    publish: (target, report, execution) =>
      manager.write(importOptions(target, execution), async (context) => {
...
function importOptions(target: Readonly<{ id: string; scope: string }>, execution: Readonly<{ signal: AbortSignal; deadline: number }>) {
  return {
    ...
    trace: target.id,
    operation: 'job.catalog.import.preflight',
