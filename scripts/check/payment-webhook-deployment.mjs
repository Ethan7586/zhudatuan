...
const [main, runtime, entryTest, config, configTest, service, environment, caddy, delivery,
  ...
  read('services/commerce/src/entry/PaymentWebhookApiMain.ts'),
  read('services/commerce/src/bootstrap/PaymentWebhookApiRuntime.ts'),
  read('services/commerce/src/entry/PaymentWebhookApiEntrypoint.test.ts'),
  read('packages/config/src/PaymentWebhookApiEnvironment.ts'),
  read('packages/config/src/PaymentWebhookApiEnvironment.test.ts'),
  read('infrastructure/zhudatuan/aliyun/systemd/zhudatuan-payment-webhook-api.service'),
  read('infrastructure/zhudatuan/aliyun/payment-webhook-api.env.example'),
  read('infrastructure/zhudatuan/aliyun/Caddyfile'),
  read('infrastructure/zhudatuan/aliyun/delivery.yml'),
  read('database/supabase/migrations/20260903110000_zhudatuan_payment_webhook_access.sql'),
  read('infrastructure/zhudatuan/aliyun/postgres-provision-payment-webhook-role.sh'),
  read('scripts/build-commerce.mjs'),
  read('services/commerce/src/entry/PurchaseApiMain.ts'),
  read('services/commerce/src/entry/PurchaseApiEntrypoint.test.ts'),
]);
...
for (const token of [
  'ConditionPathExists=/opt/zhudatuan/current/services/commerce/dist/PaymentWebhookApiMain.js',
  'ConditionPathExists=/opt/zhudatuan/current/services/commerce/dist/PaymentWebhookApiReadyMain.js',
  ...
  'ExecStart=/usr/bin/env PAYMENT_WEBHOOK_API_PROFILE=payment-webhook-only API_PORT=4326 API_BIND_HOST=127.0.0.1 /usr/bin/node services/commerce/dist/PaymentWebhookApiMain.js',
  'ExecStartPost=/usr/bin/env PAYMENT_WEBHOOK_API_PROFILE=payment-webhook-only API_PORT=4326 API_BIND_HOST=127.0.0.1 /usr/bin/node services/commerce/dist/PaymentWebhookApiReadyMain.js',
...
for (const token of [
  "PaymentWebhookApiMain: 'services/commerce/src/entry/PaymentWebhookApiMain.ts'",
  "PaymentWebhookApiReadyMain: 'services/commerce/src/entry/PaymentWebhookApiReadyMain.ts'",
