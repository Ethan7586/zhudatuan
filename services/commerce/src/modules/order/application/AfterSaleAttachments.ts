...
export class AfterSaleAttachments {
  constructor(private readonly objects: Pick<ObjectStore, 'create'>) {}

  async verify(request: OperationRequest): Promise<readonly VerifiedAfterSaleAttachment[]> {
    const access = requireAccess(request);
    const raw = bodyRecord(request).attachments;
    if (raw === undefined) return Object.freeze([]);
    if (!Array.isArray(raw) || raw.length > 6) throw new DomainError('VALIDATION_FAILED', { field: 'attachments' });
    const owner = createHash('sha256').update(access.membership.id).digest('hex').slice(0, 32);
