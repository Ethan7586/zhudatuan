...
describe('Singleflight', () => {
  ...
  it('cancels a waiter without cancelling work still observed by another waiter', async () => {
    const waiter = new AbortController();
    const singleflight = new Singleflight();
    const operation = (_signal: AbortSignal) => new Promise<string>((resolve) => setTimeout(() => resolve('ok'), 5));
