...
export class ActivateMembership {
  ...
  async execute(database: OperationDatabase, input: ActivationRequest, grant: ActivationGrant): Promise<number> {
    if (
    ...
    return this.versions.activate(database, input.membership, input.trace, { invitation: input.invitation, target: input.target, grantDigest: input.digest });
  }
}
