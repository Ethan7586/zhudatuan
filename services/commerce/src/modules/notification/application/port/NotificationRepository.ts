...
export interface NotificationRepository {
  ...
  changePreference(member: string, organization: string, channel: DeliveryChannelId, event: string, enabled: boolean, authorization: string): Promise<QueryResult<QueryResultRow>>;
  ...
  saveEndpoint(member: string, channel: DeliveryChannelId, envelope: Readonly<{ ciphertext: string; fingerprint: string; keyVersion: string }>): Promise<QueryResult<QueryResultRow>>;
  notifications(membership: string, includeScope: boolean, cursorTime: string | null, cursorId: string | null, fetch: number): Promise<QueryResult<QueryResultRow>>;
  acknowledge(membership: string, notification: string): Promise<QueryResult<{ id: string; readAt: Date }>>;
  ...
  saveAnnouncement(input: Readonly<{ id: string; scope: string; title: string; body: string; audience: unknown; state: string; startsAt: string; endsAt: string | null; expected: number | null }>): Promise<QueryResult<QueryResultRow>>;
