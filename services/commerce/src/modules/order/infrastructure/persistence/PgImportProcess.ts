...
export function createImportProcess(factory: ImportBatchFactoryPort, manager: TransactionManager, runtime: ImportPort, jobs: JobPort,
  orders: OrderImportRepository, authorization: TaskAuthorizationPort): ImportProcessPort {
  return factory.create({
    owner: 'order', failure: 'ORDER_IMPORT_ROW_FAILED', transactions: manager, runtime, authorization,
    ...
    continue: (context, target, sequence) => jobs.create(context, {
      idempotency: `${target.id}:${sequence}`, kind: 'orderimport', owner: 'order', scope: target.scope, queue: 'import',
      payload: Object.freeze({ import: target.id }), actor: 'system:order',
    }).then(() => undefined),
  });
}
