...
export class PgOrderExpiryRepository implements OrderExpiryRepository {
  ...
  async claim(context: WriteTransactionContext, id: string, scope: string): Promise<boolean> {
    const result = await this.transactions.database(context).query(`insert into checkout.expiryreceipt(id,scope_id,completed_at) values($1,$2,clock_timestamp()) on conflict(id) do nothing returning id`, [id, scope]);
