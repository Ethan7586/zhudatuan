...
export interface RedemptionCommand {
...
export class VoucherRedemptionWriter {
  ...
  private async replay(command: ReplayCommand): Promise<RedemptionReceipt | null> {
    ...
    if (prior.rows[0]) {
      ...
      if (
        prior.rows.length !== 1 ||
        row.voucher !== command.voucher ||
        (command.amountMinor !== undefined && Number(row.amountMinor) !== command.amountMinor) ||
        row.verification !== command.verification ||
        row.hold !== command.hold ||
        row.order !== command.order
      )
  ...
  private async lock(command: Pick<RedemptionCommand, 'context' | 'voucher' | 'scope'>): Promise<VoucherRow | undefined> {
    return (
      await this.transactions.database(command.context).query<VoucherRow>(
        `select ${VOUCHER_FIELDS} from voucher.voucher voucher
  ...
  private async record(command: RedemptionCommand, row: VoucherRow): Promise<RedemptionReceipt> {
    ...
    if (command.hold) {
      ...
      if (!hold || hold.state !== 'active') throw new DomainError('VOUCHER_HOLD_CONFLICT');
      if (hold.expires_at <= command.now) throw new DomainError('VOUCHER_HOLD_EXPIRED');
    ...
    await database.query(
      `insert into voucher.redemption(id,scope_id,voucher_id,hold_id,verification_id,order_id,amount_minor,refunded_minor,currency,state,idempotency_key,version,redeemed_at,updated_at,channel,store_id,reporting_scopes,timezone)
      values($1,$2,$3,$4,$5,$6,$7,0,$8,'succeeded',$9,1,$10,$10,$11,$12,$13,$14)`,
      [id, command.scope, row.id, command.hold, command.verification, command.order, command.amountMinor, row.currency, command.idempotency, command.now, snapshot.channel, snapshot.store, snapshot.scopes, snapshot.timezone]
    );
    ...
    await this.finance.post(command.context, {
      ...
      source: { module: 'voucher', aggregate: 'redemption', aggregateId: id, event: 'voucher.redeem', eventId: redemptionEventId(id), leg: 'redeem' },
      currency: row.currency,
      description: '卡券核销',
      debit: { code: `voucher.product.${row.product_id}`, kind: 'liability' },
