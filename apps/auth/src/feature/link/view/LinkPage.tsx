  ...
  return (
    <AuthShell>
      <AuthCard stage={2} onBack={onBack}>
        {state.kind === 'loading' ? (
          <Loading label="正在读取身份关联…" />
        ) : state.kind === 'failed' ? (
          <Alert failure={state.failure} onAction={onBack} />
        ) : (
          <section className="authstatus">
            ...
            {state.kind === 'ready' ? <p role="status">当前账号已有 {state.snapshot.links.length} 个身份关联，系统不会自动合并。</p> : <p role="status">当前会话未发现可直接处理的身份关联。</p>}
          </section>
        )}
      </AuthCard>
    </AuthShell>
