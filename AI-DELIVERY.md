...
带 `<target> <physical-node>` 的 `release/retry` 优先复用与 Source SHA 匹配的不可变制品；缓存缺失或不完整时，同一次 Runner 执行会先安装依赖、只为指定目标构建并上传，然后继续生产切换，不要求用户单独准备制品或“封板”。生产计时从构建完成后开始，只包含制品解析、SSH 下载与校验、原子更新 `current/previous`、重启、健康检查和失败自动恢复，目标为 60 秒以内。60 秒结果只是生产切换的观测数据，不是整次任务耗时，也不是阻塞发布或恢复的门禁。

用户对当前修复说“部署”时，同一开发任务先验证、提交、推送修复并取得新的完整业务 Source SHA，再调用发布入口；不得用不含修复的旧 SHA 代替。控制端仍只派发和展示，不在本地构建或部署。
