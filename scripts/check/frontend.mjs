...
for (const file of files(clientRoot)) {
  ...
  if (/#[0-9a-f]{3,8}\b|\brgba?\s*\(|\bhsla?\s*\(/i.test(source)) findings.push(`DESIGN_TOKEN_BYPASS:${path}`);
...
for (const application of ['auth', 'console', 'storefront']) {
  const source = files(join(clientRoot, application, 'src'))
...
function files(directory, result = []) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile() && /\.(?:ts|tsx|js|jsx|css)$/.test(entry.name) && !/\.(?:test|spec)\./.test(entry.name)) result.push(target);
  }
  return result;
}
