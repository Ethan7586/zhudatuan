...
const names = {
  mapping: '05_docs_ziliao/docs_wendang/requirements/mapping.json',
  requirements: '05_docs_ziliao/docs_wendang/requirements/requirements.yml',
  mvp: '05_docs_ziliao/docs_wendang/requirements/mvp.yml',
  providers: '05_docs_ziliao/docs_wendang/requirements/providers.yml',
  frontend: '05_docs_ziliao/docs_wendang/requirements/frontend.yml',
  contract: '01_core_hexin/packages/contract/src/RequirementCatalog.generated.ts',
};
...
const operations = parse(readFileSync(join(root, '01_core_hexin/packages/contract/definitions/operations.yml'), 'utf8')).operations;
...
if (workbookHash !== authority.sha256) fail('REQUIREMENT_AUTHORITY_HASH_INVALID', '02_platform_pingtai/config/authorities.yml', workbookHash);
...
for (const [prefix, sheet, count, rows] of definitions) {
  ...
  for (const requirement of records) {
    ...
    if (!existsSync(join(root, '01_core_hexin/services/commerce/src/modules', requirement.module))) fail('REQUIREMENT_MODULE_MISSING', location, requirement.module);
...
for (const provider of providers) {
  ...
  if (!required && (provider.extension !== null || existsSync(join(root, '01_core_hexin/extensions/providers', provider.id)))) fail('PROVIDER_EMPTY_EXTENSION_FORBIDDEN', location, String(provider.extension));
