...
function bindRulesCreate(client: OperationExecutor): OperationMethod<"pricing.rules.create"> {
  return bindOperation(client, defineStructuralOperation({"id":"pricing.rules.create","method":"POST","path":"/api/v1/pricing/rules","audience":"operator","idempotent":false,"pathKeys":[]}));
}
...
function bindRulesPublish(client: OperationExecutor): OperationMethod<"pricing.rules.publish"> {
  return bindOperation(client, defineStructuralOperation({"id":"pricing.rules.publish","method":"PUT","path":"/api/v1/pricing/rules/{ruleid}/publication","audience":"operator","idempotent":true,"pathKeys":["ruleid"]}));
}
...
function bindOffersRead(client: OperationExecutor): OperationMethod<"pricing.offers.read"> {
  return bindOperation(client, defineStructuralOperation({"id":"pricing.offers.read","method":"GET","path":"/api/v1/pricing/offers","audience":"member","idempotent":true,"pathKeys":[]}));
}
