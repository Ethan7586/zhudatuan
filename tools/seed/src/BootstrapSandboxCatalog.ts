...
import { SANDBOX_CATALOG_SCHEMA_CHECKSUM, SANDBOX_CATALOG_SCHEMA_VERSION, sandboxCatalogBootstrapEnvironment, sandboxCatalogBootstrapSummary } from './SandboxCatalogBootstrapPlan';
...
async function assertDatabaseBoundary(database: Client, expectedDatabase: string, sentinel: string): Promise<void> {
  ...
  if (
    ...
    row.schema_version !== SANDBOX_CATALOG_SCHEMA_VERSION ||
