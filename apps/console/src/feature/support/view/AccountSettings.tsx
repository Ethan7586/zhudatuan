...
export function AccountSettings({
  ...
  return (
    <section className="supportsettingsection">
      ...
      {row ? (
        <p className={`supportvalidation supportvalidation${row.validationState}`} role="status">
          {validationLabel(row)}
          {row.validatedAt ? ` · ${new Date(row.validatedAt).toLocaleString('zh-CN', { hour12: false })}` : ''}
        </p>
      ) : null}
...
function validationLabel(account: Account): string {
  if (account.validationState === 'verified') return '最近连接验证成功';
  if (account.validationState === 'notrequired') return '站内渠道无需外部连接';
