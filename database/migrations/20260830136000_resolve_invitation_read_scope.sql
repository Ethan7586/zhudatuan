...
do $precondition$ begin
  if not exists(select 1 from runtime.schemaversion where version='20260830135000') then
    raise exception 'INVITATION_READ_SCOPE_PREVIOUS_HEAD_MISSING';
  end if;
  if exists(select 1 from runtime.schemaversion where version='20260830136000') then
...
select runtime.record_migration_evidence('20260830136000',1,1,0,0,
  'select membership.id,access.resource_scope(''identity.invitations.read'',null,membership.id) from access.membership membership where membership.client<>''storefront'';',
...
values('20260830136000',encode(public.digest('20260830136000_resolve_invitation_read_scope','sha256'),'hex'));

do $assert$
declare candidate record; resolved text;
begin
  for candidate in
