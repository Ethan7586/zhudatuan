...
const records = [
  {
    ...
    subject: { type: 'member', id: 'actor:one' },
    object: { type: 'journal', id: 'journal:one' },
    actor: { type: 'member', id: 'actor:one' },
    request: 'request:one',
...
describe('finance audit read', () => {
  it('composes scoped Finance facts, Runtime source events and immutable Audit evidence', async () => {
    ...
    expect(auditPort.records).toHaveBeenCalledWith(transaction, {
      ...
      references: [
        { kind: 'object', id: 'journal:one' },
        { kind: 'object', id: 'entry:one' },
        { kind: 'trace', id: 'trace:one' },
      ],
    });
  ...
  it('fails closed when the reference has no fact in the authorized scope', async () => {
    ...
    const handler = new AuditReadHandler({ read: vi.fn(async () => ({ facts: [], resources: [] })) }, { descendants: vi.fn(async () => ['mall:one']) }, { events: eventsRead }, { records: auditRead });
