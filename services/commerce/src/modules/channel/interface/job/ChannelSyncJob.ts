...
import type { ChannelCatalogPort } from '../../../catalog/public/index';
import type { ChannelPricingPort } from '../../../pricing/public/index';
import type { ChannelInventoryPort } from '../../../inventory/public/index';
import type { ChannelReconciliationPort } from '../../../finance/public/index';
...
export class ChannelJobProcessor implements JobProcessor {
  ...
  private async catalog(job: ClaimedJob, run: RunRow): Promise<void> {
    ...
    try {
      ...
      for (const record of batch.records) {
        ...
        await this.dependencies.catalog.accept(client, { id: `listing:${digest(`${run.scope_id}:${run.provider}:${external}`)}`, provider: run.provider, external, scope: run.scope_id, version, payload: serialized, hash });
      }
  ...
  private async price(job: ClaimedJob, run: RunRow): Promise<void> {
    ...
    try {
      await client.query('begin');
      await this.dependencies.pricing.ensureProviderBook(client, book, run.scope_id, run.provider);
      for (const record of batch.records) {
        const external = text(record.externalId, 'PROVIDER_EXTERNAL_ID_INVALID');
        const amount = integer(record.amountMinor, 'PROVIDER_PRICE_INVALID');
        const sku = await this.dependencies.catalog.sku(client, run.provider, run.scope_id, external);
        if (!sku) continue;
        ...
        await this.dependencies.pricing.saveProviderPrice(client, {
  ...
  private async stock(job: ClaimedJob, run: RunRow): Promise<void> {
    ...
    try {
      ...
      for (const record of batch.records) {
        ...
        await this.dependencies.inventory.observe(client, {
          id,
          scope: run.scope_id,
          sku,
          location: run.region,
          ...
          version: String(record.version ?? job.id),
        });
      }
      await this.finish(client, job, run, batch.records.length, 0, keys.length < 500, keys.at(-1)?.externalId ?? null, []);
      await client.query('commit');
  ...
  private async statement(job: ClaimedJob, run: RunRow): Promise<void> {
    ...
    const statement = await this.extensions.require(run.provider, run.scope_id, 'Statement', 'statement').pullStatement(await this.context(job, run), { start, end, timezone });
    ...
    try {
      ...
      await this.dependencies.finance.receiveReconciliation(client, { id: reconciliation, scope: run.scope_id, provider: run.provider, partner, period: `${start}/${end}`, statement: id, hash: statement.sha256, run: run.run });
      await client.query(
  ...
  private async keys(run: RunRow): Promise<SourceSkuKey[]> {
    const keys = await this.dependencies.catalog.keys(this.pool, run.provider, run.scope_id, run.cursor_value);
  ...
  private async finish(
    ...
    await database.query(
      ...
      where id=$1`,
      [run.run, complete ? 'completed' : 'running', cursor, accepted + rejected, accepted, rejected, JSON.stringify(errors)]
    );
    if (complete)
      await database.query(
