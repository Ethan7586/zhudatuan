...
export interface QualificationRepository {
  ...
  previewDecision(context: ReadTransactionContext, scope: string, member: string, resource: string): Promise<readonly QualificationDecisionRecord[]>;
  previewPolicy(context: ReadTransactionContext, input: Readonly<{ scope: string; id: string; sourceVersion: number | null }>): Promise<QualificationPolicyImpactBasis | null>;
