...
export function Component() {
  ...
  const invitationAvailable = inviteContext !== undefined && context.session.permissions.includes('identity.invitation.manage') && context.session.capabilities.includes('identity.invitations.create') && context.session.csrf !== undefined;
  ...
  return (
    <>
      ...
      />
      {inviteContext === undefined ? null : <MemberInvitationDialog context={inviteContext} open={invitationOpen} onClose={() => setInvitationOpen(false)} />}
      <MemberRegistrationResetDialog
        context={context}
        target={resetTarget}
        onClose={() => setResetTarget(undefined)}
        onReset={() => { void query.refetch(); }}
        {...(inviteContext === undefined ? {} : { onInvite: () => setInvitationOpen(true) })}
      />
    </>
  );
}

function memberInvitationContext(context: ConsoleContext): ConsoleContext | undefined {
  if (context.scope.kind !== 'platform') return context;
  const tenantScopes = context.scopes.filter((scope) => scope.kind === 'tenant');
  if (tenantScopes.length !== 1 || tenantScopes[0] === undefined) return undefined;
  return Object.freeze({ ...context, scope: tenantScopes[0] });
}
