...
export class EvaluateRisk implements RiskCheck {
  ...
  async check(input: RiskCheckInput): Promise<RiskAssessment> {
    ...
    const [storedSignals, blocked] = await Promise.all([this.repository.signals(input.actor, input.scopes), this.repository.blocked(input.actor, input.scopes)]);
    const signals = Object.freeze([...storedSignals, ...input.signals]);
    const policies = records.map((record) => {
      const candidate = new RiskPolicy(record.id, record.activeVersion, record.activeRule, record.activeRollout);
      return candidate.selected(input.actor) || record.baselineVersion === null || record.baselineRule === null ? candidate : new RiskPolicy(record.id, record.baselineVersion, record.baselineRule, 100);
    });
    ...
    for (const policy of policies) {
      ...
      const result = this.engine.evaluate(policy.rule, { actor: input.actor, operation: input.operation, amountMinor: input.amountMinor, velocity, blocked, signals });
    ...
    const decision = await this.repository.decision({
      check: input,
      policy: selected.policy.id,
      version: selected.policy.version,
      outcome: selected.outcome,
      score: selected.score,
      safeReason: selected.reason,
      evidence: {
        ruleHash: selected.policy.hash,
        rolloutPercent: selected.policy.rolloutPercent,
        velocity: selected.velocity,
        signals: signals.map(({ type, value, observedAt }) => ({ type, value, observedAt })),
        amountMinor: input.amountMinor,
        blocked,
        scopeChain: input.scopes,
      },
    });
