...
export class SaveVersion {
  ...
  execute(context: ConsoleContext, draft: VersionDraft, expectedVersion: number, identity: string, signal?: AbortSignal) {
    assertOperationAccess(context, experienceOperations.save);
    return this.port.saveVersion(context, draft, expectedVersion, identity, signal);
  }
}
