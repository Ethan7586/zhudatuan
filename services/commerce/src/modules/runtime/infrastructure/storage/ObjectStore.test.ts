...
describe('HttpObjectStore', () => {
  ...
  it('classifies a dependency outage without exposing its body', async () => {
    ...
    await expect(store.inspect('object:opaque')).rejects.toMatchObject({
      code: 'OBJECT_STORE_UNAVAILABLE', kind: 'unavailable', retryable: true, status: 503,
    });
  });
  ...
  it('accepts only a bounded upload grant whose signed headers match every immutable input', async () => {
    ...
    const fetcher = vi.fn(async (_input: RequestInfo | URL, _init?: RequestInit) => json({ reference: 'object:opaque-upload', url: 'https://upload.objects.internal/signed', method: 'PUT',
      headers: { 'Content-Type': 'image/png', 'Content-Length': '8', 'X-Content-Sha256': sha256, 'X-Retention-Until': retentionUntil }, expiresAt }));
    const store = new HttpObjectStore('https://objects.internal', 'b'.repeat(43), fetcher);
    await expect(store.authorizeUpload({ path: 'tenant/owner/image.png', contentType: 'image/png', size: 8, sha256,
      expiresIn: 300, retentionUntil })).resolves.toMatchObject({ reference: 'object:opaque-upload', method: 'PUT', expiresAt,
      headers: { 'content-type': 'image/png', 'content-length': '8', 'x-content-sha256': sha256, 'x-retention-until': retentionUntil } });
    expect(JSON.parse(String(fetcher.mock.calls[0]?.[1]?.body))).toEqual({ path: 'tenant/owner/image.png', contentType: 'image/png',
      size: 8, sha256, expiresIn: 300, retentionUntil });
  });
  ...
  it.each([
    ...
    const fetcher = async () => json({ reference: 'object:opaque-upload', url: 'https://upload.objects.internal/signed', method: 'PUT', headers,
      expiresAt: new Date(Date.now() + 300_000).toISOString() });
    const store = new HttpObjectStore('https://objects.internal', 'b'.repeat(43), fetcher);
    await expect(store.authorizeUpload({ path: 'tenant/owner/image.png', contentType: 'image/png', size: 8, sha256,
      expiresIn: 300, retentionUntil })).rejects.toThrow('OBJECT_UPLOAD_AUTHORIZATION_RESPONSE_INVALID');
  });

  it('requires explicit lifecycle metadata and separates malware from an incomplete scan', async () => {
    const base = { reference: 'object:opaque', path: 'tenant/owner/file.pdf', sha256: 'a'.repeat(64), size: 10,
      contentType: 'application/pdf', scan: 'clean', retentionUntil: null, lockedUntil: null };
