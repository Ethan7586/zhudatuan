...
import { readMemberInvitees } from '../04_adapters_shixian/persistence/MemberInviteeReader';
...
export function memberOperatorReadActions(kms: Pick<KmsClient, 'decrypt'>): OperationActions {
  return {
    ...
    'member.storefront.invitees.read': async (request, database) => {
      ...
      const result = await readMemberInvitees(database, access.scope.id, request.input.path.membershipid!,
