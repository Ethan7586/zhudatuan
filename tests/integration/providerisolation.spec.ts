...
import { assertProviderReady } from '../../services/commerce/src/bootstrap/ProviderBootstrap';
...
test('provider worker is an isolated entrypoint with secret references, CAS activation and bounded execution', () => {
  const main = text('../../services/commerce/src/app/ProviderMain.ts');
  const runtime = text('../../services/commerce/src/bootstrap/ProviderRuntime.ts');
  const loader = text('../../services/commerce/src/bootstrap/ProviderLoader.ts');
  const registry = text('../../services/commerce/src/bootstrap/ExtensionRegistry.ts');
