...
import { keysetPage } from '../../../../foundation/interface/Validation';
...
export class PgRecoveryRepository implements RecoveryRepository {
  constructor(
    ...
    private readonly orders: Pick<PaymentOrderPort, 'numbers'>
  ) {}
  async read(context: ReadTransactionContext, input: Parameters<RecoveryRepository['read']>[1]) {
    ...
    >(
      ...
      [input.page.sort, input.page.id, input.page.fetch, scopes]
    );
  ...
  async resolve(context: WriteTransactionContext, input: Parameters<RecoveryRepository['resolve']>[1]) {
    ...
    if (input.action === 'resolve') {
      await database.query(
        `update payment.recoverycase set state='resolved',resolved_at=clock_timestamp(),resolution_request_id=$2
         where id=$1 and state='open'`,
        [input.case, request]
      );
      if (recovery.resource_type === 'deadletter') await runtime.reviewDeadletter(recovery.evidence.deadletter, 'payment');
    }
  ...
  private async retryRefund(database: ReturnType<PgTransactionAccess['database']>, runtime: PgRuntimeWriter, recovery: RecoveryRecord, request: string): Promise<void> {
    ...
    const selected = await database.query<{
      state: string;
    }>('select state from payment.refund where id=$1 for update', [refund]);
    ...
    if (selected.rows[0].state === 'failed') {
      await database.query(`update payment.refund set state='requested',version=version+1 where id=$1 and state='failed'`, [refund]);
