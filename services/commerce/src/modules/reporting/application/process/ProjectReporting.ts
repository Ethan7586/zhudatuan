...
function cacheKeys(scope: string, projectionVersion: number): readonly string[] {
  const metrics = ['dashboard', 'sales', 'product', 'mall', 'category', 'channel', 'voucher', 'member'] as const;
  const periods = ['realtime', 'yesterday', '7days', '30days'] as const;
  return metrics.flatMap((metric) => periods.map((period) => VersionedKey.create('reportingwatermark', { scope, metric, period, projectionversion: projectionVersion })));
}
