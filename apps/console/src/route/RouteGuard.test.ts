...
describe('Console RouteGuard', () => {
  ...
  it('rejects a compiled but hidden deep link before rendering it', async () => {
    const response = rejected('/scopes/enterprise/enterprise%3Aone/finance', { capabilities: [...access.capabilities, 'finance.overview.read'], permissions: [...access.permissions, 'finance.overview.read'] });
  ...
  it('rejects a route when its generated operation capability is absent', async () => {
    const response = rejected('/scopes/enterprise/enterprise%3Aone/settings/access', { ...access, capabilities: [] });
  ...
  it('rejects a route when its generated operation permission is absent', async () => {
    const response = rejected('/scopes/enterprise/enterprise%3Aone/settings/access', { ...access, permissions: [] });
  ...
  it('rejects a valid route pattern outside the route declared scope', async () => {
    ...
    const response = rejected('/scopes/platform/platform%3Aone/finance', access, platform);
    expect(await response.text()).toBe('SCOPE_DENIED');
  });

  it('returns the standard not-found error for an unknown route', async () => {
    const response = rejected('/scopes/enterprise/enterprise%3Aone/private', access);
