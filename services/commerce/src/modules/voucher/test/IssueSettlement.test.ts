...
describe('issue batch retry settlement', () => {
  it('posts only newly successful credentials and emits one transactional event per receipt', async () => {
    ...
    const query = vi.fn(async (sql: string, values?: readonly unknown[]) => {
      if (sql.includes('from voucher.issuebatch batch') && sql.includes('for update')) return result([{ id: 'issuebatch:one', order_id: 'issueorder:one', state: 'running', requested: 5, processed: 5, succeeded, failed: 5 - succeeded, retryable: 5 - succeeded, version }]);
      ...
      if (sql.startsWith('update voucher.issuebatch set accounted')) { accounted = Number(values?.[2]); version++; }
      return result([]);
    });
    ...
    expect(events.map(({ payload }) => payload)).toEqual([
      { batch: 'issuebatch:one', count: 3, amountMinor: 3000 }, { batch: 'issuebatch:one', count: 2, amountMinor: 2000 },
    ]);
