...
import { bootstrapPort, environment, identitySdk, membership } from '../TestData';
...
describe('MembershipGateway', () => {
  it('reads the server selection and approves only the configured target origin', async () => {
    ...
    await expect(gateway.read('storefront')).resolves.toMatchObject({ memberships: [membership] });
    await expect(gateway.select('membership-1', 'storefront')).resolves.toEqual({ redirectUrl: 'http://127.0.0.1:3000/orders' });
  ...
  it('rejects a cross-origin completion redirect', async () => {
    ...
    await expect(gateway.select('membership-1', 'storefront')).rejects.toThrow('RETURN_TARGET_INVALID');
  });
});
