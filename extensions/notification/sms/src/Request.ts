...
export function createAliyunRequest(endpoint: string, message: SmsMessage, credential: SmsAccessCredential, now: Date, nonce: string, signal?: AbortSignal): TransportRequest {
  const body = new URLSearchParams({ PhoneNumbers: message.phoneNumbers, SignName: message.signName, TemplateCode: message.templateCode, TemplateParam: message.templateParam, OutId: message.outId }).toString();
  ...
  return Object.freeze({ url: `https://${endpoint}/`, method: 'POST', headers: Object.freeze(headers), body, ...(signal ? { signal } : {}) });
}
