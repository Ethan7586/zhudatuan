...
export function MemberTable({ rows, canDecide, onDecide }: Readonly<{ rows: readonly ReferralMember[]; canDecide: boolean; onDecide: (action: ReferralAction) => void }>) {
  const columns: readonly DataColumn<ReferralMember>[] = [
    { key: 'member', label: '会员', render: (row) => row.memberId },
    ...
    { key: 'version', label: '版本', render: (row) => `v${row.version}` },
    {
      ...
      render: (row) => (
        <div className="referralrowactions">
          <Button onPress={() => onDecide({ kind: 'approve', id: row.id, version: row.version, label: `通过会员 ${row.memberId}` })} isDisabled={!canDecide || row.status !== 'applied'}>
            通过
          </Button>
          <Button onPress={() => onDecide({ kind: 'disqualify', id: row.id, version: row.version, label: `取消会员 ${row.memberId} 资格` })} isDisabled={!canDecide || row.status !== 'active'}>
            取消资格
          </Button>
        </div>
      ),
