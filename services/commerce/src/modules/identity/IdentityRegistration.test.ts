...
describe('canonical member registration security boundary', () => {
  ...
  it('creates storefront and console memberships atomically for a pending administrator invitation', async () => {
    const harness = registrationHarness({ challengeAccepted: true, subjectExists: false, invitationTarget: 'operator' });

    const response = await identityOperations(context(harness.pool)).invoke(registrationRequest('registration:administrator'));

    expect(response).toMatchObject({ status: 201, body: { client: 'storefront', status: 'active' } });
    const membershipWrites = harness.queries.filter(({ text }) => text.includes('insert into access.membership('));
    expect(membershipWrites).toHaveLength(2);
    expect(membershipWrites[0]?.text).toContain("'storefront'");
    expect(membershipWrites[1]?.text).toContain("'operator'");
    const roleWrite = harness.queries.find(({ text }) => text.includes('insert into access.membershiprole'));
    expect(roleWrite?.values).toEqual([expect.stringMatching(/^membership:/), 'role:self', expect.stringMatching(/^membership:/), 'role-console-pending-v1:tenant:one']);
    expect(roleWrite?.text.match(/'role:self'/g)).toHaveLength(2);
  });
});
...
function registrationHarness(
  input: Readonly<{
    ...
    invitationTarget?: 'storefront' | 'operator';
  }>
): Readonly<{
  pool: DatabasePool;
  ...
  const client = {
    query: async (text: string, values: readonly unknown[] = []) => {
      ...
      if (text.includes('update member.invite set use_count') && input.invitationTarget !== undefined) {
        return result([
          {
            organization_id: 'tenant:one',
            role_id: input.invitationTarget === 'operator' ? 'role-console-pending-v1:tenant:one' : 'role-employee',
            storefront_role_id: input.invitationTarget === 'operator' ? 'role:self' : 'role-employee',
            target_client: input.invitationTarget,
            terms_hash: 'f'.repeat(64),
          },
        ]);
      }
      if (text.includes('select kind from organization.organization')) return result([{ kind: 'tenant' }]);
      if (text.includes('insert into access.membership') && text.includes("'storefront'") && text.includes('returning *')) {
        return result([
          {
            id: String(values[0]),
...
function context(pool: DatabasePool): ModuleContext {
  ...
  container.bind(KMS_CLIENT, {
    encrypt: async () => ({ ciphertext: 'encrypted-mobile', fingerprint: 'mobile-fingerprint', keyVersion: 1 }),
