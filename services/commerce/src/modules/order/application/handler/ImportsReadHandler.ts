...
export class ImportsReadHandler implements DurableOperationHandler<'order.imports.read', undefined, RuntimeImportRecord, 'read'> {
  ...
  constructor(private readonly imports: ImportPort, private readonly objects: ObjectStore) {}
  prepare(_input: OperationInputFor<'order.imports.read'>, _context: PrepareContext<'order.imports.read'>): Promise<undefined> { return Promise.resolve(undefined); }
  ...
  async finalize(_input: OperationInputFor<'order.imports.read'>, record: RuntimeImportRecord,
    _context: FinalizeContext<'order.imports.read'>): Promise<OperationReply<OperationOutputFor<'order.imports.read'>>> {
    const body = record.body;
