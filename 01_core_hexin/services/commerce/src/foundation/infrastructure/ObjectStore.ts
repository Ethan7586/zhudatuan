...
export interface ObjectStore {
  ...
  find(path: string): Promise<(StoredObject & { readonly contentType: string }) | null>;
  read(reference: string, maximum: number): Promise<Uint8Array>;
  inspect(reference: string): Promise<StoredObject & { readonly contentType: string }>;
...
export class HttpObjectStore implements ObjectStore {
  ...
  constructor(private readonly endpoint: string, private readonly bearer: string, fetcher: typeof fetch = fetch) {
  ...
  async find(path: string): Promise<(StoredObject & { readonly contentType: string }) | null> {
    if (!/^[a-z0-9][a-z0-9/.-]{2,255}$/.test(path)) throw new Error('OBJECT_PATH_INVALID');
    const response = await this.http.send(`${this.endpoint.replace(/\/$/, '')}/v1/objects/metadata?path=${encodeURIComponent(path)}`, {
      headers: { accept: 'application/json', authorization: `Bearer ${this.bearer}` }, redirect: 'error',
    }, { mode: 'read' });
    if (response.status === 404) return null;
    if (!response.ok) throw new Error('OBJECT_STORE_UNAVAILABLE');
    const value = await response.json() as Record<string, unknown>;
    if (typeof value.reference !== 'string' || typeof value.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(value.sha256)
      || !Number.isSafeInteger(value.size) || (value.size as number) < 1 || value.scan !== 'clean' || typeof value.contentType !== 'string') {
      throw new Error('OBJECT_METADATA_INVALID');
    }
    return { reference: value.reference, sha256: value.sha256, size: value.size as number, scan: 'clean', contentType: value.contentType };
  }
  ...
  async inspect(reference: string): Promise<StoredObject & { readonly contentType: string }> {
    ...
    const value = await response.json() as Record<string, unknown>;
    if (value.reference !== reference || typeof value.sha256 !== 'string' || !/^[a-f0-9]{64}$/.test(value.sha256)
      || !Number.isSafeInteger(value.size) || (value.size as number) < 1 || value.scan !== 'clean'
      || typeof value.contentType !== 'string') throw new Error('OBJECT_METADATA_INVALID');
    return { reference, sha256: value.sha256, size: value.size as number, scan: 'clean', contentType: value.contentType };
  }
  ...
  async authorize(reference: string, seconds: number): Promise<Readonly<{ url: string; expiresAt: string }>> {
    ...
    const response = await this.call('/v1/objects/authorizations', { method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ reference, expiresIn: seconds }) });
    const value = await response.json() as Record<string, unknown>;
    if (typeof value.url !== 'string' || !value.url.startsWith('https://') || typeof value.expiresAt !== 'string'
      || Number.isNaN(Date.parse(value.expiresAt)) || Date.parse(value.expiresAt) <= Date.now()) throw new Error('OBJECT_AUTHORIZATION_RESPONSE_INVALID');
    return Object.freeze({ url: value.url, expiresAt: value.expiresAt });
  }

  request(path: string, init: RequestInit): Promise<Response> { return this.call(path, init, mode(init)); }

  private async call(path: string, init: RequestInit, callMode: HttpCallMode = mode(init)): Promise<Response> {
    const response = await this.http.send(`${this.endpoint.replace(/\/$/, '')}${path}`, { ...init,
      headers: { accept: 'application/json', authorization: `Bearer ${this.bearer}`, ...init.headers }, redirect: 'error' }, { mode: callMode });
    if (!response.ok) throw new Error('OBJECT_STORE_UNAVAILABLE');
    return response;
  }
}
...
class HttpUpload implements ObjectUpload {
  ...
  constructor(private readonly store: HttpObjectStore, private readonly id: string) {}
  ...
  async append(bytes: Uint8Array): Promise<void> {
    ...
    await this.store.request(`/v1/uploads/${encodeURIComponent(this.id)}/parts/${this.sequence}`, { method: 'PUT', body: bytes as BodyInit,
  ...
  async complete(): Promise<StoredObject> {
    ...
    const response = await this.store.request(`/v1/uploads/${encodeURIComponent(this.id)}/completion`, { method: 'POST',
      headers: { 'content-type': 'application/json' }, body: JSON.stringify({ parts: this.sequence, sha256, size: this.size }) });
