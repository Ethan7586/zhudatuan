...
const inventoryActions: readonly OperatorAction[] = Object.freeze([
  ...
  Object.freeze({ id: 'import', label: '上传库存文件', description: '校验并加密上传 CSV/XLSX，再创建可追踪的导入任务。', confirmation: '文件上传后由服务端异步校验和写入；页面不会把“已上传”误报为“已完成”。', tone: 'primary', identityScope: true, fields: Object.freeze([actionField('file', '库存文件', { kind: 'file' })]) }),
...
function inventoryProjection(value: unknown) {
  ...
  if (items.length > 0) return operatorCollection(value, items.map((item) => operatorRow({
    key: operatorText(item, 'sku'), title: `商品 ${operatorText(item, 'sku')}`,
    detail: `在手 ${count(item, 'onhand')} · 预占 ${count(item, 'reserved')} · 安全库存 ${count(item, 'safety')} · 可售 ${count(item, 'available')}`,
    statusLabel: inventoryStatus(operatorText(item, 'state')), timestamp: operatorText(item, 'watermark'),
  })));
  const job = operatorRecord(value);
  const id = job && operatorText(job, 'id');
  return operatorCollection(value, id ? [operatorRow({
    key: id, title: `库存导入 ${id}`, detail: `总计 ${count(job!, 'total_count')} · 成功 ${count(job!, 'success_count')} · 失败 ${count(job!, 'failure_count')}`,
    statusLabel: importStatus(operatorText(job!, 'state')), timestamp: operatorText(job!, 'updated_at'),
  })] : []);
}

function count(item: Parameters<typeof operatorText>[0], key: string): number { return operatorNumber(item, key) ?? 0; }
function inventoryStatus(value: string): string { return value === 'available' ? '库存可售' : value === 'blocked' ? '库存已冻结' : '当前不可售'; }
