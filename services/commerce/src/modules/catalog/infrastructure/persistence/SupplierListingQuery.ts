...
export async function readSupplierListings(database: SqlExecutor, filter: ListingFilter) {
  const result = await database.query<Record<string, unknown>>(
    `with supplieritems as(
      ...
      sku.code,product.id product_id,product.product_type,product.attributes->>'coverObject' cover_object,product.attributes->>'coverUrl' cover_url,
      ...
      product.attributes->>'coverObject' cover_object,product.attributes->>'coverUrl' cover_url,product.attributes->>'subtitle' subtitle,product.category_id,
      ...
      product_type,cover_object,cover_url,subtitle,category_id,category_name,source,source_partner_id,pool_name,sku_count,sku_total,
