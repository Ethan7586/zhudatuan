...
do $precondition$ begin
  if not exists(select 1 from runtime.schemaversion where version='20260831027000') then
    raise exception 'MANAGED_MEMBER_SCOPE_PREVIOUS_HEAD_MISSING';
  end if;
  if exists(select 1 from runtime.schemaversion where version='20260831028000') then
    raise exception 'MANAGED_MEMBER_SCOPE_ALREADY_APPLIED';
  end if;
end $precondition$;
...
select runtime.record_migration_evidence('20260831028000',0,0,0,0,
  'select id,organization_id,status,access_version from access.membership order by id;',
  'select access.resource_scope(''identity.members.manage'',id,id) from access.membership order by id;');
insert into runtime.schemaversion(version,checksum)
values('20260831028000','9d25803290be248ccb3fbc10cd4abd3c5df31991c1c8023aff1abb60536bc203');
