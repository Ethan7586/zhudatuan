...
const operation = OperationCatalog.get('identity.sessions.create');

describe('PublicActorFingerprint', () => {
  it('is stable across equivalent IPv4 and mapped IPv6 peers', () => {
    const input = { body: { method: 'invitation', code: 'ABCD-EFGH-JKMN-PQRS-TVWX-YZ12', target: 'storefront' } };
  ...
  it('isolates target, device, network prefix and credential without exposing them', () => {
    const actor = fingerprint.create(
      operation,
      { body: { method: 'invitation', code: 'private-invitation-code', target: 'storefront' } },
    ...
    expect(actor).not.toBe(
      fingerprint.create(operation, { body: { method: 'invitation', code: 'other-code', target: 'storefront' } }, { 'x-client-target': 'storefront', 'x-device-id': 'private-device', 'x-peer-address': '2001:db8:1:2::10' })
    );
  });
