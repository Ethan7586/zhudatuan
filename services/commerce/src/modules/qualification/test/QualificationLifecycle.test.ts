...
describe('qualification lifecycle', () => {
  it('verifies the stored digest before publication, persists once and schedules the exact expiry version', async () => {
    ...
    const input = {
      path: { qualificationid: 'qualification:one' },
      query: {},
      body: {
        title: '食品经营许可证',
  ...
  it('fails publication before opening the write transaction when evidence was replaced', async () => {
    ...
    const handler = new QualificationsPublishHandler(
      cases,
      { inspect: async (reference: string) => ({ reference, sha256: 'b'.repeat(64), size: 128, scan: 'clean', contentType: 'application/pdf', path: 'qualification/license.pdf' }) } as never,
  ...
  it('revokes the optimistic version and emits the immediate risk event', async () => {
    ...
    const result = await handler.execute({ path: { qualificationid: published.id }, query: {}, body: { reason: '证照已被监管机构撤销' } } as never, context('qualification.qualifications.revoke', 1));
...
function record(value: QualificationCaseSnapshot): QualificationCaseRecord {
  return {
    ...
    evidence_count: value.evidence.length,
