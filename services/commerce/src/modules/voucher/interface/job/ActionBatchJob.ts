...
export class ActionBatchJob implements JobProcessor {
  ...
  process(job: ClaimedJob, signal: AbortSignal, deadline = Date.now() + 120_000): Promise<void> {
    ...
    return typeof payload.batch === 'string'
      ? this.task.execute(job.id, payload.batch, text(job.scope, 'VOUCHER_SCOPE_REQUIRED'), signal, deadline)
      : this.task.maintain(job.id, signal, deadline);
  }
}
