...
describe('operationFingerprint', () => {
  it('defines the single canonical request payload used by action proofs and execution', () => {
    ...
    expect(JSON.stringify(operationFingerprint('access.scopes.manage', input, 3))).toBe('{"operation":"access.scopes.manage","input":{"body":{"targetMembership":"membership:target","effect":"allow"}},"expectedVersion":3}');
