...
describe('runtime export ownership and one-time issuance SQL', () => {
  it('binds evidence and download consumption to the original actor, owner and scope', async () => {
    ...
    try {
      for (const [actor, scope, owner] of [['actor:other', 'scope:test', 'voucher'], ['actor:test', 'scope:other', 'voucher'], ['actor:test', 'scope:test', 'reporting']]) {
      ...
      const attempts = await Promise.all(Array.from({ length: 3 }, () => withWriteTransaction(data.query,
        context => data.exports.take(context, data.id, 'scope:test', 'voucher'))));
  ...
  it('namespaces idempotency by actor and refuses changed requests under the same key', async () => {
    const data = await fixture();
    try {
      const request = { scope: 'scope:test', owner: 'voucher', kind: 'search', snapshot: { snapshot: 'snapshot:test' },
        authorization: { actor: 'actor:test' }, actor: 'actor:test', idempotency: 'export:key' };
      ...
      const another = await data.manager.write(options('actor:other', 'scope:test'), context => data.exports.create(context,
        { ...request, actor: 'actor:other', authorization: { actor: 'actor:other' } }));
...
function options(actor: string, scope: string) {
  return { tenant: scope, actor, scope, membership: 'membership:test', trace: 'trace:test', operation: 'voucher.exports.get',
    signal: new AbortController().signal, deadline: Date.now() + 10_000 };
}
...
async function fixture() {
  ...
  const created = await withWriteTransaction(query, context => exports.create(context, { scope: 'scope:test', owner: 'voucher', kind: 'search',
    snapshot: { snapshot: 'snapshot:test' }, authorization: { actor: 'actor:test' }, actor: 'actor:test', idempotency: 'export:key' }));
  await withWriteTransaction(query, context => exports.claim(context, created.id, 'scope:test', 'voucher'));
  await withWriteTransaction(query, context => exports.ready(context, created.id, 'scope:test', 'voucher', { reference: 'private/reference', sha256: 'a'.repeat(64),
    rows: 2, tokenHash: 'b'.repeat(64), expiresAt: new Date(Date.now() + 120_000).toISOString() }));
