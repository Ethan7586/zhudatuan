...
import { QuoteDependencyCall } from '../../application/service/QuoteDependencyCall';
...
export class CurrentQuoteReader {
  ...
  async execute(request: OperationRequest, context: ReadTransactionContext): Promise<OperationResult> {
    ...
    const priced = await this.calls.execute('pricing', { expiresAt: request.input.deadline, signal: request.input.signal }, () => this.pricing.quote(context, stored.quoteId, owner.member, owner.organization));
    ...
    return { status: 200, body: { quote: quoteResult(current, quote, null) } };
  }
}
