...
const organizations: Pick<OrganizationReadPort, 'scope'> = { scope: async (_context, id) =>
  ({ id, scopeKind: 'mall', timezone: 'Asia/Shanghai', tenant: 'scope:root', ancestors: ['scope:root'], descendants: [] }) };

const now = new Date('2026-09-05T00:00:00.000Z');
const source = { id: 'voucher:one', credential_id: 'credential:one', product_id: 'product:one', holder_id: 'holder:one',
  initial_minor: 1000, remaining_minor: 1000, currency: 'CNY', state: 'active', starts_at: new Date(now.getTime() - 60_000),
  expires_at: new Date(now.getTime() + 60_000), version: 3 };
...
describe('voucher settlement transaction', () => {
  it('atomically redeems a verified voucher, records history and posts once', async () => {
    ...
    await withWriteTransaction(query, context => new VoucherRedemptionWriter({ post }, organizations).redeem({ context, scope: 'mall:one',
      voucher: source.id, hold: null, verification: 'verification:one', order: null, amountMinor: 400,
      idempotency: 'redeem:one', actor: 'actor:one', now }));
    ...
    expect(post).toHaveBeenCalledExactlyOnceWith(expect.anything(), expect.objectContaining({ amountMinor: 400,
      debit: { code: 'voucher.product.product:one', kind: 'liability' }, credit: { code: 'commerce.clearing', kind: 'income' } }));
  });
  ...
  it.each([
    ...
    await expect(withWriteTransaction(query, context => new VoucherRedemptionWriter({ post }, organizations).redeem({ context, scope: 'mall:one',
      voucher: source.id, hold: 'hold:one', verification: 'verification:one', order: 'order:one', amountMinor: 400,
      idempotency: 'redeem:one', actor: 'actor:one', now }))).rejects.toThrow(code);
  ...
  it('validates cumulative refunds before restoring value and reverses the original account types', async () => {
    ...
    const query = vi.fn(async (sql: string, values?: readonly unknown[]) => {
      ...
      if (sql.includes('for update of redemption')) return result([{ ...source, voucher_id: source.id, voucher_state: 'active',
        remaining_minor: 700, amount_minor: 400, refunded_minor: 100, hold_id: null, verification_id: 'verification:one',
        channel: 'manual', store: null, order_id: null, scopes: ['mall:one', 'scope:root'], timezone: 'Asia/Shanghai',
        redemption_state: 'partiallyrefunded', redemption_version: 2 }]);
    ...
    await withWriteTransaction(query, context => new VoucherRefundWriter({ post }).refund({ context, scope: 'mall:one',
      redemption: 'redemption:one', amountMinor: 300, reason: '退款返还', idempotency: 'refund:one', actor: 'actor:one', now }));
    expect(post).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ amountMinor: 300,
      debit: { code: 'commerce.clearing', kind: 'income' }, credit: { code: 'voucher.product.product:one', kind: 'liability' } }));
    post.mockClear();
    await expect(withWriteTransaction(query, context => new VoucherRefundWriter({ post }).refund({ context, scope: 'mall:one',
      redemption: 'redemption:one', amountMinor: 301, reason: '退款返还', idempotency: 'refund:over', actor: 'actor:one', now }))).rejects.toThrow('VOUCHER_REFUND_EXCEEDS_REDEMPTION');
    expect(post).not.toHaveBeenCalled();
  });
});
