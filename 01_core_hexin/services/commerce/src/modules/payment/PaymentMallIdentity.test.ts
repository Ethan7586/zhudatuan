...
import { PaymentDeadletter } from './PaymentDeadletter';
...
describe('Payment mall identity', () => {
  ...
  it('keeps webhook, job, refund settlement, and deadletter recovery independent of OrderRecord joins', async () => {
    for (const file of [
      new URL('./PaymentWebhook.ts', import.meta.url),
      new URL('./PaymentJobs.ts', import.meta.url),
      new URL('./application/RefundPlanner.ts', import.meta.url),
      new URL('./application/RefundSettlement.ts', import.meta.url),
      new URL('./PaymentDeadletter.ts', import.meta.url),
    ]) {
