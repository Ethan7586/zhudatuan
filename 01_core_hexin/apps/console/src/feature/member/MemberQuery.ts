...
export const memberKey = (context: ConsoleContext, cursor?: string) => Object.freeze([
  'console', context.scope.kind, context.scope.id, context.session.accessVersion, 'member.members.read', cursor ?? null, 50,
] as const);
export async function readMembers(context: ConsoleContext, cursor: string | undefined, signal: AbortSignal) {
  return MemberPageSchema.parse(await membersRead({ query: { limit: 50,
