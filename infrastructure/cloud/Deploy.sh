...
kubectl -n "$namespace" scale deployment/shop-api deployment/shop-jobs deployment/shop-provider --replicas=0
kubectl -n "$namespace" wait --for=delete pod -l 'app.kubernetes.io/name in (shop-api,shop-jobs,shop-provider)' --timeout=180s

rollback() {
  trap - ERR
  "$cutover" maintenance on --release "$release_id" || true
  kubectl -n "$namespace" scale deployment/shop-api deployment/shop-jobs deployment/shop-provider --replicas=0 || true
  "$cutover" rollback --release "$previous_release" --snapshot "$previous_snapshot" || true
  fail "automatic rollback invoked for $release_id"
}
trap rollback ERR

render "$(dirname "$0")/../container/Migration.yml" | kubectl apply --server-side --field-manager=shop-release -f -
kubectl -n "$namespace" wait --for=condition=complete "job/shop-migration-$release_id" --timeout=1800s

for client in console storefront auth; do
...
render "$(dirname "$0")/../container/Runtime.yml" | kubectl apply --server-side --field-manager=shop-release -f -
render "$(dirname "$0")/../container/Provider.yml" | kubectl apply --server-side --field-manager=shop-release -f -
render "$(dirname "$0")/../network/Policy.yml" | kubectl apply --server-side --field-manager=shop-release -f -
...
kubectl -n "$namespace" rollout status deployment/shop-provider --timeout=600s
for percentage in 1 10 50 100; do
  "$cutover" provider canary --release "$release_id" --percent "$percentage"
  "$cutover" provider verify --release "$release_id" --percent "$percentage"
done
kubectl -n "$namespace" run "shop-smoke-$release_id" --restart=Never --image="$commerce_image" --env="SHOP_SMOKE_RELEASE=$release_id" --env="SHOP_SMOKE_BASE_URL=http://shop-api.$namespace.svc.cluster.local" -- services/commerce/dist/SmokeMain.js
