...
describe('PermissionPolicy', () => {
  it('accepts only known delegatable permissions that the issuer effectively allows', () => {
    ...
    for (const target of [['access.owner.transfer'], ['unknown.permission'], ['order.create']]) {
      expect(() => policy.assertSubset(new Set(['access.owner.transfer', 'unknown.permission', 'order.read']), new Set(), target)).toThrow('DELEGATION_DENIED');
    }
