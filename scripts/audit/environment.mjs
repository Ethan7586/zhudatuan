...
for (const file of productionSources()) {
  ...
  if (source.includes('wx.getExtConfigSync') && name !== 'apps/miniapp/miniprogram/app.js') {
    violations.push({ code: 'MINIAPP_ENVIRONMENT_READ_OUTSIDE_OWNER', location: name, detail: 'wx.getExtConfigSync' });
  }
}

const miniappEntry = readFileSync(join(root, 'apps/miniapp/miniprogram/app.js'), 'utf8');
if (!miniappEntry.includes("require('./config/Environment')") || !miniappEntry.includes('environment(wx.getExtConfigSync())')) {
  violations.push({ code: 'MINIAPP_ENVIRONMENT_SCHEMA_BYPASSED', location: 'apps/miniapp/miniprogram/app.js', detail: 'generated schema required' });
}
