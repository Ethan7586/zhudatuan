...
import { EventRegistry } from '../../composition/EventRegistry';

describe('RuntimeEventPublisher', () => {
  it('types polymorphic event parameters and enqueues the declared handler', async () => {
    const query = vi.fn(async (sql: string, _values?: readonly unknown[]) =>
      sql.includes('runtime.accept_inbox') ? { rows: [{ inserted: true }], rowCount: 1 } : sql.includes('select exists(select 1 from runtime.jobs') ? { rows: [{ existing: false, depth: 0 }], rowCount: 1 } : { rows: [], rowCount: 1 }
    );
