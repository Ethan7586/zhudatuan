...
do $precondition$
begin
  if not exists(select 1 from runtime.schemaversion where version='20260831019000') then
    raise exception 'CATALOG_PRODUCT_SCOPE_PREVIOUS_HEAD_MISSING';
  end if;
  if exists(select 1 from runtime.schemaversion where version='20260831020000') then
    raise exception 'CATALOG_PRODUCT_SCOPE_ALREADY_APPLIED';
  end if;
end
$precondition$;
...
begin
  if p_operation='organization.stores.manage' then
    ...
    if resolved is null then
      select listing.scope_id into resolved
      from catalog.sku sku join catalog.listing listing on listing.sku_id=sku.id
      where sku.product_id=p_resource
      order by listing.scope_id,listing.id limit 1;
    end if;
...
select runtime.record_migration_evidence(
  '20260831020000',1,1,0,0,
  'select access.resource_scope(''catalog.products.update'',$1,$2);',
  'select version,checksum from runtime.schemaversion where version=''20260831020000'';'
);

insert into runtime.schemaversion(version,checksum)
values('20260831020000','2d179f54b48381541fcc3df2b1b016742c901e12213816415b3f5e54f65d0285');

do $assert$
begin
  if position('sku.product_id=p_resource' in pg_get_functiondef('access.resource_scope(text,text,text)'::regprocedure))=0 then
    raise exception 'CATALOG_PRODUCT_SCOPE_RESOLVER_MISSING';
  end if;
  if not exists(select 1 from runtime.schemaversion where version='20260831020000'
    and checksum='2d179f54b48381541fcc3df2b1b016742c901e12213816415b3f5e54f65d0285') then
    raise exception 'CATALOG_PRODUCT_SCOPE_HEAD_MISSING';
  end if;
end
$assert$;
