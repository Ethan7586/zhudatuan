...
const events = eventMigration.slice(eventMigration.indexOf('alter table runtime.event add column'), eventMigration.indexOf('select runtime.record_migration_evidence'));
...
describe('one voucher tender writer', () => {
  it('shares reservation and release across checkout and operations with a single immutable timeline', async () => {
    ...
    try {
      ...
      expect(await data.history()).toEqual([{ previous_state: 'active', next_state: 'held' }, { previous_state: 'held', next_state: 'active' }]);
  ...
  it('consumes once, rejects release after consume and rejects rebinding the receipt to another hold/order', async () => {
    ...
    try {
      ...
      expect(event).toMatchObject({ event_version: 2, actor_id: 'principal:one', aggregate_id: 'voucher:one',
        payload: { scope: 'scope:test', channel: 'order', store: null, order: 'order:one', amountMinor: 400, currency: 'CNY' } });
  ...
  it('makes hold economic terms immutable and rejects foreign-parent redemptions in SQL', async () => {
    ...
    try {
      const hold = (await data.reserve()).hold;
      for (const assignment of [`voucher_id='voucher:other'`, `owner_id='order:other'`, `scope_id='scope:other'`,
        `amount_minor=900`, `expires_at=expires_at+interval '1 minute'`, `idempotency_key='forged'`]) {
      ...
      await data.add('voucher:two');
      await expect(data.database.query(`insert into voucher.redemption(id,scope_id,voucher_id,hold_id,verification_id,order_id,amount_minor,refunded_minor,currency,state,idempotency_key,version,redeemed_at,updated_at,channel,reporting_scopes,timezone)
        ...
        [hold.id])).rejects.toThrow(/voucher_redemption_hold_scope/);
    } finally { await data.database.close(); }
  });
  ...
  it('replays full-value verification from the receipt after balance reaches zero and the voucher expires', async () => {
    ...
    try {
      ...
      expect(await data.history()).toHaveLength(1);
      expect(await data.events()).toHaveLength(1);
      expect((await data.events())[0]).toMatchObject({ event_version: 2, payload: { scope: 'scope:test', channel: 'store', store: 'store:one',
        order: null, amountMinor: 1000, scopes: ['scope:test', 'scope:group', 'store:one'], timezone: 'Asia/Shanghai' } });
    } finally { await data.database.close(); }
  });
  ...
  it('rejects a spoofed store and an unrelated organization before publishing a redemption', async () => {
    ...
    try {
      ...
      data.scope.mockImplementation(async (_context, id) => ({ id, scopeKind: id.startsWith('store:') ? 'store' : 'mall',
        timezone: 'Asia/Shanghai', tenant: 'scope:other', ancestors: [], descendants: [] }));
  ...
  it('rejects a runtime event whose monetary payload differs from the persisted redemption', async () => {
    ...
    try {
      await data.verify();
      await expect(data.database.query(`insert into runtime.outbox select id,event_type,event_version,aggregate_type,aggregate_id,aggregate_version,
      ...
      await expect(data.database.query(`insert into runtime.outbox select id,event_type,1,aggregate_type,aggregate_id,aggregate_version,
  ...
  it('refunds the historical payer and replays their receipt after the voucher is rebound to someone else', async () => {
    const data = await fixture();
    try {
      await data.checkout(); await data.paymentConsume();
      await data.paymentRefund();
      expect((await data.states())[0]?.remaining_minor).toBe(1000);
      await data.rebind();
      await data.paymentRefund();
      await expect(data.paymentRefund({ member: 'member:other' })).rejects.toThrow('RESOURCE_NOT_FOUND');
      expect((await data.database.query(`select redemption.holder_id,holder.member_id,holder.state from voucher.redemption redemption
        join voucher.holder holder on holder.id=redemption.holder_id`)).rows)
        .toEqual([{ holder_id: 'voucher:one:holder', member_id: 'member:one', state: 'released' }]);
      expect((await data.database.query(`select amount_minor::integer,rule_version::integer from voucher.refund`)).rows)
        .toEqual([{ amount_minor: 400, rule_version: 1 }]);
  ...
  it('keeps partial refunds bounded and locks the voucher before the redemption', async () => {
    ...
    try {
      ...
      expect((await data.states())[0]?.remaining_minor).toBe(1000);
      expect((await data.database.query(`select refunded_minor::integer,state from voucher.redemption`)).rows)
        .toEqual([{ refunded_minor: 400, state: 'refunded' }]);
  ...
  it('rejects independently forged refund receipts and cumulative totals at the database transaction boundary', async () => {
    ...
    try {
      await data.checkout(); await data.paymentConsume();
      await expect(data.database.exec(`insert into voucher.refund(id,scope_id,redemption_id,amount_minor,currency,reason,state,rule_version,idempotency_key,created_at)
      ...
      await expect(data.database.exec(`update voucher.redemption set refunded_minor=100,state='partiallyrefunded',version=version+1`))
        .rejects.toThrow('VOUCHER_REFUND_TOTAL_MISMATCH');
      expect((await data.database.query(`select id from voucher.refund`)).rows).toEqual([]);
      expect((await data.database.query(`select refunded_minor::integer from voucher.redemption`)).rows).toEqual([{ refunded_minor: 0 }]);
      await data.paymentRefund();
...
describe('voucher refund facts', () => {
  it('reuses the original store and scope snapshot after organization settings change', async () => {
    ...
    try {
      ...
      expect(event).toMatchObject({ id: `event:voucher:refund:${refund}`, payload: { redemption: redemption!.id, amountMinor: 400,
        channel: 'store', store: 'store:one', scopes: original.payload.scopes, timezone: 'Asia/Shanghai', currency: 'CNY' } });
  ...
  it('rejects forged amounts, rule versions and scopes against persisted refund receipts', async () => {
    const data = await fixture();
    try {
      await data.checkout(); await data.paymentConsume(); await data.paymentRefund();
      for (const [field, value] of [['amountMinor', '1'], ['ruleVersion', '2'], ['scopes', '["scope:other"]']] as const) {
        await expect(data.database.query(`insert into runtime.outbox select id,event_type,event_version,aggregate_type,aggregate_id,aggregate_version,
          ...
          from runtime.outbox where event_type='voucher.refunded'`, [[field], value])).rejects.toThrow('VOUCHER_REFUND_EVENT_FACT_INVALID');
      }
      expect(await data.events()).toHaveLength(2);
    } finally { await data.database.close(); }
  });
...
async function fixture() {
  ...
  await database.exec(`create schema voucher; ${schema} ${holdGuard} ${voucherGuard} ${holderGuard} ${refundGuard} ${tenders} ${refunds}
    ...
    create table runtime.event(type text,version integer,owner text,schema_ref text,primary key(type,version));
  ...
  const add = async (id: string) => {
    ...
    await database.query(`insert into voucher.credential(id,scope_id,pool_id,product_id,number_ciphertext,secret_ciphertext,number_fingerprint,secret_fingerprint,number_masked,key_version,state,issue_batch_id,version,created_at,updated_at)
      values($1,'scope:test','pool:one','product:one','cipher','cipher',$2,$2,'****1234','key:one','allocated','batch:one',1,now(),now())`, [id + ':credential', fingerprint]);
    await database.query(`insert into voucher.voucher values($1,'scope:test','product:one',$2,null,$3,'****1234',1000,1000,'CNY','active',
      now()-interval '1 day',now()+interval '1 day',1,now(),now())`, [id, id + ':credential', fingerprint]);
  ...
  const scope = vi.fn<OrganizationReadPort['scope']>(async (_context, id) => ({ id, scopeKind: id.startsWith('store:') ? 'store' : 'mall', timezone: 'Asia/Shanghai',
    tenant: 'scope:group', ancestors: id.startsWith('store:') ? ['scope:test', 'scope:group'] : ['scope:group'], descendants: [] }));
  ...
  const options = (scope = 'scope:test') => ({ scope, tenant: scope, membership: 'membership:one', actor: 'principal:one', trace: 'trace:one',
    operation: 'voucher.tenderholds.create', signal: new AbortController().signal, deadline: Date.now() + 20_000 });
  const base = { scope: 'scope:test', now, actor: 'principal:one' };
  return { database, now, query, post, add, scope,
    refund: (redemption: string, amountMinor: number) => manager.write({ ...options(), operation: 'voucher.refunds.create' }, context => new VoucherRefundWriter({ post }).refund({ context, ...base,
      redemption, amountMinor, reason: '核销退款', idempotency: 'refund:manual' })),
    reserve: (change: Partial<Omit<Parameters<VoucherTenderWriter['reserve']>[0], 'context'>> = {}) => manager.write(options(), context => writer.reserve({
      context, ...base, voucher: 'voucher:one', owner: 'order:one', member: 'member:one', amountMinor: 400, ttlSeconds: 60, idempotency: 'hold:one', ...change })),
    release: (hold: string, change: Partial<Omit<Parameters<VoucherTenderWriter['release']>[0], 'context'>> = {}) => manager.write(options(), context => writer.release({
      context, ...base, hold, reason: '取消预占', ...change })),
    consume: (hold: string, change: Partial<Omit<Parameters<VoucherRedemptionWriter['redeem']>[0], 'context'>> = {}) => manager.write(options(), context => redeemer.redeem({
      context, ...base, voucher: 'voucher:one', hold, verification: 'verify:one', order: 'order:one', amountMinor: 400, idempotency: 'redeem:one', ...change })),
    ...
    paymentRefund: (change: Partial<Parameters<VoucherPort['refund']>[1]> = {}, scope?: string) => manager.write({ ...options(scope), operation: 'payment.refunds.request' },
      context => port.refund(context, { refund: 'refund:one', order: 'order:one', member: 'member:one', voucher: 'voucher:one', amountMinor: 400, ...change })),
    rebind: () => manager.write({ ...options(), operation: 'voucher.vouchers.unbind' }, async () => database.exec(`
    ...
    verify: (change: Partial<Parameters<VoucherPort['redeemVerification']>[1]> = {}) => manager.write(options('store:one'), context => port.redeemVerification(context,
      { voucher: 'voucher:one', verification: 'verification:one', scope: 'scope:test', store: 'store:one', actor: 'principal:one', ...change })),
    apiRelease: (hold: string) => manager.write(options(), context => repository.release({ context: { transaction: context }, ...base, expectedVersion: 1,
      input: { path: { holdid: hold }, body: { reason: '取消预占' } } } as never)),
