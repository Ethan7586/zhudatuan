...
当前进度：CP-00、CP-00A、AU-001/CP-01 至 AU-006/CP-06 已完成。AU-006 已形成 @shop/config 环境契约、SFL节点权威、Console运行配置和生成目录总图：深入审阅35个人工文件、6,151行，结构性审阅62个人工文件、8,947行，核对7个自动生成文件、1,070行，合计104文件、16,168行。阶段1的宏观骨架已闭合，阶段2的config单元已完成；下一单元须由Ethan另行授权。
...
CP-07 提交后结束当前单一目的会话。后续只有在 Ethan 明确授权时才执行：

- AU-008：生成契约运行链。
- 目的：从已生成 OpenAPI、SDK operation clients、Commerce Controller/Handler 壳、事件 registry、Miniapp 条件产物和数据库发布输出反向核对真实加载与发布边界；不同时深审各业务 handler 的领域正确性。
- 输入：CP-07 的契约权威图、生成目标表、问题清单、117 文件 AU 覆盖记录、覆盖总账与 RV-0008 独立复核队列。
- 禁止：修改契约、生成器或生成物，运行生成写模式，修复 F-0036–F-0043，安装依赖，改变线上状态，推送或部署。
