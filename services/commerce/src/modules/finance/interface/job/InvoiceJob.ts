...
export class InvoiceJobProcessor implements JobProcessor {
  ...
  private async issue(request: string): Promise<void> {
    const selected = await this.pool.query<InvoiceRow>(
    ...
    const original =
      invoice.red_of_request_id === null
        ? undefined
        : (
            await this.pool.query<{ id: string; external_id: string }>(
    ...
    const [title, taxid, address, lines] = await Promise.all([
      ...
      this.pool.query<{ description: string; amount_minor: number; tax_minor: number }>(
