...
import { eventLines, inventoryDigest, inventoryTime, restoreReservation, restoreStock, stockEvent, type ReservationRow, type StockRow, type TransitionRow } from './InventoryRecord';
...
export class PgReservationRepository {
  ...
  async reserve(context: WriteTransactionContext, order: string, scope: string, values: readonly StockDemand[]): Promise<void> {
    ...
    const locked = await database.query<StockRow>(
      `with locked as(select stock.id,stock.scope_id,stock.sku_id,stock.location_id,stock.onhand,stock.safety,stock.version,stock.status,stock.updated_at
       ...
       from locked order by locked.id`,
      [ids, scope]
    );
    ...
    const existing = await database.query<ReservationRow>(
      `select id,stockitem_id,owner_type,owner_id,quantity::float8 quantity,state,expires_at,created_at,version::integer
       from inventory.reservation where owner_type='order' and owner_id=$1 and stockitem_id=any($2::text[]) order by stockitem_id for update`,
      [order, ids]
    );
    ...
    for (const demand of demands) {
      ...
      created.push(Reservation.reserve({ id: `reservation:${randomUUID()}`, stockitem: demand.stockitem, ownerKind: 'order', owner: order, quantity: demand.quantity, expiresAt, createdAt: now.toISOString() }).snapshot());
    ...
    const touched = await database.query(
      `update inventory.stockitem target set version=input.version,updated_at=input."updatedAt"
    ...
    await appendMovements(
      database,
      created.map((item) => ({ id: `movement:${item.id.slice('reservation:'.length)}`, stockitem: item.stockitem, kind: 'reserve', quantity: -item.quantity, referenceKind: 'order', reference: order }))
    );
    const runtime = new PgRuntimeWriter(database);
    await runtime.schedule({ id: `job:reservationexpiry:${order.slice('order:'.length)}`, kind: 'reservationexpiry', owner: 'inventory', scope, payload: { owner: order }, priority: 100, availableAt: expiresAt });
    await runtime.appendMany([
      { id: `event:${randomUUID()}`, type: 'inventory.reservation.created', aggregateType: 'order', aggregate: order, scope, trace: context.trace, payload: { owner: order, ownerKind: 'order', lines: eventLines(demands) } },
  ...
  private async transition(context: WriteTransactionContext, ownerKind: ReservationSnapshot['ownerKind'], owner: string, state: Exclude<ReservationSnapshot['state'], 'reserved'>): Promise<void> {
    const database = this.transactions.database(context);
    const result = await database.query<TransitionRow>(
      `select reservation.id,reservation.stockitem_id,reservation.owner_type,reservation.owner_id,reservation.quantity::float8 quantity,
       ...
       where reservation.owner_type=$1 and reservation.owner_id=$2 order by stock.id for update of stock,reservation`,
      [ownerKind, owner]
    );
    ...
    for (const row of result.rows) {
      ...
      const stock = StockItem.restore({
        ...
        state: row.status,
        version: Number(row.stock_version),
    ...
    const reservationWrite = await database.query(
      `update inventory.reservation target set state=input.state,version=input.version
    ...
    const stockWrite = await database.query(
      `update inventory.stockitem target set onhand=input.onhand,version=input.version,updated_at=input."updatedAt"
    ...
    await appendMovements(
      ...
      reservations.map((item) => ({
        id: `movement:${inventoryDigest(`${item.id}:${state}`)}`,
        stockitem: item.stockitem,
        kind: movementKind,
        quantity: (state === 'committed' ? -1 : 1) * item.quantity,
        referenceKind: ownerKind,
        reference: owner,
      }))
    );
    const balances = await database.query<{ id: string; reserved: number }>(
      `select stock.id,coalesce(sum(reservation.quantity)
      ...
      left join inventory.reservation reservation on reservation.stockitem_id=stock.id where stock.id=any($1::text[]) group by stock.id`,
      [stocks.map(({ id }) => id)]
    );
    ...
    await new PgRuntimeWriter(database).appendMany([
      {
        id: `event:${randomUUID()}`,
        type: `inventory.reservation.${state === 'committed' ? 'confirmed' : state}`,
        aggregateType: ownerKind,
        aggregate: owner,
        scope: result.rows[0]!.scope_id,
        trace: context.trace,
        payload: {
          owner,
          ownerKind,
          lines: reservations.map((item) => ({
            sku: result.rows.find(({ id }) => id === item.id)!.sku_id,
            stockitem: item.stockitem,
