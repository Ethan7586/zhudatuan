...
`release.json` 必须符合 `shop.release.v1`，同时绑定 Git Commit、Contract Hash、Target Schema Head、Commerce 镜像 Digest、六端文件 Hash、SBOM、Provenance、数据库快照、预发布结果、11 个 P1 Provider 正式沙箱结果与业务发布批准。`scripts/release/validate.mjs` 失败时发布关闭。
...
```bash
infrastructure/aliyun/deploy.sh /absolute/path/to/signed-release
```
