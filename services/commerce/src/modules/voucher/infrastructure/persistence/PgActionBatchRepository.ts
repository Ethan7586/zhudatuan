...
export class PgActionBatchRepository implements ActionBatchRepository {
  ...
  async create(call: Parameters<ActionBatchRepository['create']>[0]) {
    ...
    const selected = await database.query<{ result_count: number }>(`select result_count from voucher.searchsnapshot where id=$1 and scope_id=$2 and expires_at>clock_timestamp() for update`, [snapshot, call.scope]);
    const count = Number(selected.rows[0]?.result_count ?? 0);
    if (count <= 0) throw new DomainError(selected.rows[0] ? 'VALIDATION_FAILED' : 'RESOURCE_NOT_FOUND');
    ...
    await database.query(
      ...
      values($1,$2,$3,$4,$5,$6,'queued',$7,0,0,0,0,1,$8,$9,$9)`,
      [id, call.scope, snapshot, action, text(value.reason, 'reason'), value.expiresAt ?? null, count, call.actor, call.now]
    );
    await database.query(
      ...
      select $1,$2,item.voucher_id,'queued',null,null,null,false,$1||':'||item.voucher_id,$3 from voucher.searchsnapshotitem item where item.snapshot_id=$4 and item.scope_id=$2`,
      [id, call.scope, call.now, snapshot]
    );
  ...
  async get(call: Parameters<ActionBatchRepository['get']>[0]) {
    const row = await this.transactions.database(call.context.transaction).query(`${ACTION} where batch.id=$1 and batch.scope_id=$2`, [path(call, 'batchid'), call.scope]);
    return one<'voucher.actionbatches.get'>(200, row.rows[0]);
  }
  async list(call: Parameters<ActionBatchRepository['list']>[0]) {
    const database = this.transactions.database(call.context.transaction);
    const filter = query(call);
    const fetch = limit(filter.limit);
    const rows = await database.query(`${ACTION} where batch.scope_id=$1 and ($2::text is null or batch.state=$2) and ($3::text is null or batch.action=$3) and ($4::text is null or batch.id>$4) order by batch.id limit $5`, [
      call.scope,
      optionalText(filter.state),
      optionalText(filter.action),
      cursor(filter.cursor),
      fetch + 1,
    ]);
    return page<'voucher.actionbatches.list'>(rows.rows, fetch);
  }
  async retry(call: Parameters<ActionBatchRepository['retry']>[0]) {
    const database = this.transactions.database(call.context.transaction);
    const id = path(call, 'batchid');
    const selected = await database.query<BatchRow>(`select ${ACTION_BATCH_FIELDS} from voucher.actionbatch batch where batch.id=$1 and batch.scope_id=$2 for update`, [id, call.scope]);
    const row = selected.rows[0];
    if (!row) throw new DomainError('RESOURCE_NOT_FOUND');
    if (row.version !== expected(call)) throw new DomainError('VERSION_CONFLICT');
    const retried = new ActionBatch({
      id: row.id,
  ...
  async export(call: Parameters<ActionBatchRepository['export']>[0]) {
    const database = this.transactions.database(call.context.transaction);
    const batch = text(body(call).batch, 'batch');
...
function actionValue(value: unknown): BatchRow['action'] {
  if (!['activate', 'disable', 'enable', 'void', 'extend'].includes(String(value))) throw new DomainError('VALIDATION_FAILED', { field: 'action' });
  return value as BatchRow['action'];
}
