...
describe('browser harness', () => {
  ...
  it('serves declared MSW handlers', async () => {
    const msw = createMswHarness(http.get('https://console.fufu.wang/api/health', () => HttpResponse.json({ ok: true })));
    msw.start();
    try {
      const response = await fetch('https://console.fufu.wang/api/health');
