...
export function ApprovalFields({ model }: Readonly<{ model: VoucherActionViewModel }>) {
  return (
    <section className="voucherapproval" aria-label="双人复核">
      ...
      <Button onPress={model.actions.requestApproval} isDisabled={model.reason.trim().length < 4 || model.busy}>
        生成复核请求码
      </Button>
      {model.approval ? (
        <>
          ...
          <p>交给另一位具备同一权限的管理员，由其在“完成二次验证”中签发一次性凭证。</p>
        </>
      ) : null}
      <label>
        一次性复核凭证
        <input value={model.proof} autoComplete="off" spellCheck={false} onChange={(event) => model.actions.proof(event.target.value.trim())} required />
      </label>
