...
export class ReadPayment {
  ...
  async execute(database: OperationDatabase, membership: string, paymentid: string) {
    const member = await this.members.member(database, membership);
    ...
    if (!selected || !(await this.orders.payment(database, selected.order_id, member))) throw new DomainError('RESOURCE_NOT_FOUND');
