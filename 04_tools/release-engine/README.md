...
The team command tags each dispatch with a one-time correlation ID so concurrent runs with the same Source SHA and target cannot be confused during queue observation or Hosted takeover. This ID is only for finding the GitHub run; it is not a release state or approval. If Aliyun fails before the core starts and the same workflow's Hosted job succeeds, the command reports that successful takeover while retaining the target's actual receipt as production truth.

When Aliyun has more than one build Runner, the workflow requests their shared build label rather than pinning a numbered slot. GitHub assigns an available Runner from that pool, so concurrent commands do not queue behind the same machine while another build Runner is idle.
