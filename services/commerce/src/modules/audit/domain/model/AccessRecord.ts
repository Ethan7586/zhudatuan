...
export class AccessRecord {
  ...
  constructor(
    ...
    if (!input.scope || !input.actor || !input.actorType || !input.resourceType || !input.resource || !input.purpose || !input.trace || Number.isNaN(Date.parse(accessedAt))) throw new Error('AUDIT_ACCESS_INVALID');
