...
export function ReconciliationPage({ title, model }: Readonly<{ title: string; model: ReconciliationViewModel }>) {
  ...
  return (
    <section className="financeworkspace" aria-label="财务对账">
      <FinanceHeader title={title} description="核对渠道账单、系统事实与账本凭证，差异处理遵循经办、复核和重新匹配。" fetching={model.fetching} statusText={model.fetching ? '正在同步对账批次' : '服务端对账事实已同步'} onRefresh={model.actions.refresh} />
      ...
      <div className="financecolumnbar">
        ...
        <button type="button" aria-expanded={model.columnsOpen} aria-controls="financecolumnsettings" onClick={model.actions.toggleColumns}>
