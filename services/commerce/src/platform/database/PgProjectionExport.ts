...
export async function requestProjectionExport(request: OperationRequest, database: SqlExecutor, report: ProjectionExportReport, filter: Readonly<Record<string, unknown>>, visibleFilter: Readonly<Record<string, unknown>> = filter) {
  const serialized = JSON.stringify(filter);
  if (serialized.length > 16_384 || JSON.stringify(visibleFilter).length > 16_384) throw new Error('REPORT_FILTER_TOO_LARGE');
  ...
  const result = await database.query<{ occurred_at: Date }>(
    `with boundary as materialized(select clock_timestamp() watermark)
    insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,
    scope_id,payload,trace_id,occurred_at,available_at)
    select $1,$2,1,'export',$3,$4,jsonb_set($5::jsonb,'{watermark}',to_jsonb(boundary.watermark)),$6,boundary.watermark,boundary.watermark from boundary
  ...
  return Object.freeze({
    ...
    watermark: occurredAt.toISOString(),
