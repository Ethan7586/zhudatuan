...
export function useRiskViewModel(context: ConsoleContext, dependencies: RiskDependencies, requestStepup: () => void) {
  ...
  const mallWrite = context.scope.kind === 'mall' && canUseOperation(context, OP_RISK_POLICIES_MANAGE);
  return Object.freeze({
    ...
    mallWrite,
    canReview: context.scope.kind === 'mall' && canUseOperation(context, OP_RISK_CASES_REVIEW),
    groupBoundary: context.scope.kind !== 'mall',
...
function validation(editor: RiskEditor | undefined, error: string | undefined, reviewing: boolean, assurance: number): string | undefined {
  ...
  if (!reviewing) return editor.kind === 'activate' ? '请先核对服务端历史样本回放结果。' : '请先进入最终预览并核对全部变更。';
