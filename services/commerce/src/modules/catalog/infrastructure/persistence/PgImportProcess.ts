...
export class PgImportProcess implements ImportProcessPort {
  ...
  async find(id: string, execution: ImportExecution): Promise<ImportTarget | null> {
    const result = await this.manager.read(transactionOptions({ id, scope: execution.scope }, execution.signal, execution.deadline), (context) =>
      this.transactions.database(context).query<TargetRow>('select id,scope_id,object_ref,sha256,state from catalog.importjob where id=$1', [id])
    );
  ...
  async stage(target: ImportTarget, rows: readonly Readonly<Record<string, string>>[], execution: ImportExecution): Promise<void> {
    ...
    await this.manager.write(options, async (context) => {
      ...
      await database.query("update catalog.importjob set state='validating',last_error=null,updated_at=clock_timestamp() where id=$1", [target.id]);
      await database.query('delete from catalog.importrow where job_id=$1', [target.id]);
      await database.query('delete from catalog.importerror where job_id=$1', [target.id]);
    });
    for (let offset = 0; offset < rows.length; offset += 500) {
      ...
      await this.manager.write(options, async (context) => {
        await this.transactions.database(context).query(
          `insert into catalog.importrow(job_id,scope_id,row_number,payload)
    ...
    await this.manager.write(options, async (context) => {
      await this.transactions.database(context).query(
        `update catalog.importjob set state='ready',total_count=$2,cursor_value=0,success_count=0,failure_count=0,
  ...
  async process(target: ImportTarget, signal: AbortSignal, deadline: number): Promise<boolean> {
    ...
    const selected = await this.manager.read(options, async (context) => {
      ...
      const job = await database.query<{ cursor_value: number; total_count: number }>(
        `select cursor_value,total_count from catalog.importjob
        where id=$1 and state in('ready','running')`,
        [target.id]
      );
      if (!job.rows[0]) return null;
      const staged = await database.query<StagedRow>(
        `select row_number,payload from catalog.importrow where job_id=$1 and row_number>$2
    ...
    for (const row of selected.rows) {
      ...
      try {
        await this.manager.write(options, (context) => importProduct(this.transactions.database(context), target.scope, row.payload));
        successes += 1;
      } catch (cause) {
        failures += 1;
        await this.manager.write(options, (context) =>
          this.transactions
            .database(context)
            .query(
              `insert into catalog.importerror(job_id,scope_id,row_number,reason_code,field,detail) values($1,$2,$3,$4,null,$5)
          on conflict(job_id,row_number,reason_code) do update set detail=excluded.detail`,
              [target.id, target.scope, row.row_number, importCode(cause, 'CATALOG_IMPORT_ROW_FAILED'), importDetail(cause)]
            )
            .then(() => undefined)
        );
      }
    ...
    await this.manager.write(options, async (context) => {
      ...
      await database.query(
        `update catalog.importjob set state=$2,cursor_value=$3,success_count=success_count+$4,failure_count=failure_count+$5,
  ...
  async failures(target: ImportTarget, execution: ImportExecution): Promise<readonly ImportFailure[]> {
    const result = await this.manager.read(transactionOptions(target, execution.signal, execution.deadline), (context) =>
      this.transactions
        .database(context)
        .query<{ row_number: number; reason_code: string; field: string | null; detail: string }>('select row_number,reason_code,field,detail from catalog.importerror where job_id=$1 order by row_number,reason_code', [target.id])
    );
  ...
  async complete(target: ImportTarget, report: StoredObject, execution: ImportExecution): Promise<void> {
    await this.manager.write(transactionOptions(target, execution.signal, execution.deadline), (context) =>
      this.transactions
        .database(context)
        .query(
          `update catalog.importjob set state='completed',report_object_ref=$2,report_sha256=$3,report_size=$4,
  ...
  async reject(target: ImportTarget, code: string, detail: string, execution: ImportExecution): Promise<void> {
    await this.manager.write(transactionOptions(target, execution.signal, execution.deadline), (context) =>
      this.transactions
        .database(context)
        .query(
          `update catalog.importjob set state='failed',last_error=$2,
  ...
  async fault(target: ImportTarget, detail: string, execution: ImportExecution): Promise<void> {
    await this.manager.write(transactionOptions(target, execution.signal, execution.deadline), (context) =>
      this.transactions
        .database(context)
        .query('update catalog.importjob set last_error=$2,updated_at=clock_timestamp() where id=$1', [target.id, detail])
        .then(() => undefined)
    );
  }
}
...
async function continuation(client: SqlExecutor, target: ImportTarget, cursor: number): Promise<void> {
  await new PgRuntimeWriter(client).schedule({ id: `job:${target.id}:${cursor}`, kind: 'catalogimport', owner: 'catalog', scope: target.scope, payload: Object.freeze({ import: target.id }), priority: 100 });
}

function transactionOptions(target: Pick<ImportTarget, 'id' | 'scope'>, signal: AbortSignal, deadline: number) {
  return { tenant: target.scope, membership: '', scope: target.scope, actor: 'job:catalogimport', trace: target.id, operation: 'job.catalog.import', workload: 'jobs' as const, signal, deadline };
}
