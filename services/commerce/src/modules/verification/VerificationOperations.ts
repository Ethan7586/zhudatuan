...
import { VERIFICATION_VOUCHER_PORT } from '../voucher/public/index';
import type { VerificationVoucherPort } from '../voucher/public/index';
...
export function verificationOperations(context: ModuleContext): ModuleOperations {
  ...
  const vouchers = context.ports.get(VERIFICATION_VOUCHER_PORT);
  const members = context.ports.get(MEMBER_ACCESS_PORT);
  const organizations = context.ports.get(ORGANIZATION_READ_PORT);
  return new ModuleOperations('verification', pool, context.service(AUDIT_SINK), {
    ...
    'verification.challenges.issue': async (request, database) => {
      ...
      const member = await members.profile(database, access.membership.id);
      ...
      if (purpose === 'voucher_redeem') {
        ...
        const voucherScope = await vouchers.redeemableScope(database, subject, member.member);
        if (!voucherScope) reject('VOUCHER_NOT_REDEEMABLE');
        scope = voucherScope;
      }
      ...
      const result = await database.query(
        ...
        (insert into verification.nonce(session_id,nonce_hash,issued_at) values($1,$6,clock_timestamp()))
    ...
    'verification.challenges.verify': async (request, database) => {
      ...
      if (!device.rows[0]) reject('VERIFICATION_DEVICE_DENIED');
      const visibleScopes = [access.scope.id, ...(await organizations.scope(database, access.scope.id)).ancestors];
      const consumed = await database.query<{ subject_type: string; subject_id: string; purpose: string; scope_id: string }>(
        `with consumed as (
        ...
        where nonce.session_id=$1 and nonce.nonce_hash=$2 and nonce.consumed_at is null and session.id=nonce.session_id
          and session.scope_id=any($3::text[])
      ...
      select subject_type,subject_id,purpose,scope_id from consumed`,
        [challenge, digest(nonce), visibleScopes]
      );
      ...
      await database.query(
        ...
        [`attempt:${randomUUID()}`, challenge, digest(nonce), device.rows[0]!.id, outcome, result ? 'verified' : 'nonce_unavailable', access.trace]
      );
      if (!result) reject('VERIFICATION_NONCE_REPLAYED');
      const record = result.purpose === 'voucher_redeem' ? await redeem(database, vouchers, organizations, result.subject_id, challenge, result.scope_id, access) : `attempt:${challenge}`;
    ...
    'verification.devices.manage': async (request, database) => {
      ...
      const result = await database.query(
        ...
        [request.input.path.deviceid!, access.scope.id, textField(body, 'label'), digest(textField(body, 'fingerprint', 512)), body.publicKey ?? null, status, request.input.expectedVersion ?? null]
      );
...
async function redeem(
  ...
  vouchers: Pick<VerificationVoucherPort, 'redeemVerification'>,
  organizations: Pick<OrganizationReadPort, 'scope'>,
  voucher: string,
  challenge: string,
  scope: string,
  ...
  if (!accepted) reject('VOUCHER_REDEMPTION_CONFLICT');
  const organization = await organizations.scope(database, scope);
  ...
  await database.query(
    ...
    values($1,'voucher.redeemed',1,'voucher',$2,$3,jsonb_build_object('voucher',$2,'redemption',$4,'amountMinor',$5,'currency','CNY','mall',$3,
      'store',$6,'scopes',$7::jsonb,'timezone',$8),$9,clock_timestamp(),clock_timestamp())`,
    [`event:${randomUUID()}`, voucher, scope, accepted.id, accepted.amountMinor, access.scope.id, JSON.stringify(scopes), organization.timezone, access.trace]
  );
  return accepted.id;
}
