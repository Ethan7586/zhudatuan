...
import type { AuditPort, ArchiveBatch } from '../../01_public_gongkai/AuditPort';

const BATCH_SIZE = 100;
const MAXIMUM_OBJECT = 64*1024*1024;

interface StoredArchive {
  readonly format: 'shop.audit.archive.v1'; readonly scope: string; readonly firstHash: string; readonly lastHash: string;
  readonly count: number; readonly keyVersion: string; readonly fingerprint: string; readonly ciphertext: string;
}

export class AuditArchiveJobProcessor implements JobProcessor {
  constructor(private readonly pool: DatabasePool, private readonly objects: ObjectStore, private readonly kms: KmsClient,
  ...
  async process(job: ClaimedJob, signal: AbortSignal): Promise<void> {
    ...
    const path = archivePath(batch); const existing = await this.objects.find(path);
    const archived = existing ? await this.recover(existing, batch) : await this.upload(path, batch, signal);
    const expiry = new Date(); expiry.setUTCFullYear(expiry.getUTCFullYear()+batch.archiveYears);
    const client = await this.pool.connect();
    try {
      await client.query('begin');
      await this.repository.completeArchive(client, batch, { reference:archived.object.reference, sha256:archived.object.sha256,
        size:archived.object.size, keyVersion:archived.keyVersion, expiresAt:expiry.toISOString() });
  ...
  private async upload(path: string, batch: ArchiveBatch, signal: AbortSignal) {
    const plaintext = JSON.stringify({ format:'shop.audit.plain.v1', scope:batch.scope, start:batch.start, end:batch.end,
      firstHash:batch.firstHash, lastHash:batch.lastHash, count:batch.rows.length, rows:batch.rows });
    const envelope = await this.kms.encrypt('audit/archive', plaintext, { scope:batch.scope, lastHash:batch.lastHash });
    const content: StoredArchive = { format:'shop.audit.archive.v1', scope:batch.scope, firstHash:batch.firstHash,
      lastHash:batch.lastHash, count:batch.rows.length, keyVersion:envelope.keyVersion, fingerprint:envelope.fingerprint,
  ...
  private async recover(object: StoredObject & { readonly contentType: string }, batch: ArchiveBatch) {
    ...
    if (content.format!=='shop.audit.archive.v1' || content.scope!==batch.scope || content.firstHash!==batch.firstHash
      || content.lastHash!==batch.lastHash || content.count!==batch.rows.length || typeof content.keyVersion!=='string'
      || typeof content.fingerprint!=='string' || !/^[a-f0-9]{64}$/.test(content.fingerprint)
      || typeof content.ciphertext!=='string' || content.ciphertext.length<16) throw new Error('AUDIT_ARCHIVE_OBJECT_MISMATCH');
