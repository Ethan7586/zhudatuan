...
export function getNotificationsOperations(repositories: NotificationRepositoryFactory): OperationActions {
  return {
    ...
    'notification.preferences.read': async (request, database) => {
      const access = requireAccess(request); const repository = repositories(database); const context = await repository.member(access.membership.id);
      const page = queryPage(request); return keysetResult(await repository.preferences(context.member, context.organization, page.id, page.fetch),
