...
export class PgCatalogImport implements BatchImportPort {
  ...
  async process(target: ImportTarget, signal: AbortSignal): Promise<boolean> {
    ...
    try {
      ...
      const staged = await client.query<StagedRow>(`select row_number,payload from catalog.importrow where job_id=$1 and row_number>$2
      ...
      for (const row of staged.rows) {
        ...
        try { await importProduct(client, target.scope, row.payload); successes += 1; }
        catch (cause) {
          await client.query('rollback to savepoint importrow'); failures += 1;
          await client.query(`insert into catalog.importerror(job_id,scope_id,row_number,reason_code,field,detail) values($1,$2,$3,$4,null,$5)
