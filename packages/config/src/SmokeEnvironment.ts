...
export function smokeEnvironment(source: EnvironmentSource = processEnvironment()): SmokeEnvironment {
  ...
  if (!/^https:\/\//.test(baseUrl) && !/^http:\/\/(localhost|127\.0\.0\.1)(:\d+)?$/.test(baseUrl) && !/^http:\/\/[a-z0-9-]+\.[a-z0-9-]+\.svc\.cluster\.local(?::\d+)?$/i.test(baseUrl)) throw new Error('SHOP_SMOKE_BASE_URL_INVALID');
