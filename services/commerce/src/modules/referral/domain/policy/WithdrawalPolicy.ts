...
export class WithdrawalPolicy {
  assertRequest(amount: ReferralMoney, available: ReferralMoney, minimumMinor: bigint, hasPendingReversal: boolean, monthlyUsed = 0, monthlyLimit: number | null = null): void {
    ...
    if (!Number.isSafeInteger(monthlyUsed) || monthlyUsed < 0 || (monthlyLimit !== null && (!Number.isSafeInteger(monthlyLimit) || monthlyLimit < 1 || monthlyUsed >= monthlyLimit))) throw new DomainError('REFERRAL_WITHDRAWAL_CONFLICT');
  }
