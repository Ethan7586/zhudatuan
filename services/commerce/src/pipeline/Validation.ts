...
import type { OperationResult } from './OperationHandler';
...
export function optionalText(body: Readonly<Record<string, unknown>>, field: string, maximum = 255): string | null {
  ...
  if (typeof value !== 'string' || value.trim().length > maximum) throw new DomainError('VALIDATION_FAILED', { field });
  return value.trim();
}
...
export function queryIdentifiers(input: OperationWireInput, field: string, maximum = 50): readonly string[] {
  if (!Number.isSafeInteger(maximum) || maximum < 1 || maximum > RUNTIME_LIMITS.sql.maximumRows) throw new Error('QUERY_IDENTIFIER_MAXIMUM_INVALID');
  const raw = input.query?.[field];
  const values = (Array.isArray(raw) ? raw : raw === undefined || raw === null ? [] : [raw])
    .flatMap((value) => (typeof value === 'string' ? value.split(',') : []))
    .map((value) => value.trim())
    .filter(Boolean);
  const selected = [...new Set(values)];
  if (selected.length > maximum || selected.some((value) => !/^[^\s,]{1,256}$/.test(value))) throw new DomainError('VALIDATION_FAILED', { field });
  return Object.freeze(selected);
}

export function queryText(input: OperationWireInput, field: string, maximum = 255): string | null {
  const raw = input.query?.[field];
...
export function integerField(body: Readonly<Record<string, unknown>>, field: string, minimum = 0): number {
  const value = body[field];
  if (!Number.isSafeInteger(value) || (value as number) < minimum) throw new DomainError('VALIDATION_FAILED', { field });
  return value as number;
}

export function resourceId(input: unknown, field: string, prefix: string, code: string): string {
  const value = input !== null && typeof input === 'object' ? Reflect.get(input, field) : null;
  if (typeof value !== 'string' || !new RegExp(`^${prefix}:[a-f0-9-]{36}$`, 'u').test(value)) throw new Error(code);
  return value;
}
...
export function keysetResult<T extends object>(result: Readonly<{ rows: readonly T[] }>, page: QueryPage, sort: keyof T, id: keyof T = 'id' as keyof T): OperationResult {
  return keysetRows(result.rows, page, sort, id);
}

export function keysetRows<T extends object>(rows: readonly T[], page: QueryPage, sort: keyof T, id: keyof T = 'id' as keyof T): OperationResult {
