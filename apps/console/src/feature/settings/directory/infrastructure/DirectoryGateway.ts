...
export class DirectoryGateway implements DirectoryPort {
  ...
  async synchronize(context: ConsoleContext, command: DirectoryCommand, signal?: AbortSignal) {
    const body = command.action === 'start' || command.action === 'preview' ? ({ action: command.action, mode: command.mode } as const) : ({ action: command.action, run: command.run } as const);
