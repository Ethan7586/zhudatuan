...
export class BootstrapGateway implements BootstrapPort {
  ...
  read(request: SessionRequest, signal?: AbortSignal): Promise<Bootstrap> {
    const key = `${request.target}:${request.returnTarget ?? ''}:${request.returnPath ?? ''}`;
    ...
    const input = request.returnTarget ? { query: { returntarget: request.returnTarget } } : request.returnPath ? { query: { returnpath: request.returnPath } } : {};
    const operation = this.sdk
      .bootstrapRead(input, queryContext(this.environment, request.target))
      .then(mapBootstrap)
      .then((value) => {
        if (value.target !== request.target) throw new ClientError('RETURN_TARGET_INVALID');
