...
| 能力 | 来源 | 裁定 |
|---|---|---|
| QR SVG 生成 | `wchs/packages/design/src/QrCode.tsx` | 提炼为 zdt-next Design 组件 |
| 商城二维码弹窗、复制和下载 | `wchs/apps/console/src/feature/experience/entry/EntryDialog.tsx` | 保留交互语义，按新 Console 结构实现 |
| `/s/<handle>` 入口解析 | `wchs/apps/storefront/src/route/EntryPath.ts` | 保留合同与 canonical path |
| 已发布商城才可扫码 | `wchs/apps/console/src/feature/experience/entry/EntryState.ts` | 保留生命周期规则 |
| 服务号 `snsapi_base` Provider | `wchs/services/commerce/src/modules/identity/infrastructure/adapter/wechat/WechatProvider.ts` | 作为 H5 微信 Provider 基础 |
| 联邦身份交易与 return target | `wchs/services/commerce/src/modules/identity/application/service/FederationService.ts` | 保留状态机与会话返回语义 |
...
- WCHS：`/Users/Ethan/Desktop/zdt-next/wchs@2ebf2ed91e87`
