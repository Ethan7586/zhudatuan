...
describe('payment aggregates', () => {
  it('keeps terminal payment intents monotonic and rejects expired continuation', () => {
    const intent = PaymentIntent.create({
      ...
      providerReference: 'P202609050001',
  ...
  it('requires allocations to conserve every minor unit and bind references by tender kind', () => {
    expect(
      new Allocation(
        [
          { sequence: 1, kind: 'wechat', reference: null, amountMinor: 60 },
          { sequence: 2, kind: 'benefit', reference: 'account:one', amountMinor: 40 },
        ],
        100
      ).totalMinor
    ).toBe(100);
  ...
  it('keeps refund and provider attempt terminal states final', () => {
    ...
    expect(
      () => new PaymentAttempt({ id: 'attempt:one', intent: 'intent:one', provider: 'wechat', scene: 'miniapp', applicationHash: 'a'.repeat(64), state: 'succeeded', externalTransaction: null, requestedAt: NOW, completedAt: NOW })
    ).toThrow('PAYMENT_INTENT_CONFLICT');
  });
});
