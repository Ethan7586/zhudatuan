...
describe('ExtensionRegistry', () => {
  ...
  it('keeps another provider product, order and finance capabilities available when one provider is disabled', async () => {
    ...
    await expect(registry.strategy('stable', 'scope:1', 'Statement').pullStatement(callContext(), { start: '2026-09-01', end: '2026-09-02', timezone: 'Asia/Shanghai' })).resolves.toMatchObject({ objectRef: 'object://stable/statement.csv' });
