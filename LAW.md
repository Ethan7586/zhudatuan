...
4. 在 Ethan 明确宣布项目开发结束之前，不得设计、规划、讨论实施或实现任何安全锁，且不得向代码、配置、工作流、脚本、测试或文档新增一行、一个字符；不得以修复、重构、顺手处理、拆批或混入其他改动的方式实施。项目结束后也不会自动解除禁令，只有 Ethan 再次明确修改 LAW 本节，写明允许增加的具体安全锁、适用范围和有效期限，相关改动才有资格进入项目。
5. 任何任务一旦可能触发新增安全锁，AI 必须在采取行动前立即停止，并以界面支持的最大标题、粗体和红色样式显示：`🚨 红色警报：此任务涉及新增安全锁，项目结束前严格禁止，继续可能造成严重人身危险与项目损失。` 若界面不支持红色或字号，必须使用一级标题、粗体和至少三个警报符号，不得弱化提示。
6. 发现真实漏洞时，只报告事实和影响，不得设计或提出安全锁方案。能用日志、告警、审计或事后检查处理的问题，不得升级为阻断机制。
7. 本节禁止新增安全锁，不自动授权删除身份认证、租户隔离、支付保护、备份恢复等现有基础保护；删除或削弱已有保护同样需要 Ethan 明确指定范围。
