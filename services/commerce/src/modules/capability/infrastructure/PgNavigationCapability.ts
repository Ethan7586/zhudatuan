...
export class PgNavigationCapability implements NavigationCapabilityPort {
  async read(database: DatabasePool, scopes: readonly string[]): Promise<readonly NavigationCapability[]> {
    ...
    const grouped = new Map<string, { values: Set<string>; version: number }>();
