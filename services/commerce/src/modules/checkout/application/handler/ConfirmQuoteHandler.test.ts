...
import { ConfirmQuoteUsecase } from './ConfirmQuoteHandler';

describe('ConfirmQuoteUsecase', () => {
  it('resolves an owner resource scope through its organization boundary', async () => {
    ...
    const usecase = new ConfirmQuoteUsecase(
      {} as never,
      { profile } as never,
      { lockQuote } as never,
      {} as never,
    ...
    await expect(usecase.execute(request(), {} as never)).rejects.toThrow('STOP_AFTER_SCOPE_ASSERTION');
