...
test('legacy 1.2 recovery remains visibly separate from normal 1.4 deployment', () => {
...
test('Console retains optional public acceptance metadata while Prepare and Deploy 1.4 remain exact single-target channels', () => {
  ...
  assert.match(preparedDeployWorkflow, /remote\.origin\.promisor true[\s\S]*?git -c protocol\.version=1 fetch[\s\S]*?--filter=blob:none[\s\S]*?--depth=1[\s\S]*?origin "\$CONTROL_SHA"/);
...
test('node operations is a reproducible single-service 1.4 target hosted once by L0', () => {
