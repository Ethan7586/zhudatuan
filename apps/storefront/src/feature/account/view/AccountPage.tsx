...
export function AccountPage({ viewmodel }: Readonly<{ viewmodel: ReturnType<typeof useAccountViewModel> }>) {
  ...
  if (viewmodel.section === 'favorites')
    return <FavoritePanel favorites={viewmodel.favorites} products={viewmodel.presentationProducts} open={viewmodel.actions.openProduct} remove={viewmodel.toggleFavorite} back={viewmodel.actions.profile} />;
  const actions = [
