...
describe('frozen voucher search exports', () => {
  ...
  it.each(['another actor', 'another scope', 'expired'] as const)('refuses %s snapshots before creating any export or job', async (denied) => {
    ...
    try {
      ...
      await expect(
        withWriteTransaction(data.query, (context) =>
          repository.search({
            ...call<'voucher.searchexports.create'>(context, { body: { snapshot: frozen.body.id, reason: '核对券卡' } }),
            ...(denied === 'another actor' ? { actor: 'actor:other' } : denied === 'another scope' ? { scope: 'mall:other' } : {}),
          })
        )
  ...
  it('rejects mutation or cross-scope snapshot items at the database boundary', async () => {
    ...
    try {
      ...
      await expect(
        data.database.query(
          `insert into voucher.searchsnapshotitem select snapshot_id,'mall:other','voucher:00002',ordinal+1,
  ...
  it.each([1, 3, 4])('fails closed when authorization is revoked at check %s', async (denyAt) => {
    ...
    try {
      ...
      const assert = vi.fn(async () => {
        if (++checks === denyAt) throw new DomainError('AUTHORIZATION_DENIED');
      });
...
function input() {
  return { exportId: 'export:one', scope: 'mall:one', attempts: 1, signal: new AbortController().signal, deadline: Date.now() + 10_000 };
}
...
function call<TKey extends OperationId>(transaction: ReadTransactionContext, input: unknown): VoucherCall<TKey> {
  const access = { actor: { id: 'actor:one', membership: 'membership:one', target: 'console', credentialVersion: 1 }, scope: { id: 'mall:one' }, organization: 'organization:one', accessVersion: 1, capabilityVersion: 1 } as AccessContext;
  return {
    input,
    context: { transaction, operation: 'voucher.searchexports.create', security: { kind: 'session', access } },
    scope: 'mall:one',
    actor: 'actor:one',
    member: 'member:one',
    target: 'console',
    idempotency: 'export:key',
    now: new Date(),
  } as VoucherCall<TKey>;
}
async function fixture(count: number) {
  ...
  await database.query(
    `insert into voucher.voucher(id,scope_id,number_masked,product_id,credential_id,holder_id,remaining_minor,currency,state,starts_at,expires_at,version,updated_at)
    select 'voucher:'||lpad(ordinal::text,5,'0'),'mall:one','****'||lpad(ordinal::text,4,'0'),'product:one','credential:'||ordinal,
    'holder:one',1000,'CNY','active',now()-interval '1 day',now()+interval '30 days',1,now()-interval '1 second' from generate_series(1,$1::integer) ordinal`,
    [count]
  );
  ...
  return {
    ...
    export: async (snapshot: string): Promise<RuntimeExportWork> => {
      ...
      await withWriteTransaction(query, (context) => new PgVoucherExport({ create } as ExportPort, { create: vi.fn() } as unknown as JobPort).search(call(context, { body: { snapshot, reason: '核对券卡' } })));
