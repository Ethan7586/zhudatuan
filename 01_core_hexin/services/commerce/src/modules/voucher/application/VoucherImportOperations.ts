  ...
  return {
    'voucher.cardlibraries.create': operationLifecycle({
      ...
      execute: async (_request, database, prepared) => {
        ...
        const result = await database.query(`insert into voucher.cardpool(id,scope_id,code_prefix,next_sequence,provider,mode,status,version)
        ...
        await database.query(`insert into voucher.importjob(id,cardpool_id,scope_id,object_ref,sha256,state,total_count,success_count,failure_count,created_at,updated_at)
    ...
    'voucher.imports.read': operationLifecycle({
      execute: async (request, database) => {
        ...
        return rowResult(await database.query(`select job.id,job.cardpool_id,job.state,job.total_count,job.cursor_value,job.success_count,job.failure_count,
          ...
          from voucher.importjob job where job.id=$1 and access.scope_allowed(job.scope_id)`, [request.input.path.importid!]));
      },
