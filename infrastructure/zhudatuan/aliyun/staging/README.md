...
此目錄只服務築大團隔離預發布環境，不是正式部署輸入。它提供 Auth／Console 靜態制品、Canonical API 與 `services/commerce/dist/JobsMain.js`，且不包含正式域名、正式 DNS、正式資料或正式流量切換。
...
install -m 0600 infrastructure/zhudatuan/aliyun/staging/.env.example /opt/zhudatuan-staging/shared/.env.staging
```
...
apps/auth-web/dist/
apps/console/dist/
services/commerce/dist/ApiMain.js
services/commerce/dist/JobsMain.js
node_modules/
```
