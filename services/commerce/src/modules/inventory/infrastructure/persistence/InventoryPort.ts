...
export class InventoryPort {
  ...
  async reserve(context: WriteTransactionContext, order: string, scope: string, demand: readonly StockDemand[]): Promise<void> {
    const database = this.transactions.database(context);
  ...
  async expireCheckout(context: WriteTransactionContext, checkout: string): Promise<void> {
    const database = this.transactions.database(context);
    await database.query(
