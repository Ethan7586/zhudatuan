...
export class QualificationsRevokeHandler implements OperationHandler<'qualification.qualifications.revoke', 'write'> {
  ...
  async execute(input: OperationInputFor<'qualification.qualifications.revoke'>, context: WriteHandlerContext<'qualification.qualifications.revoke'>): Promise<OperationReply<OperationOutputFor<'qualification.qualifications.revoke'>>> {
    ...
    const revoked = QualificationCase.restore(current)
      .revoke(access.actor.id, textField(bodyRecord(input), 'reason', 500), new Date().toISOString())
      .snapshot();
