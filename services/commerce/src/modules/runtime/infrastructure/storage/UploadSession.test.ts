...
describe('UploadSession', () => {
  it('issues a tenant-scoped grant with an immutable checksum, size, type and retention receipt', async () => {
    const authorizeUpload = vi.fn(async (input): Promise<UploadAuthorization> => ({
      reference: 'object:upload', url: 'https://objects.test/upload', method: 'PUT',
      headers: { 'content-type': input.contentType, 'content-length': String(input.size), 'x-content-sha256': input.sha256,
        'x-retention-until': String(input.retentionUntil) }, expiresAt: new Date(Date.now() + 300_000).toISOString(),
    }));
    ...
    expect(record).toMatchObject({ reference: 'object:upload', sha256, size: png.byteLength, contentType: 'image/png',
      retentionUntil: '2030-01-09T03:04:05.000Z' });
    expect(record.path).toMatch(new RegExp(`^tenant/${owner}/attachment/2030/01/02/[a-f0-9-]+\\.png$`, 'u'));
    expect(authorizeUpload).toHaveBeenCalledWith({ path: record.path, contentType: 'image/png', size: png.byteLength, sha256,
      expiresIn: RUNTIME_LIMITS.upload.authorizationSeconds, retentionUntil: record.retentionUntil });
  });
  ...
  it('streams the completed object once and verifies metadata, retention, magic, size and content hash', async () => {
    ...
    const chunks = vi.fn(async function* () { yield png.slice(0, 3); yield png.slice(3); });
...
function request(overrides: Partial<UploadRequest> = {}): UploadRequest {
  return { tenant: 'tenant:one', category: 'attachment', name: 'receipt.png', contentType: 'image/png', size: png.byteLength, sha256,
    retentionDays: 7, ...overrides };
}

function uploadRecord(): Omit<UploadRecord, 'upload'> {
  return { reference: 'object:upload', path: 'tenant/owner/attachment/2030/01/02/id.png', sha256, size: png.byteLength,
    contentType: 'image/png', retentionUntil: '2030-01-09T03:04:05.000Z' };
}

function metadata(record: Omit<UploadRecord, 'upload'>, overrides: Partial<ObjectMetadata> = {}): ObjectMetadata {
  return { reference: record.reference, path: record.path, sha256: record.sha256, size: record.size, scan: 'clean', contentType: record.contentType,
    retentionUntil: record.retentionUntil, lockedUntil: null, ...overrides };
}

function store(record: Omit<UploadRecord, 'upload'>, value: ObjectMetadata, bytes: Uint8Array): ObjectStore {
  return { inspect: vi.fn(async () => value), chunks: vi.fn(async function* () { yield bytes; }) } as unknown as ObjectStore;
}
