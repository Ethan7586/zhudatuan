...
import { ProviderList } from '../../federation/ui/ProviderList';
import { InvitationForm } from '../../invitation/ui/InvitationForm';
...
export function LoginPage(props: Readonly<LoginPageProps>) {
  ...
  return <AuthShell><AuthCard stage={first ? 1 : 2} onBack={props.onBack}><section className="authpanel" aria-busy={props.busy}>
    ...
    {first ? <div className="authformstack">
      ...
      <label className="authagreement"><input type="checkbox" checked={props.accepted} onChange={(event) => props.onAccepted(event.target.checked)} /><span>我已阅读并同意<button type="button" onClick={() => setPolicy('terms')}>《{props.bootstrap.legal.termsTitle}》</button>和<button type="button" onClick={() => setPolicy('privacy')}>《{props.bootstrap.legal.privacyTitle}》</button></span></label>
      {props.fields.agreement ? <p className="authfieldissue" role="alert">{props.fields.agreement}</p> : null}
    </div> : props.stage}
  </section>{policy === undefined ? null : <PolicyDialog policy={props.bootstrap.legal} kind={policy} onClose={() => setPolicy(undefined)} onAccept={() => { props.onAccepted(true); setPolicy(undefined); }} />}</AuthCard></AuthShell>;
}
