...
describe('EnrollmentGateway', () => {
  it('reads the one-time enrollment through its generated operation', async () => {
    ...
    const gateway = new EnrollmentGateway(identitySdk({ enrollmentsRead: read }), environment, bootstrapPort());
  ...
  it('singleflights duplicate completion and never carries an administrator-created password', async () => {
    ...
    const gateway = new EnrollmentGateway(identitySdk({ enrollmentsComplete: complete }), environment, bootstrapPort());
