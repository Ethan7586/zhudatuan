...
describe('requirement authority', () => {
  it('binds the repository 260821 workbook by content hash', async () => {
    ...
    expect(result.authority.logicalSource).toBe('RepositoryAuthority 05_docs_ziliao/docs_wendang/福利商城功能清单260821.xlsx');
    expect(result.authority.repositoryRelativePath).toBe('05_docs_ziliao/docs_wendang/福利商城功能清单260821.xlsx');
  ...
  it('rejects a repository symlink that resolves outside the repository', async () => {
    const fixture = await createFixture('05_docs_ziliao/docs_wendang/authority.xlsx');
    try {
      await mkdir(join(fixture.root, '05_docs_ziliao/docs_wendang'), { recursive: true });
      await symlink(fixture.outside, join(fixture.root, '05_docs_ziliao/docs_wendang/authority.xlsx'));
...
async function createFixture(repositoryRelativePath: string) {
  ...
  await mkdir(join(root, '02_platform_pingtai/config'), { recursive: true });
  await writeFile(outside, bytes);
  await writeFile(join(root, '02_platform_pingtai/config/authorities.yml'), [
    'version: 1',
    'requirements:',
