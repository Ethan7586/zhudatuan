...
describe('Finance reconciliation query', () => {
  it('parses signed differences and preserves authoritative evidence', async () => {
    const result = await readFinanceReconciliations(context(), { limit: 50 }, new AbortController().signal);
    expect(result.items[0]).toMatchObject({ difference_minor: -11_900, item_counts: { matched: 2, difference: 1 } });
  ...
  it('sends only cursor and limit and isolates the query key', async () => {
    await readFinanceReconciliations(context(), { limit: 50, cursor: 'cursor:2' }, new AbortController().signal);
  ...
  it('propagates cancellation', async () => {
    ...
    const pending = readFinanceReconciliations(context(), { limit: 50 }, controller.signal);
  ...
  it('rejects a page whose count does not match its item payload', async () => {
    ...
    await expect(readFinanceReconciliations(context(), { limit: 50 }, new AbortController().signal)).rejects.toThrow('FINANCE_PAGE_COUNT_MISMATCH');
  });
});
