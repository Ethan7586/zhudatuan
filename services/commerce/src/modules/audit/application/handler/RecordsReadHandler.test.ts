...
const row = Object.freeze({
  id: 'audit:one', kind: 'command', scope_id: 'mall:one', actor_id: 'actor:secret', actor_type: 'console',
  request_id: 'request:secret', operation: 'catalog.listings.publish', subject_type: 'actor', subject_id: 'actor:secret',
  object_type: 'catalog', object_id: 'listing:secret', outcome: 'succeeded', reason: 'http:200', before_hash: null,
  after_hash: 'a'.repeat(64), evidence: { decision: 'approved' }, trace_id: 'trace:secret', previous_hash: null,
  record_hash: 'b'.repeat(64), occurred_at: '2026-09-06T01:00:00.000Z',
});

describe('RecordsReadHandler', () => {
  it('masks sensitive fields in summary mode and carries one watermark through the cursor', async () => {
    const records = vi.fn<AuditHistoryRepository['records']>().mockResolvedValueOnce([row, { ...row, id: 'audit:two' }]).mockResolvedValueOnce([]);
    const access = vi.fn();
    const handler = new RecordsReadHandler({ records }, { record: vi.fn(), access });
    await withWriteTransaction(async () => result([]), async (transaction) => {
...
function context(transaction: HandlerContext<'audit.records.read'>['transaction'], assurance: number): HandlerContext<'audit.records.read'> {
  return {
    requestId: 'request:query', traceId: 'trace:query', deadline: Date.now() + 10_000, signal: new AbortController().signal,
    operation: 'audit.records.read', headers: {}, rawBody: '', transaction,
    security: { kind: 'session', access: {
      ...
      roles: [], organization: 'organization:one', scope: { id: 'mall:one', kind: 'mall', path: [] }, accessVersion: 1,
      capabilities: new Set(['audit.records.read']), capabilityVersion: 1, assurance: { level: assurance }, trace: 'trace:query',
    } },
  };
}
