...
export function withdrawalPersistence(repository: FinanceRepositoryFactory): Pick<FinancePersistence, 'withdrawalsCreate' | 'withdrawalsDecide' | 'withdrawalsRecover'> {
  return {
...
const create: FinanceAction = async (request, database) => {
  ...
  const result = await database.query(
    ...
    [id, amount, textField(body, 'destinationRef', 500), access.actor.id, textField(body, 'reason', 1000), JSON.stringify(record(body.evidence)), body.settlement, access.scope.id]
  );
...
async function decide(request: OperationRequest, database: SqlExecutor, repository: FinanceRepositoryFactory) {
  ...
  const result = await database.query(
    ...
    and requested_by<>$3 returning *`,
    [request.input.path.withdrawalid!, decision, access.actor.id, JSON.stringify({ decisionReason: textField(body, 'reason', 1000), decisionEvidence: record(body.evidence) }), access.scope.id]
  );
...
async function recover(request: OperationRequest, database: SqlExecutor, repository: FinanceRepositoryFactory) {
  const access = requireAccess(request);
  const body = bodyRecord(request.input);
