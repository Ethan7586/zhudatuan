import { chineseDomainLabel, presentProductPoolKind, presentProductStatus } from '@shop/presentation';
...
export const skuColumns: readonly DataColumn<Sku>[] = Object.freeze([
  { key: 'code', label: '商品规格编码', render: (row) => row.code },
...
export const priceColumns: readonly DataColumn<Price>[] = Object.freeze([
  { key: 'sku', label: '商品规格', render: (row) => row.skuCode },
  { key: 'scope', label: '价格范围', render: (row) => row.scopeName },
...
export const stockColumns: readonly DataColumn<Stock>[] = Object.freeze([
  { key: 'sku', label: '商品规格', render: (row) => row.skuCode },
  { key: 'scope', label: '库存范围', render: (row) => row.scopeName },
  { key: 'location', label: '库位', render: (row) => row.locationName },
...
export const qualificationColumns: readonly DataColumn<Qualification>[] = Object.freeze([
  { key: 'listing', label: '商城投放', render: (row) => row.listingTitle },
...
export const channelColumns: readonly DataColumn<Channel>[] = Object.freeze([
  ...
  { key: 'external', label: '渠道商品编号', render: (row) => row.externalId },
  { key: 'status', label: '映射状态', render: (row) => presentProductStatus(row.status).label },
  { key: 'version', label: '来源版本', render: (row) => `第 ${row.sourceVersion} 版` },
...
export const timelineColumns: readonly DataColumn<Timeline>[] = Object.freeze([
  ...
  { key: 'reference', label: '业务对象', render: (row) => row.referenceLabel ?? '—' },
]);
