...
export function useOrderDetailViewModel(orderId: string) {
  ...
  const act = async (kind: 'receive' | 'remind') => {
    ...
    try {
      ...
      identity.showToast(kind === 'receive' ? '确认收货失败，请刷新订单后重试' : '催发货过于频繁或当前状态不允许', 'error');
    } finally {
      setBusy(null);
    }
  };
  return Object.freeze({
    order,
    busy,
    actions: Object.freeze({ back: () => void navigate(ROUTES.storeorders), remind: () => act('remind'), receive: () => act('receive'), aftersale: () => order && void navigate(routePath('storeaftersale', { orderId: order.id })) }),
  });
}
