...
import type { CatalogRiskDecisionPort } from '../../../catalog/public/index';
...
export class RiskReplayJobProcessor implements JobProcessor {
  ...
  private async replay(policyid: string, version: number, abort: AbortSignal): Promise<void> {
    ...
    try {
      ...
      const repository = new PgRiskRepository(client);
      const selected = await repository.replay(policyid, version);
      if (!selected) {
        await client.query('commit');
        return;
      }
      const policy = new RiskPolicy(policyid, version, selected.rule, 100);
      const sample = await repository.replaySample(selected.scope);
      ...
      await repository.completeReplay(policyid, version, { sample: sample.length, changed, outcomes, changedRate: changed / Math.max(1, sample.length), falsePositiveRate: falsePositives / Math.max(1, sample.length) });
      await client.query('commit');
    } catch (cause) {
  ...
  private async applyCatalogDecision(decisionid: string, abort: AbortSignal): Promise<void> {
    ...
    try {
      ...
      if (command) await this.catalog.execute(client, { decision: command.decision, scope: command.scope, listing: command.resource });
      await client.query('commit');
    } catch (cause) {
      await client.query('rollback');
      throw cause;
