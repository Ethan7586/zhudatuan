...
export class InventoryPort {
  ...
  async reserve(database: OperationDatabase, order: string, scope: string, demand: readonly StockDemand[]): Promise<void> {
    ...
    const locked = await database.query<{ id: string; onhand: number; safety: number; reserved: number }>(`with locked as(
      ...
      select locked.id,locked.onhand::float8 onhand,locked.safety::float8 safety,coalesce((select sum(reservation.quantity)
        from inventory.reservation reservation where reservation.stockitem_id=locked.id and reservation.state='active'
          and reservation.expires_at>clock_timestamp()),0)::float8 reserved from locked order by array_position($1::text[],locked.id)`, [ids, scope]);
    ...
    for (const line of lines) {
      ...
      await database.query(`insert into inventory.movement(id,stockitem_id,kind,quantity_delta,reference_type,reference_id,occurred_at)
        values($1,$2,'reserve',$3,'order',$4,clock_timestamp())`, [`movement:${randomUUID()}`, stock.id, -line.quantity, order]);
  ...
  async commit(database: OperationDatabase, order: string): Promise<void> {
    const reservations = await database.query<{ id: string; stockitem_id: string; quantity: number }>(`select id,stockitem_id,quantity::float8 quantity
      from inventory.reservation where owner_type='order' and owner_id=$1 and state='active' order by stockitem_id for update`, [order]);
    for (const reservation of reservations.rows) {
      const stock = await database.query(`update inventory.stockitem set onhand=onhand-$2,version=version+1,updated_at=clock_timestamp()
        where id=$1 and onhand-safety>=$2 returning id`, [reservation.stockitem_id, reservation.quantity]);
  ...
  async release(database: OperationDatabase, order: string): Promise<void> {
    const reservations = await database.query<{ stockitem_id: string; quantity: number }>(`update inventory.reservation
      set state='released',version=version+1 where owner_type='order' and owner_id=$1 and state='active'
      returning stockitem_id,quantity::float8 quantity`, [order]);
