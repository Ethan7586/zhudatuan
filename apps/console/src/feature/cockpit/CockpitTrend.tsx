      ...
      <header>
        <div>
          ...
          <div className="measuretabs" role="tablist" aria-label="趋势指标">
            <button type="button" role="tab" aria-selected={measure === 'sales'} onClick={() => setMeasure('sales')}>净成交额</button>
            <button type="button" role="tab" aria-selected={measure === 'orders'} onClick={() => setMeasure('orders')}>支付订单</button>
          </div>
        </div>
        <div className="grainbuttons" role="group" aria-label="趋势粒度">
          <button type="button" aria-pressed={grain === 'day'} onClick={() => setGrain('day')}>按日</button>
...
function TrendGraphic({ rows, events, geometry, measure }: Readonly<{
  rows: readonly Trend[];
  ...
  return (
    <svg className="cockpitchart" viewBox="0 0 560 224" role="img" aria-label="净成交额折线与支付订单柱形组合趋势">
      ...
      {geometry.orders.map((height, index) => <rect key={rows[index]?.date} className="orderbar" opacity={measure === 'orders' ? 0.75 : 0.28}
        x={(geometry.x[index] ?? 0) - 5} y={176 - height} width="10" height={height} rx="2" />)}
      <polyline className="salesline" opacity={measure === 'sales' ? 1 : 0.42} points={geometry.points} />
      {eventMarkers(rows, events, geometry).map((marker) => <g key={marker.id} className={marker.kind === 'warning' ? 'eventwarning' : 'eventnormal'}>
        <line x1={marker.x} x2={marker.x} y1="28" y2={marker.y - 4} />
        <circle cx={marker.x} cy={marker.y} r="4" />
        <text x={marker.x} y="14" textAnchor="middle">{marker.label}</text>
        <text x={marker.x} y="26" textAnchor="middle">{marker.title}</text>
      </g>)}
      ...
      <g className="chartlegend"><line x1="44" x2="66" y1="214" y2="214" /><text x="72" y="218">净成交额（元）</text>
        <rect x="170" y="208" width="10" height="8" rx="2" /><text x="186" y="218">支付订单（单）</text></g>
    </svg>
  );
}
...
function eventMarkers(rows: readonly Trend[], events: readonly BusinessEvent[], geometry: Geometry) {
  return events.flatMap((event) => {
    ...
    if (index < 0) return [];
    return [{ id: event.id, kind: event.kind, x: geometry.x[index] ?? 0, y: geometry.sales[index] ?? 0,
      label: monthDay(event.date), title: event.title.replace(/^鸿泰(惠民通|甄选)/, '') }];
  });
}
