...
describe('PgMessageRepository', () => {
  it('writes the append-only message before deterministically linking clean evidence', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      calls.push(sql);
      if (sql.includes('insert into support.message('))
        return result([
          {
            id: 'message:one',
    ...
    const stored = await withWriteTransaction(query, (context) =>
      new PgMessageRepository().append(context, {
        scope: 'mall:one',
        ...
        visibility: 'external',
        sequence: 3,
        message: { id: 'message:one', clientMessageId: 'client:0001', ciphertext: 'encrypted-message', fingerprint: 'a'.repeat(64), keyVersion: 'v1', body: '不会进入 SQL' },
        attachments: ['evidence:two', 'evidence:one'],
      })
    );
