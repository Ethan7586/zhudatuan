...
export function createImportProcess(factory: ImportBatchFactoryPort, manager: TransactionManager, runtime: ImportPort, jobs: JobPort,
  identities: IdentityPrincipal, access: MemberImportAccessPort, profiles: Pick<MemberAccessPort, 'syncProfile'>,
  authorization: TaskAuthorizationPort): ImportProcessPort {
  const members = new MemberPort(profiles);
  return factory.create({
    owner: 'member', failure: 'MEMBER_IMPORT_ROW_FAILED', transactions: manager, runtime, authorization,
    ...
    continue: (context, target, sequence) => jobs.create(context, {
      idempotency: `${target.id}:${sequence}`, kind: 'memberimport', owner: 'member', scope: target.scope, queue: 'import',
      payload: Object.freeze({ import: target.id }), actor: 'system:member',
    }).then(() => undefined),
  });
}
