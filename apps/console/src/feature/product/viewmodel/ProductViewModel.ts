...
export function useProductViewModel(context: ConsoleContext, dependencies: ProductDependencies) {
  ...
  const batch = useMutation({ mutationFn: (published: boolean) => dependencies.changePublication.execute(command(context), rows.filter((row) => visible.has(row.id)), published), onSuccess: (receipt) => { setMessage(`批量操作完成：${receipt.count ?? visible.size} 条记录已更新`); setSelected(new Set()); void query.refetch(); } });

  const updateSearch = (update: (next: URLSearchParams) => void) => {
    const next = new URLSearchParams(search); next.delete('selected'); update(next); setSelected(new Set()); setSearch(next);
  };
  ...
  const actionvm = useProductActionViewModel(action, context, dependencies, () => {
    ...
    closeAction(); setMessage('商品操作已完成'); void query.refetch();
  ...
  const state = productState({ pending: query.isPending, fetching: query.isFetching, ...(query.data === undefined ? {} : { data: query.data }), empty: rows.length === 0, ...(query.error === null ? {} : { error: query.error }), updatedAt: new Date(query.dataUpdatedAt).toISOString() });
  return Object.freeze({
    resource: Object.freeze({ state, condition: productCondition(state), ...(error === undefined ? {} : { error }), refresh: () => void query.refetch() }),
    rows, filter: filterdraft, page, limit, columns, selected: visible, selectedid, message,
    batch: Object.freeze({ submitting: batch.isPending, ...(batch.error === null ? {} : { error: presentError(batch.error).message }), submit: (published: boolean) => { if (!batch.isPending) batch.mutate(published); } }),
    drawer: drawervm, action: actionvm, pool: poolvm, columnsopen,
    actions: Object.freeze({
      changeFilter: setFilterDraft,
      applyFilter: () => { const next = new URLSearchParams(); if (filterdraft.q !== '') next.set('q', filterdraft.q); if (filterdraft.category !== '') next.set('category', filterdraft.category); if (limit !== 50) next.set('limit', String(limit)); cursors.current = new Map([[1, undefined]]); setSelected(new Set()); setSearch(next); },
      resetFilter: () => { setFilterDraft(EMPTY_PRODUCT_FILTER); cursors.current = new Map([[1, undefined]]); setSelected(new Set()); setSearch(new URLSearchParams()); },
      open: (listing: Listing) => { const next = new URLSearchParams(search); next.set('selected', listing.id); setSearch(next); },
      close: () => { const next = new URLSearchParams(search); next.delete('selected'); setSearch(next); },
      select: (id: string) => setSelected((current) => toggleSelection(current, id)),
      selectPage: () => setSelected((current) => { const next = new Set(current); const remove = rows.length > 0 && rows.every((row) => next.has(row.id)); for (const row of rows) remove ? next.delete(row.id) : next.add(row.id); return next; }),
      openAction: setAction, closeAction, openPools: () => setPoolsOpen(true), closePools: () => setPoolsOpen(false), openColumns: () => setColumnsOpen(true), closeColumns: () => setColumnsOpen(false), toggleColumn: (key: ProductColumnKey) => setColumns((current) => toggleSelection(current, key)),
      next: () => { const cursor = query.data?.nextCursor; if (cursor === undefined) return; cursors.current.set(page + 1, cursor); updateSearch((next) => { next.set('cursor', cursor); next.set('page', String(page + 1)); }); },
      previous: () => { if (page <= 1) return; const target = page - 1; const cursor = cursors.current.get(target); updateSearch((next) => { if (target === 1) next.delete('cursor'); else if (cursor !== undefined) next.set('cursor', cursor); next.set('page', String(target)); }); },
      limit: (value: number) => { cursors.current = new Map([[1, undefined]]); updateSearch((next) => { next.delete('cursor'); next.delete('page'); value === 50 ? next.delete('limit') : next.set('limit', String(value)); }); },
    }),
