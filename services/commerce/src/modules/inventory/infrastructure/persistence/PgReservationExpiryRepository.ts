  ...
  async expire(context: WriteTransactionContext, owner: string | null, at: Date, limit: number): Promise<Readonly<{ expired: number; more: boolean }>> {
    ...
    const result = await database.query<ExpiryRow>(
      `select reservation.id,reservation.stockitem_id,reservation.owner_type,reservation.owner_id,reservation.quantity::float8 quantity,
       ...
       order by stock.id,reservation.id limit $3 for update of stock,reservation skip locked`,
      [at.toISOString(), owner, limit + 1]
    );
    ...
    await database.query(
      ...
      [JSON.stringify(reservations.map((item) => ({ id: `movement:${digest(`${item.id}:expired`)}`, stockitem: item.stockitem, quantity: item.quantity, ownerKind: item.ownerKind, owner: item.owner }))), at.toISOString()]
    );
    const balances = await database.query<{ id: string; reserved: number }>(
      `select stock.id,coalesce(sum(reservation.quantity) filter(where reservation.state='reserved' and reservation.expires_at>$2),0)::float8 reserved
       ...
       where stock.id=any($1::text[]) group by stock.id`,
      [[...stocks.keys()], at.toISOString()]
    );
    ...
    await runtime.appendMany([
      ...[...owners.values()].map((items) => ({
        id: `event:${randomUUID()}`,
        type: 'inventory.reservation.expired',
        aggregateType: items[0]!.ownerKind,
        aggregate: items[0]!.owner,
        scope: rows.find(({ owner_id }) => owner_id === items[0]!.owner)!.scope_id,
        trace: context.trace,
        payload: {
          owner: items[0]!.owner,
          ownerKind: items[0]!.ownerKind,
          lines: items.map((item) => ({
            sku: rows.find(({ id }) => id === item.id)!.sku_id,
      ...
      ...[...stocks.values()].map((stock) => ({
        ...
        scope: stock.scope,
        trace: context.trace,
        payload: { stockitem: stock.id, sku: stock.sku, available: Math.max(0, stock.onhand - stock.safety - (reserved.get(stock.id) ?? 0)), reserved: reserved.get(stock.id) ?? 0, version: stock.version },
      })),
    ]);
