...
export class AfterSaleAttachmentsCreateHandler implements DurableOperationHandler<'order.aftersaleattachments.create', PreparedAfterSaleAttachment, Reply, 'write'> {
  ...
  commit(
    _input: OperationInputFor<'order.aftersaleattachments.create'>,
    prepared: PreparedAfterSaleAttachment,
    _context: CommitContext<'order.aftersaleattachments.create'>
  ): Promise<DurableCommit<Reply, OperationOutputFor<'order.aftersaleattachments.create'>>> {
