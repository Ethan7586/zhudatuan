...
| 可否删除 | 禁止 |
| 二次复核 | 已完成：RV-0006（2026-09-15）从聚合入口、relay/scheduler职责和部署目标重新取证；维持GX、禁止删除 |
...
| 二次复核 | RV-0027 已完成入口/批准清单/发布制品复核（2026-09-15），维持GX；产品裁定后仍须重新检查App、owner-approved、bundle、BFF和视觉 |
...
| 二次复核 | RV-0029 已完成切换迁移、后继回填/退役和当前库存运行入口复核（2026-09-15），维持GX；生产ledger、库存/预留对账和恢复演练仍须经授权专项核验。 |
...
| 二次复核 | RV-0030 已完成事务骨架、敏感stage、领域代表样本、后续对账/退役序列与契约计划复核（2026-09-15），维持GX；逐scope数据对账与恢复仍须在隔离备份副本专项核验。 |
...
| 二次复核 | RV-0031 已完成对账evidence/hash、fail-closed断言及后续退役序列复核（2026-09-15），维持GX；真实对账输出、ledger和恢复方案仍须在隔离备份副本专项核验。 |
...
| 二次复核 | RV-0032 已完成退役命令、target-head fail-closed约束和契约序列复核（2026-09-15），维持GX；备份、实际ledger、维护窗口和恢复演练仍须经授权专项核验。 |
...
| 二次复核 | RV-0034 已完成资金事实模型、RLS/操作约束与当前结算/提现Worker入口复核（2026-09-15），维持GX；历史账本对账、渠道付款状态与恢复仍须经授权专项核验。 |
...
| 二次复核 | RV-0040 已完成：维持 GX；静态复核确认四类正式导入作业、cursor、暂存授权和分行失败隔离仍依赖目标结构。生产回填、对象、取消/重试和恢复未验证。 |
...
| 二次复核 | RV-0041 已完成：维持 GX；静态复核确认成员/券 API、Worker、续跑、敏感暂存权限和券指纹去重仍依赖目标结构。生产回填、成员归属、密钥、重试和恢复未验证。 |
...
| 可否删除 | 禁止 |
| 二次复核 | RV-0050 已完成：维持 GX；静态复核确认两个顺序版本均 fail-closed 验证 target-head checksum，目标断言文件在固定序列中。执行器、ledger 与恢复未验证。 |
...
| 可否删除 | 禁止 |
| 二次复核 | RV-0051 已完成：维持 GX；静态复核确认历史政策/条款回填 fail-closed，当前邀请/注册/治理校验仍依赖该事实。生产政策、scope 和恢复未验证。 |
...
| 可否删除 | 禁止 |
| 二次复核 | RV-0053 已完成：维持 GX；静态复核确认 session API/event 注册、后续 revoked 条件和手机号变更撤销链仍依赖该契约。原子性、消费者、跨 realm 和恢复未验证。 |
...
## DC-0090｜供应商业务模拟数据 seed/cleanup 的高风险运行责任

| 字段 | 记录 |
| --- | --- |
| 分类 | GX：高风险，禁止删除，需专项设计 |
| 对象 | `04_tools/scripts/data/{seed,cleanup}-supplier-business-simulation.sql`。 |
| 风险证据 | 两脚本会在一个事务中删除并写入订单、售后、渠道、财务、报表等多业务表；虽使用固定 simulation namespace 和配对 cleanup，误连错误数据库或schema漂移均可能造成实质数据风险。 |
| 保留证据 | seed生成可重复1000笔临时供应商业务事实并有末尾数量断言；cleanup以同一命名空间逆序清理，是唯一已见恢复路径。 |
| 可否删除 | 否；不得以无根自动入口推断闲置。任何替代/删除须单独设计、隔离数据库演练、回滚证明和二次独立复核。 |
| 二次复核 | 是；由数据/财务/供应商演示负责人确认当前人工入口、目标数据库安全互锁、执行回执与cleanup演练。 |

- 供应商业务模拟SQL归 DC-0090 / GX-0049。RV0069 已从脚本事务、命名空间、人工入口与配对清理重新取证；未执行写入，专项数据/财务复核仍需在隔离环境授权后进行。累计 G0 62、G1 100、G2 5、G3 0、GX 46；未删除任何文件。
...
- Console release build归 DC-0092 / GX-0050。RV0070 已重查人工封板、常规构建、外部制品与浏览器验收边界；未执行构建、写入或浏览器验证。累计 G0 62、G1 101、G2 5、G3 0、GX 47；未删除任何文件。
...
- RequirementGen主生成器归 DC-0093 / GX-0051。RV0071 已重查 authority、默认写入、`--check` 与质量门消费者；未运行生成器或安装缺失依赖。累计 G0 62、G1 101、G2 5、G3 0、GX 48；未删除任何文件。

## DC-0094｜VI 1.5 发布归档包的保留责任

| 字段 | 记录 |
| --- | --- |
| 分类 | GX：高风险，禁止删除，需专项设计 |
| 对象 | `05_docs_ziliao/VI_shijue/release-packages/ZHU-VI-1.5.zip`。 |
| 风险/保留证据 | 2.0 MiB 的可交付归档包包含视觉资产、构建脚本、PDF、预览和交付说明；现有产品直接依赖同版本 `current/` 资产，归档可能是复现、验收或外部分发的唯一证据。 |
| 可否删除 | 否；不可从其无源码导入或发布包名称推断无用。 |
| 二次复核 | 是；由品牌/发布负责人核实外部分发、版本追溯和可恢复来源后，才能讨论替代或归档策略。 |

- VI 1.5 发布归档包归 DC-0094 / GX-0052。RV0072 已复查 ZIP 只读清单、同版本 current 资产与 Auth Web 直接导入边界；未解压、执行、重建或修改归档。累计 G0 62、G1 101、G2 5、G3 0、GX 49；未删除任何文件。

## DC-0095｜发布制品校验清单写入器的高风险职责

| 字段 | 记录 |
| --- | --- |
| 分类 | GX：高风险，禁止删除，需专项设计 |
| 对象 | `04_tools/scripts/release/inventory.mjs`。 |
| 风险/保留证据 | 对显式目录递归、稳定排序地计算所有普通文件 SHA-256，并重写 `checksums.sha256`；会主动排除现有清单及 `release.sigstore.json`，防止自身/签名递归进入散列。 |
| 未排除项 | 未发现仓内自动调用，但人工发布、CI 外部编排和签名流程尚未得到运行复核。 |
| 可否删除 | 否；它是制品可验证性的写入边界，不可因无静态调用而降格为垃圾。 |
| 二次复核 | 是；由 Release Owner 在隔离制品目录核实清单、签名顺序及恢复流程。 |

- 发布制品校验清单写入器归 DC-0095 / GX-0053。RV0073 已重查显式目录、稳定遍历、清单/签名排除与制品交接边界；未对任何制品目录执行写入。累计 G0 62、G1 101、G2 5、G3 0、GX 50；未删除任何文件。

## DC-0096｜Storefront 跨版本资产池写入与回滚职责

| 字段 | 记录 |
| --- | --- |
| 分类 | GX：高风险，禁止删除，需专项设计 |
| 对象 | `04_tools/scripts/release/storefront-assets.mjs`。 |
| 风险/保留证据 | 以 content hash 保存共享对象并将多个 release assets 硬链接到同一对象池，使切换和回滚页面能够继续读取对方版本的静态资产；现有 recovery test 覆盖 A/B 版本和同名冲突。 |
| 已知边界 | F-0319：输入 root 可与 pool 重叠，现有测试未覆盖该反例。 |
| 可否删除 | 否；不能从无仓内自动入口推断闲置，可能仍由人工/外部发布编排使用。 |
...
## DC-0098｜2026-08 Operations 交接与发布历史记录集

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | Operations 目录下 10 份 2026-08 API/VI/登录/财务/短信/部署交接记录及 MallTopology.md。 |
| 证据 | 文件均显式标注 2026-08 日期、历史分支或交接/显示发布状态；未找到当前 runtime、workflow 或代码消费者。MallTopology 只被已漂移的 frontend evidence 引用。 |
| 保留原因 | 保存过往发布是否实际切流、未完成事项、历史 baseline/dirty tree 边界和拓扑决策，不可由零代码引用推断无价值。 |
| 可否删除 | 否；需先确认审计、合规、事故追溯和外部交接保留期。 |
| 二次复核 | 由 Operations/Release Owner 决定正式归档位置和保留周期。 |

- 8月 Operations 历史记录集归 DC-0098。累计 G0 62、G1 103、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0099｜历史 AI/实施提示词集合

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | prompts/ 下 9 份商品、订单、个人信息、权限及商城历史提示词。 |
| 证据 | 治理台账明确它们是 HISTORY，未发现程序消费者；但 architecture/strategy 仍链接其中部分，且文本记录了历史任务范围和设计取舍。 |
| 风险 | 近重复 zhudatuan/无前缀版本内容不同，可能被人工/AI 误用。 |
| 可否删除 | 否；应先建立当前任务模板和历史指针，再由治理负责人决定保留期。 |
| 二次复核 | 由治理/产品 Owner 确认 canonical 与归档状态。 |

- 历史提示词集合归 DC-0099。累计 G0 62、G1 104、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0100｜SFL 历史标准与候选发布引擎设计

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | SFL 1.5、1.6、1.8 标准和 SFL 统一 AI 发布引擎 V1.0 候选标准。 |
| 证据 | 版本文件包含节点、跨域、发布和回滚设计；无 runtime/workflow consumer，且治理台账将历史 SFL 版本/未启用架构建议降为 HISTORY/REFERENCE。 |
| 保留原因 | 版本演进、规格继承、取舍及候选引擎模型可能是后续设计/事故分析的唯一来源。 |
| 可否删除 | 否；必须先建立当前权威索引和历史保留策略。 |
| 二次复核 | 由治理与 Release Owner 确认 current authority、候选引擎状态和归档位置。 |

- SFL 历史标准与候选发布设计归 DC-0100。累计 G0 62、G1 105、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0101｜SFL 2.2 历史验收与统一节点实验报告

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `15-SFL-2.2-全量机器与真实业务验收-2026-09-12.md` 与 `16-SFL-统一节点模型全项目总报告与实验计划-2026-09-12.md`。 |
| 证据 | 两份文件均锁定 2026-09-12 的旧 `origin/zdt-next` SHA，明确 `production_touched: false`；后者明确将 44 项结果定义为历史证据框架、实验未执行且不能替代生产证据。第一份由第二份链接，无运行、构建、部署或工作流消费者。 |
| 保留原因 | 保存当时 44 项验收的失败/未知边界、统一节点模型的历史取舍及 E01—E12 实验设计；零程序引用不能证明这些证据没有审计、复盘或治理价值。 |
| 可否删除 | 否；应先由治理/架构 Owner 确定当前权威索引、归档位置及保留期。 |
| 二次复核 | 重新核对当前基线的实际运行入口、源代码及生产收据；不得把历史 PASS/FAIL/UNKNOWN 直接迁移为当前结论。 |

- SFL 2.2 历史验收与统一节点实验报告归 DC-0101。累计 G0 62、G1 106、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0102｜2026-09-02 GitHub 分支关闭账本

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/branch-closures/` 下 README 与 001—025 的六份历史关闭记录。 |
| 证据 | 记录均锁定 2026-09-02 的分支、tip、Bundle/Tag、恢复步骤和当时分支计数；未发现 runtime 或 workflow 消费，只有 architecture/02—04 与顶层文档的历史参考链接。 |
| 保留原因 | 保存删除前承接、生产保护、未提交 worktree 保留和可恢复路径；不能从当前远程引用不存在或零程序消费者推断可删除。 |
| 可否删除 | 否；需确认 Git 历史、事故追溯、恢复资产和保留周期。 |
| 二次复核 | Release/Git Owner 核对当前远程与归档资产；任何清理或恢复均须在独立授权任务中执行。 |

- 2026-09-02 分支关闭账本归 DC-0102。累计 G0 62、G1 107、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0103｜旧系统审计与 zdt-next 目标架构草图

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `00-zhudatuan-架构审计.md` 与 `01-zdt-next-目标系统架构图.md`。 |
| 证据 | 00 明示审计对象是已判失效的旧 `main` 工作区；01 明示为“供 Ethan 审核、尚未成为最终实现授权”的 2026-09-02 草图，且列出未决决策。二者由文档导航/策略/历史架构文档引用，未发现 runtime、构建或工作流消费者。 |
| 保留原因 | 分别保存旧系统资产/问题提炼和新主轴原始边界设计；当前基线不能覆盖其历史上下文。 |
| 可否删除 | 否；应先形成经授权的当前架构权威索引，并保持旧基线可追溯。 |
| 二次复核 | 架构 Owner 核对当前运行图、现行 LAW/治理索引与未来目标的取舍；不得把草图直接转成运行指令。 |

- 旧系统审计与目标架构草图归 DC-0103。累计 G0 62、G1 108、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0104｜2026-09-02 GitHub 分支谱系与收口图

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/02-github-分支谱系与收口图.md`。 |
| 证据 | 该图冻结于 2026-09-02，记录 32→8 的分支收口和历史承接关系；由 architecture/01、04 与策略文档引用，无 runtime/workflow consumer。当前远程有 44 个 head，不能用该图代替当前真值。 |
| 保留原因 | 保存旧历史关系、逐支提炼边界、Bundle/Tag 保护背景与关闭授权约束。 |
| 可否删除 | 否；需先建立当前 Git 事实来源与历史快照导航。 |
| 二次复核 | Release/Git Owner 在任何分支操作前重新读取目标 ref、承接关系、保护规则和恢复资产。 |

- 历史 GitHub 分支谱系图归 DC-0104。累计 G0 62、G1 109、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0105｜阿里云旧运行取证与生产反推架构

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/03-阿里云运行真值与生产反推架构.md` 与已单列覆盖的 `evidence/production-state-2026-09-02.json`。 |
| 证据 | 文档与 JSON 都明确记录 2026-09-02 的只读生产快照；文档首屏明确后续 current 已变化，且不能代表最新状态。只被目标架构/文档导航引用，未发现 runtime/workflow consumer。 |
| 保留原因 | 保存旧系统 Caddy、systemd、release、多 SHA 组合制品和生产提交保护的直接取证；这是迁移、恢复与历史事故复盘证据。 |
| 可否删除 | 否；须确认审计、生产迁移、恢复资产与合规保留期。 |
| 二次复核 | Release/Operations Owner 对当前控制面、实际路由、制品和运行收据重新取证；不得直接执行报告中的历史 Git/主机操作。 |

- 阿里云旧运行取证与反推架构归 DC-0105。累计 G0 62、G1 110、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0106｜20 条零独有分支逐条历史审计

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/04-20条零独有分支逐条审计.md`。 |
| 证据 | 文件锁定 2026-09-02 01:23 快照；明确随后远程变化使候选失效，且后续 005—025 以独立账本为准。表中严格区分 Git 承接、生产锚点与本地 worktree，未发现 runtime/workflow consumer。 |
| 保留原因 | 记录零独有分支不等于可删除的判断方法、生产保护前提、A/B/C 分类与重算纪律，是历史治理和事故复盘依据。 |
| 可否删除 | 否；需要当前 Git 事实来源、历史 Bundle/Tag 保留策略和 Release Owner 复核。 |
| 二次复核 | 任何未来分支动作均须重新读取目标 ref、独有提交、承接、保护规则和运行锚点；不得复用本表的候选集合。 |

- 20 条零独有分支逐条历史审计归 DC-0106。累计 G0 62、G1 111、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0107｜Mall 产品边界历史取证与裁定

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/05-Mall产品边界取证与裁定.md`。 |
| 证据 | 文档锁定 2026-09-02 的旧原仓库、WCHS 和阿里云 release；Mall Scope 的 `mall_id`/Membership 机制有当前代码事实支持，但 Mall Product 层级是 reference 设计。文档由 architecture/01 和导航引用，无 runtime/workflow consumer。 |
| 保留原因 | 保存产品边界与数据 Scope 的术语区分、旧系统取证和迁移设计背景，不能因没有程序入口而删除。 |
| 可否删除 | 否；应先由治理/产品/架构 Owner 确认当前术语和权威入口。 |
| 二次复核 | 重新核对现行 LAW、产品所有权、运行单元及实际数据 Scope；不得把历史裁定直接转换为数据/代码变更。 |

- Mall 产品边界历史取证与裁定归 DC-0107。累计 G0 62、G1 112、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0108｜H5 微信一键绑定与购物入口方案

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/06-H5微信一键绑定与购物入口架构.md`。 |
| 证据 | 文档首屏明示入口方案尚未迁移代码、接入微信配置或发布生产；当前代码保留小程序 `jscode2session` 路径、`identity.wechat.*` 合同和微信支付适配，未发现文档所述 `identity.federations.*` 服务号 H5 OAuth 入口注册。 |
| 保留原因 | 保存 H5 与小程序身份/会话/支付的边界、反向约束与分阶段验收方案；不能因未实现或没有程序消费者而删除。 |
| 可否删除 | 否；需由 Identity、Payments 与产品 Owner 决定是否实施、替代或正式归档。 |
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `07-登录系统总体设计图.md`、同名 PNG 与 SVG。 |
| 证据 | 文档明确是 2026-09-06 设计基线候选、2026-09-12 快照，且列出未完成的通用 Domain Binding、Compatibility 迁移、OTP UI、企微/SSO；其中列出的 Canonical 登录、Session Loader、Access Pipeline 与宏泰代理代码仍存在。PNG/SVG 只被该 Markdown 嵌入/链接。 |
| 保留原因 | 保存身份/会话模型、候选验收矩阵、当时架构差距及可视化关系。 |
| 可否删除 | 否；需 Identity/Security Owner 确认当前设计索引和历史设计资产保留策略。 |
| 二次复核 | 重新按当前代码、迁移、契约、浏览器入口和生产证据验证其中每项“当前事实”；图件随说明文档一并管理。 |

- 登录系统设计候选及图件归 DC-0110。累计 G0 62、G1 115、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0111｜SFL L0/L1 兼容边界历史清单

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `architecture/09-SFL-L0-L1兼容边界清单.md`。 |
| 证据 | 文档为 2026-09-08 的 SFL 1.6 兼容债台账，明确将候选与生产退出分开。当前 `hbbtzn-alias` 源码仍为 `routes: []` 的 410 哨兵，但文档所列生产切流、Tunnel 和合同路径不能由静态源码证明，且其中 `production-domain-boundary.json` 路径已不存在。 |
| 保留原因 | 保存“零独有/品牌字符串不等于污染”、节点边界、生产退出、回滚和关闭式失败的治理逻辑。 |
| 可否删除 | 否；须由 Release/Identity/Security Owner 重新核对当前控制面、路由、Tunnel、运行进程、制品和现行权威标准。 |
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `strategy/zhudatuan-主打团-战略全景图.md`、三份 Canvas、SVG 和 PNG 预览。 |
| 证据 | Markdown 为 2026-09-02 的整体观摩战略图；三份 Canvas 为 59–60 节点同主题变体，SVG/PNG 为同一可视化资产。没有页面、API、Worker、构建、部署或迁移入口；现行 SFL 标准仍引用 Markdown 并采用其 L0—L5／L6—L11 的同向语义。 |
| 保留原因 | 保存战略决策、节点术语、产品/经营/消费四流关系与视觉沟通背景；没有程序消费者不能证明无责任。 |
| 可否删除 | 否；须由治理、产品和架构 Owner 明确其现行参考／历史记录／设计资产身份，并检查外部设计、交接或合规引用。 |
| 二次复核 | 先以现行 LAW、架构标准和产品权威目录重新核对术语；不得把图中“永久裁定”措辞单独作为代码或数据变更依据，相关权威层级问题已登记 F-0327。 |

- 主打团战略全景图组归 DC-0113。累计 G0 62、G1 118、G2 5、G3 0、GX 51；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `voucher/Architecture.md`。 |
| 证据 | 文档保存单一 Commerce、Voucher/Finance 单向事实、异步作业和目标 B2B 边界；固定基线的 Voucher manifest 仍注册 19 个运行 operation 与四类 jobs，另有 74 个明确 `frozen` 的 target operation。未发现该 Markdown 被运行、构建、部署或迁移入口消费。 |
| 保留原因 | 保存目标模型、模块边界、迁移/切换前置条件与对当前运行链不得直接删除的约束。 |
| 可否删除 | 否；目标设计、当前运行链和 74 项冻结契约的正式替代/落地关系尚未由 Owner 定稿。 |
| 二次复核 | 先由治理、产品和架构 Owner 确认当前目标设计索引；修正 F-0332 的路径/占位迁移歧义后，重查是否存在外部实施或交接引用。 |

- 卡券目标架构设计归 DC-0114。累计 G0 62、G1 119、G2 5、G3 0、GX 51；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `voucher/Domain.md`。 |
| 证据 | 文件规定跨模块事实所有权、目标聚合/状态机/事务/事件/财务事实；当前 Voucher schema 和 manifest 与其关键边界同向，但目标实体/表模型尚未替换当前运行表，未发现运行、构建、部署或迁移入口直接消费该 Markdown。 |
| 保留原因 | 是未来 B2B 卡券实现、迁移和验收的领域规格；其状态/金额/幂等约束不能从无程序入口推断为无价值。 |
| 可否删除 | 否；需先完成目标设计、现行 contract、数据迁移和财务事实的正式映射，并确认外部产品/交接引用。 |
| 二次复核 | 由产品、架构和财务 Owner 对目标聚合逐项确认当前映射或退役决定；不得因目标名称不同而删除现行 `voucher.*` 表或 operation。 |

- 卡券目标领域模型归 DC-0115。累计 G0 62、G1 120、G2 5、G3 0、GX 51；未删除任何文件。
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `voucher/Flows.md`。 |
| 证据 | 记录目标命令、审批、发行、核销、退款、投影、导出及失败恢复；当前实际注册四类 Voucher job，部分发行/outbox 机制同向，但 74 个 B2B target operation 保持 frozen，未发现该 Markdown 被运行、构建、部署或迁移入口消费。 |
| 保留原因 | 保存跨模块调用方向、事务/最终一致边界、失败与恢复约束；是目标交付和验收的重要过程规格。 |
| 可否删除 | 否；须先完成 Target Contract 启用、实际链路验证及外部产品/交接引用复核。 |
| 二次复核 | 由产品、架构、财务 Owner 逐流程映射当前/目标状态；不能把部分已注册 job 视作整条 B2B 流程的等价替代。 |

- 卡券目标调用与数据流归 DC-0116。累计 G0 62、G1 121、G2 5、G3 0、GX 51；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `voucher/Delivery.md`。 |
| 证据 | 文件明确要求冻结 target contract 后才实施、生产快照迁移演练/对账/回滚/端到端验收后才切换或删除；当前 74 项 target operation 仍 frozen，release policy 也没有目标交付单元。未发现 Markdown 被运行/发布控制面消费。 |
| 保留原因 | 保存一次性切换的数据安全前置条件、回滚边界、删除门槛和验收定义；不能因未被程序消费而删除。 |
| 可否删除 | 否；须由产品、架构和发布 Owner 确认当前交付计划及现行控制面映射。 |
| 二次复核 | 在任何目标实施前，以当时最新主线建立独立迁移/发布计划复核，验证实际制品、运行单元、数据库快照、回滚和外部域名基线；不得复用本文件作为执行授权。 |

- 卡券目标交付与验收方案归 DC-0117。累计 G0 62、G1 122、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0118｜MVP 历史浏览器验收报告与截图组

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `evidence/MVP真实浏览器验收报告-20260822.md` 与 `evidence/mvp/` 5 张截图。 |
| 证据 | 记录 2026-08-22 对 hbbtzn 旧站的失败验收；报告与同目录截图没有逐项链接、散列、URL、时间或会话绑定，不能充当当前基线证据。当前源码仍存部分相关历史文案/迁移，但未做运行复现。 |
| 保留原因 | 保存发布阻断、旧站行为和验收范围的历史线索；缺 provenance 不能作为删除证据。 |
| 可否删除 | 否；先由产品/发布 Owner 确认历史浏览器会话、外部证据引用与保留期，并补 F-0336 所述 provenance 或明确归档身份。 |
| 二次复核 | 重新核对图像来源、报告版本、URL/时间、账号/环境边界和外部交接引用；当前浏览器验收必须另建可复跑收据。 |

- MVP 历史浏览器验收报告与截图组归 DC-0118。累计 G0 62、G1 123、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0119｜VI 1.1 验证证据组

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `evidence/vi/2026-08-28/README.md` 与 Desktop/Mobile PNG。 |
| 证据 | 2026-08-28 Upgrade Draft 的验证索引和全页设计图，受 VI 升级记录与品牌 1.1 草案引用；没有运行消费者，README 还明确 Owner 视觉确认、Dark Mode、Zoom、读屏和跨端迁移未完成。 |
| 保留原因 | 保存设计 token/组件/响应式状态、历史验证范围及未完成边界；不是当前生产设计或浏览器验收结论。 |
| 可否删除 | 否；须由 Design/Brand Owner 确认历史证据、批准候选总板和当前设计系统之间的正式保留/归档关系。 |
| 二次复核 | 修正 F-0337 的 viewport/导出图元数据后，核对外部设计、品牌批准和 VI 升级记录引用。 |

- VI 1.1 验证证据组归 DC-0119。累计 G0 62、G1 124、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0120｜2026-08-20 上线准备计划

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `上线准备计划-2026-08-20.md`。 |
| 证据 | 文件明确基于 `integration/merge-20260820@4ea30bd`，使用重组前路径、69 迁移、旧 deploy.sh 和历史 main 发布方式；未发现构建、部署或运行消费者。 |
| 保留原因 | 保存库存、支付、退款、财务、mock、部署和凭据边界的早期风险线索及计划反模式；历史 P0/P1 不等于当前事故。 |
| 可否删除 | 否；先由发布、安全、支付、库存和财务 Owner 将历史风险与当前控制面/现行 finding 做正式映射，并确认外部交接引用。 |
| 二次复核 | 任何基于其建议的行动均从当时最新主线重新取证、建立独立小批次；不得运行文中命令、删除文件、接触凭据或部署。 |

- 2026-08-20 上线准备计划归 DC-0120。累计 G0 62、G1 125、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0121｜2026-08-29 主打团代码审计 PDF

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `主打团代码审计.pdf`。 |
| 证据 | PDF 元数据为 2026-08-29，正文明确审计 `main@01f1ed4` 与 188 项未提交工作树，包含旧 six-app 目录、旧 quality/CI、历史统计、问题和治理建议；未发现当前运行/构建/发布消费者。 |
| 保留原因 | 保存质量工具、ESLint、架构门禁、Identity、测试和历史工作树的独立审计线索与推荐顺序。 |
| 可否删除 | 否；先由架构/质量 Owner 将其关键历史结论与现行 findings、外部交接或合规保留需求完成映射。 |
| 二次复核 | 任何采用其建议的变更都须在最新主线重新反向追踪入口、调用链、数据责任、验证与回滚；不得把 PDF 的 P0 或“可删除”意见直接照搬。 |

- 2026-08-29 主打团代码审计 PDF 归 DC-0121。累计 G0 62、G1 126、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0122｜2026-08-29 主打团登录系统评估

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `主打团登录系统评估.html`。 |
| 证据 | 文件明确固定在 `be37030`/`13781f1` 工作树，且排除之后的 ACL 修复；记录当时 Identity 的 ACL、Cookie、页面复杂度、兼容链路和测试结论。未发现当前运行、构建、发布或文档消费者。 |
| 保留原因 | 保存安全设计与最小权限测试的历史线索；当前仍有同名 Cookie、scrypt、PKCE 票据、登录失败写入和大型登录页，但旧统计/评级不能代替现行证据。 |
| 可否删除 | 否；先由身份/安全 Owner 映射外部交接、保留期和其结论与现行审计 findings 的关系。 |
| 二次复核 | 在最新主线重新追踪 Cookie、会话、ACL、兼容身份和真实最小权限数据库验证；不得执行历史报告中的改动建议。 |

- 2026-08-29 主打团登录系统评估归 DC-0122。累计 G0 62、G1 127、G2 5、G3 0、GX 51；未删除任何文件。

## DC-0123｜2026-08-20 双重事实源审计

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `双重事实源审计-2026-08-20.md`。 |
| 证据 | 文件保存当时 canonical 矩阵和生产不可验证边界；当前仍可找到 membership context、scope bindings、idempotency_keys、注册条款 contract 与库存迁移，但未发现运行、构建、发布或质量脚本消费者。 |
| 保留原因 | 保存关于双重业务事实的判定口径与数据/权限/资金收敛线索；旧目录、历史迁移和当时生产未知项不能直接证明当前状态。 |
| 可否删除 | 否；先由数据、支付与身份 Owner 将每项历史 canonical 结论映射至现行迁移、运行图和测试。 |
| 二次复核 | 从最新主线重查每项写入口、DB 约束、异步消费者、真实最小权限/生产证据与回滚责任；不得据此执行 SQL、迁移或删除。 |

- 2026-08-20 双重事实源审计归 DC-0123。累计 G0 62、G1 128、G2 5、G3 0、GX 51；未删除任何文件。
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `多端前端集成说明.md`。 |
| 证据 | 文中五个路径及多端 UI 仍有当前源码；但 `showcaseAccess.ts` 将其定义为 showcase，production 对 mini-program/Android/tablet 切换设限，且未发现该文档被运行、构建、发布或质量脚本消费。 |
| 保留原因 | 保存多端 UI、统一服务端事实和真实 SDK 未接入的历史声明；不能把“生产型 MVP/已验证”直接当当前发布承诺。 |
| 可否删除 | 否；先由产品和发布 Owner 确认其对外承诺、展示环境和替代交付说明。 |
| 二次复核 | 在最新主线逐端核验 host、route、API access、身份/支付、构建制品和真实原生 SDK；与 F-0063 的交付矩阵裁决保持一致。 |

- 多端前端集成说明归 DC-0124。累计 G0 62、G1 129、G2 5、G3 0、GX 51；未删除任何文件。

## GX-0041｜历史测试验收单中的登录材料

| 字段 | 记录 |
| --- | --- |
| 分类 | GX：高风险，禁止删除、改写、验证登录或按静态信息判断失效，需身份/安全专项设计。 |
| 对象 | `测试环境验收-会员与支付.md:11-22` 中的测试环境登录材料。 |
| 直接证据 | 文档声称是 hbbtzn 历史测试验收并明文列出多组账号材料；hbbtzn 仍出现在当前节点和发布配置。账号有效性、权限、访问范围、数据性质、轮换和撤销状态均未验证。 |
| 运行边界 | 文档不是运行入口，但可能被人工用于登录受控环境；当前节点配置含 hbbtzn storefront/API/accounts 等运行单元。不得以“历史”或“测试”推断材料无效。 |
| 可否删除 | 否；先由身份/安全 Owner 以授权只读方式确认账号生命周期、realm/role、可达性、日志、数据隔离、保留期和轮换收据，再决定受控替换或归档。 |
| 二次复核 | RV0063 已完成：文档受 Git 跟踪且被当前验收材料目录引用；同域仍有受控 node、身份 API、Storefront/API/Console/accounts 配置。未读取、复述、验证或使用任何材料，账号生命周期仍未验证；GX 维持。 |
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `福利商城296条需求修改点.md`。 |
| 证据 | 文档自称从 mapping 展开却记录 `6f5a…` 工作簿哈希；当前权威工作簿和 mapping 为 `78cfc3…`。generator outputs 与 requirement graph gate 均不包含该 Markdown。 |
| 保留原因 | 保存当时的 296 项人工阅读索引和“Implemented 不等于验收/发布”边界；不能作为现行需求、路由、操作或状态权威源。 |
| 可否删除 | 否；先由需求 Owner 确认外部需求合同、人工阅读和交接引用，并决定从当前 mapping 重生或正式标记历史。 |
| 二次复核 | 从最新主线重查 workbook authority、mapping/YAML/contract、生成输出、入口引用和状态语义；不得通过勾选或删行改变任何需求事实。 |

- 296 条需求人工展开索引归 DC-0125。累计 G0 62、G1 130、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0126｜福利商城前端代码修改清单

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `福利商城前端代码修改清单.md`。 |
| 证据 | 文件明确基于 `integration/merge-20260820@358bc…` 且状态为尚未执行；P0/P1、六端/605 文件盘点、框架/目录/部署目标均属历史目标。未发现运行、构建、发布或质量脚本消费者，仅被根治/代码蓝图作为规划线索引用。 |
| 保留原因 | 保存前端问题分类、目标 ADR、六端文件处置和“不以 mock/页面代替真实链路”的历史规划。 |
| 可否删除 | 否；先由产品、前端与架构 Owner 确认外部交接、当前前端运行图和是否从现行事实重生计划。 |
| 二次复核 | 从最新主线重查当前六端入口、路由、SDK、质量门、构建/发布和证据；不得运行文中框架迁移、安装、删除或部署建议。 |

- 历史前端根治计划归 DC-0126。累计 G0 62、G1 131、G2 5、G3 0、GX 52；未删除任何文件。
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `福利商城功能清单.xlsx`。 |
| 证据 | SHA-256 为 `6f5a7954…`，OpenXML 含九张需求/接口/MVP 表；当前 authority、mapping、generator 与 requirement graph gate 使用 `福利商城功能清单260821.xlsx` 的 `78cfc3…`，旧工作簿没有当前机器消费者。 |
| 保留原因 | 保存早期需求、MVP 与接口范围的历史迁移线索；工作簿可能含集成元数据，不能复制、运行或将旧数据当当前合同。 |
| 可否删除 | 否；先由需求/安全 Owner 复核外部合同、历史验收、留存期和敏感单元格治理，再决定受控归档。 |
| 二次复核 | 重新核对 workbook authority、mapping/YAML/contract、外部引用和敏感信息处理；不得将旧单元格值写入代码、日志、测试或文档。 |

- 旧版需求工作簿归 DC-0127。累计 G0 62、G1 132、G2 5、G3 0、GX 52；未删除任何文件。
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `福利商城全系统根治方案.md`。 |
| 证据 | 文件明确为 2026-08-25 目标蓝图，历史代码/工作簿/生成物/通过结果均标为迁移输入或 SnapshotOnly；MallTopology 将跨仓章节退役。未发现当前运行、构建、发布或质量脚本消费者。 |
| 保留原因 | 保存目标架构、领域/数据/质量合同、Authority Snapshot 设计、证据门和旧新需求迁移判定；不能将旧 P0、旧路径或历史安全计划当当前事实。 |
| 可否删除 | 否；先由架构、发布和安全 Owner 确认外部交接、目标架构审批、authority consumer 与保留期。 |
| 二次复核 | 从最新主线重新追踪实际 authority、运行图、需求生成、发布控制面、凭据治理和每个拟采纳的行动；F-0342 的 hash 冲突须独立复核。 |

- 全系统目标根治方案归 DC-0128。累计 G0 62、G1 133、G2 5、G3 0、GX 52；未删除任何文件。
...
| --- | --- |
| 分类 | G0：不是垃圾，仍有真实规划职责 |
| 对象 | `订单管理升级前详细预案-WCHS-MCBS-20260901.md`。 |
| 证据 | 两份 OMS 提示词均把该文件列为实施/评分的输入；`requirements/order.yml` 的 profile、14 个 OMS 需求和工作簿 SHA 与预案的需求重组保持可追溯关联。未发现当前运行、构建、发布或自动生成消费者。 |
| 保留原因 | 保存订单事实源裁决、OMS-001–014、升级顺序、验收矩阵和“不得整分支合并/不得新增订单双写”的规划约束。F-0343 表明其中生产筛选事实已漂移，但不构成删除理由。 |
| 可否删除 | 否；先由订单、产品与架构 Owner 决定其是否更新为现行订单事实，或正式冻结为历史预案并更新人工入口。 |
...
## DC-0131｜认证、短信与运行可靠性评估

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `AUTH-RELIABILITY-ARCHITECTURE-2026-08-15.md`。 |
| 证据 | 无当前运行、构建、发布、质量脚本或现行文档消费者；内容固定于 2026-08-15，单 ECS/单进程/手工部署陈述与当前多节点 systemd policy 不一致。 |
| 保留原因 | 保存认证、短信、缓存、事务和历史控制面故障的决策语境；不应据“零机器引用”删除。 |
| 可否删除 | 否；由身份/发布 Owner 核实外部运行手册、事故复盘、合规留存及历史启用记录后，决定冻结、替代或归档。 |
| 二次复核 | 从最新主线重新追踪运行/发布消费者、迁移/SMS 现状和外部交接，不执行文中任何环境、迁移或云操作。 |

- 认证可靠性评估归 DC-0131。累计 G0 64、G1 134、G2 5、G3 0、GX 52；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `AUTHORIZATION-IMPLEMENTATION.md`。 |
| 证据 | 摘要所述 authz 和 AccessPipeline 仍在当前代码/测试中，但未发现当前机器或文档消费者；角色/路径内容已部分漂移。 |
| 保留原因 | 保留会话、Scope、StepUp、风险与拒绝优先的高层安全设计；不能把无入口或不完整角色列表当删除依据。 |
| 可否删除 | 否；先由身份/数据库 Owner 核对外部合规、运行手册和架构入口，决定更新为受控摘要或冻结历史。 |
| 二次复核 | 从最新主线独立核对 authz、AccessPipeline、数据库角色矩阵和外部引用；不得连接数据库或改动权限。 |

- 授权实现摘要归 DC-0132。累计 G0 64、G1 135、G2 5、G3 0、GX 52；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `CORE-BUSINESS-READ-MIRROR.md`。 |
| 证据 | 无当前消费者；独立 cache 服务未见当前源码/受控发布 target，仅有 compatibility PM2 配置和仍在使用的 Commerce API client。 |
| 保留原因 | 保留单写读镜像、缓存安全、失效和目标扩展的架构决策；缓存客户端的回源实现使其仍有历史解释价值。 |
| 可否删除 | 否；先由缓存/发布 Owner 核实外部 Tair、运维、备份与性能验收材料，并决定冻结、产品化或受控退休。 |
| 二次复核 | 从最新主线重新追踪 cache client、target、制品、健康检查和外部配置；不得启动 sidecar 或访问云资源。 |

- 核心业务读镜像架构归 DC-0133。累计 G0 64、G1 136、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0134｜商品数据同源与微信支付执行方案

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `DATA-AND-PAYMENT-EXECUTION-PLAN.md`。 |
| 证据 | 未发现当前消费者；路由和 UnionID identity 工作已在当前源码/迁移出现，文档仍为预实施阶段口径。 |
| 保留原因 | 保存服务端事实、支付幂等/验签、同源和历史决策背景；不含当前凭据实值，不能据无消费者直接删除。 |
| 可否删除 | 否；先由支付、身份和产品 Owner 核实外部验收/决策记录，决定冻结、替代或归档。 |
| 二次复核 | 从最新主线重核 handler、契约、迁移、受控配置与外部引用；不得访问微信、商户凭据、数据库或线上支付。 |

- 商品数据同源与微信支付执行方案归 DC-0134。累计 G0 64、G1 137、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0135｜L0/L1 域名采购插座

| 字段 | 记录 |
| --- | --- |
| 分类 | G0：不是垃圾，承担当前领域/契约安全边界 |
| 对象 | `DOMAIN-PURCHASE-SOCKET.md`。 |
| 证据 | provisioning 的 DomainPurchasePolicy/Lifecycle、公共导出和单元测试，以及 contract 的 DomainRegistrarPort 均存在；未注册外部 adapter/Operation 与文档的未接通声明一致。 |
| 保留原因 | 保存保留域、所有权、审批、幂等、未知提交窗口与采购/DNS/发布隔离的边界。 |
| 可否删除 | 否；外部采购尚未接通不等于无职责。 |
| 二次复核 | 是；未来任何 registrar、Finance、DNS 或发布接通前，独立核对合同、状态机、权限、审计、幂等、恢复与真实注册商契约。 |

- 域名采购插座归 DC-0135。累计 G0 65、G1 137、G2 5、G3 0、GX 52；未删除任何文件。
...
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `MULTI-PLATFORM-DELIVERY-STANDARD.md`。 |
| 证据 | 当前 platform adapter contracts 存在，但文档所述 gate 不存在、matrix 已由 F-0063 证明断链，未发现当前消费者。 |
| 保留原因 | 保存跨端状态机、事务真相、同步/异步与平台适配边界；不能因交付事实过期而删除。 |
| 可否删除 | 否；先由产品/多端/发布 Owner 确认外部交付合同和新的唯一 authority。 |
...
| 字段 | 记录 |
| --- | --- |
| 分类 | G0：不是垃圾，承担当前上位产品/治理决策职责 |
| 对象 | `SMART-WING-MALL-MASTER-CHARTER.md`。 |
| 证据 | 数据支付与 VI 收敛方案引用该总纲；冻结决议定义 Owner、Member、授权、资格、支付和多端等上位边界。 |
| 保留原因 | 是产品/业务/安全/交付共同解释和冲突裁决源；路径漂移/F-0063 不构成删除依据。 |
| 可否删除 | 否；任何重写需 Owner 明确裁决并保存冻结语义、外部合同和历史可追溯性。 |
| 二次复核 | 是；从最新主线重核每个 current index、引用、机器 authority 和外部产品/合规约束；不得借更新索引改变冻结决议。 |

- 商城总纲归 DC-0138。累计 G0 66、G1 139、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0139｜智慧翼 VI 收敛执行计划

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `VI-CONVERGENCE-EXECUTION-PLAN.md`。 |
| 证据 | 数据支付方案与 pilot notes 仍引用其治理约束，但 paths/commands/current generator 已漂移；无机器消费者。 |
| 保留原因 | 保存机制优先、生成物保护、视觉裁决和外部素材授权边界。 |
| 可否删除 | 否；先由 VI/前端/工具 Owner 决定重生为现行计划或冻结历史，并核实外部设计验收材料。 |
| 二次复核 | 从最新主线重核 source/output、scripts、quality gates、各端设计入口与外部素材授权；不得生成、构建或改 UI。 |

- VI 收敛执行计划归 DC-0139。累计 G0 66、G1 140、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0140｜VI 收敛 T1 打样记录

| 字段 | 记录 |
| --- | --- |
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `VI-CONVERGENCE-PILOT-NOTES.md`。 |
| 证据 | VI 计划仍直接引用其经验；参照提交/路径/命令和测试结果已不对应当前工具链，无机器消费者。 |
| 保留原因 | 保存生成器、调用面、漂移、豁免与单一目的提交的历史经验和 Owner 待裁决项。 |
| 可否删除 | 否；先由 VI/工具 Owner 核实外部设计记录和打样提交来源，决定历史冻结或受控重生。 |
| 二次复核 | 从最新主线重核 git history、source/output、scripts、设计 authority 和外部验收；不得生成或改 token。 |

- VI T1 打样记录归 DC-0140。累计 G0 66、G1 141、G2 5、G3 0、GX 52；未删除任何文件。
...
| 分类 | G1：疑似闲置，证据不足 |
| 对象 | `VI-PORT-LOG.md`。 |
| 证据 | 无当前消费者；仅记录历史工作树/Fresh Replay/浏览器结果，Operation 数和文档 hash 均已漂移。 |
| 保留原因 | 保存 P0-P3 施工顺序、当时发现的跨域/权限/生成问题和未完成项。 |
| 可否删除 | 否；先由 VI/架构/质量 Owner 确认外部验收、事故复盘与交接留存。 |
| 二次复核 | 从最新主线重核 current contract、路径、hash、quality records 与外部引用；不得运行迁移/浏览器/发布。 |

- VI 移植执行日志归 DC-0141。累计 G0 66、G1 142、G2 5、G3 0、GX 52；未删除任何文件。

## DC-0142｜第三方许可声明

| 字段 | 记录 |
| --- | --- |
| 分类 | G0：不是垃圾，承担当前第三方许可/归属义务 |
| 对象 | `THIRD_PARTY_NOTICES.md`。 |
| 证据 | 当前 lock/依赖含 qrcode-generator 1.4.4 与 lucide-react 0.546.0；Commerce API 直接导入 qrcode-generator；license policy 允许 MIT/ISC。 |
| 保留原因 | 分发及修改第三方软件时的许可/归属文本不能因引用少或生成来源未验证而删除。 |
| 可否删除 | 否；任何依赖/打包资产变更前由法务/供应链 Owner 更新并验证 notices。 |
| 二次复核 | 是；从最新锁文件、制品清单和生成资产追踪第三方组件、版本、许可和归属要求；不得联网下载或更改依赖。 |

- 第三方许可声明归 DC-0142。累计 G0 67、G1 142、G2 5、G3 0、GX 52；未删除任何文件。

## 独立复核范围对账（AU-905）

- [FACT] AU-962 至 RV-0074 已将 12 个可追溯对象编为 GX-0042 至 GX-0053；当前可枚举 `GX-####` 唯一编号为 50，历史累计仍为 GX 52，余下差额为两个没有对象标识的计数单位。
- [FACT] G3 累计为 0；不得因未发现编号而推断不存在未定位高风险对象。
- [ACTION] 最终统计保留“GX 52（历史累计，50 已枚举、2 个无对象标识）”；P1 与所有可定位 GX 的第二轮复核已完成。两个差额只能由原始工作记录或受控历史证据继续追溯，不得为凑数补造对象。
- [BOUNDARY] 不以本对账为由删除、降级或合并任何 GX/P1/G3 项。
