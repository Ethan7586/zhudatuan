...
> 需求基线：`05_docs_ziliao/docs_wendang/福利商城功能清单.xlsx`  
> 架构基线：`05_docs_ziliao/docs_wendang/福利商城架构和补齐修改清单.md`  
> 逐文件落地清单：`05_docs_ziliao/docs_wendang/福利商城代码修改清单.md`  
...
| 事实 | 唯一Owner | 生成或消费方式 |
| --- | --- | --- |
| 需求 | `05_docs_ziliao/docs_wendang/requirements/mapping.json` | 由Excel生成，CI校验Hash和数量 |
...
```ts
interface CatalogSource { pullCatalog(cursor?: string): Promise<CatalogBatch> }
interface PriceSource { pullPrice(keys: readonly SourceSkuKey[]): Promise<PriceBatch> }
interface StockSource { pullStock(keys: readonly SourceSkuKey[]): Promise<StockBatch> }
interface RemoteOrderSubmitter { submit(order: RemoteOrderDraft): Promise<RemoteOrderReceipt> }
interface RemoteOrderCanceller { cancel(reference: string, reason: string): Promise<RemoteOrderState> }
interface TrackingSource { pullTracking(reference: string): Promise<TrackingSnapshot> }
interface RemoteRefundProvider { refund(request: RemoteRefundRequest): Promise<RemoteRefundReceipt> }
interface StatementSource { pullStatement(period: StatementPeriod): Promise<StatementFile> }
interface VerificationProvider { verify(request: RemoteVerificationRequest): Promise<RemoteVerificationReceipt> }
```
