    ...
    <div className="min-h-dvh bg-[var(--sw-background)] text-content">
      ...
      <header className="sticky top-0 z-40 border-b border-edge bg-surface/95 backdrop-blur">
        ...
        <div className="mx-auto flex max-w-[1280px] gap-1 overflow-auto px-3 pb-2 sm:px-5" aria-label="商城导航">
          <button type="button" onClick={viewmodel.actions.home} className="min-h-10 whitespace-nowrap rounded-xl px-3 text-xs font-bold hover:bg-brand-light">
            首页
          </button>
          {viewmodel.navigation
            ...
            .map((item) => (
              <button type="button" key={item.id} onClick={() => viewmodel.actions.navigate(item.route)} className="min-h-10 whitespace-nowrap rounded-xl px-3 text-xs font-bold hover:bg-brand-light">
                {item.title}
              </button>
            ))}
