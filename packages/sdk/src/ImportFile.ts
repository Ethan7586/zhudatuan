import { IMPORT_CAPACITY } from '@shop/config/runtime';
import type { OperationBodyFor } from '@shop/contract';
import { uploadObject } from './ObjectUpload';
import type { RequestContext } from './RequestContext';
import { createIdempotencyKey } from './RequestContextFactory';
import type { CommerceClient } from './operations/CommerceClient';

const INITIAL = new Uint32Array([0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19]);
const ROUND = new Uint32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
]);

export interface UploadedImport {
  readonly objectRef: string;
  readonly sha256: string;
  readonly fileName: string;
  readonly mediaType: ImportMediaType;
  readonly size: number;
}

export async function uploadImportFile(
  runtime: Pick<CommerceClient['runtime'], 'uploadsCreate'>,
  context: RequestContext,
  file: File,
  progress?: (processed: number) => void
): Promise<UploadedImport> {
  const contentType = validateImportFile(file);
  const sha256 = await hashFile(file, context.signal, progress);
  const intent = await runtime.uploadsCreate(
    { body: { name: file.name, contentType, size: file.size, sha256 } },
    { ...context, idempotencyKey: context.idempotencyKey ?? createIdempotencyKey() }
  );
  await uploadObject({ url: intent.upload.url, headers: intent.upload.headers, body: file, ...(context.signal === undefined ? {} : { signal: context.signal }) }).catch(() => {
    throw new Error('文件上传失败，请检查网络后重新选择文件。');
  });
  return Object.freeze({ objectRef: intent.reference, sha256, fileName: file.name, mediaType: contentType, size: file.size });
}

export function validateImportFile(file: File): ImportMediaType {
  const contentType = importContentType(file);
  const maximum = contentType === 'text/csv' ? IMPORT_CAPACITY.maximumFileBytes : IMPORT_CAPACITY.maximumSpreadsheetBytes;
  if (file.size < 1 || file.size > maximum) throw new Error(contentType === 'text/csv' ? 'CSV 文件不可为空且不得超过 1 GiB。' : 'XLSX 文件不可为空且不得超过 32 MiB。');
  return contentType;
}

export async function hashFile(file: File, signal?: AbortSignal, progress?: (processed: number) => void): Promise<string> {
  const hash = new Sha256();
  const reader = file.stream().getReader();
  let processed = 0;
  try {
    while (true) {
      if (signal?.aborted) throw signal.reason ?? new Error('IMPORT_UPLOAD_ABORTED');
      const result = await reader.read();
      if (result.done) break;
      hash.update(result.value);
      processed += result.value.byteLength;
      progress?.(processed);
    }
    return hash.hex();
  } finally {
    reader.releaseLock();
  }
}

class Sha256 {
  private readonly state = INITIAL.slice();
  private readonly buffer = new Uint8Array(64);
  private buffered = 0;
  private bytes = 0n;
  private done = false;

  update(source: Uint8Array): void {
    if (this.done) throw new Error('SHA256_ALREADY_FINALIZED');
    this.bytes += BigInt(source.byteLength);
    let offset = 0;
    if (this.buffered > 0) {
      const length = Math.min(64 - this.buffered, source.byteLength);
      this.buffer.set(source.subarray(0, length), this.buffered);
      this.buffered += length;
      offset += length;
      if (this.buffered === 64) {
        this.compress(this.buffer);
        this.buffered = 0;
      }
    }
    while (offset + 64 <= source.byteLength) {
      this.compress(source.subarray(offset, offset + 64));
      offset += 64;
    }
    if (offset < source.byteLength) {
      this.buffer.set(source.subarray(offset), 0);
      this.buffered = source.byteLength - offset;
    }
  }

  hex(): string {
    if (this.done) throw new Error('SHA256_ALREADY_FINALIZED');
    this.done = true;
    const tail = new Uint8Array(this.buffered < 56 ? 64 : 128);
    tail.set(this.buffer.subarray(0, this.buffered));
    tail[this.buffered] = 0x80;
    const bits = this.bytes * 8n;
    for (let index = 0; index < 8; index += 1) tail[tail.length - 1 - index] = Number((bits >> BigInt(index * 8)) & 0xffn);
    for (let offset = 0; offset < tail.length; offset += 64) this.compress(tail.subarray(offset, offset + 64));
    return [...this.state].map((value) => value.toString(16).padStart(8, '0')).join('');
  }

  private compress(block: Uint8Array): void {
    const words = new Uint32Array(64);
    for (let index = 0; index < 16; index += 1) {
      const offset = index * 4;
      words[index] = ((block[offset]! << 24) | (block[offset + 1]! << 16) | (block[offset + 2]! << 8) | block[offset + 3]!) >>> 0;
    }
    for (let index = 16; index < 64; index += 1) {
      const left = words[index - 15]!;
      const right = words[index - 2]!;
      words[index] = (words[index - 16]! + (rotate(left, 7) ^ rotate(left, 18) ^ (left >>> 3)) + words[index - 7]! + (rotate(right, 17) ^ rotate(right, 19) ^ (right >>> 10))) >>> 0;
    }
    let [a, b, c, d, e, f, g, h] = this.state;
    for (let index = 0; index < 64; index += 1) {
      const first = (h! + (rotate(e!, 6) ^ rotate(e!, 11) ^ rotate(e!, 25)) + ((e! & f!) ^ (~e! & g!)) + ROUND[index]! + words[index]!) >>> 0;
      const second = ((rotate(a!, 2) ^ rotate(a!, 13) ^ rotate(a!, 22)) + ((a! & b!) ^ (a! & c!) ^ (b! & c!))) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d! + first) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (first + second) >>> 0;
    }
    [a, b, c, d, e, f, g, h].forEach((value, index) => {
      this.state[index] = (this.state[index]! + value!) >>> 0;
    });
  }
}

function importContentType(file: File): ImportMediaType {
  const suffix = file.name.normalize('NFKC').toLowerCase();
  if (suffix.endsWith('.csv') && (file.type === '' || file.type === 'text/csv' || file.type === 'application/csv')) return 'text/csv';
  if (suffix.endsWith('.xlsx') && (file.type === '' || file.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')) return 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet';
  throw new Error('仅支持扩展名与内容类型一致的 CSV 或 XLSX 文件。');
}

function rotate(value: number, bits: number): number {
  return (value >>> bits) | (value << (32 - bits));
}

type ImportMediaType = OperationBodyFor<'RuntimeUploadsCreateInput'>['contentType'];
