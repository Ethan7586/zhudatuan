...
export class LoginGateway implements LoginPort {
  ...
  async proof(reference: string, code: string, context: Pick<Credential, 'target' | 'returns'>, signal?: AbortSignal): Promise<LoginOutcome> {
    const [authorization, bootstrap] = await Promise.all([this.authorizations.create(), this.bootstrap.read(context.target, context.returns, signal)]);
    const result = await this.sdk.sessionsComplete(
      ...
      commandContext(this.environment, context.target, bootstrap.csrf, signal)
    );
    const redirectUrl = await exchangeSession(this.sdk, this.environment, result, authorization, context.target, bootstrap.csrf, signal);
    return Object.freeze({ kind: 'authenticated', redirectUrl });
  }
}
