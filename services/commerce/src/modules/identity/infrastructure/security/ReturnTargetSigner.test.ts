...
const TARGETS = Object.freeze({ console: 'https://console.zhudatuan.com', storefront: 'https://zhudatuan.com' });
...
describe('ReturnTargetSigner', () => {
  it('accepts only the exact configured target before expiry', () => {
    ...
    expect(signer.verify(issued.proof, new Date(NOW.getTime() + 59_000))).toMatchObject({ target: 'console', url: TARGETS.console });
    expect(() => signer.verify(issued.proof, new Date(NOW.getTime() + 60_000))).toThrow('RETURN_TARGET_INVALID');
  });
  ...
  it.each(['https://attacker.example', 'https://console.zhudatuan.com.attacker.example', 'https://user@console.zhudatuan.com', 'https://console.zhudatuan.com/#token'])('rejects open redirect tampering for destination %s', (url) => {
    const issued = signer.issue('console', NOW);
    const [payload, signature] = issued.proof.split('.') as [string, string];
    const value = JSON.parse(Buffer.from(payload, 'base64url').toString('utf8')) as Record<string, unknown>;
