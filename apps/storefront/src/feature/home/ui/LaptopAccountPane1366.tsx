...
export const LaptopAccountPane1366: React.FC<{
  ...
  const count = (status: string) => presentationOrders.filter((order) => order.status === status).length;
  return (
    <>
      ...
      <div className="sw-web-account-sidebar w-[240px] flex-shrink-0 space-y-2.5">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-2xs">
          ...
          <div className="mt-2.5 space-y-2">
            <div className="bg-blue-50/80 border border-blue-200/80 rounded-md p-2">
              ...
              <div className="text-base font-black text-[var(--sw-brand-dark)] mt-0.5">¥{formatMinor(user.welfareBalanceMinor)}</div>
            </div>
            ...
            <div className="bg-emerald-50/80 border border-emerald-200/80 rounded-md p-2">
              <div className="flex items-center justify-between text-[10px] text-gray-500">
                ...
                <span className="text-[9px] bg-emerald-700 text-white font-bold px-1 rounded">生鲜/园区餐饮</span>
              </div>
              <div className="text-base font-black text-emerald-700 mt-0.5">¥{formatMinor(user.mealBalanceMinor)}</div>
            </div>
          </div>

          {/* 快捷跳转入口 */}
          <div className="grid grid-cols-2 gap-1.5 mt-2.5 pt-2 border-t border-gray-100 text-[11px]">
            <button onClick={() => openFeature('卡券包')} className="p-1.5 bg-gray-50 hover:bg-blue-50 text-gray-700 hover:text-[var(--sw-brand)] rounded text-center font-bold transition-colors cursor-pointer">
              我的卡券包
            </button>
            <button onClick={() => openFeature('账户明细')} className="p-1.5 bg-gray-50 hover:bg-blue-50 text-gray-700 hover:text-[var(--sw-brand)] rounded text-center font-bold transition-colors cursor-pointer">
              账户流水
            </button>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-2xs">
          ...
          <div className="grid grid-cols-3 gap-1 text-center">
            <button type="button" onClick={() => openFeature('待付款')} className="p-1.5 bg-gray-50 hover:bg-blue-50 rounded cursor-pointer transition-colors">
              <div className="text-xs font-black text-[var(--sw-promotion)]">{count('pending_payment')}</div>
              <div className="text-[10px] text-gray-500">待付款</div>
            </button>
            <button type="button" onClick={() => openFeature('待发货')} className="p-1.5 bg-gray-50 hover:bg-blue-50 rounded cursor-pointer transition-colors">
              <div className="text-xs font-black text-[var(--sw-brand)]">{count('pending_shipment')}</div>
              <div className="text-[10px] text-gray-500">待发货</div>
            </button>
            <button type="button" onClick={() => openFeature('待收货')} className="p-1.5 bg-gray-50 hover:bg-blue-50 rounded cursor-pointer transition-colors">
              <div className="text-xs font-black text-emerald-600">{count('pending_receipt')}</div>
