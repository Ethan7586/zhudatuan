...
export function usePartnerViewModel(context: ConsoleContext, dependencies: PartnerDependencies, requestStepup: () => void) {
  ...
  const query = useQuery<PartnerPage | StorePage>({
    ...
    queryFn: ({ signal }) => (section === 'store' ? dependencies.readStores.execute(context, cursor, signal) : dependencies.readPartners.execute(context, section, cursor, signal)),
  });
  ...
  return Object.freeze({
    section,
    cursor,
    partners: section === 'store' ? undefined : (page as PartnerPage | undefined),
    ...
    canManage: section === 'store' ? canUseOperation(context, OP_ORGANIZATION_STORES_MANAGE) : canUseOperation(context, OP_PARTNER_PARTNERS_MANAGE),
