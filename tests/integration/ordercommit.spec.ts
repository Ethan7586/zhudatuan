...
test('order commit has one fixed lock order and returns order plus prepared payment in one use case', () => {
  ...
  const source = text('../../services/commerce/src/modules/checkout/application/handler/ConfirmQuoteHandler.ts');
...
test('same checkout and same idempotency identity cannot create two orders', () => {
  ...
  const runtime = text('../../services/commerce/src/foundation/application/ModuleOperations.ts');
  assert.match(order, /checkout_id text not null unique/);
  assert.match(idempotency, /primary key\(scope,actor_id,operation,key\)/);
  assert.match(runtime, /record\.request_hash !== hash[\s\S]+IDEMPOTENCY_KEY_REUSED/);
  assert.match(runtime, /state === 'completed'[\s\S]+response/);
});
