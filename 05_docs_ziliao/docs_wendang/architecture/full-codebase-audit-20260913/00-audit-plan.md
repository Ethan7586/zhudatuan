...
CP-06 提交后结束当前单一目的会话。后续只有在 Ethan 明确授权时才执行：

- AU-007：contract definitions 与 contractgen。
- 目的：逐文件深审契约定义、operation/event/error/schema来源、生成器、生成清单、版本兼容与真实消费者；不同时进入SDK实现或业务handler。
- 输入：CP-06的配置权威图、运行图、问题清单、覆盖总账与独立复核队列。
- 禁止：修改契约或生成物、运行生成写模式、修复F-0029–F-0035、安装依赖、改变线上状态、推送或部署。
