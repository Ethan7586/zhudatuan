import { ArrowLeft, BellRing, CheckCircle2, CircleAlert, Clock3, PackageCheck, RefreshCw, ShieldCheck, Truck } from 'lucide-react';
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4 sm:p-6">
      ...
      <section className="grid gap-3 rounded-2xl bg-surface p-5 shadow-sm sm:grid-cols-2">
        <div><h2 className="flex items-center gap-2 font-black"><ShieldCheck size={18} className="text-brand" />支付与退款</h2><SectionState title="支付与退款" value={order.sections.payment} retry={actions.refresh} />{order.sections.payment.state === 'ready' ? <p className="mt-3 text-xs text-muted">实付 ¥{formatMinor(order.payment.capturedMinor)} · 已退 ¥{formatMinor(order.payment.refundedMinor)} · 可退 ¥{formatMinor(order.payment.refundableMinor)}</p> : null}</div>
