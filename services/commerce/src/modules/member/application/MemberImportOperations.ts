...
export function memberImportOperations(context: ModuleContext): OperationActions {
  const objects = context.container.get(OBJECT_STORE);
  return {
    'member.imports.create': operationLifecycle({
      prepare: (request) => prepareImport(request, objects),
      execute: async (_request, database, prepared) => {
        const id = `memberimport:${randomUUID()}`;
        const result = await database.query(`insert into member.importjob(id,organization_id,object_ref,sha256,state,created_at,updated_at)
        ...
        await database.query(`insert into runtime.job(id,kind,owner,scope_id,payload,state,priority,available_at,created_at,updated_at)
          values($1,'memberimport','member',$2,jsonb_build_object('import',$3),'queued',100,clock_timestamp(),clock_timestamp(),clock_timestamp())`,
        [`job:${id}:0`, prepared.access.scope.id, id]);
        return rowResult(result, 202);
      },
    }),
    'member.imports.read': operationLifecycle({
      execute: async (request, database) => {
        ...
        return rowResult(await database.query(`select job.id,job.state,job.total_count,job.cursor_value,job.success_count,job.failure_count,
          ...
          from member.importjob job where job.id=$1 and job.organization_id=$2`, [request.input.path.importid!, access.scope.id]));
      },
