...
describe('inventory reservation locking', () => {
  it('locks stock rows before aggregating reservations', async () => {
    ...
    const database = {
      query: async (text: string) => {
        ...
        if (text.startsWith('with locked as'))
          return result([{ id: 'stock:1', scope_id: 'mall:1', sku_id: 'sku:1', location_id: 'warehouse:1', onhand: 10, safety: 1, reserved: 2, version: 1, status: 'active', updated_at: new Date('2026-09-05T00:00:00.000Z') }]);
  ...
  it('treats a matching existing reservation as an idempotent replay', async () => {
    ...
    const database = {
      query: async (text: string) => {
        queries.push(text);
        if (text.startsWith('with locked as'))
          return result([{ id: 'stock:1', scope_id: 'mall:1', sku_id: 'sku:1', location_id: 'warehouse:1', onhand: 10, safety: 1, reserved: 2, version: 2, status: 'active', updated_at: new Date('2026-09-05T00:00:00.000Z') }]);
        if (text.startsWith('select id,stockitem_id'))
          return result([
            {
              id: 'reservation:one',
              stockitem_id: 'stock:1',
              owner_type: 'order',
              owner_id: 'order:1',
              quantity: 2,
              state: 'reserved',
              expires_at: new Date('2026-09-05T00:30:00.000Z'),
  ...
  it('ignores a duplicate provider source version without touching stock or emitting an event', async () => {
    const query = vi.fn(async (text: string, _values?: readonly unknown[]) => {
      if (text.startsWith('select stock.id'))
        return result([{ id: 'stock:1', scope_id: 'mall:1', sku_id: 'sku:1', location_id: 'warehouse:1', onhand: 10, safety: 1, reserved: 0, version: 2, status: 'active', updated_at: new Date('2026-09-05T00:00:00.000Z') }]);
      return result([]);
    });
    await withWriteTransaction(query, (context) => new InventoryPort().observe(context, { id: 'stock:1', scope: 'mall:1', sku: 'sku:1', location: 'warehouse:1', onhand: 10, safety: 1, provider: 'jdproduct', version: 'cursor:one' }));
