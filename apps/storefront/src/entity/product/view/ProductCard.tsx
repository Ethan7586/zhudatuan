  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-edge bg-surface shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="flex items-center justify-between gap-2 px-3 pb-3">
        {product.saleability.reasons.includes('price_unavailable') ? <b className="text-xs text-warning-strong">报价暂不可用</b> : <b className="text-danger">¥{formatMinor(product.priceWelfareMinor)}</b>}
        <button type="button" disabled={!availability.canPurchase} onClick={() => add(product)} aria-label={`将${product.title}加入购物车`} title={availability.canPurchase ? '加入购物车' : availability.availabilityText} className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-brand text-inverse disabled:cursor-not-allowed disabled:bg-disabled">
