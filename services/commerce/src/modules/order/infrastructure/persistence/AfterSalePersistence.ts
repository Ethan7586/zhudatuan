...
export class AfterSalePersistence {
  ...
  async read(request: OperationRequest, database: SqlExecutor): Promise<OperationResult> {
    ...
    const availableOrder = member && filter.order ? await loadAfterSaleOrder(database, filter.order, access.scope.id, false) : null;
  ...
  async apply(request: OperationRequest, database: SqlExecutor, attachments: readonly VerifiedAfterSaleAttachment[]): Promise<OperationResult> {
    ...
    for (const line of lines) {
      ...
      const decision = await this.policies.evaluate(database.transaction, {
  ...
  async review(request: OperationRequest, database: SqlExecutor, decision: 'approved' | 'rejected'): Promise<OperationResult> {
    ...
    if (decision === 'rejected') {
      ...
      await scheduleAfterSaleJob(database, `job:return:${sale.id}`, 'fulfillment', 'fulfillment', sale.scope_id, { aftersale: sale.id });
    } else {
