...
export const connectionViewModel = defineSupplierViewModel({
  routes: ['supplierconnections'],
  title: '接口与渠道连接',
  description: '查看当前供应商连接健康状态并发起安全测试；密钥和平台连接配置不可见。',
...
function connectionActions(item: OperatorRecord | undefined): readonly OperatorAction[] {
  ...
  return Object.freeze([
    Object.freeze({
      id: 'test',
      label: '测试连接',
      description: '使用平台托管密钥验证当前接口可用性。',
      confirmation: '测试不会展示或修改密钥，仅记录健康结果。',
      tone: 'primary',
  ...
  return operatorCollection(
    ...
    operatorItems(value).map((item) =>
      operatorRow({
        ...
        title: `${operatorText(item, 'provider')} · ${operatorText(item, 'region')}`,
        detail: `${latency(operatorNumber(item, 'health_latency_ms'))} · ${operatorText(item, 'health_reason') || '最近检查无异常说明'}`,
        statusLabel: healthStatus(operatorText(item, 'health_state'), operatorText(item, 'status')),
        timestamp: operatorText(item, 'checked_at') || operatorText(item, 'updated_at'),
      })
    )
  );
}
...
function latency(value: number | undefined): string {
  return value === undefined ? '延迟待检测' : `延迟 ${value} 毫秒`;
}
