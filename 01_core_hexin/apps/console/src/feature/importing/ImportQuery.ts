...
export const importKey = (context: ConsoleContext, kind: ImportKind, jobId: string) => Object.freeze([
  'console', context.scope.kind, context.scope.id, context.session.accessVersion, `${kind}.imports.read`, jobId,
] as const);
export async function readImport(context: ConsoleContext, kind: ImportKind, jobId: string, signal: AbortSignal) {
  ...
  const value = kind === 'member' ? await memberImportsRead(input, request)
    : kind === 'catalog' ? await catalogImportsRead(input, request) : await voucherImportsRead(input, request);
