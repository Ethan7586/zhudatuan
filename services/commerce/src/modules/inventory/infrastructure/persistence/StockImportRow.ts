...
export async function importStock(database: SqlExecutor, catalog: CatalogSku, scope: string, importid: string, rowNumber: number,
  row: Readonly<Record<string, string>>): Promise<StockImportResult | null> {
  ...
  const selected = await database.query<{
    id: string; scope_id: string; sku_id: string; location_id: string; onhand: number; safety: number; status: StockItemState;
    version: number; updated_at: Date | string; reserved: number;
  }>(
    `select stock.id,stock.scope_id,stock.sku_id,stock.location_id,stock.onhand::float8 onhand,stock.safety::float8 safety,
     ...
     where stock.scope_id=$1 and stock.sku_id=$2 and stock.location_id=$3 for update of stock`, [scope, sku, location]
  );
  const now = new Date().toISOString();
  const existing = selected.rows[0];
  const stock = existing
    ? StockItem.restore({ id: existing.id, scope: existing.scope_id, sku: existing.sku_id, location: existing.location_id,
      onhand: Number(existing.onhand), safety: Number(existing.safety), state: existing.status, version: Number(existing.version),
  ...
  if (!existing) {
    ...
    await database.query(`insert into inventory.stockitem(id,scope_id,sku_id,location_id,onhand,safety,status,version,updated_at)
      values($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [value.id, value.scope, value.sku, value.location, value.onhand, value.safety, value.state, value.version, value.updatedAt]);
  }
  ...
  const observed = await database.query(
    ...
    [`observation:${createHash('sha256').update(`${stock.snapshot().id}:${reference}`).digest('hex')}`, stock.snapshot().id, reference, onhand,
      JSON.stringify({ import: importid, row: rowNumber, safety, status }), now]
  );
  ...
  await database.query(
    `insert into inventory.snapshot(stockitem_id,observed_at,source,onhand,source_version) values($1,$2,'import',$3,$4)
     on conflict(stockitem_id,source,source_version) do nothing`, [stock.snapshot().id, now, onhand, reference]
  );
  ...
  if (existing && next !== stock) {
    ...
    const updated = await database.query(`update inventory.stockitem set onhand=$2,safety=$3,status=$4,version=$5,updated_at=$6
      where id=$1 and version=$7 returning id`, [value.id, value.onhand, value.safety, value.state, value.version, value.updatedAt, stock.snapshot().version]);
