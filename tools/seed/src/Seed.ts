...
async function ensureLocalCommercialCatalog(database: Client): Promise<void> {
  ...
  const readiness = await database.query<{ invalid_listing: number; invalid_price: number; invalid_stock: number; missing_price: number; missing_stock: number }>(
    `select
      ...
      (select count(*)::integer from inventory.stockitem stock where stock.scope_id='mall-zhudatuan'
        and stock.id!~'^stock:[A-Za-z0-9][A-Za-z0-9.:/-]*$') invalid_stock,
  ...
  if (!row || row.invalid_listing !== 0 || row.invalid_price !== 0 || row.invalid_stock !== 0 || row.missing_price !== 0 || row.missing_stock !== 0) throw new Error(`LOCAL_COMMERCIAL_CATALOG_INCOMPLETE:${JSON.stringify(row)}`);
}
