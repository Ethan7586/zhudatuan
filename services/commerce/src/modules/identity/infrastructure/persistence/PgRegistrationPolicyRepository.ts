...
export class PgRegistrationPolicyRepository implements RegistrationPolicyRepository {
  async current(database: OperationDatabase): Promise<RegistrationPolicyRecord | null> {
  ...
  async read(database: OperationDatabase, id: string, active: boolean): Promise<RegistrationPolicyRecord | null> {
