...
export class PgCheckoutSessionStore implements CheckoutSessionStore {
  ...
  async lockQuote(context: WriteTransactionContext, quote: string, member: string, mall: string): Promise<StoredCheckoutQuote> {
    ...
    const locked = await database.query<StoredCheckoutQuote>(
      ...
      session.quote_id "quoteId",session.quote_hash "quoteHash",session.input,session.version::integer version
