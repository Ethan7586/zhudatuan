...
export function supportCasePersistence(kms: KmsClient, ports: SupportPersistenceFactory): Pick<SupportPersistence, 'createCase'> {
  const assignment = new AssignmentPolicy();
  return {
    createCase: supportLifecycle({
      ...
      execute: async (_request, database, { access, body, ticket, conversation, message }) => {
        ...
        if (selected)
          await new PgRuntimeWriter(database).append({
            ...
            aggregate: ticket,
            scope,
            payload: { ticket, conversation, agent: selected.id, member },
            trace: access.trace,
          });
