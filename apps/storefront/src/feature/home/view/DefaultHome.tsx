...
export function DefaultHome({ viewmodel }: Readonly<{ viewmodel: ReturnType<typeof useHomeViewModel> }>) {
  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-ink via-brand to-brand-dark p-5 text-inverse shadow-lg sm:p-8">
        <Sparkles className="absolute -right-10 -top-10 h-48 w-48 opacity-15" />
        <p className="relative flex items-center gap-1 text-[11px] font-bold text-inverse-label sm:text-xs">
          <BadgeCheck size={15} aria-hidden="true" />
          {viewmodel.currentMall.enterpriseName} · 员工专享
        </p>
        <h1 className="relative mt-2 max-w-2xl text-2xl font-black leading-tight sm:text-4xl">员工专享福利季</h1>
        <p className="relative mt-2 max-w-2xl text-xs leading-5 text-inverse-label sm:mt-3 sm:text-sm sm:leading-6">严选好物与专属权益，价格、库存和购买资格实时核验，安心选购。</p>
        <button type="button" onClick={() => viewmodel.navigatePage('catalog')} className="relative mt-4 inline-flex min-h-11 items-center gap-2 rounded-full bg-warning px-5 text-sm font-black text-content sm:mt-6">
          进入福利专区
          <ArrowRight size={17} />
        </button>
      </section>
