...
describe('PgCartOfferRepository', () => {
  it('reads price and inventory owners on every projection instead of freezing a cart quote', async () => {
    ...
    const currentMany = vi.fn().mockResolvedValueOnce(new Map([['sku:1', { amountMinor: 100, currency: 'CNY', version: 'price:1' }]])).mockResolvedValueOnce(new Map([['sku:1', { amountMinor: 130, currency: 'CNY', version: 'price:2' }]]));
