...
describe('IntegrationClient', () => {
  it('rejects undeclared operations before transport', async () => {
    const client = new IntegrationClient(
      { id: 'test', baseUrl: 'https://vendor.test', secret: { token: 'x' }, endpoints: { health: '/health' }, healthOperation: 'health', limits },
  ...
  it('retries an idempotent operation and preserves the request context', async () => {
    let calls = 0;
    const client = new IntegrationClient(
      { id: 'test', baseUrl: 'https://vendor.test', secret: { token: 'x' }, endpoints: { health: '/health', read: '/read' }, healthOperation: 'health', limits },
      new HeaderAuthenticator('authorization', 'token'),
      async (_url, init) => {
        calls += 1;
  ...
  it('never retries a non-idempotent write without an idempotency key', async () => {
    let calls = 0;
    const client = new IntegrationClient(
      { id: 'test', baseUrl: 'https://vendor.test', secret: { token: 'x' }, endpoints: { health: '/health', write: '/write' }, healthOperation: 'health', limits },
      new HeaderAuthenticator('authorization', 'token'),
      async () => {
        calls += 1;
    ...
    const context: ProviderCallContext = { tenantId: 't', requestId: 'r', traceId: 'trace', deadline: Date.now() + 1000 };
    await expect(client.invoke(context, { operation: 'write', method: 'POST', idempotent: false, body: {} })).rejects.toThrow('PROVIDER_HTTP_503');
    expect(calls).toBe(1);
  });
  ...
  it('opens an isolated circuit after the signed failure threshold', async () => {
    ...
    const client = new IntegrationClient(
      { id: 'test', baseUrl: 'https://vendor.test', secret: { token: 'x' }, endpoints: { health: '/health', read: '/read' }, healthOperation: 'health', limits: strict },
      new HeaderAuthenticator('authorization', 'token'),
      async () => {
        calls += 1;
        return new Response('{}', { status: 503 });
    ...
    const context: ProviderCallContext = { tenantId: 't', requestId: 'r', traceId: 'trace', deadline: Date.now() + 1000 };
    await expect(client.invoke(context, { operation: 'read', method: 'GET', idempotent: true })).rejects.toThrow('PROVIDER_HTTP_503');
