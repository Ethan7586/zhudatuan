...
describe('capability assignment persistence', () => {
  it('conditions both entitlement and capability-set versions, appends history and emits navigation invalidation', async () => {
    ...
    const change = new CapabilitySet('mall:one', 7, 'enterprise:one', 4).change(current, { id: current.id, capability: current.capability, state: 'disabled', quota: 20, expiresAt: null }, new Date('2026-09-04T04:00:00.000Z'), {
      operations: 19,
      dependentCapabilities: 1,
    });
    const query = async (sql: string, values: readonly unknown[] = []) => {
      ...
      if (sql.startsWith('with effective as materialized')) {
        return row({
          id: 'entitlement:one',
          scope_id: 'mall:one',
          capability_id: 'voucher.lifecycle',
          name: '卡券完整生命周期',
          kind: 'feature',
          state: 'disabled',
          ...
          inherited_from: null,
          quota: 20,
          effective_at: '2026-09-04T04:00:00.000Z',
          expires_at: null,
          version: 3,
          capability_version: 18,
          dependency_healthy: true,
          disabled_reason: 'explicitdisabled',
          dependencies: [],
          operations: 19,
          dependent_capabilities: 1,
        });
      }
      return { rows: [], rowCount: 1 } as unknown as QueryResult;
    };

    const result = await withWriteTransaction(query, (context) =>
      new PgAssignmentRepository().save(context, change, {
