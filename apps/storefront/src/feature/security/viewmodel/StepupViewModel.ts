...
export function useStepupViewModel(open: boolean, onVerified: () => void) {
  ...
  useEffect(() => {
    if (!open) { setChallenge(null); setCode(''); setError(null); setPhoneMasked(undefined); return; }
    if (!session.session) return;
    const controller = new AbortController();
    void dependencies.stepup.phoneMasked(session.session, controller.signal).then(setPhoneMasked).catch((cause) => setError(presentError(cause).message));
  ...
  async function send() {
    ...
    try {
      ...
      setChallenge((await dependencies.stepup.start(session.session)).id); setCode('');
    } catch (cause) { setError(presentError(cause).message); } finally { setBusy(false); }
  }
  async function verify(event: FormEvent) {
    event.preventDefault(); if (!challenge || code.length !== 6) return;
    setBusy(true); setError(null);
    try {
      if (!session.session) return setError(presentFailure({ kind: 'client', code: 'SESSION_CONTEXT_MISSING', retryable: false }).message);
