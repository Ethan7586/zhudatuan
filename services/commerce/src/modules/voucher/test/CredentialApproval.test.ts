...
const receipt = { status: 202, body: { id: 'export:one', kind: 'credential', state: 'queued' as const,
  expiresAt: '2026-09-06T01:00:00.000Z', createdAt: '2026-09-05T01:00:00.000Z', updatedAt: '2026-09-05T01:00:00.000Z' } };
...
describe('credential export maker/checker entry boundary', () => {
  it('consumes the exact request, pool version, scope and maker binding before calling the export application', async () => {
    ...
    expect(data.query).toHaveBeenCalledWith(expect.stringContaining('access.consume_action_proof'), [
      createHash('sha256').update('a'.repeat(43)).digest(), operation, 'scope:test', executionRequestHash(operation, input, 4),
      4, 'console', 'scope:test', 'membership:maker', 'voucher.credential.export' ]);
...
function fixture() {
  ...
  const application = vi.fn(async () => receipt); const complete = vi.fn();
  const claim = vi.fn<import('../../../foundation/application/IdempotencyRepository').IdempotencyRepository['claim']>(async () => ({ state: 'started' }));
  const executor = new OperationExecutor(transactionManager(query), { claim, complete, checkpoint: vi.fn() }, new PgMakerCheckerGuard(new PgTransactionAccess()),
    new AuditDecorator({ append: vi.fn() }), { append: vi.fn() });
  const access = { actor: { id: 'principal:maker', membership: 'membership:maker', target: 'console', credentialVersion: 1 },
    membership: { id: 'membership:maker' }, scope: { id: 'scope:test', kind: 'platform' }, organization: 'organization:test',
    accessVersion: 1, capabilityVersion: 1 } as AccessContext;
  const execution: ExecutionContext<typeof operation> = { operation, requestId: 'request:test', traceId: 'trace:test',
    deadline: Date.now() + 10_000, signal: new AbortController().signal, headers: {}, rawBody: '', idempotencyKey: 'export:key', expectedVersion: 4,
    security: { kind: 'session', access } };
  return { query, application, complete, claim, execute: (proof = true) => executor.execute(new CredentialExportsCreateHandler({ credentialexportsCreate: application }),
    input, { ...execution, ...(proof ? { actionProof: 'a'.repeat(43) } : {}) }) };
}
