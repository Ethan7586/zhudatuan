...
describe('Referral gateway', () => {
  it('maps and deeply freezes the authoritative promotion page', async () => {
    ...
    expect(result.items[0]).toMatchObject({ amountMinor: 1288, status: 'available' });
  ...
  it('binds a write to scope, version, stable identity, CSRF and one-time proof', async () => {
    ...
    const values = { action, reason: '年度分销政策调整', enabled: false, firstTouchDays: 30, rateBasisPoints: 800, minimumWithdrawalMinor: 5000, currency: 'CNY' };
    await gateway.execute(context(), createReferralCommand(values), 'a'.repeat(43), 'command:referral:1');
    expect(command?.body).toEqual({ enabled: false, firstTouchDays: 30, rateBasisPoints: 800, minimumWithdrawalMinor: 5000, currency: 'CNY', expectedVersion: 7, reason: '年度分销政策调整' });
...
function commissionPage() {
  return {
    items: [{ id: 'referralcommission:1', orderId: 'order:1', promoterId: 'referralmember:1', status: 'available', amountMinor: 1288, currency: 'CNY', availableAt: '2026-08-26T00:00:00Z', version: 3 }],
    count: 1,
    nextCursor: 'cursor:3',
  };
}
