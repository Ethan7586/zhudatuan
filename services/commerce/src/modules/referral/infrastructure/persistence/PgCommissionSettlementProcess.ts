...
import type { CommissionSettlementProcess } from '../../application/port/CommissionSettlementProcess';
...
export class PgCommissionSettlementProcess implements CommissionSettlementProcess {
  ...
  settle(scopeId: string, orderId: string | null, signal: AbortSignal, deadline: number): Promise<void> {
    ...
    return this.manager.write({ tenant: scopeId, membership: '', scope: scopeId, actor: 'system:referral', trace: `referralsettlement:${scopeId}`, operation: 'referralsettlement', workload: 'jobs', signal, deadline }, async (context) => {
      ...
      await this.commissions(context, transaction, scopeId, orderId);
      await this.withdrawals(context, transaction, scopeId);
      await reschedule(transaction, scopeId, orderId, this.clock.now().toISOString());
    });
  }

  private async commissions(context: WriteTransactionContext, transaction: SqlExecutor, scopeId: string, orderId: string | null): Promise<void> {
  ...
  private async withdrawals(context: WriteTransactionContext, transaction: SqlExecutor, scopeId: string): Promise<void> {
