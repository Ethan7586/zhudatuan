...
import { requireAccess, rowResult, type OperationActions } from '../../foundation/application/ModuleOperations';
...
export function voucherCatalogOperations(organizations: Pick<OrganizationReadPort, 'descendants'>): OperationActions {
  return {
    ...
    'voucher.cardlibraries.allocate': async (request, database) => {
      ...
      const allowed = await organizations.descendants(database, organizationScope(access.scope));
