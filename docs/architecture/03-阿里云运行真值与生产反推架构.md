...
<<<<<<< HEAD
> **后续状态变化（2026-09-02 02:01 +08:00）**：`/opt/zhudatuan/current` 已由其他任务切换为 `/opt/zhudatuan/releases/0d9bcb13e8a-console-perf95-stable`，远程 `ethan/iam-reliability-95` 也已前进到 `0d9bcb13e8a1c5452d8a541e1bfaa76a0ac9ad8f`。下文保留 00:17 快照用于历史取证，不再代表最新 current；最新 Mall 产品结构取证见 `05-Mall产品边界取证与裁定.md`。
...
=======
在关闭这些本地分支或清理 worktree 前，必须先把 `443cdb0` 保存为 GitHub 不可变生产证据标签或独立归档。
>>>>>>> 1d1ee336 (docs(architecture): derive target system from production)
=======
2026-09-02 已完成远程保护：annotated tag `prod-evidence/2026-09-02/web-business-api-443cdb0` 精确指向该提交。后续仍不得删除该 Tag，直到生产迁移完成并由 Ethan 明确确认。
...
## 十二之二、当前 20 条零独有分支的生产血缘审计

2026-09-02 01:23 +08:00 再次只读核对生产实例身份、`current`、9 个 systemd 进程实际 CWD、39 个主 release 目录和 Referral 历史制品；运行状态与本文件前述快照一致。

当前 28 条远程分支中有 20 条独有提交为 0，但其中有多条 tip 已进入在线后代，三条还分别精确对应在线 Support 来源、在线 Identity 来源或 current 的 `RELEASE-BASE`。因此不能只凭“独有提交为 0”批量删除。逐条双向祖先关系、精确 release、本地 worktree 和关闭等级见 `04-20条零独有分支逐条审计.md`；机器证据见 `evidence/zero-unique-branches-20-2026-09-02.json`。

本次只读审计没有删除 GitHub 分支、没有清理阿里云 release、没有改 Caddy，也没有重启服务。
=======
但在真正删除前，必须先完成 `443cdb0` 的远程证据保护，并建立第一版 Production State Manifest。本文件只确定候选，不授权删除。
>>>>>>> 1d1ee336 (docs(architecture): derive target system from production)
=======
`443cdb0` 远程证据保护完成后，Ethan 明确授权开始减少第一条分支。该远程分支已删除，分支总数由 32 降为 31；完整记录见 `branch-closures/001-canonical-registration-release.md`。
>>>>>>> 0a6459e3 (docs(branches): record first legacy branch closure)
...
=======
- 不删除任何本地 worktree 或本地分支，直到 `443cdb0` 被保护。
- 不删除 GitHub 远程分支，直到逐支账本开始执行。

当前正确动作是：先保存生产证据，再提炼架构，再迁移能力，最后一条一条减少分支。
>>>>>>> 1d1ee336 (docs(architecture): derive target system from production)
=======
- 不批量清理本地 worktree 或本地分支；每个运行 SHA 仍需逐项核对。
- 不批量删除 GitHub 远程分支；只能按逐支账本并经 Ethan 明确授权执行。
