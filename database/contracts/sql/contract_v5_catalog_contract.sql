...
do $contract$
begin
  if (select count(*) from runtime.operation)<>383 then raise exception 'CONTRACT_V5_OPERATION_CATALOG_MISMATCH'; end if;
  if (select count(*) from capability.operation)<>383 then raise exception 'CONTRACT_V5_CAPABILITY_BINDING_MISMATCH'; end if;
  ...
  if not exists(
    ...
    where artifact='commerce' and version='5.0.0' and checksum='788f19669c6e665ae615436a18454ce14ff8c66f0d066cd5a3e57f75d1a0d23e'
      and operation_count=383 and event_count=145 and status='active'
