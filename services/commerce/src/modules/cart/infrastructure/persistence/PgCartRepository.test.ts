...
function database(rows: ReadonlyArray<ReadonlyArray<Record<string, unknown>>>): OperationDatabase {
  let index = 0;
  return { query: vi.fn(async () => ({ rows: [...(rows[index++] ?? [])], rowCount: 0, command: '', oid: 0, fields: [] })) } as unknown as OperationDatabase;
}
...
describe('PgCartRepository', () => {
  it('rejects a stale cart version after locking the active cart', async () => {
    ...
    await expect(new PgCartRepository().lockOrCreate(target, 'member:1', 'mall:1', 'app:1', 3)).rejects.toMatchObject({ code: 'VERSION_CONFLICT' });
  });
  ...
  it('locks line versions in stable listing order', async () => {
    ...
    await new PgCartRepository().lineVersions(target, 'cart:1', ['listing:b', 'listing:a']);
  ...
  it('mutates a whole batch in one CTE and increments the cart exactly once', async () => {
    ...
    await new PgCartRepository().mutate(target, 'cart:1', [
