...
describe('after-sale attachment', () => {
  it('authorizes direct upload under the member-scoped private path', async () => {
    ...
    const service = new AfterSaleAttachment({ authorizeUpload, inspect: vi.fn() });
  ...
  it('accepts only a clean uploaded object with matching owner and checksum receipt', async () => {
    ...
    const service = new AfterSaleAttachment({ authorizeUpload: vi.fn(), inspect });
