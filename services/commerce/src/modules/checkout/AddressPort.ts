...
export class AddressPort {
  async snapshot(database: OperationDatabase, id: string | null, member: string): Promise<unknown | null> {
    if (id === null) return null;
    return (
      (
        await database.query(
          `select id,recipient_ciphertext,mobile_ciphertext,address_ciphertext,
      recipient_masked,mobile_masked,address_masked,region_code,version from checkout.address
  ...
  list(database: OperationDatabase, member: string, after: string | null, limit: number) {
    return database.query(
      `select id,recipient_masked,mobile_masked,address_masked,region_code,status,version from checkout.address
  ...
  remove(database: OperationDatabase, id: string, member: string, expectedVersion: number | null) {
    return database.query(
      `update checkout.address set status='deleted',version=version+1
  ...
  save(database: OperationDatabase, input: AddressInput) {
    return database.query(
      `insert into checkout.address(id,member_id,recipient_ciphertext,mobile_ciphertext,address_ciphertext,region_token,address_token,
...
export type MemberAddressPort = Pick<AddressPort, 'list' | 'remove' | 'save'>;
export const MEMBER_ADDRESS_PORT = publicPort<MemberAddressPort>('checkout', 'member');
