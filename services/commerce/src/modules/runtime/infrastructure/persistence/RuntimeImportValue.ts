...
export interface RuntimeImportRow extends Record<string, unknown> {
  readonly id: string; readonly scope_id: string; readonly owner: string; readonly object_key: string; readonly file_hash: string; readonly state: string;
  readonly total_count?: number | string; readonly cursor_value?: number | string; readonly success_count?: number | string; readonly failure_count?: number | string;
  readonly created_at?: string; readonly updated_at?: string; readonly validation_summary?: Readonly<Record<string, unknown>>;
  readonly last_error?: string | null;
  readonly errors?: readonly Readonly<{ row_number: number; reason_code: string; field: string | null; detail: unknown }>[];
  readonly report_object_ref: string | null; readonly report_sha256: string | null; readonly report_size: number | string | null;
...
export function runtimeImportView(row: RuntimeImportRow): RuntimeImportView {
  return Object.freeze({ ...runtimeImportCreated(row),
    validation_summary: Object.freeze({ ...(row.validation_summary ?? {}) }), last_error: row.last_error ?? null,
    errors: Object.freeze([...(row.errors ?? [])].map((failure) => Object.freeze({ row_number: Number(failure.row_number), reason_code: failure.reason_code,
      field: failure.field, detail: failure.detail }))) });
}

export function runtimeImportCreated(row: RuntimeImportRow): RuntimeImportCreated {
  return Object.freeze({ id: row.id, state: runtimeImportState(row.state), total_count: Number(row.total_count ?? 0), cursor_value: Number(row.cursor_value ?? 0),
    success_count: Number(row.success_count ?? 0), failure_count: Number(row.failure_count ?? 0), created_at: String(row.created_at), updated_at: String(row.updated_at) });
}
...
function runtimeImportReport(row: RuntimeImportRow): RuntimeImportRecord['report'] {
  ...
  if (typeof row.report_object_ref !== 'string' || typeof row.report_sha256 !== 'string' || !/^[0-9a-f]{64}$/.test(row.report_sha256) ||
    !Number.isSafeInteger(size) || size < 0) throw new Error('RUNTIME_IMPORT_REPORT_INVALID');
