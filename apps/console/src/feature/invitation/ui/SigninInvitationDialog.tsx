...
import type { InvitationMembershipPage } from '../infrastructure/InvitationGateway';
import type { InvitationDraft } from '../model/InvitationDraft';

export function SigninInvitationDialog({ open, memberships, busy, error, onClose, onSubmit }: Readonly<{ open: boolean; memberships: InvitationMembershipPage['items']; busy: boolean; error?: string; onClose: () => void; onSubmit: (draft: InvitationDraft) => Promise<void> }>) {
  const [target, setTarget] = useState<'console' | 'storefront'>('console');
  ...
  return (
    <Dialog open={open} title="创建登录邀请" eyebrow="高风险操作 · 指定成员" onClose={onClose} dismissable={!busy}>
      <Form label="登录邀请" className="invitationform" onSubmit={(event) => void submit(event)}>
        ...
        <select id="signinTarget" value={target} onChange={(event) => setTarget(event.target.value as typeof target)} disabled={busy}><option value="console">管理控制台</option><option value="storefront">员工商城</option></select>
        <label htmlFor="signinMembership">指定成员</label>
        <select id="signinMembership" value={membership} onChange={(event) => setMembership(event.target.value)} disabled={busy} required>{eligible.map((item) => <option key={item.id} value={item.id}>{item.id}</option>)}</select>
        ...
        <footer className="invitationactions"><Button onPress={onClose} isDisabled={busy}>取消</Button><Button type="submit" tone="primary" isDisabled={busy || membership === '' || reason.trim().length < 4}>{busy ? '正在创建…' : '确认创建'}</Button></footer>
      </Form>
    </Dialog>
  );
}
