...
On a cache miss, the shared core installs its minimal isolated control dependencies and only the selected source workspaces concurrently; a target with no declared workspace falls back to the full source install. Cache absence or damage triggers a normal rebuild, not an approval or repair step. Tests and typechecks run concurrently before building. `DELIVERY_END_TO_END_MS` measures from the team command to workflow completion; only a real nonempty cold release's total can prove the 60-second goal. `DELIVERY_PRE_CORE_TIMINGS` reports final-run queue, routing, and checkout time; the release receipt reports dependencies, tests, typecheck, build, package, upload, and target cutover. These intervals may overlap and should not be summed. A Hosted fallback's cancelled first run is included in the total but not final-run stage breakdown. The release result already contains the target node's `current`, `previous`, and health returned after deployment. For an immediate completion answer, use that target result; a later question about current production state uses a fresh read-only `status`.
