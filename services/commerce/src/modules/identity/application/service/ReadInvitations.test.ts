...
describe('ReadInvitations', () => {
  it('composes invitation accounts through bounded public ports without N+1 reads', async () => {
    ...
    expect((response.body as Readonly<{ items: readonly Readonly<Record<string, unknown>>[] }>).items[0]).toMatchObject({
      recipient_display_name: '李小明', recipient_employee_no: 'E1002', recipient_mobile_masked: '139****0002',
      issuer_display_name: '王主管', issuer_employee_no: 'E1001', issuer_mobile_masked: '138****0001',
    });
...
function record() {
  return {
    id: 'invitation:one', kind: 'enrollment', target: 'storefront', organization_id: 'mall:one',
    membership_id: 'membership:employee', issuer_membership_id: 'membership:owner', issuer_access_version: 3,
    minimum_assurance: 2, max_uses: 1, use_count: 0, not_before: '2026-09-03T00:00:00.000Z',
    expires_at: '2026-09-06T00:00:00.000Z', status: 'active', reason: '新员工入职',
    created_at: '2026-09-03T00:00:00.000Z', revoked_at: null, revoked_by: null, revoke_reason: null, version: 1,
  } as const;
}
