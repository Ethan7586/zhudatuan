...
beforeEach(() => { state.activate.mockReset(); state.toast.mockReset(); state.session = { membership: 'member:one', scope: { kind: 'mall', id: 'mall:one' }, accessVersion: 1, csrfToken: 'csrf:one' }; });
afterEach(cleanup);
...
describe('activation command state', () => {
  it('normalizes the card number, keeps secret case, clears the form and returns the real voucher', async () => {
    ...
    act(() => { result.current.actions.open(); result.current.actions.number('vc001234'); result.current.actions.secret('Secret123'); });
  ...
  it('blocks duplicate submits and reuses the idempotency key for the unchanged failed command', async () => {
    ...
    state.activate.mockImplementationOnce(() => new Promise((_resolve, fail) => { reject = fail; }));
    const { result } = renderHook(() => useActivationViewModel(vi.fn(), vi.fn()));
    act(() => { result.current.actions.open(); result.current.actions.mode('secret'); result.current.actions.secret('Secret123'); });
    const key = result.current.draft!.key;
    let first!: Promise<void>;
    act(() => { first = result.current.actions.submit(); void result.current.actions.submit(); });
    expect(state.activate).toHaveBeenCalledTimes(1);
    act(() => result.current.actions.close());
    expect(result.current.draft).not.toBeNull();
    await act(async () => { reject(new Error('CONNECTION_FAILED')); await first; });
  ...
  it('aborts on scope change and ignores a late successful response from the old account', async () => {
    ...
    state.activate.mockImplementationOnce(() => new Promise(done => { resolve = done; }));
    const done = vi.fn();
    const { result, rerender } = renderHook(() => useActivationViewModel(done, vi.fn()));
    act(() => { result.current.actions.open(); result.current.actions.mode('secret'); result.current.actions.secret('Secret123'); });
    let pending!: Promise<void>;
    act(() => { pending = result.current.actions.submit(); });
    ...
    expect(result.current.draft).toBeNull();
    await act(async () => { resolve(voucher); await pending; });
