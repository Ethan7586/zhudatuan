    ...
    const reader = new QuoteReader(
      {
        ...
        catalog: {
          items: () => {
            starts.push('catalog');
            return catalog.promise as never;
          },
        },
        ...
        pricing: {
          offers: () => {
            starts.push('pricing');
            return pricing.promise as never;
          },
        } as never,
...
function selection() {
  return {
    cartVersion: 2,
    lines: [{ listingId: 'listing:one', quantity: 2, lineVersion: 1 }],
    addressId: 'address:one',
    invoiceId: null,
    delivery: { method: 'standard' as const, note: null, scheduledAt: null },
    voucherIds: [],
    benefits: [],
