...
export function CampaignInvitationDialog({ open, targets, busy, error, onClose, onSubmit }: Readonly<{ open: boolean; targets: readonly InvitationTarget[]; busy: boolean; error?: string; onClose: () => void; onSubmit: (draft: InvitationDraft) => Promise<void> }>) {
  ...
  return (
    <Dialog open={open} title="创建共享注册邀请" eyebrow="高风险操作 · 仅员工商城" onClose={onClose} dismissable={!busy}>
      <Form label="共享注册邀请" className="invitationform" onSubmit={(event) => void submit(event)}>
        ...
        <footer className="invitationactions"><Button onPress={onClose} isDisabled={busy}>取消</Button><Button type="submit" tone="primary" isDisabled={busy || organizationId === '' || reason.trim().length < 4}>{busy ? '正在创建…' : '确认创建'}</Button></footer>
      </Form>
    </Dialog>
  );
}
