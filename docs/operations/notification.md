...
- Diagnosis: inspect dispatch, template version, endpoint state, attempts, provider response and correlation ID. Aliyun `SendSms=OK` is provider acceptance, not a carrier delivery receipt; use the BizId with `QuerySendDetails` or the configured SMS report channel when final-delivery evidence is required.
