...
const riskSignal = (value: number, sensitivity: 'public' | 'personal' | 'sensitive' = 'personal') =>
  signal({ type: 'trust.score', version: 2, value, source: 'checkout', sensitivity, observedAt: '2026-08-21T00:00:00Z' });

describe('RiskEngine', () => {
  it('applies hard deterministic limits before scoring and never lowers their outcome', () => {
    expect(
      evaluate({ maximumAmountMinor: 100, scores: [{ signal: 'trust.score', minimum: 1, points: 1 }], thresholds: { challenge: 1, review: 2, deny: 3 } }, { amountMinor: 101, signals: [riskSignal(0)] })
    ).toMatchObject({ outcome: 'deny', safeReason: 'amount', policy: { id: 'policy', version: 7 } });
  ...
  it('returns explainable evidence without persisting sensitive values', () => {
    const decision = evaluate(
      { scores: [{ signal: 'trust.score', minimum: 1, points: 9 }], thresholds: { challenge: 5, review: 8, deny: 10 } },
      { signals: [riskSignal(3, 'sensitive')] }
    );
