...
export class PgQualificationRepository implements QualificationRepository {
  ...
  async previewDecision(context: WriteTransactionContext, scope: string, member: string, resource: string): Promise<readonly QualificationDecisionRecord[]> {
    const database = this.transactions.database(context);
  ...
  async previewPolicy(context: WriteTransactionContext, input: Readonly<{ scope: string; id: string; sourceVersion: number | null }>): Promise<QualificationPolicyImpactBasis | null> {
    const database = this.transactions.database(context);
