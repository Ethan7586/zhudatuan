...
| API             | `services/commerce/src/entry/ApiMain.ts`       | 保留后移动   | 目标 `entry/ApiMain.ts`                     |
| Jobs            | `services/commerce/src/entry/JobsMain.ts`      | 保留后移动   | 目标 `entry/JobsMain.ts`                    |
| Provider        | `services/commerce/src/entry/ProviderMain.ts`  | 保留后移动   | 目标 `entry/ProviderMain.ts`                |
| Migration       | `services/commerce/src/entry/MigrationMain.ts` | 保留后移动   | 目标 `entry/MigrationMain.ts`               |
| Smoke           | `services/commerce/src/entry/SmokeMain.ts`     | 发布验证专用 | 目标 `entry/SmokeMain.ts`，禁止造业务数据   |
...
| `apps/console/src/shell.css`              | `bca1a95ccc1db466e4100cd8570bd11cf12d998a964c46248d46ab70544c7d7d` | `apps/console/src/shell/Header.css`、`Navigation.css`、`style/Layout.css`、`Profile.css`、`Scope.css`、`packages/design/src/template/WorkspaceShell.css` | 侧栏/顶栏比例、工作台留白、遮罩、折叠与移动端布局            | 按组合职责拆分；未整文件复制，不形成第二套 Shell                       |
