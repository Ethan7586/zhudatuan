...
describe('ServiceLevel', () => {
  it('evaluates a ratio without rounding across an SLO boundary', () => {
    ...
    expect(
      level.evaluate([
        { name: 'commerce.operation.count', value: 999, result: 'success' },
        { name: 'commerce.operation.count', value: 1, result: 'failure' },
      ])
    ).toMatchObject({ current: 99.9, burnRate: 2, status: 'atrisk', total: 1000 });
  });
  ...
  it('evaluates a bounded percentile and exposes actionable ownership', () => {
    const level = new ServiceLevel({
      id: 'latency',
      title: '延迟',
      indicator: { metric: 'commerce.operation.duration', type: 'percentile', percentile: 95 },
      target: 100,
      unit: 'milliseconds',
      direction: 'maximum',
      windowSeconds: 300,
