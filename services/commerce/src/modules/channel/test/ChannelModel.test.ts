...
describe('channel domain model', () => {
  it('freezes the connection contract, capability snapshot and opaque Secret reference', () => {
    ...
    const connection = new Connection({
      ...
      capabilities,
      secretRef: 'vault/channel/one',
      region: 'CN',
      version: 3,
      limits: { connectionTimeoutMs: 100, responseTimeoutMs: 200, totalDeadlineMs: 500, maxConcurrency: 4, requestsPerSecond: 10, maxAttempts: 3, failureThreshold: 5, recoveryMs: 1_000 },
    });
  ...
  it('binds a committed sync cursor and watermark to a classified, versioned run', () => {
    const run = new SyncRun({
      ...
      failure: { classification: 'timeout', code: 'PROVIDER_TIMEOUT', retryable: true },
      version: 2,
    });
  ...
  it('makes external identity scope-safe and versioned', () => {
    const mapping = new ExternalMapping({ provider: 'supplier', scope: 'mall:one', objectType: 'product', externalId: 'A', internalType: 'sku', internalId: 'sku:one', sourceVersion: '7', watermark, version: 1 });
  ...
  it('retains only webhook evidence hashes and classified failure metadata', () => {
    const webhook = new WebhookInbox({
      id: 'webhook:one',
      receipt: 'webhookreceipt:one',
      connection: 'connection:one',
      externalId: 'event:one',
      eventType: 'refund',
      state: 'failed',
      attempts: 2,
      rawHash: hash,
      signatureHash: hash,
      watermark,
      failure: { classification: 'authentication', code: 'PROVIDER_SIGNATURE_INVALID', retryable: false },
      version: 2,
    });
    expect(webhook).not.toHaveProperty('raw');
    expect(webhook.failure?.retryable).toBe(false);
    const receipt = new WebhookReceipt({
  ...
  it('protects distributor termination, binding periods and quota boundaries', () => {
    const distributor = new Distributor({ id: 'distributor:one', organization: 'distributor:one', code: 'DIST.1', name: '渠道一', settlementMode: 'monthly', metadata: {}, state: 'active', activeBindings: 1, hasContact: true, version: 1 });
    expect(() => distributor.requireTermination()).toThrow('CHANNEL_DISTRIBUTOR_BINDING_ACTIVE');
    expect(() => new Binding({ id: 'binding:one', distributor: 'distributor:one', tenant: 'tenant:one', state: 'active', effectiveAt: watermark, expiresAt: '2026-09-05T00:00:00.000Z', version: 0 })).toThrow('CHANNEL_BINDING_INVALID');
    const quota = new Quota({ id: 'quota:one', scope: 'mall:one', capability: 'channel.orders.submit', state: 'enabled', limit: 2, effectiveAt: watermark, expiresAt: null, version: 0 });
  ...
  it('stores only a whitelisted provider response summary', () => {
    const response = providerResponse({ state: 'accepted', externalReference: 'external:one', secret: 'do-not-store', mobile: '13800000000', address: { line: 'do-not-store' }, instruction: 'do-not-store' });
    const operation = new ProviderOperation({
      id: 'operation:one',
      provider: 'supplier',
      scope: 'mall:one',
      kind: 'order',
      idempotency: 'fulfillment:one',
      internalReference: 'fulfillment:one',
  ...
  it('centralizes rate admission, cursor commit and retry classification', () => {
    const connection = new Connection({
      ...
      region: 'CN',
      version: 1,
      limits: { connectionTimeoutMs: 100, responseTimeoutMs: 200, totalDeadlineMs: 500, maxConcurrency: 2, requestsPerSecond: 2, maxAttempts: 3, failureThreshold: 5, recoveryMs: 1_000 },
    });
    ...
    expect(() =>
      sync.requireCheckpoint({ pulled: 1, accepted: 1, rejected: 0, phase: 'apply', cursor: 'a', watermark }, { phase: 'apply', progress: { pulled: 2, accepted: 2, rejected: 0, phase: 'apply', cursor: 'b', watermark } })
    ).toThrow('CHANNEL_SYNC_CURSOR_UNCOMMITTED');
    expect(sync.retry(new Error('PROVIDER_RESPONSE_TIMEOUT'), 1, 3)).toMatchObject({ retry: true, delayMs: 250, failure: { classification: 'timeout', retryable: true } });
  });
});
