...
import { benefitQuery } from './BenefitQueryKey';
...
export function useBenefitViewModel() {
  ...
  const query = useQuery({
    queryKey: benefitQuery(session.query.scoped),
  ...
  return Object.freeze({
    ...
    accounts: query.data?.accounts ?? Object.freeze([]),
    ledger: query.data?.ledger ?? Object.freeze([]),
