...
export class VoucherTenderWriter {
  ...
  async reserve(command: ReserveCommand): Promise<Readonly<{ created: boolean; hold: OperationOutputFor<'voucher.tenderholds.create'> }>> {
    if (!command.voucher || !command.owner || !command.scope || !command.idempotency || !Number.isSafeInteger(command.amountMinor) || command.amountMinor <= 0 || !Number.isSafeInteger(command.ttlSeconds) || command.ttlSeconds <= 0) {
      throw new DomainError('VALIDATION_FAILED');
    }
    const database = this.transactions.database(command.context);
    const selected = await database.query<VoucherRow>(
      `select ${VOUCHER_FIELDS},holder.member_id from voucher.voucher voucher
      ...
      where voucher.id=$1 and voucher.scope_id=$2 for update of voucher`,
      [command.voucher, command.scope]
    );
    const row = selected.rows[0];
    if (!row || (command.member !== null && row.member_id !== command.member)) throw new DomainError('VOUCHER_NOT_USABLE');
    const prior = await database.query<HoldRow>(`select ${HOLD_FIELDS} from voucher.tenderhold hold where hold.scope_id=$1 and hold.idempotency_key=$2 for update`, [command.scope, command.idempotency]);
    if (prior.rows[0]) {
      const hold = prior.rows[0];
      if (hold.voucher_id !== command.voucher || hold.owner_id !== command.owner || Number(hold.amount_minor) !== command.amountMinor || !['active', 'consumed'].includes(hold.state)) throw new DomainError('VOUCHER_HOLD_CONFLICT');
    ...
    const hold = new TenderHold({ id, voucher: row.id, owner: command.owner, amountMinor: command.amountMinor, state: 'active', expiresAt, idempotency: command.idempotency, version: 1 });
    ...
    await database.query(`update voucher.voucher set state=$3,version=$4 where id=$1 and scope_id=$2`, [row.id, command.scope, held.value.state, held.value.version]);
  ...
  async release(command: ReleaseCommand): Promise<OperationOutputFor<'voucher.tenderholds.release'>> {
    ...
    const reference = (await database.query<{ voucher_id: string }>(`select voucher_id from voucher.tenderhold where id=$1 and scope_id=$2`, [command.hold, command.scope])).rows[0];
    if (!reference) throw new DomainError('RESOURCE_NOT_FOUND');
    const row = (await database.query<VoucherRow>(`select ${VOUCHER_FIELDS} from voucher.voucher voucher where voucher.id=$1 and voucher.scope_id=$2 for update`, [reference.voucher_id, command.scope])).rows[0];
    const source = (await database.query<HoldRow>(`select ${HOLD_FIELDS} from voucher.tenderhold hold where hold.id=$1 and hold.voucher_id=$2 and hold.scope_id=$3 for update`, [command.hold, reference.voucher_id, command.scope])).rows[0];
    if (!row || !source) throw new DomainError('RESOURCE_NOT_FOUND');
    if (source.state === 'released' || source.state === 'expired' || (command.ifActive && source.state !== 'active') || (command.expiredOnly && new Date(source.expires_at) > command.now)) return this.read(command, source.id);
    ...
    const released = new TenderHold({
      ...
      idempotency: source.idempotency_key,
      version: Number(source.version),
    }).release(command.now);
    const restored = aggregate(row).release(command.now);
    await database.query(`update voucher.tenderhold set state=$3,version=$4 where id=$1 and scope_id=$2`, [source.id, command.scope, released.value.state, released.value.version]);
    await database.query(`update voucher.voucher set state=$3,version=$4 where id=$1 and scope_id=$2`, [row.id, command.scope, restored.value.state, restored.value.version]);
