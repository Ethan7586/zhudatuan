...
import { StorefrontStepup } from '../public';
        ...
        {state === 'loading' ? (
          ...
          <>
            <div className="grid gap-4 md:grid-cols-2">
              <form onSubmit={(event) => void actions.changePassword(event)} className="rounded-xl border bg-white p-4 shadow-sm">
                <h2 className="flex items-center gap-2 text-base font-black">
                  <KeyRound size={18} />
                  修改密码
                </h2>
                <p className="mt-1 text-gray-500">最近修改：{security?.passwordChangedAt ? format(security.passwordChangedAt) : '尚未设置本地密码'}</p>
              ...
              <form onSubmit={(event) => void actions.changeMobile(event)} className="rounded-xl border bg-white p-4 shadow-sm">
                <h2 className="flex items-center gap-2 text-base font-black">
                  <Smartphone size={18} />
                  更换手机号
                </h2>
                <p className="mt-1 text-gray-500">当前手机号：{security?.phoneMasked ?? '未绑定'}</p>
                <input
                  value={mobileValue}
            ...
            <section className="mt-4 rounded-xl border bg-white p-4 shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-gray-500">当前验证等级：L{security?.assurance ?? 0}</p>
                </div>
                <button type="button" disabled={busy !== null} onClick={() => void actions.revokeSession('others')} className="rounded-lg border px-3 py-2 font-bold text-red-600 disabled:opacity-50">
                  撤销其他设备
                </button>
              </div>
              <div className="mt-4 divide-y">
                {security?.sessions.map((item) => (
                  <article key={item.id} className="flex flex-wrap items-center justify-between gap-3 py-3">
                    ...
                    {!item.current ? (
                      <button type="button" disabled={busy !== null} onClick={() => void actions.revokeSession(item.id)} className="inline-flex items-center gap-1 rounded-lg bg-red-50 px-3 py-2 font-bold text-red-600 disabled:opacity-50">
                        <LogOut size={14} />
                        撤销
                      </button>
                    ) : null}
