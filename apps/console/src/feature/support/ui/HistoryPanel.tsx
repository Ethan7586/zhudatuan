...
export function HistoryPanel({
  ...
  onNext,
  onRetry,
}: Readonly<{ open: boolean; items: readonly History[]; condition: ResourceCondition; error?: string | undefined; nextCursor?: string | undefined; onClose: () => void; onNext: () => void; onRetry: () => void }>) {
  if (!open) return null;
  return (
    <aside className="supporthistory" aria-label="工单历史">
      ...
      <ResourceState condition={condition} {...(error === undefined ? {} : { error })} retry={onRetry}>
        <ol>
          {items.map((item) => (
            <li key={item.cursor_id}>
              <span>{item.sequence}</span>
              <div>
                <strong>{historyLabel(item.kind)}</strong>
                <p>{chineseReference('操作人', item.actor_id)}</p>
