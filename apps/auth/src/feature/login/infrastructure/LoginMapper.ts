...
export function mapLogin(value: Exclude<SessionOutput, { kind: 'session' }>): LoginOutcome {
  switch (value.kind) {
    case 'selection':
      return Object.freeze({ kind: 'membership', transaction: value.transaction, memberships: Object.freeze(value.memberships.map((membership) => Object.freeze(membership))) });
    case 'proofRequired': {
    ...
    case 'enrollment':
      return Object.freeze({ kind: 'enrollment', id: value.enrollment.id, expiresAt: value.enrollment.expiresAt, target: value.enrollment.target });
  }
}
