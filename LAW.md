...
4. 在 Ethan 明确宣布项目开发结束之前，任何安全锁不得向代码、配置、工作流、脚本、测试或文档新增一行、一个字符；不得以修复、重构、顺手处理、拆批或混入其他改动的方式实施。项目结束后也不会自动解除禁令，只有 Ethan 再次明确修改 LAW 本节，写明允许增加的具体安全锁、适用范围和有效期限，相关改动才有资格进入项目。
