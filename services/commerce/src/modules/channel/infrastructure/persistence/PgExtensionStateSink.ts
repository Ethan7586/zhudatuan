...
export class PgExtensionStateSink implements ExtensionStateSink {
  ...
  async recover(context: ReadTransactionContext, id: string, scope: string): Promise<void> {
    const database = this.transactions.database(context);
    await database.query(
      `update channel.connection set status='enabled',version=version+1,updated_at=clock_timestamp()
      where id=$1 and scope_id=$2 and status='degraded'`,
      [id, scope]
    );
  }
}
