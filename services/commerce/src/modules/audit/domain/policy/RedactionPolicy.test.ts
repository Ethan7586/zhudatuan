...
describe('RedactionPolicy', () => {
  ...
  it('bounds depth, collection size and text size', () => {
    const result = new RedactionPolicy().redact({ values: Array.from({ length: 120 }, (_, index) => index), text: 'x'.repeat(700), deep: { one: { two: { three: { four: { five: { six: 'hidden' } } } } } } }) as Record<string, unknown>;
