...
export interface SettlementFulfillment {
  create(database: OperationDatabase, input: Readonly<{ order: string; payment: string; plans: readonly OrderFulfillmentPlan[] }>): Promise<readonly string[]>;
}
...
export class PaymentSettlementCore {
  ...
  async capture(database: OperationDatabase, target: SettlementTarget, source: 'wechat' | 'internal' | 'mixed'): Promise<string> {
    ...
    const fulfillmentPlans = await this.orders.fulfillment(database, target.order);
    const fulfillments = await this.fulfillment.create(database, { order: target.order, payment, plans: fulfillmentPlans });
  ...
  const snapshot =
    ...
    (
      await database.query<{ payload: unknown }>(
        `select payload from runtime.outbox where event_type='order.placed'
