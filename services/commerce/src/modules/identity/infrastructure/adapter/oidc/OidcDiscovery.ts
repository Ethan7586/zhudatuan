...
import { OIDC_PROVIDER_CONFIGURATION, oidcIssuer } from '@shop/config/server';
  ...
  async read(issuerValue: string, signal?: AbortSignal, deadline?: number): Promise<OidcMetadata> {
    ...
    return this.flights.run(
      ...
      async () => {
        ...
        url.pathname = `${url.pathname.replace(/\/$/, '')}${OIDC_PROVIDER_CONFIGURATION.discoveryPath}`;
        ...
        this.cache.set(issuer, Object.freeze({ value, expires: Date.now() + OIDC_PROVIDER_CONFIGURATION.discoveryTtlSeconds * 1_000 }));
        return value;
      },
      { signal, deadline }
    );
