...
当前进度：CP-00、CP-00A、AU-001/CP-01 至 AU-252 已完成。AU-252 完成 Commerce console support runtime contract independent review。覆盖总账不变：深入审阅1,748文件/124,103行、结构性审阅799文件/117,353行、自动生成70文件/172,651行、暂未审阅1,110文件。F-0158/P1、F-0159/P1、F-0173/P1、F-0186/P1、F-0227/P1、F-0229/P1均已双轮确认；按Ethan最新指令仅确认P0时中断，否则连续进入下一审计单元。
...
审阅 Provisioning public ports、domain purchase policy/lifecycle、mall create/template clone application、full/selected module、manifest 和现有测试，并反查独立 Mall Provisioning API runtime/route。

执行结果：CreateMall 先 plan/preflight 再按 organization→catalog→experience→owner 顺序创建，稳定 ID 和冲突在写入前确定；Hosted-node/template clone 交由 canonical database function；domain policy 验证购买域名、报价、资金 hold、approval/assurance 并在 registrar registration 收口。新增 F-0178/P2：没有 HTTP action fixture 直接调用 provisioningOperations create/read，route test 只验证注册；未发现 P0/P1，Vitest 未运行。
...
审阅 MemberPort 的邀请/注册/Hosted 开通/主权升级端口、对应 HTTP actions、导入 command/worker persistence/processor、public surface 与 port tests，并从 jobs catalog、Web Business API 和 Identity Registration API 反查运行入口。

执行结果：MemberPort 将 Hosted/sovereign 事务限制为服务端 access node authority 并回检 principal/membership/realm/node；主 jobs catalog 实际注册 memberimport（import queue、4 并发、120 秒 timeout、lease/retry/deadletter）。导入以 staged rows、每行 savepoint、500 行续作和 report 完成收口。新增 F-0179/P2：完整及 identity-selected Member module 的 spread 顺序均以 `MemberReadOperations` 覆盖 `MemberImportOperations` 的同名 `member.imports.read`，导致已完成 report 不会通过 `projectImport` 获得授权下载投影，并返回内部 object reference 字段。Member 19/19 基线文件现已取得文件级审阅状态；无 P0。
...
审阅 MallContext resolver、public index/manifest 与两项 direct test。

执行结果：AccessPipeline 在 capability/permission/scope 判定后才投影 mall context；resolver 优先真实 mall scope，否则仅接受唯一 direct mall grant，拒绝从 Scope.path 推断或多 mall 模糊选择。tests 覆盖 immutable DTO、请求 preferred mall 不覆盖现有 mall scope、ambiguity rejection 与 job/event restore。无 P0–P3 新问题。
...
审阅 Commerce API signed session、miniapp token、membership runtime、authorization wrapper、credential 与 phone assurance。

执行结果：host-only admin/storefront cookie 分隔签名密钥与 target；miniapp bearer 强制 storefront channel/key。受保护路径不信任客户端 permission/scope，每次用 session id回查 RPC membership context，并核对 member/membership/target/authzVersion。phone assurance 缺失或未验证均拒绝 payment-eligible行为。session/assurance tests覆盖 host separation、tamper、legacy payload、miniapp target、scope row 与 fail-closed。无 P0–P3 新问题。
...
审阅 Commerce API HTTP response/error mapping/body/scope transport helpers。

执行结果：JSON响应统一no-store、request-id和browser safety headers；全路由顶层通过固定错误代码表归一化业务错误，未知错误仅返回通用500。所有兼容写操作共用32KB声明/实际字节双检 JSON parser；资源scope只可由security-definer RPC读取。routerSupport/errorResponse没有direct tests，形成F-0188/P3；无P0–P2新问题。
...
审阅 Commerce API WeChat prepay、payment status 与 order-by-number compatibility route。

执行结果：prepay在phone assurance、idempotency、config和服务端order scope后创建attempt，provider失败会best-effort标记失败；可复用未过期prepay。status以数据库为权威、provider query失败标记deferred。现有test只覆盖状态映射与read route，未直接覆盖prepay成功/reuse/provider失败，形成F-0190/P2；无P0/P1新问题。

## 179. AU-177 连续审计点

审阅 Commerce API WeChat Pay Provider client、configuration、crypto和transaction models。

执行结果：固定WeChat API origin，out-trade-no/description/amount/openid均先校验；64KB有界UTF-8 response在JSON解析前必须通过平台签名验证。配置要求approved HTTPS notify URL、PKCS8/SPKI、32-byte APIv3 key；client tests以临时RSA key验证request signing、provider response signature、transaction parse和retryable error。无P0–P3新问题。

## 180. AU-178 连续审计点

审阅 Commerce API WeChat response/notification signature verifier 与测试密钥支撑。

执行结果：verifier要求四个header、平台key id一致、10位timestamp在至多3600秒且默认300秒窗口、可打印bounded nonce和RSA验签；不满足即protocol error。test support只临时生成RSA key和AES-GCM notification resource，不包含生产凭据。无P0–P3新问题。

## 181. AU-179 连续审计点

审阅 Commerce API public WeChat payment callback、notification decrypt/validation和apply RPC。

执行结果：公开回调限制POST/64KB；验签后才decrypt，严格只接收`TRANSACTION.SUCCESS`/transaction/AEAD_AES_256_GCM，并最小化保存payer hash/summary。notification parser tests覆盖authentic/tamper/stale/attempt mismatch；实际public route没有direct RPC/response fixture，形成F-0191/P2；无P0/P1新问题。

## 182. AU-180 连续审计点

审阅 Commerce API address book 的 PII read/write/delete compatibility route。

执行结果：地址簿要求order.create及PII key；读取仅用server user scope读取并decrypt，写入先校验字段再encrypt整段recipient后RPC upsert，删除同样携带server scope。handler没有direct test，形成F-0192/P2；无P0/P1新问题。

## 183. AU-181 连续审计点

审阅 Commerce API cart snapshot/upsert/delete compatibility route。

执行结果：所有cart动作要求order.create，snapshot/写入/删除均传server membership/user scope；quantity强制1–99，public cover URL从server product values再经media policy重写。现有test只覆盖GET qualified snapshot，没有PUT/delete/permission/invalid-input fixture，形成F-0193/P2；无P0/P1新问题。

## 184. AU-182 连续审计点

审阅 Commerce API dynamic member-code issue/revoke/verify compatibility route。

执行结果：签发只向客户端返回随机64-hex credential/QR payload，RPC仅得到hash及server scope/authz version；撤销绑定member/user；核验额外需要memberCodeVerify并只传hash。direct tests覆盖credential不写库、hash verify、permission和invalid revoke。无P0–P3新问题。

## 185. AU-183 连续审计点

审阅 Commerce API qualification admin center/config compatibility route。

执行结果：center在admin target、权限和mall scope后按capability服务端裁剪selector/资源数据；config按kind选择permission，draft可直接保存，发布/停用需15分钟内step-up并先preview，critical影响改为approval request。direct tests覆盖target/permission/scope/redaction/idempotency/draft/publish/approval。无P0–P3新问题。

## 186. AU-184 连续审计点

审阅 Commerce API qualification governance、preview/review/history/rollback/simulation/employee事实变更 routes。

执行结果：governance按caller capability裁剪requests/employee facts；review和employee write绑定current actor/evidence，rollback额外要求fresh step-up/idempotency且复用config persistence；simulation只以current mall scope读取；tags时间窗和数量均有输入约束。direct tests覆盖redaction、approval step-up/actor、employee tags/step-up和simulation scope。无P0–P3新问题。
...
审阅 Commerce API 后台 member operations、invite/create/import/profile-update routes 及其direct tests。

执行结果：后台router注册全链；列表把PII、历史、导入错误分别按权限请求；所有写入都要求相应capability、fresh step-up及server scope/actor evidence。建档使用PBKDF2 hash且导入错误回执剔除明文密码。tests覆盖读权限裁剪、邀请码step-up、建档密码脱敏、导入错误脱敏，但没有邀请成功/停用、资料更新、有效导入和导入边界fixture，形成F-0196/P2；无P0/P1新问题。

## 190. AU-188 连续审计点

审阅 Commerce API 后台 permission command center、membership access/status mutation routes 及direct tests。

执行结果：命令中心必须同时具备member.read与role.read，PII字段独立裁剪；access变更要求role+scope grant、禁止自改、输入去重并把grant ceiling留给database RPC；status变更禁止自改且offboard需要独立permission。tests覆盖核心read、self、step-up、access body/hierarchy及offboard拒绝，但没有active/suspended成功RPC fixture，形成F-0197/P2；无P0/P1新问题。

## 191. AU-189 连续审计点

审阅 Commerce API 后台 custom role center/create/update/status routes 及direct tests。

执行结果：role center以role.read读取；create/update/status分别绑定role.create/role.update/role.delete、fresh step-up、输入去重、server scope与actor evidence。clone source与手选permissions同时出现被拒绝。tests覆盖read/step-up/create payload、clone冲突、enable/disable权限分流和update拒绝；更新成功RPC及禁用成功的body未完整assert，但现有关键授权和创建/启用成功路径已直接验证，记录为测试缺口说明而不单列新问题；无P0–P3新问题。

## 192. AU-190 连续审计点

审阅 Commerce API admin catalog、overview和product status routes 及direct tests。

执行结果：catalog需要catalog.read；overview拒绝非admin target且同时要求catalog/order read，self-only membership被限制为当前user订单scope；商品状态变更先作coarse permission再加载server resource scope，要求idempotency/request hash和actor evidence。tests覆盖read/target/coarse拒绝与overview profile；未验证status变更的resource denial、输入/idempotency与成功RPC，形成F-0198/P2；无P0/P1新问题。

## 193. AU-191 连续审计点

审阅 Commerce API Storefront home composition 单元。

执行结果：`/api/v1/home`只允许GET，复用bootstrap/accounts/orders/ledgers四个已深审handler，以同一authorization并行请求，任一失败直接透传；正式Storefront router已注册。没有独立test，缺口已纳入F-0195/P2；无P0/P1新问题。

## 194. AU-192 连续审计点

审阅 Commerce API security center/password-reset/phone-change/session compatibility handlers 及direct tests。

执行结果：handler实现session、current password、OTP、phone PII encrypt及session revoke边界；现有tests覆盖password change、production OTP闭合、phone-change anonymous拒绝和revoke others。真实`routeApi`对全部`/api/v1/auth/*`固定404，仓内未找到这些handler生产import；旧Auth LoginPage、文档、测试和public exports仍是兼容契约，扩展DC-0048/G1而非删除。security center、reset/change phone、single-session revoke等尚无direct fixture，形成F-0199/P2；无P0/P1新问题。

## 195. AU-193 连续审计点

审阅 Commerce API OTP delivery adapter 及direct tests。

执行结果：registration/security OTP都通过同一adapter；成功与失败都会将delivery状态写入challenge RPC，失败记录本身失败时不覆盖原provider error。实际provider availability由smsProvider决定。现有test只断言resend constant，未验证send、delivery record或failure mapping，形成F-0200/P2；无P0/P1新问题。

## 196. AU-194 连续审计点

审阅 Commerce API SMS provider adapter 及direct tests。

执行结果：debug code仅在development/test；生产只接受完整Aliyun配置，使用单次发送、3秒timeout并把provider code净化为稳定错误码。tests覆盖debug隔离、Aliyun request/timeout/no retry及provider rejection净化；未覆盖缺失sign/template与client transport exception映射，形成F-0201/P3；无P0–P2新问题。

## 197. AU-195 连续审计点

审阅 Commerce API shared order/payment simulation validation helpers 及direct tests。

执行结果：order、after-sale、internal payment/refund和simulation输入统一限制类型、数量、金额、渠道、voucher和手机格式；order拒绝重复SKU，金额均使用safe integer。direct tests覆盖每个export的正常与关键无效输入，且调用方仅为已审订单/未注册simulation route；无P0–P3新问题。

## 198. AU-196 连续审计点

审阅 Commerce API payment simulation wallet/recharge/benefit/mixed-payment routes。

执行结果：simulation router只在development/test进入，handler还要求matching AUTH_MODE，生产返回404；wallet/recharge仅storefront，benefit仅admin，mixed payment加载server order scope；所有写入要求capability、idempotency/hash与actor evidence。未找到payment simulation direct route fixture，形成F-0202/P2；无P0/P1新问题。

## 199. AU-197 连续审计点

审阅 Commerce API router target-routing 与business target-guard tests。

执行结果：top-level router test验证storefront/admin不会跨dispatch，`/auth/*`在resolve session之前保持固定404；business router tests验证admin/storefront的业务及simulation路径在错误target返回null。测试以mocked routers核对选择顺序，真实handler路径已在AU-168及各子模块审阅；无P0–P3新问题。

## 200. AU-198 连续审计点

审阅 Commerce API WeChat routing-boundary test。

执行结果：test以真实`routeApi`确认退休WeChat auth paths保持404、payment notification在public router可达且method受限、prepay在anonymous请求时先被认证边界拒绝。该test不替代notification/支付handler内部RPC验证（已分别记录F-0190/F-0191）；无P0–P3新问题。

## 201. AU-199 连续审计点

审阅 Commerce API WorkerEnv、AuthorizationContext和RequestContext types。

执行结果：WorkerEnv集中声明数据库、cache、SMS、identity/PII、session、miniapp和WeChat Pay运行变量；AuthorizationContext明确为单一resolved membership投影，不从其他membership拼接权限。types被API router/handlers/tests广泛直接消费，未发现独立运行或数据边界问题；无P0–P3新问题。
...
审阅 Commerce bootstrap manifest SignatureVerifier。

执行结果：CommerceRuntime用配置public key构造verifier，扩展安装和Channel route通过其token消费；实现对canonical manifest payload作Node crypto验签。仓内未找到有效/篡改/错误key的direct verifier fixture，形成F-0204/P2；无P0/P1新问题。

## 205. AU-203 连续审计点

审阅 Commerce ExtensionRegistry canary/activation test。

执行结果：registry生产实现已在AU-041深审并支撑已有P1 capability失配结论。direct test证明不健康canary被discard后旧实例保留、健康candidate激活后原实例停止。签名、freeze/require、disable、healthAll和stop分支由其它运行/专项审计承接；无P0–P3新问题。

## 206. AU-204 连续审计点

审阅 Commerce Catalog Operator API runtime 与direct tests。

执行结果：runtime加载固定L1 node manifest、限定console origin，读取workload secret建立api pool与object store，并在启动前核对database role/schema/contract/relations/functions/writes；entry/ready均实际消费。tests覆盖manifest origin/node context和database role mismatch；没有secrets/object probe/完整runtime close的direct fixture，形成F-0205/P2；无P0/P1新问题。

## 207. AU-205 连续审计点

审阅 Commerce ProviderFactories。

执行结果：不可变factory表列出11个外部provider，`providerFactory`以ID选择且unknown fail-fast；ProviderLoader是唯一仓内运行消费者。未找到factory表完整性、唯一性或unknown ID的direct fixture，形成F-0206/P3；无P0–P2新问题。

## 208. AU-206 连续审计点

审阅 Commerce foundation CommandBus。

执行结果：CommandBus以command type注册单一handler，freeze后拒绝注册，重复/缺失handler fail-fast，API/Jobs bootstrap均创建后freeze。未找到direct CommandBus fixture，形成F-0207/P3；无P0–P2新问题。

## 209. AU-207 连续审计点

审阅 Commerce foundation QueryBus。

执行结果：QueryBus按query type注册单一handler，freeze后拒绝注册，重复/缺失handler fail-fast，API/Jobs bootstrap均创建后freeze。未找到direct QueryBus fixture，形成F-0208/P3；无P0–P2新问题。
...
审阅 Commerce foundation ModuleOperations 与direct lifecycle/idempotency/audit tests。

执行结果：该公共操作层逐操作核对catalog ownership，GET进入query workload，transactional write交由ExecutionKernel，其他写入在command transaction中锁定idempotency记录、写审计并存可重放响应；identity及observability审计字段另作允许列表和脱敏。direct tests覆盖敏感数据不落审计、prepare/execute/finalize顺序、transport字段不影响重放、public actor及expectedVersion绑定。未覆盖catalog mismatch、deadline/abort、lifecycle discard及write short-circuit拒绝，形成F-0209/P3；无P0–P2新问题。
...
审阅 Commerce foundation Command message contract。

执行结果：三行泛型契约将command唯一约束为stable type；CommandBus直接将它用于注册、handler typed dispatch及execute输入，API/Jobs bootstrap再实例化bus。类型本身没有独立副作用或数据边界，既有F-0207覆盖bus不变量测试缺口；无P0–P3新问题，也不是删除候选。

## 216. AU-214 连续审计点

审阅 Commerce transactional write ExecutionKernel 与direct test。

执行结果：transactional operation先原子claim scoped idempotency，创建临时write context，执行action与审计，再同事务写outbox和completed checkpoint；OperationHandler要求写结果带enforced标记。fixture覆盖五并发同key单次执行/replay、hash复用拒绝、注入失败完整rollback后可重试、必需key、状态转换及write context关闭。provider key fallback、OperationRejection完成语义与checkpoint丢失分支没有direct fixture，形成F-0211/P3；无P0–P2新问题。

## 217. AU-215 连续审计点

审阅 Commerce foundation Handler contract。

执行结果：Handler是仅含异步`handle`的泛型应用层接口；CommandBus和QueryBus的注册/dispatch直接消费它，生成的OperationHandler以同一契约桥接HTTP operation。它没有独立运行、数据或权限副作用，是真实G0类型职责；无P0–P3新问题。

## 218. AU-216 连续审计点

审阅 Commerce identity credential replay direct test。

执行结果：fixture经真实ModuleOperations idempotency路径连续调用，验证sessions.create和tickets.exchange首次响应含敏感session/csrf/ticket/cookie后，持久化回放固定为不含secret的409；invitations.create的one-time code亦同。重放不会再执行业务action。后续AU-218确认OwnerActionCredentialPersistence覆盖的是ownership preview proof，非`identity.stepup.complete` action-proof；后者没有direct fixture，见F-0212/P3；无P0–P2新问题。

## 219. AU-217 连续审计点

审阅 Commerce Job message contract。

执行结果：Job/JobContext定义stable job ID、attempt和abort signal，以及异步execute形状；QueueJob、JobRegistry、Commerce/Catalog/Payment/IdentityNotification runtimes和多个entry直接消费。该类型不定义领取、重试或业务逻辑，相关行为由已审JobRunner/QueueJob承接；无P0–P3新问题。

## 220. AU-218 连续审计点

审阅 Commerce ownership action credential persistence direct test，并复核one-time replay分支映射。

执行结果：fixture经真实ModuleOperations验证`access.ownership.transfers.preview`首次返回proof后，audit与idempotency replay都不含secret、第二次返回固定409且不重执行业务action。实现同样保护accept/cancel preview；`identity.stepup.complete`返回`actionProof.proof`时走不同的`ACTION_PROOF_ONE_TIME_RESPONSE`分支，仓内未找到direct fixture，形成F-0212/P3。无P0–P2新问题。

## 221. AU-219 连续审计点

审阅 Commerce Query message contract。

执行结果：三行泛型契约只定义query stable type；QueryBus直接使用它进行typed handler registration和execute dispatch，API/Jobs bootstrap实例化bus。类型本身没有独立副作用或数据边界，既有F-0208记录QueryBus行为fixture缺口；无P0–P3新问题，也不是删除候选。

## 222. AU-220 连续审计点

审阅 Commerce cache VersionedKey。

执行结果：键名由generated CACHE_CATALOG声明的field集合严格决定，值以base64url编码并拒绝空/超长，体验、报表和webbusiness缓存读取/投影失效实际消费。direct test只覆盖experience active/version和缺字段；未验证reporting catalog shape、extra field、空值/超长值边界，形成F-0213/P3；无P0–P2新问题。

## 223. AU-221 连续审计点

审阅 Commerce foundation domain primitives。

执行结果：Aggregate、Entity、ValueObject、DomainEvent与Policy为`@shop/kernel`的兼容领域转发，分别被库存、订单、支付、checkout/order/purchase及support实际导入；DomainError承载领域/权限错误code及details并由ErrorMapper映射。Specification/And/Or无仓内业务caller但已在AU-009归DC-0011/G1，因为仍是workspace公开兼容转发；无P0–P3新问题或删除动作。

## 224. AU-222 连续审计点

审阅 Commerce release HealthProbe。

执行结果：SmokeMain在发布后依次验证live/startup/ready并携带release header，HttpClient以read mode和redirect error执行。HealthProbe对非2xx和非JSON/status失败会拒绝，但只接受三种status任一值，不要求与请求probe对应；例如`/health/live`错误返回`ready`仍可使smoke通过。未找到HealthProbe direct fixture，形成F-0214/P2；无P0/P1新问题。

## 225. AU-223 连续审计点

审阅 Commerce HttpClient direct test。

执行结果：fixture覆盖safe read transport retry、unsafe write不重试、business-key write的204 body保持；与Executor重试模式接线一致。HttpClient实际还区分connection/response timeout、外部deadline取消及redirect error，但未被direct fixture覆盖，形成F-0215/P2；无P0/P1新问题。
...
审阅 Commerce Bulkhead contract。

执行结果：Bulkhead是`@shop/kernel`兼容转发，Commerce Executor以RUNTIME_LIMITS external maximumConcurrency/maximumQueue建立唯一实例，并由HttpClient间接消费。该文件没有独立逻辑、静态消费者明确，为G0；无P0–P3新问题。

## 229. AU-227 连续审计点

审阅 Commerce CircuitBreaker contract。

执行结果：CircuitBreaker/CircuitState是`@shop/kernel`兼容转发，Executor由runtime failure threshold/recovery time构造并把外部调用包入rate→bulkhead→circuit→retry链。该文件无独立逻辑、唯一消费者明确，为G0；无P0–P3新问题。

## 230. AU-228 连续审计点

审阅 Commerce Parallel contract。

执行结果：mapParallel验证正整数并发度，以共享cursor分发固定数量worker，保持输入顺序；Voucher issue/import在KMS券码加密阶段以16并发调用。失败会拒绝调用方但不提供取消已在途operation的契约，且没有direct fixture覆盖并发上限、顺序、空输入、无效并发或失败后在途行为，形成F-0217/P3；无P0–P2新问题。

## 231. AU-229 连续审计点

审阅 Commerce RateLimiter contract。

执行结果：RateLimiter是`@shop/kernel`兼容转发；Executor以`RUNTIME_LIMITS.external.requestsPerSecond`构造唯一实例，并在bulkhead/circuit/retry之前获取令牌。内核实现已在AU-009深审，Commerce文件无独立逻辑、唯一消费者明确，为G0；无P0–P3新问题。

## 232. AU-230 连续审计点

审阅 Commerce Semaphore contract。

执行结果：Semaphore是`@shop/kernel`兼容转发；JobRunner、OutboxRelay、MigrationRunner和RegistrationMigrationRunner分别以它限制job处理、outbox发布及迁移KMS加密。内核实现已在AU-009深审，Commerce文件无独立逻辑、消费者明确，为G0；无P0–P3新问题。

## 233. AU-231 连续审计点

审阅 Commerce CursorCodec contract。

执行结果：CursorCodec输出versioned canonical base64url复合游标，并以kernel Cursor预校验、JSON schema、长度和canonical round-trip防止格式漂移；Validation作为风险、报表、支持与审计keyset查询的公共入口。direct fixture覆盖canonical round-trip、malformed与unsupported version，但“non-canonical”仅出现于测试标题而无实际case，也未覆盖invalid position与长度边界，形成F-0218/P3；无P0–P2新问题。

## 234. AU-232 连续审计点

审阅 Commerce HttpRequest contract。

执行结果：HttpRequest定义路由handler接收的method/path、headers、path parameters、query、parsed/raw body和deadline/signal。HttpApp在路由、鉴权/门禁后的唯一调用点以Fetch Request建立该形状；RouteRegistry和OperationController以该协议承接处理。该接口无独立执行分支、消费者明确，为G0；无P0–P3新问题。

## 235. AU-233 连续审计点

审阅 Commerce HttpResponse contract。

执行结果：HttpResponse统一status、可选headers/body；json为非204响应补JSON content-type并强制204无body。OperationController和ErrorMapper实际生产该类型，HttpApp的204断言与NodeServer 204写回fixture间接覆盖无body结果。无P0–P3新问题。

## 236. AU-234 连续审计点

审阅 Commerce Validation/pagination shared boundary。

执行结果：Validation统一body、string/secret/integer、limit/cursor与keyset分页结果，已被大量业务写入和读取操作复用。Pagination fixture只验证lookahead nextCursor与terminal page；没有direct fixture覆盖body/field校验、limit/cursor query、多值query、keyset的number/invalid position。该共享输入边界的回归会跨模块暴露，形成F-0219/P2；无P0/P1新问题。

## 237. AU-235 连续审计点

审阅 Commerce NodeServer ingress test boundary。

执行结果：NodeServer实际在全部生产API入口以127.0.0.1监听，先绑定node context，严格仅信任loopback反代传入的单IP `x-real-ip`；fixture覆盖节点解析一次、未知Host拒绝、health node-neutral及可信地址规则。现有fixture不覆盖请求体上限、client abort、通用500/listen error及Response headers/cookies writeback，形成F-0220/P2；无P0/P1新问题。

## 238. AU-236 连续审计点

审阅 Commerce ErrorMapper contract。

执行结果：ErrorMapper以generated errorStatus表映射已注册code，剥离`CODE:detail`后缀并对unknown/500/non-Error统一返回无内部详情的500；HttpApp捕获链实际消费。fixture只验证数个409和unknown 500，未覆盖DomainError details、冒号分支、非Error以及其他正式HTTP status族，形成F-0221/P2；无P0/P1新问题。

## 239. AU-237 连续审计点

审阅 Commerce HttpApp gates fixture。

执行结果：gate被证明为声明式观察链而非访问控制替代：唯一声明gate的`catalog.listings.read`恰执行一次且响应不变，无gate operation完全旁路，无plugin和plugin throw均记录not_applicable/error并保持handler结果。fixture与generated OperationCatalog声明闭合；无P0–P3新问题。

## 240. AU-238 连续审计点

审阅 Commerce Catalog controller scope fixture。

执行结果：生成控制器对`catalog.imports.read`的特殊资源规则经direct fixture锁定：`catalogpublication:*`不传具体resource、改由selected mall scope授权，普通`catalogimport:*`仍以具体资源授权；latest及task形式均覆盖。无P0–P3新问题。

## 241. AU-239 连续审计点

审阅 Commerce Finance controller scope fixture。

执行结果：生成控制器对`finance.policies.preview/manage`的新policy例外经direct fixture锁定：authorizer在请求处理前接收undefined resource，改由selected mall scope决定权限，而不会把尚未持久化的policy ID当作资源；两条operation均覆盖。无P0–P3新问题。

## 242. AU-240 连续审计点

审阅 Commerce WeChat controller authentication fixture。

执行结果：`identity.wechat.session`以request body mode区分两条真实边界：authenticated通过`identity.session.read`当前会话policy得到access，anonymous保持public并传入null access；fixture直接断言authorizer实参和handler access。无P0–P3新问题。

## 243. AU-241 连续审计点

审阅 Commerce QueryMetrics。

执行结果：QueryMetrics以workload累积count/failure/total/max并输出排序、冻结snapshot；PoolSet在pool connect/query及proxy client query的成功/异常两侧实际调用，runtime dependency健康响应读取该snapshot。没有direct fixture固定聚合、排序、失败或snapshot不可变性，形成F-0222/P3；无P0–P2新问题。

## 244. AU-242 连续审计点

审阅 Commerce migration execution plan。

执行结果：genericMigrationSql仅匹配固定owner-transfer迁移中的唯一`session_hash` add-column语句，替换为if-not-exists以兼容不可改历史迁移；目标不存在或匹配次数不为一即失败。MigrationRunner在实际执行前调用它；fixture覆盖目标转换、旁路与drift failure。无P0–P3新问题。

## 245. AU-243 连续审计点

审阅 Commerce registration migration plan。

执行结果：RegistrationMigrationPlan对白名单环境owner fixture明确省略、对15个历史断言明确变换、对post-history原文与legacy/reconciled ledger严格校验、从最新迁移抽取唯一runtime marker。RegistrationMigrationRunner在独立数据库、advisory lock、history/ledger/target复核链内消费；fixture逐类覆盖省略、变换/漂移、original、历史账本、reconciled账本和marker。无P0–P3新问题。

## 246. AU-244 连续审计点

审阅 Commerce RegistrationMigrationRunner。

执行结果：Runner锁定独立registration数据库、验证history/ledger/target，并对owner fixture、省略/变换迁移、secret backfill及两类受控已存在runtime marker执行专门恢复路径。对新migration仍先执行SQL、后INSERT ledger，扩展既有F-0013/P2到registration runner；没有Runner级direct fixture覆盖数据库边界、SQL/ledger故障窗口或deferred repair状态，形成F-0223/P2；无P0/P1新问题。

## 247. AU-245 连续审计点

审阅 Commerce live database boundary。

执行结果：LiveDatabaseBoundary从deployment oracle验证运行role、registration database、单Owner、migration/runtime/boundary role、retired membership和各database owner；RuntimeCompatibility/jobs与identity runtime实际调用。严格主边界fixture逐字段fail-closed，但Identity runtime的`businessRoles:false/retiredRoles:false`分支没有direct fixture，形成F-0224/P2；无P0/P1新问题。

## 248. AU-246 连续审计点

审阅 Commerce IdentityRegistrationApiRuntime。

执行结果：独立身份API在监听前加载manifest/node definition、秘密和database，执行registration compatibility、identity database boundary、node database manifest及object-store probe；成功后绑定访问、审计、密钥、KMS、object store与可选WeChat网关。现有fixture只覆盖manifest/compatibility，不执行runtime factory、cleanup、容器绑定或可选WeChat分支，形成F-0225/P2；无P0/P1新问题。
...
审阅 Commerce ConsoleSupportRuntime。

执行结果：console support API以专用roles和最小访问权限装配metrics、访问、risk、audit和KMS。compatibility query计算`contract`但predicate遗漏该字段，contract marker缺失/drift仍会启动，记录F-0229/P1待独立复核；factory/metrics生命周期无direct fixture，记录F-0230/P2；无P0问题。
