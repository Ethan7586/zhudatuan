...
import { DelegationService, delegatedPermissions } from './DelegationService';
...
describe('delegatedPermissions', () => {
  it('validates a prepared employee membership with enrollment semantics', async () => {
    ...
    const assert = vi.fn();
    const service = new DelegationService({} as never, { assert, assertCampaign: vi.fn() } as never, {} as never, { execute } as never, {} as never);
