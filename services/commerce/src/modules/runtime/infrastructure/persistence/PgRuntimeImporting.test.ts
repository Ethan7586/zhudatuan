...
describe('PgRuntimeImporting', () => {
  it('maps persistence rows without leaking tenant or object-store columns', async () => {
    const query = vi.fn(async () => ({ rows: [{ id: 'import:one', state: 'uploaded', total_count: 0, cursor_value: 0, success_count: 0, failure_count: 0,
      created_at: '2026-09-05T00:00:00.000Z', updated_at: '2026-09-05T00:00:00.000Z', validation_summary: {}, last_error: null, errors: [],
      report_object_ref: null, report_sha256: null, report_size: null, scope_id: 'mall:one', owner: 'catalog', object_key: 'object:one', file_hash: 'a'.repeat(64) }], rowCount: 1 }));
    const importing = new PgRuntimeImporting(access(query));
    const created = await importing.create(context, { id: 'import:one', scope: 'mall:one', owner: 'catalog', kind: 'product', reference: 'object:one',
      sha256: 'a'.repeat(64), name: 'catalog.csv', mediaType: 'text/csv', size: 12, actor: 'principal:one', authorization: {} });
    expect(created).toEqual({ id: 'import:one', state: 'uploaded', total_count: 0, cursor_value: 0, success_count: 0, failure_count: 0,
  ...
  it('keeps the API view and confidential report reference in separate public values', async () => {
    const query = vi.fn(async () => ({ rows: [{ id: 'import:one', state: 'completed', total_count: 2, cursor_value: 2, success_count: 1, failure_count: 1,
      created_at: '2026-09-05T00:00:00.000Z', updated_at: '2026-09-05T00:01:00.000Z', validation_summary: { columns: ['sku'] }, last_error: null,
      errors: [{ row_number: 3, reason_code: 'SKU_INVALID', field: 'sku', detail: '商品编码无效' }], report_object_ref: 'object:report',
      report_sha256: 'b'.repeat(64), report_size: '128', scope_id: 'mall:one', owner: 'catalog', object_key: 'object:one', file_hash: 'a'.repeat(64) }], rowCount: 1 }));
    const importing = new PgRuntimeImporting(access(query));
    ...
    expect(found).toEqual({
      body: {
        id: 'import:one', state: 'completed', total_count: 2, cursor_value: 2, success_count: 1, failure_count: 1,
        created_at: '2026-09-05T00:00:00.000Z', updated_at: '2026-09-05T00:01:00.000Z', validation_summary: { columns: ['sku'] },
        last_error: null, errors: [{ row_number: 3, reason_code: 'SKU_INVALID', field: 'sku', detail: '商品编码无效' }],
      },
  ...
  it('fails closed when report metadata is partially persisted', async () => {
    const query = vi.fn(async () => ({ rows: [{ id: 'import:one', state: 'completed', total_count: 0, cursor_value: 0, success_count: 0, failure_count: 0,
      created_at: '2026-09-05T00:00:00.000Z', updated_at: '2026-09-05T00:00:00.000Z', validation_summary: {}, last_error: null, errors: [],
      report_object_ref: 'object:report', report_sha256: null, report_size: null, scope_id: 'mall:one', owner: 'catalog', object_key: 'object:one', file_hash: 'a'.repeat(64) }], rowCount: 1 }));

    await expect(new PgRuntimeImporting(access(query)).read({} as never, 'import:one', 'mall:one', 'catalog'))
      .rejects.toThrow('RUNTIME_IMPORT_REPORT_INVALID');
  });
  ...
  it('exposes validated progress through the Runtime port instead of cross-schema SQL', async () => {
    ...
    await expect(importing.progress({ scope: 'mall:one' } as never, 'import:one', 'finance'))
      .resolves.toEqual({ total: 4, processed: 3, succeeded: 2, failed: 1 });
  ...
  it('rejects impossible persisted progress', async () => {
    ...
    await expect(new PgRuntimeImporting(access(query)).progress({ scope: 'mall:one' } as never, 'import:one', 'finance'))
      .rejects.toThrow('RUNTIME_IMPORT_PROGRESS_INVALID');
  });
  ...
  it('stores normalized chunks and preflight row errors under the same transaction context', async () => {
    ...
    await importing.stage({ scope: 'mall:one' } as never, 'import:one', 'catalog', [{ sequence: 0, rows: [{ row: 2, payload: { invalid: 'CATALOG_SKU_INVALID' } }] }], [
