...
const operations = parse(readFileSync(resolve(root, 'packages/contract/definitions/operations.yml'), 'utf8')).operations;
...
for (const path of ['services/commerce/src/entry/ApiMain.ts', 'services/commerce/src/entry/JobsMain.ts', 'services/commerce/src/entry/MigrationMain.ts']) {
  if (!existsSync(resolve(root, path))) throw new Error(`P0_ENTRY_MISSING:${path}`);
}
const compose = parse(readFileSync(resolve(root, 'infrastructure/local/docker-compose.yml'), 'utf8'));
...
for (const path of [
  'tools/localsecrets/src/Main.ts', 'tools/localkms/src/Main.ts', 'tools/localobjects/src/Main.ts',
  'tools/seed/src/Migrate.ts', 'tools/seed/src/Seed.ts', 'tools/seed/src/Verify.ts',
...
for (const value of ['.env*', 'secrets.local.json', 'infrastructure/local/.tls/', 'infrastructure/local/data/']) {
  if (!ignored.split(/\r?\n/).includes(value)) throw new Error(`P0_LOCAL_SECRET_NOT_IGNORED:${value}`);
}
const portContracts = new Map([
  ['apps/console/vite.config.ts', 'port: 5173'],
  ['apps/store/vite.config.ts', 'port: 5174'],
  ['apps/supplier/vite.config.ts', 'port: 5175'],
  ['apps/auth/vite.config.ts', 'port: 5176'],
...
for (const path of ['apps/console/.env.example', 'apps/store/.env.example', 'apps/supplier/.env.example', 'apps/storefront/.env.example', 'apps/auth/.env.example', 'apps/miniapp/.env.example']) {
...
for (const path of [
  ...
  'services/commerce/src/foundation/infrastructure/KmsClient.ts',
  'services/commerce/src/foundation/infrastructure/ObjectStore.ts',
]) {
