...
const SURFACES = new Set(['console', 'storefront', 'auth']);

export function clientErrorOperations(context: ModuleContext): ModuleOperations {
  const telemetry = context.service(TELEMETRY);
  return new ModuleOperations('observability', context.service(DATABASE_POOL), context.service(AUDIT_SINK), {
    'observability.clienterrors.create': async (request, database) => {
      ...
      const recorded = telemetry.clientErrors.record({
        scope,
        surface,
    ...
    'observability.clienterrors.read': async (request) => {
      const access = requireAccess(request);
      const items = telemetry.clientErrors.list(access.scope, limit(request, 200));
...
async function organizationScope(database: OperationDatabase, membership: string): Promise<TelemetryScope> {
  const result = await database.query<{ scope: TelemetryScope }>(
    `select access.scope_object(organization_id) scope
    from access.membership where id=$1 and status='active'`,
    [membership]
  );
  const scope = result.rows[0]?.scope;
  if (!scope) throw new DomainError('MEMBERSHIP_INACTIVE');
  return scope;
}
