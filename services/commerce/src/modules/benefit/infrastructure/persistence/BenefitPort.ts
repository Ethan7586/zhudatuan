...
export class BenefitPort implements BenefitGateway {
  ...
  async preview(context: ReadTransactionContext, member: string, scope: string, accounts: readonly string[]): Promise<readonly BenefitChoice[]> {
    const database = this.transactions.database(context);
    if (accounts.length === 0) return [];
    return (
      await database.query<BenefitChoice>(
        `select account.id,account.version::float8 version,account.kind,
      ...
      and account.member_id=$2 and account.scope_id=$3 and account.currency='CNY' and account.status='active' order by account.id`,
        [accounts, member, scope]
      )
    ).rows;
  }
  async available(context: ReadTransactionContext, member: string, scope: string): Promise<readonly BenefitChoice[]> {
    const database = this.transactions.database(context);
    const ids = await database.query<{
      id: string;
    }>(`select id from benefit.account where member_id=$1 and scope_id=$2 and currency='CNY' and status='active' order by id limit 100`, [member, scope]);
    return this.preview(
      context,
      member,
      scope,
  ...
  async reserve(context: WriteTransactionContext, order: string, member: string, scope: string, tenders: readonly BenefitTender[]): Promise<void> {
    const database = this.transactions.database(context);
    ...
    const rows = await database.query<{
      ...
      greatest(0,least(balance.balance_minor,coalesce((select sum(lot.remaining_minor) from benefit.lot lot where lot.account_id=account.id
        ...
        -coalesce((select sum(reservation.amount_minor) from benefit.reservation reservation where reservation.account_id=account.id
          and reservation.state='active' and reservation.expires_at>clock_timestamp()),0))::float8 available_minor
      from benefit.account account join benefit.balance balance on balance.account_id=account.id where account.id=any($1::text[])
      and account.member_id=$2 and account.scope_id=$3 and account.status='active' order by account.id for update of account`,
      [ids, member, scope]
    );
    ...
    for (const tender of tenders) {
      ...
      if (account.available_minor < tender.amountMinor) throw new DomainError('BENEFIT_BALANCE_INSUFFICIENT');
  ...
  async consume(context: WriteTransactionContext, order: string, accountid: string, amountMinor: number): Promise<void> {
    const database = this.transactions.database(context);
    const account = await database.query<{
      ...
      `select id,scope_id,currency from benefit.account
    ...
    const reservation = await database.query(
      `select id from benefit.reservation where account_id=$1 and owner_id=$2 and state='active'
      and expires_at>clock_timestamp() and amount_minor=$3 for update`,
      [accountid, order, amountMinor]
    );
    ...
    const lots = await database.query<{
      ...
      `select id,remaining_minor::float8 remaining_minor from benefit.lot
      ...
      order by expires_at nulls last,effective_at,id for update`,
      [accountid]
    );
    if (lots.rows.reduce((sum, lot) => sum + lot.remaining_minor, 0) < amountMinor) throw new DomainError('BENEFIT_BALANCE_INSUFFICIENT');
    let remaining = amountMinor;
    for (const lot of lots.rows) {
      const amount = Math.min(remaining, lot.remaining_minor);
      if (amount === 0) continue;
      await database.query(
        `update benefit.lot set remaining_minor=remaining_minor-$2,
        state=case when remaining_minor=$2 then 'consumed' else 'active' end,version=version+1 where id=$1`,
        [lot.id, amount]
      );
      await database.query(
        `insert into benefit.lotmovement(id,lot_id,kind,amount_minor,reference_type,reference_id,occurred_at)
        values($1,$2,'consume',$3,'order',$4,clock_timestamp())`,
        [`movement:${randomUUID()}`, lot.id, amount, order]
      );
      remaining -= amount;
    ...
    await this.financial().post(context, {
      ...
      credit: { code: 'commerce.benefit', kind: 'income' },
      amountMinor,
    });
    await database.query(`update benefit.reservation set state='consumed' where id=$1`, [reservation.rows[0].id]);
  }
  async refund(context: WriteTransactionContext, input: BenefitRefund): Promise<void> {
    const database = this.transactions.database(context);
    const account = await database.query<{
      id: string;
      currency: string;
    }>(
      `select id,currency from benefit.account
      where id=$1 and member_id=$2 and scope_id=$3 for update`,
      [input.account, input.member, input.scope]
    );
    const selected = account.rows[0];
    if (!selected) throw new Error('BENEFIT_REFUND_ACCOUNT_MISSING');
    const sources = await database.query<Source>(
      `select movement.id,movement.lot_id,movement.amount_minor::float8 amount_minor,
      ...
      order by movement.occurred_at,movement.id for update of lot`,
      [input.order, input.account]
    );
    ...
    for (const source of sources.rows) {
      ...
      if (reusable)
        await database.query(
          `update benefit.lot set remaining_minor=remaining_minor+$2,state='active',version=version+1
        where id=$1 and remaining_minor+$2<=total_minor`,
          [source.lot_id, amount]
        );
      else {
      ...
      await database.query(
        `insert into benefit.lotmovement(id,lot_id,kind,amount_minor,reference_type,reference_id,source_id,occurred_at)
        values($1,$2,'refund',$3,'refund',$4,$5,clock_timestamp())`,
        [`movement:${randomUUID()}`, target, amount, input.id, source.id]
      );
      remaining -= amount;
      if (remaining === 0) break;
    }
    await this.financial().post(context, {
      ...
      credit: { code: `benefit.${selected.id}`, kind: 'liability' },
