...
export interface IssueBatchValue {
  readonly id: string;
...
export class IssueBatch {
  constructor(readonly value: IssueBatchValue) {
    if (
      ![value.requested, value.processed, value.succeeded, value.failed, value.retryable, value.version].every(Number.isSafeInteger) ||
      value.requested <= 0 ||
      value.processed !== value.succeeded + value.failed ||
      value.processed > value.requested ||
  ...
  progress(succeeded: number, failed: number, retryable: number): IssueBatch {
    ...
    return new IssueBatch(
      Object.freeze({
        ...this.value,
        processed,
        succeeded: this.value.succeeded + succeeded,
        failed: this.value.failed + failed,
        retryable: this.value.retryable + retryable,
        state: processed === this.value.requested ? (this.value.failed + failed > 0 ? 'failed' : 'completed') : 'running',
