...
try {
  ...
  const sources = [...new Set([
    'infrastructure/zhudatuan/aliyun/staging/artifacts.yml',
    'infrastructure/zhudatuan/aliyun/staging/delivery.yml',
...
async function assertBuildMarkers(hosts, solutionOrigin) {
  const contracts = [
    [resolve(repository, 'apps/auth-web/dist'), [`https://${hosts.api}`, `https://${hosts.console}`]],
    [resolve(repository, 'apps/console/dist'), [`https://${hosts.api}`, `https://${hosts.accounts}`, solutionOrigin]],
  ];
