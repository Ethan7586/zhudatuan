...
export function supportTransitionPersistence(ports: SupportPersistenceFactory): Pick<SupportPersistence, 'updateCase' | 'closeCase' | 'reopenCase'> {
  return {
    updateCase: (request, database) => update(request, database, ports),
    closeCase: (request, database) => transition(request, database, 'closed', ports),
    reopenCase: (request, database) => transition(request, database, 'open', ports),
  };
}

async function update(request: OperationRequest, database: SqlExecutor, ports: SupportPersistenceFactory) {
...
async function transition(request: OperationRequest, database: SqlExecutor, target: TicketState, ports: SupportPersistenceFactory) {
  const access = requireAccess(request);
  const id = request.input.path.caseid!;
  const repository = ports(database);
  const current = await load(database, id, await repository.descendants(access.scope.id));
...
async function load(database: SqlExecutor, id: string, scopes: readonly string[]) {
