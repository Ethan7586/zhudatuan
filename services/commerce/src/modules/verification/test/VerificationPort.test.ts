...
describe('verification public proof boundary', () => {
  it('consumes a proof only for its exact purpose, operation, subject and scope', async () => {
    ...
    const receipt = await withWriteTransaction(query, (context) => port.verify(context, { proof, scope: 'mall:one', subjectType: 'resource',
      subject: 'settlement:one', purpose: 'financial_approval', operation: 'finance.approvals.decide', actor: 'principal:one', now: new Date('2026-09-05T00:00:00.000Z') }));
  ...
  it('fails closed for purpose mismatch and proof replay', async () => {
    ...
    await expect(withWriteTransaction(async () => result([]), (context) => port.verify(context, { proof: 'a'.repeat(43), scope: 'mall:one',
      subjectType: 'resource', subject: 'settlement:one', purpose: 'financial_approval', operation: 'finance.approvals.decide', actor: 'principal:one' }))).rejects.toThrow('PROOF_REQUIRED');
  });
});
