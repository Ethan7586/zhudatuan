    ...
    try {
      ...
      for (let offset = 0; offset < rows.length; offset += 5_000) {
        const values = rows.slice(offset, offset+5_000).map((row, index) => ({ sequence: offset+index+1,reference: row.reference,
          ...
          occurred: row.occurredAt ?? null,hash: createHash('sha256').update(JSON.stringify(row)).digest('hex') }));
        await client.query(`insert into finance.statementline(id,reconciliation_id,scope_id,sequence,external_reference,kind,amount_minor,
      ...
      await match(client, id, source.scope_id);
      const internal = await client.query<{ net: number; differences: number }>(`select coalesce(sum(case line.kind when 'refund' then
      ...
      if (summary.differences > 0 || providerNet !== summary.net) await difference(client, id, source.scope_id,
        providerNet-summary.net, summary.differences);
      await client.query('commit');
    } catch (cause) { await client.query('rollback'); throw cause; } finally { client.release(); }
  }
}

async function match(database: Database, id: string, scope: string): Promise<void> {
  await database.query(`with candidates as(
      select line.id line_id,'payment' internal_type,payment.id internal_id,payment.amount_minor internal_minor,1 priority
        from finance.statementline line join payment.attempt attempt on attempt.external_transaction=line.external_reference
        join payment.payment payment on payment.intent_id=attempt.intent_id where line.reconciliation_id=$1 and line.kind='payment'
      union all select line.id,'refund',refund.id,refund.amount_minor,1 from finance.statementline line join payment.refund refund
        on refund.external_transaction=line.external_reference or refund.provider_reference=line.external_reference
        where line.reconciliation_id=$1 and line.kind='refund'
      union all select line.id,'fulfillment',fulfillment.id,coalesce(fulfillment.amount_minor,0),1 from finance.statementline line
        join fulfillment.fulfillmentorder fulfillment on fulfillment.external_reference=line.external_reference
        where line.reconciliation_id=$1 and line.kind='fulfillment'
...
async function difference(database: Database, id: string, scope: string, amount: number, count: number): Promise<void> {
  await database.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,
