...
export class PgPublicationRepository implements PublicationRepository {
  ...
  async references(context: ReadTransactionContext, document: ExperienceDocument, pool: string): Promise<boolean> {
    ...
    const [catalog, marketing] = await Promise.all([
      this.catalog.references(context, pool, { products: references.products, categories: references.categories, collections: references.collections }),
