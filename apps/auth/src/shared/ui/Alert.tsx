  ...
  if (failure)
    return (
      <div ref={summary} className="authalert" data-severity={failure.severity} role="alert" aria-live="assertive" tabIndex={-1}>
        ...
        <div className="authalertcopy"><strong>{failure.title}</strong><p>{failure.message}</p>{failure.requestId ? <small>请求编号：{failure.requestId}</small> : null}</div>
        {failure.requestId ? <button type="button" aria-label="复制请求编号" onClick={() => void copy(failure.requestId ?? '').then(setCopied)}>{copied ? '已复制' : '复制编号'}</button> : null}
        {onAction && failure.action.kind !== 'none' ? <button type="button" onClick={onAction}>{failure.action.label}</button> : null}
      </div>
    );
  if (notice)
    return (
