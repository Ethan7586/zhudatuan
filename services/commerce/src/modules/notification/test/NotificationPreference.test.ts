...
describe('notification preference command', () => {
  it('persists consent source, quiet hours and the observed optimistic version', async () => {
    ...
    const changePreference = vi.fn(async () => ({
    ...
    await handler.execute(
      {
        path: { channel: 'email', eventtype: 'order.paid' },
        query: {},
        body: {
          enabled: true,
      ...
      context(transaction, 2)
    );

    expect(changePreference).toHaveBeenCalledWith(transaction, 'member:one', 'mall:one', 'email', 'order.paid', true, { start: '22:00', end: '07:00', timezone: 'Asia/Shanghai' }, 2);
  });
  ...
  it('uses version zero for the first preference write', async () => {
    ...
    await handler.execute({ path: { channel: 'email', eventtype: 'order.paid' }, query: {}, body: { enabled: true } } as never, context(transaction, undefined));

    expect(changePreference).toHaveBeenCalledWith(transaction, 'member:one', 'mall:one', 'email', 'order.paid', true, null, 0);
  });
});
...
function context(transaction: WriteTransactionContext, expectedVersion: number | undefined): WriteHandlerContext<'notification.preferences.manage'> {
  return {
    ...
    operation: 'notification.preferences.manage',
    headers: {},
    rawBody: '',
    idempotencyKey: 'preference:one',
    ...
    security: {
      ...
      access: {
        ...
        accessVersion: 1,
        roles: [],
        capabilities: new Set(['notification.preferences.manage']),
        capabilityVersion: 1,
        assurance: { level: 1 },
        trace: 'trace:one',
      },
