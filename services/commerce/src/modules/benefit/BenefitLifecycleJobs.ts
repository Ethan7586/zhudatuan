...
import { digest, event, moveReservedToGranted, post } from './BenefitJobSupport';

export class BenefitLifecycleJobs {
  constructor(
  ...
  async expire(): Promise<void> {
    ...
    try {
      ...
      for (const lot of lots.rows) {
        await post(this.finance, client, lot.scope_id, 'benefit.expire', lot.id, lot.currency, 'Benefit expiry', `benefit.${lot.account_id}`, 'liability', 'benefit.expiry', 'income', lot.remaining_minor);
