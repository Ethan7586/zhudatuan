...
export class PgTransactionalOutbox implements TransactionalEventWriter {
  ...
  async appendMany(context: WriteTransactionContext, events: readonly DomainEvent[]): Promise<void> {
    ...
    const rows = events.map((event) => ({
      id: event.event, eventType: event.type, eventVersion: event.version, aggregateType: event.aggregate.type,
      aggregateId: event.aggregate.id, aggregateVersion: event.aggregate.version, scope: event.tenant,
      payload: parseEventPayload(event.type, event.payload), trace: event.trace, actor: event.actor,
      correlation: event.correlation, causation: event.causation, payloadVersion: event.payloadVersion, occurred: event.occurred,
    }));
