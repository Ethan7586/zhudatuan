...
export function useQualificationViewModel(context: ConsoleContext, dependencies: QualificationDependencies, requestStepup: () => void) {
  ...
  const preview = useMutation({
    mutationFn: async (target: QualificationEditor) => {
      if (target.kind === 'decision') return Object.freeze({ kind: 'decision' as const, decisions: await dependencies.preview.decision(context, required(target.member), required(target.resource), identity) });
      const command = previewCommand(target);
      return Object.freeze({ kind: 'policy' as const, impact: await dependencies.preview.policy(context, command, identity) });
    },
    onSuccess: (result) => {
