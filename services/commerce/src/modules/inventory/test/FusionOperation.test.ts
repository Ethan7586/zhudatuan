...
describe('inventory availability read', () => {
  it('defines a typed inventory import preview instead of a browser-owned estimate', () => {
    const parsed = OPERATION_SCHEMAS['inventory.imports.read'].output.parse({
      id: 'import:one', state: 'validated', total_count: 2, cursor_value: 2, success_count: 1, failure_count: 1,
      created_at: '2026-09-04T08:00:00.000Z', updated_at: '2026-09-04T08:01:00.000Z', validation_summary: {}, last_error: null, errors: [],
      preview: {
        hash: 'a'.repeat(64), state: 'ready', total: 2, valid: 1, invalid: 1, onhandDelta: 10,
        samples: [{ row: 2, sku: 'sku:one', location: 'warehouse:one', source: 'jdproduct', onhand: 10, safety: 1, valid: true, issues: [] }],
        watermark: '2026-09-04T08:00:00.000Z', expiresAt: '2026-09-05T08:00:00.000Z',
      },
    });
  ...
  it('uses the authorized scope, source selector and latest authoritative watermark', async () => {
    ...
    const reply = await new AvailabilityReadHandler(inventory).execute(
      { query: { sku: ['sku:one', 'sku:one'], source: 'jdproduct' } } as never,
      readHandlerContext('inventory.availability.read', transaction)
    );
...
function projection() {
  return {
    sku: 'sku:one', scope: 'mall:one', onhand: 10, safety: 1, reserved: 2, available: 7, state: 'available' as const,
    ...
    version: '2', watermark: '2026-09-04T08:00:00.000Z',
  };
}

function row(overrides: Readonly<Record<string, unknown>>) {
  return {
    id: 'stock:one', sku: 'sku:one', scope: 'mall:one', location: 'warehouse:one', onhand: 10, safety: 1,
    reserved: 2, active_count: 1, earliest_expiry: new Date('2026-09-04T08:30:00.000Z'), status: 'active', version: '2',
    source: 'jdproduct', source_reference: 'source:one', watermark: new Date('2026-09-04T08:00:00.000Z'), ...overrides,
  };
}
