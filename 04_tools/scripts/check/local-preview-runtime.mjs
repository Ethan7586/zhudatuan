...
function validateDirtyFiles(records) {
  const allowed = new Set([
    ...
    '01_core_hexin/apps/console/src/shared/config/BuildInfo.ts',
    '01_core_hexin/apps/console/src/shared/config/BuildInfo.test.ts',
    '01_core_hexin/apps/console/src/shell/ScopeShell.tsx',
    '01_core_hexin/apps/console/src/shell/ScopeShell.test.tsx',
    '04_tools/scripts/check/local-preview-database.mjs',
    '04_tools/scripts/check/local-preview-runtime.mjs',
    '05_docs_ziliao/docs_wendang/本地预览唯一版本说明.md',
  ]);
...
function prepareRuntimeConfig() {
  const secretsPath = resolve(ROOT, '02_platform_pingtai/infrastructure/local/secrets.local.json');
...
function validateMigrationLedger(records, head) {
  ...
  for (const path of paths) {
    if (!path.startsWith('02_platform_pingtai/database/supabase/migrations/') || !path.endsWith('.sql')) continue;
...
function candidateOwner(path) {
  if (path.startsWith('01_core_hexin/apps/auth-web/')) return 'EXCLUDED-LOGIN';
  if (/Payment|payment/i.test(path) && (path.startsWith('01_core_hexin/extensions/providers/core/') || path.startsWith('01_core_hexin/packages/contract/src/provider/'))) {
    return 'EXCLUDED-PAYMENT';
  }
  if (path.startsWith('01_core_hexin/packages/design/')) return 'PRODUCT-013-APPROVAL-REQUIRED';
  if (path === 'pnpm-lock.yaml' || path === 'package-lock.json' || path === '01_core_hexin/packages/config/src/Release.ts' || path.startsWith('04_tools/scripts/check/local-preview-') || path === '05_docs_ziliao/docs_wendang/本地预览唯一版本说明.md') return 'PRODUCT-000A';
  if (path === '04_tools/scripts/audit/database-contracts.mjs' || path.endsWith('/RegistrationMigrationRunner.ts')) return 'PRODUCT-000R';
  if (path.startsWith('02_platform_pingtai/database/supabase/migrations/')) return 'PRODUCT-000R-MIGRATION';
  if (path.startsWith('02_platform_pingtai/database/contracts/') || path === '05_docs_ziliao/docs_wendang/requirements/mvp.yml') return 'PRODUCT-014';
  if (path.startsWith('01_core_hexin/apps/console/public/product-preview/')) return 'PRODUCT-002-REMOVE-PREVIEW';
  if (path.startsWith('01_core_hexin/apps/console/')) return consoleOwner(path);
  if (path.startsWith('01_core_hexin/extensions/providers/')) return 'PRODUCT-005';
  if (path.startsWith('01_core_hexin/packages/sdk/') || path.startsWith('04_tools/tools/contractgen/')) return 'PRODUCT-005-SERIAL-GENERATED';
  if (path.startsWith('01_core_hexin/packages/contract/')) return contractOwner(path);
  if (path === '04_tools/scripts/build-web-tokens.mjs') return 'PRODUCT-013';
  if (path.includes('/callgraph/events.mjs')) return 'PRODUCT-008';
  if (path.includes('/callgraph/extensions.mjs')) return 'PRODUCT-005';
  if (path.startsWith('01_core_hexin/services/commerce/src/modules/extension/') || path.startsWith('01_core_hexin/services/commerce/src/bootstrap/Extension') || path.endsWith('/ProviderFactories.ts') || path.endsWith('/ProviderLoader.ts')) return 'PRODUCT-005';
  if (path.startsWith('01_core_hexin/services/commerce/src/modules/channel/')) return channelOwner(path);
  if (path.startsWith('01_core_hexin/services/commerce/src/modules/reporting/')) {
    return /ProjectEvent|ReportingPort|PgReportingRepository/.test(path) ? 'PRODUCT-008' : 'PRODUCT-001';
  }
  if (
    path.startsWith('01_core_hexin/services/commerce/src/modules/catalog/') ||
    path.startsWith('01_core_hexin/services/commerce/src/modules/inventory/') ||
    path.startsWith('01_core_hexin/services/commerce/src/modules/pricing/') ||
  ...
  if (path.startsWith('04_tools/tools/seed/')) return 'PRODUCT-004';
  return 'UNASSIGNED';
}
...
async function validateServices() {
  const services = [
    ...
    { name: 'console', origin: CONSOLE_ORIGIN, cwd: resolve(ROOT, '01_core_hexin/apps/console'), kind: 'web' },
    { name: 'auth-web', origin: AUTH_ORIGIN, cwd: resolve(ROOT, '01_core_hexin/apps/auth-web'), kind: 'web' },
  ];
