...
describe('canonical payment settlement injection', () => {
  it('keeps the captured sequence while allowing the purchase benefit boundary', async () => {
    const events: string[] = [];
    const benefit = { consume: vi.fn(async () => { events.push('benefit'); }) };
    const voucher = { consume: vi.fn(async () => { events.push('voucher'); }) };
    const inventory = { commit: vi.fn(async () => { events.push('inventory'); }) } as unknown as InventoryPort;
    const marketing = { commit: vi.fn(async () => { events.push('marketing'); }) } as unknown as MarketingPort;
    const fulfillment = { create: vi.fn(async () => { events.push('fulfillment'); return ['fulfillment:one']; }) } as unknown as FulfillmentPort;
    const orders = { paymentState: vi.fn(async () => 'unpaid'), markPaid: vi.fn(async () => { events.push('order-paid'); }) } as unknown as OrderPort;
  ...
  it('retains default construction and rejects external tender capture before side effects', async () => {
    expect(new PaymentSettlement()).toBeInstanceOf(PaymentSettlement);
    ...
    const orders = { paymentState: vi.fn(async () => 'unpaid'), markPaid: vi.fn() } as unknown as OrderPort;
    const settlement = new PaymentSettlement(benefit, voucher, inventory, marketing, fulfillment, orders);
    await expect(settlement.capture(databaseWithTender('wechat'), target(), 'internal'))
      .rejects.toThrow('PAYMENT_EXTERNAL_TENDER_NOT_CAPTURED');
...
function databaseWithTender(kind: 'benefit' | 'wechat'): OperationDatabase {
  return { query: vi.fn(async (text: string) => {
    ...
    if (text.includes('from payment.intenttender')) return result([{
      sequence: 1, kind, reference_id: kind === 'benefit' ? 'account:one' : null, amount_minor: 100, state: 'held',
    }]);
...
function target() {
  return { intent: 'intent:one', order: 'order:one', scope: 'mall:one', mall: 'mall:one', member: 'member:one',
