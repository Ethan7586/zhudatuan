...
export function FinanceTabs({ model, onImport }: Readonly<{ model: FinanceNavigationViewModel; onImport?: () => void }>) {
  return (
    <div className="financenavigation">
      ...
      {model.canImport && model.active === 'statements' ? <div className="financecontextactions"><button className="financeimportlink" type="button" onClick={onImport ?? model.importStatement}>导入账单</button></div> : null}
      <details className="financegovernance" open={model.governance.some((item) => item.key === model.active)}>
        ...
        <div role="navigation" aria-label="财务二级治理工具">
          {model.governance.map((item) => <button key={item.key} type="button" aria-current={item.key === model.active ? 'page' : undefined} onClick={() => model.select(item.route)}><strong>{item.label}</strong><span>{item.description}</span></button>)}
        </div>
      </details>
    </div>
  );
