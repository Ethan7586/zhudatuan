...
export class VoucherProductReference implements ProductReference {
  constructor(
    private readonly customers: VoucherCustomerPort,
    private readonly qualifications: Pick<CheckoutQualificationPort, 'policies'>
  ) {}
  ...
  async validate(context: Parameters<ProductReference['validate']>[0], scope: string, customer: string, qualification: string): Promise<void> {
    const [approved, policies] = await Promise.all([this.customers.approved(context, customer, scope), this.qualifications.policies(context, scope)]);
