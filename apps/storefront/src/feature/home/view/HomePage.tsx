      ...
      <div className="mx-auto grid max-w-[1280px] gap-4 lg:grid-cols-[220px_1fr_260px]">
        ...
        <div className="order-1 min-w-0 space-y-4 lg:order-2">
          <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-brand-ink via-brand to-brand-dark p-6 text-inverse shadow-xl sm:p-8">
            ...
            <button type="button" onClick={() => viewmodel.navigatePage('catalog')} className="relative mt-6 inline-flex min-h-11 items-center gap-2 rounded-full bg-warning px-5 font-black text-content">
              开始选购
              <ArrowRight size={17} />
            </button>
          </section>
              ...
              <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
                {products.map((product) => (
                  <article key={product.id} className="group overflow-hidden rounded-2xl border border-edge bg-surface shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <button type="button" onClick={() => viewmodel.openProduct(product.id)} className="block w-full text-left">
