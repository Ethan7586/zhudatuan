...
describe('runtime job execution recovery', () => {
  it('turns a crashed processor into a retry without losing the claimed receipt', async () => {
    ...
    const runner = new RunJob(transactionManager(async () => result([])), queue, deadletters, config(), { assert: vi.fn() });
    await runner.execute('catalogimport', { process: async () => { throw new Error('DATABASE_UNAVAILABLE'); } }, controller.signal);
    expect(queue.fail).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ id: 'job:one', token: 7 }),
      'worker-one', false, expect.any(Number), 'DATABASE_UNAVAILABLE');
  ...
  it('records permanent processor corruption once before deadletter settlement', async () => {
    ...
    const runner = new RunJob(transactionManager(async () => result([])), queue, deadletters, config(), { assert: vi.fn() }, domainDeadletter);
    await runner.execute('catalogimport', { process: async () => { throw new Error('JOB_PAYLOAD_INVALID'); } }, controller.signal);
...
function queuePort(controller: AbortController, claimed: ClaimedJob = job()): JobQueuePort & { fail: ReturnType<typeof vi.fn> } {
  ...
  return {
    ...
    complete: vi.fn(async () => { controller.abort(); }),
    heartbeat: vi.fn(async () => true),
    rejectAuthorization: vi.fn(async () => { controller.abort(); }),
    fail: vi.fn(async () => { controller.abort(); }),
  };
}

function job(): ClaimedJob {
  return Object.freeze({ id: 'job:one', kind: 'catalogimport', scope: 'scope:one', payload: { import: 'import:one' }, attempts: 1, token: 7,
...
function config(): JobRunnerConfig {
  return Object.freeze({ worker: 'worker-one', workload: 'jobs', owner: 'catalog', queue: 'import', batch: 1, lease: 5,
    concurrency: 1, attempts: 8, poll: 1, deadline: 10_000, retryMinimum: 10, retryMaximum: 100 });
}
