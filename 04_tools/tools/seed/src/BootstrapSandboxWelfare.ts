...
try {
  ...
  const result = await database.query<{ welfare: Readonly<Record<string, unknown>> }>(
    'select deployment.sandbox_member_welfare_bootstrap($1,$2,$3,$4,$5) welfare',
    [environment.sentinel, environment.membership, environment.amountMinor, environment.currency, environment.confirmation],
  );
  const welfare = result.rows[0]?.welfare;
  if (!welfare || welfare.membership !== environment.membership || welfare.scope !== 'mall-zhudatuan'
    || welfare.amountMinor !== environment.amountMinor || welfare.currency !== 'CNY' || welfare.sandboxOnly !== true
    || typeof welfare.account !== 'string' || typeof welfare.batch !== 'string') {
...
async function assertDatabaseBoundary(database: Client): Promise<void> {
  const result = await database.query<{
    ...
    schema_version: string | null; schema_checksum: string | null; function_allowed: boolean;
    benefit_usage: boolean; finance_usage: boolean;
  ...
  if (!row || row.database_name !== environment.expectedDatabase || row.database_role !== 'zhudatuansandboxbootstrap'
    || row.role_safe !== true || row.sentinel_valid !== true || row.schema_version !== SANDBOX_WELFARE_SCHEMA_VERSION
    || row.schema_checksum !== SANDBOX_WELFARE_SCHEMA_CHECKSUM || row.function_allowed !== true
    || row.benefit_usage !== false || row.finance_usage !== false) {
