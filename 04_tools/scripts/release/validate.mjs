...
if (value.sbom?.path !== 'sbom.cdx.json' || !sha.test(value.sbom?.sha256 ?? '')
  || value.provenance?.path !== 'stage.json' || !sha.test(value.provenance?.sha256 ?? '')) throw new Error('RELEASE_SUPPLY_CHAIN_EVIDENCE_INVALID');
if (Object.keys(value.clients ?? {}).sort().join(',') !== requiredClients.join(',')) throw new Error('RELEASE_CLIENT_SET_INVALID');
...
if (!/^[a-z0-9]{8,64}$/.test(value.rollback?.releaseId ?? '')
  || !/^oss:\/\/[a-z0-9.-]+\/.+/.test(value.rollback?.databaseSnapshot ?? '')
  || !sha.test(value.rollback?.pointerSha256 ?? '')) throw new Error('RELEASE_ROLLBACK_EVIDENCE_INVALID');
