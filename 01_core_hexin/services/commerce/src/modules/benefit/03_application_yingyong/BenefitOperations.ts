...
import { GrantPolicy, planState } from '../02_domain_yewu/policy/GrantPolicy';
...
export function benefitOperations(context: ModuleContext): ModuleOperations {
  ...
  return new ModuleOperations('benefit', pool, context.container.get(AUDIT_SINK), {
    'benefit.accounts.read': async (request, database) => {
      ...
      const result = await database.query(`select account.id,account.kind,account.currency,account.status,account.version,
        ...
        coalesce((select jsonb_agg(jsonb_build_object('id',lot.id,'batch',lot.batch_id,'totalMinor',lot.total_minor,
          'remainingMinor',lot.remaining_minor,'state',lot.state,'effectiveAt',lot.effective_at,'expiresAt',lot.expires_at)
        ...
        from access.membership membership join benefit.account account on account.member_id=membership.member_id
        join benefit.balance balance on balance.account_id=account.id
        where membership.id=$1 and ($2::text is null or (account.kind,account.id)>($2,$3))
        order by account.kind,account.id limit $4`, [access.membership.id, page.sort, page.id, page.fetch]);
      return keysetResult(result, page, 'kind');
    },
    'benefit.ledgers.read': async (request, database) => {
      const access = requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select entry.id,account.id as account,account.kind,account.currency,
        case entry.side when 'credit' then entry.amount_minor else -entry.amount_minor end::float8 as "amountMinor",
        journal.reference_type as "referenceType",journal.reference_id as "referenceId",journal.description,
        journal.posted_at as "occurredAt" from access.membership membership
        join benefit.account account on account.member_id=membership.member_id
        join finance.entry entry on entry.account_id=account.finance_account_id
        join finance.journal journal on journal.id=entry.journal_id and journal.state='posted'
        where membership.id=$1 and ($2::timestamptz is null or (journal.posted_at,entry.id)<($2::timestamptz,$3))
        order by journal.posted_at desc,entry.id desc limit $4`, [access.membership.id, page.sort, page.id, page.fetch]);
      return keysetResult(result, page, 'occurredAt');
    },
    'benefit.plans.read': async (request, database) => {
      requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select plan.id,plan.scope_id,plan.name,plan.kind,plan.currency,plan.state,plan.version,
        ...
        from benefit.plan plan where access.scope_allowed(plan.scope_id) and ($1::text is null or plan.id>$1)
        order by plan.id limit $2`, [page.id, page.fetch]);
    ...
    'benefit.plans.manage': async (request, database) => {
      ...
      const current = await database.query<{ state: string }>('select state from benefit.plan where id=$1 and scope_id=$2 for update',
      [request.input.path.planid!, access.scope.id]);
      ...
      const result = await database.query<{ id: string; name: string; kind: string; currency: string; state: string; version: number }>(
        ...
        returning id,name,kind,currency,state,version::integer`, [request.input.path.planid!, access.scope.id, textField(body, 'name'), kind,
          currency, state, request.input.expectedVersion ?? null]);
      const changed = result.rows[0];
      if (!changed) throw new Error('VERSION_CONFLICT');
      await database.query(`insert into benefit.planversion(plan_id,version,name,kind,currency,state,changed_by,changed_at)
        values($1,$2,$3,$4,$5,$6,$7,clock_timestamp())`, [changed.id, changed.version, changed.name, changed.kind, changed.currency,
        changed.state, access.actor.id]);
      return rowResult(result);
    },
    'benefit.budgets.read': async (request, database) => {
      requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select budget.id,budget.plan_id,plan.name plan_name,budget.period,budget.total_minor::float8 total_minor,
        ...
        from benefit.budget budget join benefit.plan plan on plan.id=budget.plan_id where access.scope_allowed(plan.scope_id)
        and ($1::text is null or budget.id>$1) order by budget.id limit $2`, [page.id, page.fetch]);
      return keysetResult(result, page, 'id');
    },
    'benefit.budgets.manage': async (request, database) => {
      ...
      const result = await database.query(`insert into benefit.budget(id,plan_id,period,total_minor,granted_minor,reserved_minor,version)
        ...
        where benefit.budget.plan_id=excluded.plan_id and benefit.budget.reserved_minor+benefit.budget.granted_minor<=excluded.total_minor
          and ($6::bigint is null or benefit.budget.version=$6) returning *`, [request.input.path.budgetid!, period, total, body.plan,
          access.scope.id, request.input.expectedVersion ?? null]);
    ...
    'benefit.grants.create': async (request, database) => {
      ...
      const plan = await database.query<{ version: number }>(`select plan.version::integer from benefit.plan plan
      ...
      const result = await database.query(`insert into benefit.grantbatch(id,plan_id,budget_id,state,requested_by,requested_count,
      ...
      [id, body.plan, body.budget, access.actor.id, members.length, amount, textField(body, 'reason', 1000), plan.rows[0].version,
        effective.toISOString(), expires.toISOString(), timezone, digest(snapshot)]);
      await database.query(`insert into benefit.grantitem(batch_id,member_id,amount_minor,state)
        select $1,member,$2,'queued' from unnest($3::text[]) member`, [id, amount, members]);
    ...
    'benefit.grants.decide': async (request, database) => {
      ...
      const batch = await database.query<{ id: string; budget_id: string; requested_count: number; amount_minor: number; scope_id: string; requested_by: string }>(
        ...
        and batch.state='submitted' and batch.requested_by<>$3 for update of batch`, [request.input.path.batchid!, access.scope.id, access.actor.id]);
      ...
      if (decision === 'approved') {
        const reserved = await database.query(`update benefit.budget set reserved_minor=reserved_minor+$2,version=version+1 where id=$1
          and total_minor-reserved_minor-granted_minor >= $2 returning id`, [selected.budget_id, total]);
        if (!reserved.rows[0]) throw new Error('BENEFIT_BUDGET_INSUFFICIENT');
      }
      const result = await database.query(`update benefit.grantbatch set state=$2,approved_by=case when $2='approved' then $3 else null end,
        updated_at=clock_timestamp() where id=$1 returning *`, [selected.id, decision, access.actor.id]);
      await database.query(`insert into benefit.grantdecision(batch_id,sequence,decision,actor_id,membership_id,reason,evidence,trace_id,occurred_at)
    ...
    'benefit.grants.read': async (request, database) => {
      requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select batch.*,plan.name plan_name,budget.period,
        ...
        where access.scope_allowed(plan.scope_id) and ($1::timestamptz is null or (batch.created_at,batch.id)<($1::timestamptz,$2))
    ...
    'benefit.grants.control': async (request, database) => {
      ...
      const current = await database.query<{ state: string }>(`select batch.state from benefit.grantbatch batch join benefit.plan plan on plan.id=batch.plan_id
        where batch.id=$1 and plan.scope_id=$2 for update of batch`, [batchid, access.scope.id]);
      ...
      let result;
      if (action === 'pause') result = await database.query(`update benefit.grantbatch batch set state='paused',pause_reason=$3,updated_at=clock_timestamp()
      ...
      [batchid, access.scope.id, reason]);
        ...
        result = await database.query(`update benefit.grantbatch batch set state=case when exists(select 1 from benefit.grantitem item where item.batch_id=batch.id
          ...
          where batch.id=$1 and plan.id=batch.plan_id and plan.scope_id=$2 and batch.state='paused' returning batch.*`, [batchid, access.scope.id]);
        if (result.rows[0]) await enqueue(database, access.scope.id, { kind: 'benefitgrant', batch: batchid });
      } else if (action === 'cancel') {
        result = await database.query<{ id: string; budget_id: string }>(`update benefit.grantbatch batch set state='cancelled',pause_reason=$3,updated_at=clock_timestamp()
          from benefit.plan plan where batch.id=$1 and plan.id=batch.plan_id and plan.scope_id=$2
          and batch.state in('approved','paused','scheduled') returning batch.id,batch.budget_id`, [batchid, access.scope.id, reason]);
    ...
    'benefit.grants.revoke': async (request, database) => {
      ...
      const body = bodyRecord(request);
      const batchid = request.input.path.batchid!;
      const result = await database.query<{ requested_by: string }>(`update benefit.grantbatch batch set state='revoking',updated_at=clock_timestamp()
        from benefit.plan plan where batch.id=$1 and plan.id=batch.plan_id and plan.scope_id=$2
    ...
    'benefit.lots.read': async (request, database) => {
      requireAccess(request);
      const page = queryPage(request);
      const result = await database.query(`select lot.*,account.kind,account.currency,account.scope_id,
        ...
        from benefit.lot lot join benefit.account account on account.id=lot.account_id where access.scope_allowed(account.scope_id)
        and ($1::text is null or lot.id>$1) order by lot.id limit $2`, [page.id, page.fetch]);
      return keysetResult(result, page, 'id');
    },
  });
}

async function cancelUnexecuted(database: Queryable, batch: string, budget: string) {
  const released = await database.query(`with changed as(update benefit.grantitem set state='skipped',error_code='BATCH_CANCELLED'
    where batch_id=$1 and state in('queued','scheduled') returning amount_minor)
    select coalesce(sum(amount_minor),0)::float8 amount from changed`, [batch]);
...
async function actionRecord(database: Queryable, batch: string, action: string, actor: string, reason: string, evidence: Readonly<Record<string, unknown>>) {
  await database.query(`insert into benefit.action(id,batch_id,action,actor_id,reason,evidence,occurred_at)
    values($1,$2,$3,$4,$5,$6::jsonb,clock_timestamp())`, [`action:${randomUUID()}`, batch, action, actor, reason, JSON.stringify(evidence)]);
}

async function enqueue(database: Queryable, scope: string, payload: unknown, id = `job:${randomUUID()}`) {
  await database.query(`insert into runtime.job(id,kind,owner,scope_id,payload,state,priority,available_at,created_at,updated_at)
    values($1,'benefitgrant','benefit',$2,$3::jsonb,'queued',20,clock_timestamp(),clock_timestamp(),clock_timestamp()) on conflict(id) do nothing`,
  [id, scope, JSON.stringify(payload)]);
}

interface Queryable { query(text: string, values?: readonly unknown[]): Promise<{ readonly rows: readonly Record<string, unknown>[] }> }

function memberSnapshot(value: unknown): readonly string[] {
  if (!Array.isArray(value) || value.length === 0 || value.length > 100_000 || value.some((member) => typeof member !== 'string' || member.length === 0)) {
    throw new Error('VALIDATION_FAILED:members');
  }
...
function instant(value: unknown, field: string): Date {
  if (typeof value !== 'string') throw new Error(`VALIDATION_FAILED:${field}`);
  const parsed = new Date(value);
  if (!Number.isFinite(parsed.valueOf())) throw new Error(`VALIDATION_FAILED:${field}`);
  return parsed;
}
