...
describe('voucher approval boundaries', () => {
  it('declares approval configuration and concurrent decision failures in the caller contracts', () => {
    for (const operation of ['voucher.stockrequests.submit', 'voucher.issueorders.submit'] as const) expect(OperationCatalog.get(operation).errorUnion).toContain('APPROVAL_TEMPLATE_DISABLED');
    for (const operation of ['voucher.stockrequests.cancel', 'voucher.issueorders.cancel'] as const) expect(OperationCatalog.get(operation).errorUnion).toContain('APPROVAL_INSTANCE_CONFLICT');
  });
  ...
  describe.each(['stock', 'issue'] as const)('%s request cancellation', (kind) => {
    it.each(['approved', 'rejected', 'cancelled', 'expired', null])('rejects a %s approval even while its business projection remains submitted', async (state) => {
      ...
      await expect(withWriteTransaction(query, (context) => execute(kind, { cancel } as unknown as ApprovalPort, { read } as unknown as ApprovalReadPort, context))).rejects.toThrow('VOUCHER_STATE_INVALID');
    ...
    it('rolls back cancellation if a decision wins the version race', async () => {
      ...
      const query = vi.fn(async () => result([row()]));
      await expect(withWriteTransaction(query, (context) => execute(kind, { cancel } as unknown as ApprovalPort, { read } as unknown as ApprovalReadPort, context))).rejects.toThrow('VERSION_CONFLICT');
  ...
  it.each(['stock', 'issue'] as const)('submits and cancels %s through the real Approval port using membership, not principal identity', async (kind) => {
    ...
    expect(data.create).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        requesterId: 'membership:test',
        subjectKind: kind === 'stock' ? 'voucherstock' : 'voucherissue',
        subjectVersion: 1,
        amountMinor: kind === 'stock' ? null : 1000,
        currency: kind === 'stock' ? null : 'CNY',
        constraints: expect.objectContaining({ quantity: 1 }),
      })
    );
...
function approvalFixture(kind: 'stock' | 'issue') {
  ...
  const query = vi.fn(async (sql: string, parameters?: readonly unknown[]) => {
    ...
    if (sql.startsWith('select product.id product'))
      return result([
        { product: 'product:one', productVersion: 3, pool: 'pool:one', faceMinor: 1000, currency: 'CNY', qualification: 'qualification:one', activation: 'secret', startsAt: new Date('2026-01-01'), expiresAt: new Date('2099-01-01') },
      ]);
    if (sql.startsWith('update voucher.') && sql.includes('approval_instance_id=')) {
      current.state = 'submitted';
      current.approval_instance_id = String(parameters![2]);
      current.version = Number(parameters![3]);
    }
    return result([{ ...current, available: 100, approved: 100 }]);
  });
  const call = (transaction: Parameters<Parameters<typeof withWriteTransaction>[1]>[0]) => ({
    input: {
      path: { requestid: 'stock:one', orderid: 'issue:one' },
      body: { reason: '取消申请' },
    },
    context: { transaction },
    actor: 'actor:test',
...
function execute(kind: 'stock' | 'issue', approval: ApprovalPort, read: ApprovalReadPort, transaction: Parameters<Parameters<typeof withWriteTransaction>[1]>[0]): Promise<unknown> {
  const call = { input: { path: { orderid: 'issue:one', requestid: 'stock:one' }, body: { reason: '取消申请' } }, context: { transaction }, scope: value.scope, actor: value.requester, expectedVersion: 2, now: new Date('2026-09-05') };
  return kind === 'stock'
...
function row() {
  return {
    ...
    stock_request_id: value.stockRequest,
    pool_id: 'pool:one',
    ...
    recipient_snapshot: value.recipientSnapshot,
    reason: value.reason,
    requested_by: value.requester,
    approval_instance_id: value.approval,
    state: value.state,
