    ...
    >
      ...
      >
        ...
        <label>
          服务商
          {action.kind === 'update' ? (
            <strong>{channelProviders.find(({ id }) => id === model.provider)?.label ?? model.provider}</strong>
          ) : (
        ...
        <div className="channelgrid">
          <label>
            部署区域
            <input value={model.region} onChange={(event) => model.actions.region(event.target.value)} placeholder="cn" required />
          </label>
          <label>
            健康检查操作
            <input value={model.healthOperation} onChange={(event) => model.actions.healthOperation(event.target.value)} readOnly={supplier} required />
          </label>
        </div>
        {!supplier ? (
          <>
            <label>
              服务地址
              <input type="url" value={model.baseUrl} onChange={(event) => model.actions.baseUrl(event.target.value)} placeholder="https://provider.example.com" autoComplete="off" required />
            </label>
            <label>
              密钥引用
              <input value={model.secretRef} onChange={(event) => model.actions.secretRef(event.target.value)} placeholder="vault:channel/credential" autoComplete="off" spellCheck={false} required />
              <small>填写密钥管理系统中的引用；禁止粘贴 API Key、口令或私钥原文。</small>
            </label>
            <label>
              操作端点 JSON
              <textarea value={model.endpoints} onChange={(event) => model.actions.endpoints(event.target.value)} spellCheck={false} rows={7} required />
              <small>键为服务商操作名，值必须是“/”开头的相对路径；至少包含健康检查操作。</small>
            </label>
          </>
        ) : (
          <p className="channelnotice">自有供应商使用本地适配器，无远程地址、端点和密钥。</p>
        )}
