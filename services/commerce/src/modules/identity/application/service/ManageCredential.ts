...
export class ManageCredential {
  ...
  change(): OperationLifecycle<PasswordChange> {
    return operationLifecycle({
      ...
      execute: async (_request, database, value) => {
        const credential = await this.credentials.password(requireWriteTransaction(database), value.actor.actor.id);
  ...
  verify(): OperationAction {
    return async (request, database) => {
      ...
      const credential = await this.credentials.password(requireWriteTransaction(database), actor.actor.id);
      if (!(await this.passwords.verify(textField(bodyRecord(request.input), 'password', 128), credential?.secretHash ?? null))) reject('CREDENTIAL_INVALID');
      const verifiedAt = new Date().toISOString();
      await this.assurances.record(requireWriteTransaction(database), { principal: actor.actor.id, method: 'password', level: 2, evidenceHash: this.digest(actor.actor.session), expiresIn: '10minutes' });
  ...
  reset(): OperationLifecycle<PasswordReset> {
    return operationLifecycle({
      ...
      execute: async (request, database, value) => {
        const consumed = await this.challenges.consume(requireWriteTransaction(database), value.challenge, textField(value.body, 'code'), (id, code) => this.code(id, code), undefined, { purpose: 'password_reset' });
  ...
  mobile(): OperationLifecycle<MobileChange> {
    return operationLifecycle({
      ...
      execute: async (_request, database, value) => {
        ...
        const result = await this.members.changeMobile(requireWriteTransaction(database), value.actor.actor.id, value.envelope.ciphertext, value.envelope.fingerprint, mask(value.mobile));
        await this.assurances.expire(requireWriteTransaction(database), value.actor.actor.id, 'phone_otp');
        await this.assurances.record(requireWriteTransaction(database), { principal: value.actor.actor.id, method: 'phone_otp', level: 2, evidenceHash: this.digest(value.mobile), expiresIn: '365days' });
