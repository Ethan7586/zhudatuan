...
export class PgEnrollmentRepository implements EnrollmentRepository {
  ...
  async findPrincipal(context: ReadTransactionContext, subjectHash: string): Promise<string | null> {
  ...
  async createPassword(context: WriteTransactionContext, principal: string, subjectHash: string, secretHash: string): Promise<void> {
    const database = this.transactions.database(context);
    await database.query(
