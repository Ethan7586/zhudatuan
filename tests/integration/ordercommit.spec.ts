...
test('order commit has one fixed lock order and returns order plus prepared payment in one use case', () => {
  assert.deepEqual(CHECKOUT_LOCK_ORDER, ['idempotency', 'quote', 'cart', 'inventory', 'voucher', 'marketing', 'benefit', 'order', 'payment', 'finance', 'audit', 'outbox']);
...
test('same checkout and same idempotency identity cannot create two orders', () => {
  ...
  const runtime = text('../../services/commerce/src/platform/database/PgIdempotencyRepository.ts');
