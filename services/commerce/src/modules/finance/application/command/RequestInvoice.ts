...
const requestInvoice: NonNullable<OperationActions['invoice.requests.create']> = async (request, database) => {
  ...
  const eligible = await database.query<InvoiceLine>(`select line.id,line.source_type,line.source_id,line.invoice_minor::float8 amount_minor,
    ...
    order by line.id for update of settlement,line`, [settlement, access.scope.id, lines]);
  ...
  const sourceHash = createHash('sha256').update(selected.map((line) => `${line.id}:${line.amount_minor}:${line.tax_minor}`).join(',')).digest('hex');
  const result = await database.query(`insert into invoice.request(id,profile_id,settlement_id,amount_minor,currency,state,created_at,version,
    ...
    and profile.owner_id=$9 and profile.status='active' and settlement.scope_id=$9 returning *`, [id, settlement, amount, access.actor.id,
    textField(body, 'reason', 1000), JSON.stringify(record(body.evidence)), sourceHash, body.profile, access.scope.id]);
...
async function decideInvoice(request: OperationRequest,
  database: Parameters<FinanceRepositoryFactory>[0], repository: FinanceRepositoryFactory) {
  ...
  const result = await database.query(`update invoice.request request set state=$2,approved_by=case when $2='approved' then $3 else null end,
...
const redInvoice: NonNullable<OperationActions['invoice.requests.red']> = async (request, database) => {
  ...
  await database.query(`insert into invoice.requestprofile(request_id,owner_id,title_ciphertext,title_key_version,taxid_ciphertext,taxid_token,
    ...
    from invoice.requestprofile where request_id=$2`, [id, request.input.path.requestid!]);
  await database.query(`insert into invoice.requestline(id,request_id,settlement_line_id,kind,amount_minor,tax_minor,source_hash)
    select 'invoiceline:'||$1||':'||settlement_line_id,$1,settlement_line_id,'red',amount_minor,tax_minor,source_hash
    from invoice.requestline where request_id=$2 and kind='original'`, [id, request.input.path.requestid!]);
  await database.query(`insert into invoice.line(request_id,sequence,description,amount_minor,tax_minor,source_line_id)
    select $1,sequence,'红冲：'||description,amount_minor,tax_minor,source_line_id from invoice.line where request_id=$2`, [id, request.input.path.requestid!]);
  await database.query(`insert into invoice.statusevent(request_id,sequence,state,occurred_at) values($1,1,'submitted',clock_timestamp())`, [id]);
  return rowResult(result, 201);
};

interface InvoiceLine { readonly id: string; readonly source_type: string; readonly source_id: string;
  readonly amount_minor: number; readonly tax_minor: number }
