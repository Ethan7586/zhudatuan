...
declare created jsonb; updated jsonb; disabled jsonb; custom_role_id text; before_version bigint; after_version bigint;
  ...
  low_user_id text; low_member_id text; low_membership_id text; low_role_id text;
...
begin
  low_user_id:='user-role-low-'||suffix;
  low_member_id:='member-role-low-'||suffix;
  low_membership_id:='membership-role-low-'||suffix;
  low_role_id:='role-low-'||suffix;
  insert into public.users(
    id,tenant_id,enterprise_id,employee_no,display_name,identity_subject,status
  ) values(
    low_user_id,'tenant-smart-wing','enterprise-demo','ROLE-LOW-'||suffix,
    '低权限角色契约用户','contract:role-low:'||suffix,'active'
  );
  insert into public.members(id,user_id,primary_identifier,status)
  values(low_member_id,low_user_id,'contract:role-low:'||suffix,'active');
  insert into public.memberships(
    id,member_id,context_user_id,tenant_id,enterprise_id,mall_id,target,status
  ) values(
    low_membership_id,low_member_id,low_user_id,'tenant-smart-wing',
    'enterprise-demo','mall-demo','admin','active'
  );
  insert into public.membership_scopes(membership_id,scope_kind,resource_id)
  values(low_membership_id,'mall','mall-demo');
  insert into public.roles(
    id,tenant_id,code,name,description,is_system,is_owner,is_editable,sort_order,status
  ) values(
    low_role_id,'tenant-smart-wing','contract_low_'||suffix,'低权限角色',
    '仅用于验证不能超出操作者权限',false,false,true,900,'active'
  );
  insert into public.role_permissions(role_id,permission_id)
  select low_role_id,permission.id from public.permissions permission
  where permission.code='role.create';
  insert into public.membership_roles(
    membership_id,role_id,granted_by_membership_id
  ) values(low_membership_id,low_role_id,'membership-test-owner-admin');
  ...
  begin
    perform public.api_create_custom_role(
      low_membership_id,low_user_id,'tenant-smart-wing','enterprise-demo','mall-demo',
