...
export class OrderPort extends OrderAfterSalePort {
  async purchases(database: OperationDatabase, member: string) {
  ...
  async create(database: OperationDatabase, input: CreateOrderIntent): Promise<Readonly<{ number: string; record: Record<string, unknown> }>> {
    const number = await this.number(database);
  ...
  async scheduleExpiry(database: OperationDatabase, order: string, scope: string): Promise<void> {
    await database.query(
      `insert into runtime.job(id,kind,owner,scope_id,payload,state,priority,available_at,created_at,updated_at)
      values($1,'orderexpiry','order',$2,jsonb_build_object('order',$3::text),'queued',10,
        clock_timestamp()+interval '30 minutes',clock_timestamp(),clock_timestamp())`,
      [`job:expiry:${order}`, scope, order]
    );
  }
  ...
  async expirable(database: OperationDatabase, orders: readonly string[]) {
    if (orders.length === 0) return Object.freeze([]);
    const result = await database.query<{ id: string; scope: string }>(
  ...
  async completeFulfillment(database: OperationDatabase, order: string, lines: readonly Readonly<{ line: string; quantity: number }>[]): Promise<void> {
  ...
  async snapshot(database: OperationDatabase, order: string) {
    const result = await database.query<{ id: string; scope: string; member: string }>(`select id,scope_id scope,member_id member from ordering.orderrecord where id=$1`, [order]);
    const row = result.rows[0];
    return row ? Object.freeze(row) : null;
  }

  async fulfillment(database: OperationDatabase, order: string) {
    const result = await database.query<{
      suborder: string;
  ...
  async lineSkus(database: OperationDatabase, order: string, lines: readonly string[]) {
    if (lines.length === 0) return Object.freeze([]);
    const result = await database.query<{ line: string; sku: string }>(`select id line,sku_id sku from ordering.line where order_id=$1 and id=any($2::text[]) order by id`, [order, lines]);
    return Object.freeze(result.rows.map((row) => Object.freeze(row)));
  }

  private async number(database: OperationDatabase): Promise<string> {
    const row = (
