...
describe('server-driven navigation tree', () => {
  it('renders server order, labels, disabled state and routes', async () => {
    ...
    render(
      <NavigationTree
        ...
        nodes={[
          ...
          node('groupreferral', '分销与返佣', 'referral', 'consolereferral', '/scopes/:scopeKind/:scopeId/referral', 'referral', 25),
          node('groupreporting', '数据报表', 'report', 'consolereporting', '/scopes/:scopeKind/:scopeId/reporting', 'reporting', 30, true),
          node('groupsupport', '客服中心', 'support', 'consolesupport', '/scopes/:scopeKind/:scopeId/support', 'support', 40),
        ]}
    ...
    expect(labels).toEqual(['築店 · 商城管理', '商品治理台', '分销与返佣', '数据报表', '客服中心']);
  ...
  it('expands only the active branch and marks the exact active page', () => {
    ...
    const settings = node('groupsettings', '设置', 'settings', 'consolesettings', '/scopes/:scopeKind/:scopeId/settings', 'settings', 90, false, null, 'primary', [members, notices, runtime]);
    ...
    render(<NavigationTree active="groupmembers" collapsed={false} displayName="商城管理员" roleLabel="集团 · 鸿泰" nodes={[product, settings]} onNavigate={vi.fn()} onDismiss={vi.fn()} onToggle={vi.fn()} />);
    ...
    expect(screen.getByRole('group', { name: '组织与人员' })).not.toBeNull();
    expect(screen.getByRole('group', { name: '连接' })).not.toBeNull();
