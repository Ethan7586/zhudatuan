...
const routerFile = join(root, '01_core_hexin/apps/console/src/route/ConsoleRouter.tsx');
const workstationFile = join(root, '01_core_hexin/apps/console/src/shell/Workstation.ts');
const professionalFile = join(root, '01_core_hexin/apps/console/src/route/ProfessionalRouteCatalog.ts');
...
const miniapp = JSON.parse(readFileSync(join(root, '01_core_hexin/apps/miniapp/miniprogram/app.json'), 'utf8'));
const actionDispatcher = join(root, '01_core_hexin/apps/miniapp/miniprogram/navigation/actions.js');
const violations = [];
...
if (!routePaths.includes('/scopes/:scopeKind/:scopeId')) {
  violations.push({ code: 'CONSOLE_SCOPE_ROUTE_MISSING', location: '01_core_hexin/apps/console/src/route/ConsoleRouter.tsx', detail: 'URL scope root' });
}
...
if (/workstations|ResourceBoard|features\/workstation/.test(routes)) {
  violations.push({ code: 'CONSOLE_RETIRED_ROUTE_REGISTRY', location: '01_core_hexin/apps/console/src/route/ConsoleRouter.tsx', detail: 'hard-cut semantic router required' });
}

for (const page of miniapp.pages ?? []) {
  for (const extension of ['.js', '.json', '.wxml']) {
    const file = join(root, '01_core_hexin/apps/miniapp/miniprogram', `${page}${extension}`);
    if (!existsSync(file)) violations.push({ code: 'MINIAPP_PAGE_FILE_MISSING', location: page, detail: extension });
  }
}
if (!existsSync(actionDispatcher)) violations.push({ code: 'EXPERIENCE_ACTION_DISPATCHER_MISSING', location: '01_core_hexin/apps/miniapp', detail: 'seven registered actions' });
else {
