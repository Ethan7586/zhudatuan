...
export class MigrationRunner {
  ...
  async run(phase: MigrationPhase): Promise<void> {
    ...
    try {
      ...
      await this.assertPrerequisites(files, historyHead, phase, applied);
      for (const file of files.filter((candidate) => phaseOfMigration(candidate, historyHead) === phase)) {
      ...
      await this.assertPhaseComplete(files, historyHead, phase, applied);
      if (phase === 'retire') {
        await client.query('reset role');
        await client.query('alter role shopmigration nologin noinherit nosuperuser nocreatedb nocreaterole noreplication nobypassrls');
        await client.query('set role shopmigration');
  ...
  private async assertFinalRole(client: PoolClient): Promise<void> {
    const result = await client.query<{ readonly valid: boolean }>(
      `select exists(select 1 from pg_roles where rolname=current_user and rolname='shopmigration' and not rolcanlogin and not rolinherit
        and not rolsuper and not rolcreatedb and not rolcreaterole and not rolreplication and not rolbypassrls) valid`
    );
  ...
  private async assertPrerequisites(files: readonly string[], historyHead: string, phase: MigrationPhase, applied: ReadonlySet<string>): Promise<void> {
    const phaseIndex = MIGRATION_PHASES.indexOf(phase);
    const missing = files.filter((file) => MIGRATION_PHASES.indexOf(phaseOfMigration(file, historyHead)) < phaseIndex && !applied.has(file.slice(0, 14)));
    if (missing.length > 0) throw new Error(`MIGRATION_PHASE_PREREQUISITE_MISSING:${phase}:${missing[0]}`);
  }

  private async assertPhaseComplete(files: readonly string[], historyHead: string, phase: MigrationPhase, applied: ReadonlySet<string>): Promise<void> {
    const missing = files.filter((file) => phaseOfMigration(file, historyHead) === phase && !applied.has(file.slice(0, 14)));
    if (missing.length > 0) throw new Error(`MIGRATION_PHASE_INCOMPLETE:${phase}:${missing[0]}`);
  }
