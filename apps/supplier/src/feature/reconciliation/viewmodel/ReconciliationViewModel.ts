...
export const reconciliationViewModel = defineSupplierViewModel({
  routes: ['supplierreconciliation'], title: '对账差异', description: '查看当前供应商对账差异及平台处理状态；复核和审批决定由平台完成。',
  ...
  project: (value) => operatorCollection(value, operatorItems(value).map((item) => operatorRow({
    key: operatorText(item, 'id'), title: `${operatorText(item, 'provider')} · ${operatorText(item, 'period')}`,
    detail: `借方 ${money(item, 'debit_minor')} · 贷方 ${money(item, 'credit_minor')} · 差额 ${money(item, 'difference_minor')}`,
    statusLabel: reconciliationStatus(operatorText(item, 'state')), timestamp: operatorText(item, 'updated_at'),
  }))),
});

function money(item: Parameters<typeof operatorText>[0], key: string): string { const value = operatorNumber(item, key); return value === undefined ? '待同步' : `¥${(value / 100).toFixed(2)}`; }
