...
describe('PublicActorFingerprint', () => {
  ...
  it('isolates target, device, network prefix and credential without exposing them', () => {
    const actor = fingerprint.create(operation, { body: { code: 'private-invitation-code', target: 'storefront' } }, { 'x-client-target': 'storefront', 'x-device-id': 'private-device', 'x-peer-address': '2001:db8:1:2::10' });
    ...
    expect(actor).not.toBe(fingerprint.create(operation, { body: { code: 'other-code', target: 'storefront' } }, { 'x-client-target': 'storefront', 'x-device-id': 'private-device', 'x-peer-address': '2001:db8:1:2::10' }));
  });
