...
export class PgCartPublicPort implements CartReadPort, CartWritePort {
  ...
  async current(context: ReadTransactionContext, member: string, mall: string): Promise<CartSnapshot> {
    ...
    const selected = await database.query<{
      id: string;
    }>(`select id from cart.cart where member_id=$1 and mall_id=$2 and state='active' order by updated_at desc,id limit 1`, [member, mall]);
  ...
  async read(context: ReadTransactionContext, cart: string, member: string, mall: string): Promise<CartSnapshot> {
    ...
    const selected = await database.query<{
      ...
      version: number;
    }>(`select id,application_id,version::integer from cart.cart where id=$1 and member_id=$2 and mall_id=$3 and state='active'`, [cart, member, mall]);
    ...
    const items = await database.query<{
      ...
      version: number;
    }>(`select listing_id,sku_id,title_snapshot,quantity::integer,listing_version,unit_minor::float8 unit_minor,currency,price_version,version::integer from cart.item where cart_id=$1 order by listing_id`, [row.id]);
