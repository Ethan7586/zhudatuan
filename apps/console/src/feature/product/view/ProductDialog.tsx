    ...
    <div className="productflowoverlay">
      ...
      <form className="productflowdialog" aria-label={actionTitle(action.kind)} onSubmit={submit}>
        ...
        <div className="productflowbody">
          {action.kind === 'create' || action.kind === 'edit' ? (
            <>
              <label>
                商品名称
                <input value={viewmodel.title} onChange={(event) => viewmodel.setTitle(event.target.value)} required maxLength={160} />
              </label>
              <label>
                商品分类
                <input value={viewmodel.category} onChange={(event) => viewmodel.setCategory(event.target.value)} required maxLength={120} />
              </label>
              {action.kind === 'create' ? (
                <label>
                  商品类型
                  <select value={viewmodel.type} onChange={(event) => viewmodel.setType(event.target.value as ProductActionViewModel['type'])}>
              ...
              {action.kind === 'edit' ? (
                <label>
                  商品状态
                  <select value={viewmodel.status} onChange={(event) => viewmodel.setStatus(event.target.value as ProductActionViewModel['status'])}>
          ...
          {action.kind === 'price' ? (
            <label>
              销售价（元）
              <input type="number" min="0.01" max="999999.99" step="0.01" value={viewmodel.amount} onChange={(event) => viewmodel.setAmount(event.target.value)} required />
            </label>
          ) : null}
        ...
        <footer>
          ...
          <button className="productactionprimary" type="submit" disabled={viewmodel.submitting}>
            {viewmodel.submitting ? '正在提交…' : actionSubmit(action.kind)}
          </button>
        </footer>
      </form>
    </div>
