...
export class SupportJobProcessor implements JobProcessor {
  ...
  private async scan(id: string): Promise<void> {
    const selected = await this.pool.query<{ object_ref: string; sha256: string; size_bytes: number; kind: string }>(
      `select object_ref,
      sha256,size_bytes::float8 size_bytes,kind from support.evidence where id=$1 and state='pending'`,
      [id]
    );
    ...
    let valid = false;
    try {
      const metadata = await this.objects.inspect(item.object_ref);
      valid =
        metadata.sha256 === item.sha256 &&
        metadata.size === item.size_bytes &&
        metadata.contentType === item.kind &&
  ...
  private async escalate(ticket: string, reason: 'response' | 'resolution'): Promise<void> {
    ...
    try {
      ...
      const created = await client.query<{ id: string; scope_id: string; member_id: string | null }>(
        `with eligible as(
      ...
      if (escalation) {
        ...
        if (escalation.member_id)
          ...
          jsonb_build_object('ticket',$2::text,'member',$4::text,'reason',$5::text,'escalation',$6::text),$1,clock_timestamp(),clock_timestamp())`,
