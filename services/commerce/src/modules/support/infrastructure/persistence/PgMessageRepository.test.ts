...
describe('PgMessageRepository', () => {
  it('writes the append-only message before deterministically linking clean evidence', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      calls.push(sql);
      if (sql.includes('insert into support.message(')) return result([{ id: 'message:one', client_message_id: 'client:0001', conversation_id: 'conversation:one', author_type: 'agent', author_id: 'actor:one', kind: 'attachment', visibility: 'external', body_hash: 'a'.repeat(64), sequence: 3, version: 1, created_at: '2026-09-02T00:00:00.000Z' }]);
      return result([]);
    });
    const stored = await withWriteTransaction(query, (context) => new PgMessageRepository().append(context, { scope: 'mall:one', conversation: 'conversation:one', authorType: 'agent', authorId: 'actor:one', kind: 'attachment', visibility: 'external', sequence: 3, message: { id: 'message:one', clientMessageId: 'client:0001', ciphertext: 'encrypted-message', fingerprint: 'a'.repeat(64), keyVersion: 'v1', body: '不会进入 SQL' }, attachments: ['evidence:two', 'evidence:one'] }));
  ...
  it('finds idempotent replay by conversation, author and client message id', async () => {
    const query = vi.fn(async () => result([{ id: 'message:one', client_message_id: 'client:0001', conversation_id: 'conversation:one', author_type: 'member', author_id: 'actor:one', kind: 'text', visibility: 'external', body_hash: 'b'.repeat(64), sequence: 8, version: 1, created_at: '2026-09-02T00:00:00.000Z' }]));
