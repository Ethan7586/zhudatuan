...
export class RedactionPolicy {
  redact(value: unknown): unknown { return redact(value, '', 0); }
}
...
function redact(value: unknown, key: string, depth: number): unknown {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).slice(0, 100).map(([name, item]) => [name, redact(item, name, depth+1)]));
}

function minimize(key: string, value: string): string {
  if (PHONE.test(key)) return value.length > 4 ? `${value.slice(0, 2)}***${value.slice(-2)}` : '***';
  if (EMAIL.test(key)) { const separator = value.lastIndexOf('@'); return separator > 0 ? `***${value.slice(separator)}` : '***'; }
