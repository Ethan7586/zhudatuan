...
export function useChannelActionViewModel(action: ChannelAction | null, context: ConsoleContext, dependencies: ChannelDependencies, requestStepup: () => void, done: (command: ChannelCommand) => void) {
  ...
  const initialProvider = action?.kind === 'update' ? action.connection.provider : (channelProviders[0]?.id ?? '');
  const initialRegion = action?.kind === 'update' ? action.connection.region : 'cn';
  const initialHealthOperation = action?.kind === 'update' && action.connection.provider === 'supplier' ? 'local' : 'health';
  const initialConnection = action?.kind === 'startsync' ? (action.connection?.id ?? '') : '';
  useEffect(() => {
    setProvider(initialProvider);
    setRegion(initialRegion);
    setBaseUrl('');
    setHealthOperation(initialHealthOperation);
    setEndpoints('{\n  "health": "/health"\n}');
    setSecretRef('');
  ...
  }, [actionIdentity, dependencies, initialConnection, initialHealthOperation, initialProvider, initialRegion]);
  const endpointResult = useMemo(() => parseEndpoints(endpoints), [endpoints]);
  const draft = useMemo<ConnectionDraft>(
    () => Object.freeze({ provider, region: region.trim(), ...(baseUrl.trim() ? { baseUrl: baseUrl.trim() } : {}), healthOperation: healthOperation.trim(), endpoints: endpointResult.value, secretRef: secretRef.trim() }),
    [baseUrl, endpointResult.value, healthOperation, provider, region, secretRef]
  );
  ...
  const chooseProvider = (value: string) => {
    setProvider(value);
    setHealthOperation(value === 'supplier' ? 'local' : 'health');
    setBaseUrl('');
    setEndpoints(value === 'supplier' ? '{}' : '{\n  "health": "/health"\n}');
    setSecretRef('');
    setConfirmed(false);
    setIdentity(dependencies.createIdentity());
  };
  return Object.freeze({
    ...
    actions: Object.freeze({
      ...
      region: reset(setRegion),
      baseUrl: reset(setBaseUrl),
      healthOperation: reset(setHealthOperation),
      endpoints: reset(setEndpoints),
      secretRef: reset(setSecretRef),
...
function validate(action: ChannelAction | null, draft: ConnectionDraft, sync: SyncDraft, endpointError: string | undefined, proof: string, confirmed: boolean): string | undefined {
  ...
  if (operation(action) && requiredAssurance(operation(action)) === 3 && !/^[A-Za-z0-9_-]{43,128}$/.test(proof)) return '请输入有效的一次性复核凭证。';
...
function parseEndpoints(value: string): Readonly<{ value: Readonly<Record<string, string>>; error?: string }> {
  try {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed) || !Object.values(parsed).every((item) => typeof item === 'string' && item.startsWith('/'))) throw new Error();
    return { value: Object.freeze({ ...(parsed as Record<string, string>) }) };
  } catch {
    return { value: Object.freeze({}), error: '端点必须是由操作名和“/”开头路径组成的 JSON 对象。' };
  }
}

function operation(action: ChannelAction) {
