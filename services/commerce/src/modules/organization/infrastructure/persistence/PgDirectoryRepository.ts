...
import { PgDirectorySubjectRepository } from './PgDirectorySubjectRepository';
export class PgDirectoryRepository extends PgDirectorySubjectRepository implements DirectoryRepository {
  ...
  async active(context: ReadTransactionContext, run: string): Promise<boolean> {
    return (await this.transactions.database(context).query("select 1 from organization.syncrun where id=$1 and state in('queued','running')", [run])).rowCount === 1;
  }
  async event(
