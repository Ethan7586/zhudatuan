...
export async function freezeIssueTerms(database: SqlExecutor, input: Readonly<{
  order: string; scope: string; product: string; customer: string; stockRequest: string; quantity: number;
  startsAt: Date; expiresAt: Date; now: Date;
}>): Promise<IssueTerms> {
  const row = (await database.query<Omit<IssueTermsValue, 'startsAt' | 'expiresAt'> & { startsAt: Date; expiresAt: Date }>(`select product.id product,product.version::integer as "productVersion",
    ...
    for share of product,pool`, [input.product, input.scope, input.customer, input.stockRequest])).rows[0];
  ...
  const captured = await database.query(`insert into voucher.issueterms(order_id,scope_id,product_id,product_version,pool_id,face_minor,currency,qualification_id,activation,starts_at,expires_at)
    select $1,$2,id,version,pool_id,face_minor,currency,qualification_id,activation,starts_at,expires_at
    from voucher.product where id=$3 and scope_id=$2 and version=$4`, [input.order, input.scope, terms.value.product, terms.value.productVersion]);
