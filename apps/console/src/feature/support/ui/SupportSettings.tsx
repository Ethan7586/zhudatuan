...
export function SupportSettings({ gateway }: Readonly<{ gateway: SupportGateway }>) {
  ...
  const mutation = useMutation<void, Error, Change>({
    mutationFn: async (change) => {
      if (change.kind === 'agents') await manager.agent(context, change.id, change.version, change.body);
      else if (change.kind === 'accounts') await manager.account(context, change.id, change.version, change.body);
      else if (change.kind === 'rules') await manager.rule(context, change.id, change.version, change.body);
      else await manager.sla(context, change.id, change.version, change.body);
    },
    ...
    onError: async (cause, change) => {
      ...
      if (conflict) await cache.invalidateQueries({ queryKey: supportSettingKey(context.scope.id, context.session.accessVersion, change.kind) });
    },
  });
    ...
    <section className="supportsettings">
      ...
      {disabled ? (
        ...
        <>
          ...
          {active === 'agents' ? (
            <AgentSettings rows={agents.data?.items ?? []} busy={mutation.isPending} disabled={false} onSave={(id, version, body) => mutation.mutate({ kind: 'agents', id, version, body })} />
          ) : active === 'accounts' ? (
            <AccountSettings rows={accounts.data?.items ?? []} busy={mutation.isPending} disabled={false} onSave={(id, version, body) => mutation.mutate({ kind: 'accounts', id, version, body })} />
          ) : active === 'rules' ? (
            <RuleSettings rows={rules.data?.items ?? []} busy={mutation.isPending} disabled={false} onSave={(id, version, body) => mutation.mutate({ kind: 'rules', id, version, body })} />
          ) : (
            <SlaSettings rows={slas.data?.items ?? []} busy={mutation.isPending} disabled={false} onSave={(id, version, body) => mutation.mutate({ kind: 'slas', id, version, body })} />
          )}
        </>
      )}
    </section>
...
const label = (value: Setting): string => ({ agents: '客服人员', accounts: '渠道账号', rules: '分配规则', slas: '服务时限' })[value];
