...
do $contract$
begin
  if (select count(*) from runtime.operation)<>382 then raise exception 'CONTRACT_V5_OPERATION_CATALOG_MISMATCH'; end if;
  if (select count(*) from capability.operation)<>382 then raise exception 'CONTRACT_V5_CAPABILITY_BINDING_MISMATCH'; end if;
  ...
  if not exists(
    ...
    where artifact='commerce' and version='5.0.0' and checksum='6be4d54e11ee3e0ae76d98725e4ae86da12ee98d0c547481bd96d0c8449d42e3'
      and operation_count=382 and event_count=145 and status='active'
