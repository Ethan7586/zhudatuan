import { createFetchFinanceEntriesRead, createFetchFinanceReconciliationsRead, createFetchFinanceSettlementsRead, createFetchFinanceStatementsRead, createFetchFinanceWithdrawalsRead } from '@shop/sdk/finance';
import { createFetchInvoiceRequestsRead } from '@shop/sdk/invoice';
import type { ConsoleContext } from '../../entity/session/ConsoleSession';
import { consoleRequest } from '../../shared/api/Client';
import { appConfig } from '../../shared/config/AppConfig';
import { EntryPageSchema, InvoicePageSchema, ReconciliationPageSchema, SettlementPageSchema, StatementPageSchema, WithdrawalPageSchema, type FinanceRecord, type FinanceRecordPage, type FinanceSection } from './FinanceProfessionalSchema';
...
export const financeProfessionalKey = (context: ConsoleContext, section: FinanceSection, cursor?: string) =>
  Object.freeze(['console', context.scope.kind, context.scope.id, context.session.accessVersion, financeOperation(section), cursor ?? null, 50] as const);

export async function readFinanceProfessional(context: ConsoleContext, section: FinanceSection, cursor: string | undefined, signal: AbortSignal): Promise<FinanceRecordPage> {
  ...
  switch (section) {
    ...
    case 'reconciliations': {
      ...
      return mapPage(page, (row) => ({ id: row.id, label: `${row.provider} · ${row.period}`, reference: row.partner_id, amountMinor: row.difference_minor, currency: 'CNY', state: row.state, occurredAt: row.updated_at, version: null }));
    }
    case 'settlements': {
    ...
    case 'withdrawals': {
      ...
      return mapPage(page, (row) => ({ id: row.id, label: row.id, reference: row.settlement_id, amountMinor: row.amount_minor, currency: row.currency, state: row.state, occurredAt: row.created_at, version: row.version }));
    }
    case 'invoices': {
