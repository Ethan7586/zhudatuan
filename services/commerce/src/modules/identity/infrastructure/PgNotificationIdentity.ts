...
export class PgNotificationIdentity implements NotificationIdentityPort {
  recipient(database: OperationDatabase, membership: string) {
    return database.query<{ id: string; subject_ciphertext: string }>('select id,subject_ciphertext from identity.notification_recipient($1)', [membership]);
  }

  challenge(database: OperationDatabase, id: string) {
    return database.query<IdentityChallenge>(
      `select challenge.purpose,secret.code_ciphertext,secret.destination_ciphertext
  ...
  beginAttempt(database: OperationDatabase, id: string, provider: string) {
    return database.query<IdentityChallengeAttempt>(
      `with uncertain as(
