...
const source = Object.freeze({
  ...
  ZHUDATUAN_SANDBOX_QUALIFICATION_DATABASE_URL: `postgresql://zhudatuansandboxbootstrap:${password}@127.0.0.1:55432/zhudatuan_registration`,
...
test('qualification bootstrap accepts only one explicit registered sandbox membership', () => {
  ...
  assert.throws(() => sandboxQualificationBootstrapEnvironment({ ...source, ZHUDATUAN_SANDBOX_QUALIFICATION_CONFIRM: 'yes' }), /CONFIRMATION_REQUIRED/);
  assert.throws(() => sandboxQualificationBootstrapEnvironment({ ...source, ZHUDATUAN_SANDBOX_QUALIFICATION_DATABASE_URL: `postgresql://shopmigration:${password}@127.0.0.1:55432/zhudatuan_registration` }), /ENDPOINT_INVALID/);
  assert.throws(() => sandboxQualificationBootstrapEnvironment({ ...source, ZHUDATUAN_SANDBOX_QUALIFICATION_MEMBERSHIP: '../../all-members' }), /MEMBERSHIP_INVALID/);
