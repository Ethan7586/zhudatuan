...
export function ExperienceActionDialog({ action, context, onClose, onDone }: Readonly<{ action: ExperienceAction | null; context: ConsoleContext; onClose: () => void; onDone: () => void }>) {
  ...
  return <ExperienceActionForm key={`${action.kind}:${'record' in action ? action.record.id : 'new'}`} action={action} context={context} onClose={onClose} onDone={onDone} />;
}
...
function ExperienceActionForm({ action, context, onClose, onDone }: Readonly<{ action: ExperienceAction; context: ConsoleContext; onClose: () => void; onDone: () => void }>) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      await commands.versionsPublish({ path: { versionid: saved.id }, body: {} }, commandContext(context, action.record.version + 1));
      return commands.applicationsUpdate({ path: { applicationid: action.record.id }, body: { status: 'active' } }, commandContext(context, action.record.version + 1));
    },
    onSuccess: onDone,
  });
  const restore = useMutation({
    mutationFn: async () => {
      ...
      const source = record.history.find((version) => version.id !== record.head_id) ?? record.history[0];
      ...
      await commands.versionsPublish({ path: { versionid: restored.id }, body: {} }, commandContext(context, record.version + 1));
      return commands.applicationsUpdate({ path: { applicationid: record.id }, body: { status: 'active' } }, commandContext(context, record.version + 1));
    },
    onSuccess: onDone,
  });
    ...
    <div className="commerceoverlay">
      ...
      <form className="commerceflowdialog" aria-label={actionTitle(action.kind)} onSubmit={submit}>
        ...
        <footer className="commerceactionfooter">
          {action.kind === 'design' && (record?.history.length ?? 0) > 1 ? (
...
function experienceContent(record: Experience | undefined): Readonly<{ title: string; announcement: string }> {
  if (record?.head_configuration === null || record?.head_configuration === undefined || typeof record.head_configuration !== 'object') {
    return { title: record?.name ?? '主打团福利商城', announcement: '欢迎进入企业福利商城' };
  }
  const source = record.head_configuration as { pages?: readonly { blocks?: readonly { component?: string; content?: Record<string, unknown> }[] }[] };
  ...
  return {
    title: typeof hero?.title === 'string' ? hero.title : record.name,
