...
describe('keysetResult', () => {
  it('returns a cursor only when one fetched lookahead row proves another page', () => {
    const result = keysetResult(query([
  ...
  it('does not emit a speculative cursor for the terminal page', () => {
    expect(keysetResult(query([{ id: 'order:1', created_at: '2026-08-21T01:00:00Z' }]), page, 'created_at').body)
      .toEqual({ items: [{ id: 'order:1', created_at: '2026-08-21T01:00:00Z' }], count: 1 });
  });
});
