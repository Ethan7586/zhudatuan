...
await withMigrationOwnership(connectionString, async (database) => {
  await database.query(`update cart.cart set state='abandoned',updated_at=clock_timestamp(),version=version+1 where member_id=$1 and mall_id=$2 and application_id<>$3 and state='active'`, [
    LOCAL_OWNER.member,
    LOCAL_OWNER.mall,
    application,
  ]);
  ...
  const result = await database.query<{ valid: boolean }>(
    ...
    and exists(select 1 from member.favorite where member_id=$2 and listing_id=$4) valid`,
    [cart, LOCAL_OWNER.member, LOCAL_OWNER.mall, listing]
  );
