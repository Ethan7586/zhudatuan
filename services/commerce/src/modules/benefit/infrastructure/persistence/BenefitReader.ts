...
export function benefitReader(context: ModuleContext) {
  ...
  return {
    ...
    readLedger: async (request: Parameters<ReturnType<typeof benefitAccountReader>>[0], database: Parameters<ReturnType<typeof benefitAccountReader>>[1]) => {
      ...
      const entries = await finance.benefitLedger(database.transaction, [...accountByFinance.keys()], { occurredAt: page.sort, entry: page.id }, page.fetch);
      return keysetResult(
        { rows: entries.map((entry) => ({ ...entry, account: accountByFinance.get(entry.account)!.id, kind: accountByFinance.get(entry.account)!.kind, currency: accountByFinance.get(entry.account)!.currency })), rowCount: entries.length } as never,
        page,
        'occurredAt'
      );
    },
