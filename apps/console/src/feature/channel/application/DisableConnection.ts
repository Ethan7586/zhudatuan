...
export class DisableConnection {
  ...
  execute(context: ConsoleContext, connection: string, version: number, proof: string, identity: string, signal?: AbortSignal) {
    assertOperationAccess(context, channelOperations.disable, proof);
    return this.port.disable(context, connection, version, proof, identity, signal);
  }
}
