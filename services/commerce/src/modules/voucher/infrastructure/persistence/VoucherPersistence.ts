...
import { voucherQueryPersistence } from './VoucherQueryPersistence';
import { voucherCatalogPersistence } from './VoucherCatalogPersistence';
import { voucherImportPersistence } from './VoucherImportPersistence';
import { MEMBER_ACCESS_PORT } from '../../../access/public';
import { ORGANIZATION_READ_PORT } from '../../../organization/public';
import { PgRuntimeWriter } from '../../../../adapter/database/PgRuntimeWriter';

export function voucherPersistence(context: ModuleContext): VoucherPersistence {
  const members = context.ports.get(MEMBER_ACCESS_PORT);
  const organizations = context.ports.get(ORGANIZATION_READ_PORT);
  return {
    ...voucherImportPersistence(context),
    ...voucherCatalogPersistence(organizations),
    issueBatch: async (request, database) => {
      ...
      await new PgRuntimeWriter(database).schedule({ id: `job:${id}`, kind: 'voucherissue', owner: 'voucher', scope: access.scope.id, payload: { batch: id }, priority: 20 });
      return rowResult(result, 202);
    },
    readBatches: async (request, database) => {
    ...
    retryBatch: async (request, database) => {
      const access = requireAccess(request);
      ...
      if (await runtime.hasActiveJob('voucherissue', 'batch', batchid)) throw new Error('VOUCHER_ISSUE_JOB_ACTIVE');
      ...
      await runtime.schedule({ id: `job:${randomUUID()}`, kind: 'voucherissue', owner: 'voucher', scope: selected.scope_id, payload: { batch: batchid }, priority: 30 });
      await runtime.reviewDeadlettersByPayload('voucher', 'batch', batchid);
      return rowResult(result, 202);
    },
    changeStatus: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      ...
      await new PgRuntimeWriter(database).schedule({ id: `job:${batchid}`, kind: 'voucherstatus', owner: 'voucher', scope: access.scope.id, payload: { batch: batchid }, priority: 20 });
      return rowResult(result, 202);
    },
    manageBinding: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
      const voucherid = request.input.path.voucherid!;
      const member = textField(body, 'member');
      const eligible = await members.activeIn(database.transaction, member, await organizations.descendants(database.transaction, organizationScope(access.scope)));
    ...
    reverseRedemption: async (request, database) => {
      const access = requireAccess(request);
      const body = bodyRecord(request.input);
    ...
    ...voucherQueryPersistence(organizations),
  };
}
