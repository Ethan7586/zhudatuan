  ...
  async execute(session: StorefrontSession, preference: NotificationPreference, change: Readonly<{ enabled?: boolean; quietHours?: Readonly<{ start: string; end: string; timezone: string }> | null }>): Promise<void> {
    ...
    const quietHours =
      change.quietHours === undefined
        ? preference.quietStart && preference.quietEnd && preference.quietTimezone
          ? { start: preference.quietStart.slice(0, 5), end: preference.quietEnd.slice(0, 5), timezone: preference.quietTimezone }
          : null
        : change.quietHours;
    await this.gateway.changePreference(session, preference, { enabled, quietHours }, `preference:${preference.channel}:${preference.eventType}:${crypto.randomUUID()}`);
  }
}
