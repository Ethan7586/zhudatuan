...
export function ConversationPage({ viewmodel }: Readonly<{ viewmodel: ReturnType<typeof useConversationViewModel> }>) {
  ...
  return (
    <main className="storesupportconversation">
      <header className="storesupportconversationheader">
        <button type="button" aria-label="返回客服中心" onClick={actions.back}><ArrowLeft size={18} /></button>
        <div>
          <p>智慧翼 · 客户服务</p>
          <h1>{ticket?.subject ?? '工单会话'}</h1>
          <span>{chineseReference('工单', caseId)} · {connected ? '实时连接正常' : '正在恢复实时连接'}</span>
        </div>
        <button type="button" aria-label="刷新会话" onClick={actions.refresh}><RefreshCw size={17} /></button>
      </header>
      ...
      <section
        ...
        onScroll={(event) => {
          ...
          if (bottom) { actions.setNewMessage(false); actions.markRead(messages.at(-1)?.sequence ?? 0); }
        }}
      >
        {hasEarlier ? <button className="storesupportearlier" type="button" disabled={loadingEarlier} onClick={actions.loadEarlier}>{loadingEarlier ? '加载中…' : '加载更早消息'}</button> : null}
        {state === 'loading' ? <p className="storesupportloading" role="status"><LoaderCircle size={18} />正在读取会话…</p> : null}
        ...
        {newMessage ? <button className="storesupportnewmessage" type="button" onClick={() => { if (viewport.current) viewport.current.scrollTop = viewport.current.scrollHeight; actions.setNewMessage(false); }}>有新消息</button> : null}
      ...
      <SupportComposer
        ...
        sending={sending}
        failed={failed !== null}
        attachments={draft.attachments as readonly PendingAttachment[]}
...
function MessageBubble({ message, attachments, showDate }: Readonly<{ message: SupportMessage; attachments: readonly SupportAttachment[]; showDate: boolean }>) {
  return <>
    ...
    <article className="storesupportmessage" data-author={message.authorType}>
      <div><strong>{message.authorType === 'member' ? '我' : '客服'}</strong><time dateTime={message.createdAt}>{format(message.createdAt)}</time></div>
      <p>{message.body}</p>
      {attachments.length ? <ul>{attachments.map((item) => <li key={item.id}>
        <Paperclip size={14} />
        {item.download ? <a href={item.download.url} rel="noreferrer">{item.name}</a> : <span>{item.name}</span>}
...
function DraftBubble({ draft, state }: Readonly<{ draft: MessageDraft; state: 'sending' | 'failed' }>) {
  return <article className="storesupportmessage" data-author="member" data-delivery={state}><div><strong>我</strong></div><p>{draft.message}</p><small>{state === 'sending' ? '正在发送…' : '发送失败，可原样重试'}</small></article>;
}
