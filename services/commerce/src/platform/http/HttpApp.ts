...
import { CONTRACT_VERSION, isOperationTarget, OperationCatalog } from '@shop/contract';
...
import { ErrorPresenter } from '../../pipeline/ErrorPresenter';
import { operationController } from '../../pipeline/OperationController';
import { isHttpStream } from './HttpStream';
import { httpPreflight as preflight, httpResponse as secure, httpStreamResponse as secureStream } from './HttpWireResponse';
...
export class HttpApp {
  ...
  constructor(
    ...
    private readonly errors = new ErrorPresenter(),
...
function assertRequestPolicy(request: Request, origin: string | null, operation: ReturnType<typeof OperationCatalog.get>, protector: CsrfProtector): void {
  ...
  if (!isOperationTarget(target)) throw new ApplicationError('CSRF_TOKEN_INVALID');
  const supplied = request.headers.get('x-csrf-token');
  const optionalAnonymous = operation.assuranceLevel === 'optional' && !cookieValue(cookie ?? '', `__Host-${target}-session`);
  if (operation.assuranceLevel === 'anonymous' || operation.assuranceLevel === 'preauth' || optionalAnonymous) {
...
function errorDetails(body: unknown): Readonly<Record<string, import('../error/ApplicationError').ErrorDetail>> {
  if (body === null || typeof body !== 'object' || Array.isArray(body)) return {};
  const record = body as Readonly<Record<string, unknown>>;
  const nested = record.details;
  if (nested !== null && typeof nested === 'object' && !Array.isArray(nested)) return nested as Readonly<Record<string, import('../error/ApplicationError').ErrorDetail>>;
  const entries = Object.entries(record).filter(([key, value]) => key !== 'code' && isErrorDetail(value));
  return Object.freeze(Object.fromEntries(entries) as Record<string, import('../error/ApplicationError').ErrorDetail>);
}
function isErrorDetail(value: unknown): value is import('../error/ApplicationError').ErrorDetail {
