...
const allowedHttpPrefixes = ['01_core_hexin/packages/sdk/', '01_core_hexin/services/commerce/src/foundation/http/', '01_core_hexin/extensions/vendors/', '04_tools/tools/localinfra/src/'];
...
function semanticKind(name, sourceName) {
  ...
  if (sourceName.startsWith('01_core_hexin/extensions/vendors/') && /retry/i.test(name)) return 'providerRetry';
  return 'general';
}
  ...
  for (const source of sourceList) {
    ...
    const visit = (node, parent) => {
      ...
      if (envKey) {
        ...
        if (!sourceName.startsWith('01_core_hexin/packages/config/src/')) {
      ...
      if (
        ...
        const owner = sourceName.startsWith('01_core_hexin/apps/') ? sourceName.split('/').slice(0, 2).join('/') : 'global';
      ...
      if (sourceName.startsWith('01_core_hexin/extensions/vendors/') && ts.isPropertyAssignment(node) && ((ts.isIdentifier(node.name) && node.name.text === 'retry') || (ts.isStringLiteralLike(node.name) && node.name.text === 'retry'))) {
