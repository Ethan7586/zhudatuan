...
describe('frozen credential, issuance and action exports', () => {
  ...
  it('binds reads and replay to the original scope, actor, kind and filters', async () => {
    ...
    try {
      ...
      for (const changed of [
        { ...work, scope: 'foreign' },
        { ...work, kind: 'action' },
        { ...work, authorization: { ...work.authorization, actor: 'other' } },
        { ...work, snapshot: { ...work.snapshot, pool: 'other' } },
      ]) {
        await expect(data.read(changed)).rejects.toThrow('VOUCHER_EXPORT_SNAPSHOT_INVALID');
      }
      await expect(
        data.manager.write(options(), (context) =>
          captureExportSnapshot(new PgTransactionAccess().database(context), {
            id: work.id,
            scope: work.scope,
            kind: 'credential',
            filter: { ...work.snapshot, pool: 'other' },
            actor: 'actor:test',
          })
        )
  ...
  it('rolls back the Runtime export and the entire projection if queuing fails', async () => {
    ...
    try {
      ...
      for (const table of ['runtime.exports', 'voucher.exportsnapshot', 'voucher.exportsnapshotitem']) expect((await data.database.query(`select count(*)::integer count from ${table}`)).rows).toEqual([{ count: 0 }]);
  ...
  it('rejects updates, extra rows, cross-scope rows and credential fields in business-only exports', async () => {
    ...
    try {
      ...
      for (const change of ['ordinal+2,scope_id', "ordinal,'foreign'"]) {
        ...
        await expect(
          data.database.query(
            `insert into voucher.exportsnapshotitem(snapshot_id,ordinal,scope_id,cells)
          select snapshot_id,${ordinal},${scope},cells from voucher.exportsnapshotitem where snapshot_id=$1 limit 1`,
            [work.id]
          )
        ).rejects.toThrow('VOUCHER_EXPORT_SNAPSHOT_INVALID');
      }
      await data.database.query(`delete from voucher.exportsnapshotitem where snapshot_id=$1 and ordinal=2`, [work.id]);
      await expect(
        data.database.query(
          `insert into voucher.exportsnapshotitem(snapshot_id,scope_id,ordinal,cells,credential_id,pool_id,number_ciphertext,secret_ciphertext)
        select snapshot_id,scope_id,2,cells,'credential:evil','pool:one','cipher','cipher' from voucher.exportsnapshotitem where snapshot_id=$1`,
          [work.id]
        )
      ).rejects.toThrow('VOUCHER_EXPORT_SNAPSHOT_INVALID');
    } finally {
  ...
  it.each(['missing row', 'expired'] as const)('never publishes a %s snapshot and fails without returning a file', async (corruption) => {
    ...
    try {
      ...
      const fail = vi.fn();
      await expect(
        runExport(new PgVoucherExportProcess(data.manager, { claim: vi.fn(async () => work), ready, fail } as unknown as ExportPort, {} as CredentialProtector, 3, { assert: vi.fn() }), output.objects, input(work.id))
      ).rejects.toThrow(corruption === 'expired' ? 'VOUCHER_EXPORT_SNAPSHOT_INVALID' : 'EXPORT_ROW_COUNT_MISMATCH');
...
function options() {
  return { tenant: 'scope:test', scope: 'scope:test', membership: 'membership:test', actor: 'actor:test', trace: 'trace:test', operation: 'voucher.actionexports.create', signal: new AbortController().signal, deadline: Date.now() + 20_000 };
}
function input(id: string) {
  return { exportId: id, scope: 'scope:test', attempts: 1, signal: new AbortController().signal, deadline: Date.now() + 20_000 };
}
function runExport(renderer: PgVoucherExportProcess, objects: ObjectStore, value: ReturnType<typeof input>): Promise<void> {
  return new RunExport('voucherexport', objects).execute(
    value.exportId,
    {
      id: `job:${value.exportId}`,
      kind: 'voucherexport',
      scope: value.scope,
      payload: { export: value.exportId },
      authorization: {},
...
function call<TKey extends OperationId>(context: WriteTransactionContext, kind: Kind, body: unknown): VoucherCall<TKey> {
  ...
  return {
    input: { body },
    context: { transaction: context, operation: operations[kind], security: { kind: 'session', access } },
    scope: 'scope:test',
    actor: 'actor:test',
    member: 'member:test',
    target: 'console',
    idempotency: `export:${kind}`,
    expectedVersion: 1,
    now: new Date(),
  } as VoucherCall<TKey>;
}
async function fixture(count: number) {
  ...
  await database.query(
    `insert into voucher.credential select 'credential:'||lpad(n::text,5,'0'),'scope:test','pool:one','product:one','available','key:one',now()-interval '1 minute','cipher-number:'||n,'cipher-secret:'||n from generate_series(1,$1::integer) n`,
    [count]
  );
  ...
  return {
    ...
    create: async (kind: Kind, reference?: string) => {
      const record = await manager.write(options(), (context) =>
        kind === 'credential'
          ? credentials.export(call(context, kind, { pool: reference ?? 'pool:one', watermark, reason: '核对导出' }))
          : kind === 'issueorder'
            ? issues.export(call(context, kind, { order: reference ?? 'order:one', reason: '核对导出' }))
