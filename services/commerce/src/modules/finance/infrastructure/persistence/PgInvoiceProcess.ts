...
import { assertDocument, date, invoiceHash, invoiceResultHash, requiredHash, transactionOptions, type InvoiceDocument, type InvoiceLineRow, type InvoiceRow, type OriginalDocument, type PreparedInvoice } from './InvoiceRecord';
...
export class PgInvoiceProcess implements InvoiceJobProcess {
  ...
  async issue(input: Readonly<{ request: string; business: string }>, execution: FinanceJobExecution): Promise<void> {
    ...
    await this.manager.write(options, async (transaction) => {
      ...
      await database.query(
        ...
        [`document:${invoice.id}`, invoice.id, issued.provider, issued.externalId, stored.reference, stored.sha256, invoice.kind, prepared.original?.id ?? null]
      );
      const document = await database.query<InvoiceDocument>(`select id,provider,external_id,object_ref,sha256,kind,red_of_id from invoice.document where request_id=$1 for update`, [invoice.id]);
      ...
      const updated = await database.query(
        ...
        [invoice.id, completed.state, completed.version, invoice.version, prepared.watermark.snapshot().hash, issued.provider, issued.externalId, invoiceResultHash(issued, stored.sha256)]
      );
      if (!updated.rows[0]) {
        const complete = await database.query(`select 1 from invoice.request where id=$1 and state='issued' and issue_hash=$2`, [invoice.id, prepared.watermark.snapshot().hash]);
      ...
      if (invoice.red_of_request_id !== null) {
        const red = await database.query(`update invoice.request set state='red',version=version+1 where id=$1 and state='issued' returning id`, [invoice.red_of_request_id]);
      ...
      await new PgRuntimeWriter(database).append({
        ...
        payload: {
          ...
          providerReference: issued.externalId,
    ...
    return this.manager.write(options, async (context) => {
      ...
      if (invoice.state === 'approved') {
        if (invoice.issue_hash !== null) {
          InputWatermark.restore({ hash: invoice.issue_hash, count: Number(invoice.issue_count), occurredAt: date(invoice.issue_watermark) }).assert(hash, lines.rows.length);
        }
        ...
        claimed = { ...invoice, state: 'issuing', version: update.version, issue_hash: update.issue_hash, issue_count: lines.rows.length, issue_watermark: update.issue_watermark };
      }
      const watermark = InputWatermark.restore({ hash: requiredHash(claimed.issue_hash), count: Number(claimed.issue_count), occurredAt: date(claimed.issue_watermark) });
  ...
  private async store(request: string, issued: IssuedInvoice): Promise<StoredObject> {
    ...
    if (existing) {
      if (existing.contentType !== issued.contentType || existing.sha256 !== expected || existing.size !== issued.document.byteLength) throw new Error('INVOICE_DOCUMENT_RECOVERY_CONFLICT');
      return existing;
    }
