...
export function useConversationViewModel(
  ...
  const onEvent = useCallback((event: SupportEvent) => {
  ...
  const actions = useMemo(() => Object.freeze({
    draft: (value: string) => { if (ticketId) setDrafts((current) => ({ ...current, [ticketId]: value })); },
    send: sendCurrent,
    retrySend,
    file: (file: File) => { if (ticketId && !upload.isPending) upload.mutate(file); },
