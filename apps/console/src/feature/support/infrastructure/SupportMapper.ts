...
export class SupportMapper {
  ...
  slas(value: OperationOutputFor<'support.slas.read'>): ConfigPage<Sla> {
    return freezePage(value, (item) => Object.freeze({ id: item.id, priority: item.priority, responseSeconds: item.response_seconds, resolutionSeconds: item.resolution_seconds, reopenSeconds: item.reopen_seconds, version: item.version }));
  }
