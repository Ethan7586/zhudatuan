...
export function defineModule(manifest: ModuleManifest, assembly: ModuleAssembly): CommerceModule {
  ...
  return Object.freeze({
    ...
    bind(context: ModuleContext): readonly PublicPortBinding[] {
      ...
      assertExact(`${id}:${context.workload}:publicports`, bindings.map(({ token }) => token.key), manifest.workloads[context.workload].publicPorts);
    ...
    register(context: ModuleContext): void {
      ...
      if (context.workload === 'api') {
        ...
        assertExact(`${id}:api:operations`, handlers.map(({ operation }) => operation), manifest.operations);
      ...
      assertExact(`${id}:${context.workload}:jobs`, jobs.map(({ id: job }) => job), manifest.workloads[context.workload].jobs);
      for (const job of jobs) context.jobs?.add(job);
      if (context.workload === 'jobs') {
        const workers = assembly.workers?.(context) ?? [];
        assertExact(`${id}:jobs:workers`, workers.map(({ id: worker }) => worker), manifest.workloads.jobs.workers);
