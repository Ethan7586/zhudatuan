...
describe('OrderReadFilter', () => {
  it('normalizes an empty query to the authoritative unfiltered view', () => {
    expect(OrderReadFilter.from({ query: {} })).toEqual({ search: '', order: '', view: 'all', placed: '', from: '', to: '', lifecycle: '', payment: '', fulfillment: '', mall: '', channel: '', product: '', member: '', minimumMinor: null, maximumMinor: null });
  });
  ...
  it('accepts the bounded contract values and the first transport value', () => {
    ...
    ).toEqual({ search: 'ZD202609050001', order: 'order:one', view: 'unshipped', placed: '7days', from: '', to: '', lifecycle: 'paid', payment: 'partially_refunded', fulfillment: 'allocated', mall: 'mall:one', channel: 'jdproduct', product: '福利礼盒', member: '张三', minimumMinor: 100, maximumMinor: 10000 });
  });
