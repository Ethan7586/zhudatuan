...
export class IssueBatchJob implements JobProcessor {
  ...
  process(job: ClaimedJob, signal: AbortSignal, deadline = Date.now() + 120_000): Promise<void> {
    ...
    if (typeof payload.eventId === 'string') return this.task.decide(payload.eventId, text(payload.event, 'VOUCHER_EVENT_TYPE_REQUIRED'), scope,
      record(payload.payload), signal, deadline);
