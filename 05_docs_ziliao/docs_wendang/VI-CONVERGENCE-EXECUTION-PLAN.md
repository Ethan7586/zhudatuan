...
1. `05_docs_ziliao/docs_wendang/brand/SMART-WING-UNIFIED-VI-1.0.md` —— VI 硬规则
2. `05_docs_ziliao/docs_wendang/SMART-WING-MALL-MASTER-CHARTER.md` —— 商城总纲与冻结决议
...
**根因已确认：`packages/design-system/src/tokens.css` 是手写文件**，没有 `GENERATED` 头，`04_tools/scripts/build-brand-assets.mjs` 也不生成它。它从未接入任何管道，因此必然与 `tokens.json` 漂移。
...
> 已由打样完成，提交 `fafc0b7`。实现见 `04_tools/scripts/build-web-tokens.mjs`，
> **执行 T2 前必读 `05_docs_ziliao/docs_wendang/VI-CONVERGENCE-PILOT-NOTES.md`** —— 里面是这次踩到的坑。
...
- 参考实现：`04_tools/scripts/build-miniapp-assets.mjs`（同一套读取逻辑，输出目标不同；Web 用 px，小程序用 rpx）
...
- 以 `04_tools/scripts/check-miniapp-vi.mjs` 为模板，新建 `04_tools/scripts/check-vi.mjs`，覆盖 `apps/storefront-web`、`apps/admin-web`、`apps/auth-web`
- 规则：
  | 规则 | 拦什么 |
  | --- | --- |
...
- **必须实现 baseline 机制**：
  - 首次运行生成 `04_tools/scripts/vi-baseline.json`，记录当前每个文件每条规则的违规计数
...
**硬要求**（`05_docs_ziliao/docs_wendang/mobile/WING-CODE-WECHAT-MINIAPP-MASTER-PLAN.md`）
