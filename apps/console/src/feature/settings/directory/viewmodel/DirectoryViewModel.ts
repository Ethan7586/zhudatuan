...
import type { DirectoryCommand, DirectorySyncRun } from '../model/SyncRun';

export interface SyncEditor {
  readonly action: 'start' | 'cancel' | 'resume';
...
export function useDirectoryViewModel(context: ConsoleContext, dependencies: DirectoryDependencies, requestStepup: () => void) {
  ...
  const [receipt, setReceipt] = useState<Readonly<{ id: string; state: string; mode: string }>>();
  const mutation = useMutation({
    mutationFn: async (command: DirectoryCommand) => {
      const value = command.action === 'start' ? await dependencies.start.execute(context, command) : command.action === 'cancel' ? await dependencies.cancel.execute(context, command) : await dependencies.resume.execute(context, command);
  ...
  const submit = useCallback(() => {
    ...
    const command: DirectoryCommand =
      editor.action === 'start'
        ? { action: 'start', directory: editor.directory.id, mode: editor.mode, proof: editor.proof, identity }
...
function validate(editor: SyncEditor | undefined, assurance: number): string | undefined {
  ...
  if (editor.action !== 'start' && !editor.run) return '同步运行不存在，请重读后重试。';
