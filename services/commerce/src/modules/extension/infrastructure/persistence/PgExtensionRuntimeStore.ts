...
export class PgExtensionRuntimeStore implements ExtensionRuntimeStore {
  ...
  enabled(signal: AbortSignal, deadline = Date.now() + 30_000): Promise<readonly RuntimeInstallation[]> {
    return this.transactions.read(systemOptions(signal, 'load', deadline), async (context) => (
...
function systemOptions(signal: AbortSignal, action: string, deadline: number) {
  return {
    tenant: '', membership: '', scope: 'extension', actor: 'system:extension', trace: `extension:${action}`,
    operation: `extension.provider.${action}`, workload: 'jobs' as const, deadline, signal,
  };
}
