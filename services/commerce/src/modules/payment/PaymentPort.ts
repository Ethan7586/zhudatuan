...
export class PaymentPort {
  async externalAmount(database: OperationDatabase, kind: 'payment' | 'refund', reference: string): Promise<number> {
    const result =
      kind === 'refund'
        ? await database.query<{ amount: number }>(
  ...
  async reconciliation(database: OperationDatabase, references: readonly string[]) {
    ...
    const result = await database.query<{ reference: string; kind: 'payment' | 'refund'; id: string; amountMinor: number }>(
  ...
  async prepare(
    ...
    const inserted = await database.query<{ expires_at: string }>(
    ...
    if (!expiresAt) {
      const selected = await database.query<{ expires_at: string }>('select expires_at from payment.intent where id=$1', [intent]);
  ...
  async expirations(database: OperationDatabase, order: string | null) {
    const result = await database.query<{ intent: string; order: string; external: boolean }>(
