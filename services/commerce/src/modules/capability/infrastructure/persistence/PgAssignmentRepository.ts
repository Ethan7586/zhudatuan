...
import { assignment, date, dependency, entitlement, nullableInteger, nullableUtc, type AssignmentRow, type EntitlementRow } from './AssignmentRecord';
export class PgAssignmentRepository implements AssignmentRepository {
  ...
  private async projectionRows(context: ReadTransactionContext, scope: string, capability: string | null, sort: string | null, id: string | null, fetch: number): Promise<readonly AssignmentRow[]> {
    const database = this.transactions.database(context);
