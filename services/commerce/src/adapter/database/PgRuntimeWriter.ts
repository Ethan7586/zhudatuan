...
export class PgRuntimeWriter {
  ...
  constructor(private readonly database: RuntimeSql) { this.admission = new QueueAdmission(database); }
  ...
  async schedule(job: RuntimeJobInput): Promise<void> {
    ...
    await this.database.query(
      ...
      [job.id, job.kind, definition.owner, job.scope ?? 'organization-platform-root', JSON.stringify(job.payload), job.priority, job.availableAt ?? null, definition.queue, job.authorization === undefined ? null : JSON.stringify(job.authorization)]
    );
  }
