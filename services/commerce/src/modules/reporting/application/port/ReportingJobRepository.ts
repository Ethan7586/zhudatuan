...
export interface ReportingJobRepository {
  ...
  exportRows(context: ReadTransactionContext, id: string, report: Exclude<ExportReport, 'metrics'>, cursor: string | null, fetch: number): Promise<readonly ExportRow[]>;
  metricExportRows(context: ReadTransactionContext, id: string, cursor: string | null, fetch: number): Promise<readonly MetricExportRow[]>;
