...
describe('frozen issuance terms', () => {
  it('reclaims expired holds through the shared writer and records exactly one restoration in the timeline', async () => {
    ...
    try {
      ...
      await data.manager.write(options(), (context) =>
        new VoucherTenderWriter().reserve({
          context,
          scope: 'scope:test',
          voucher: active.body.id,
          owner: 'order:checkout',
          member: null,
          amountMinor: 100,
          ttlSeconds: 60,
          idempotency: 'hold:expired',
          now: new Date(Date.now() - 120_000),
          actor: 'principal:consumer',
        })
      );
      await data.maintain();
      await data.maintain();
      expect((await data.database.query(`select state from voucher.tenderhold`)).rows).toEqual([{ state: 'expired' }]);
      expect((await data.database.query(`select state,remaining_minor::integer from voucher.voucher where id=$1`, [active.body.id])).rows).toEqual([{ state: 'active', remaining_minor: 1000 }]);
      expect((await data.database.query(`select previous_state,next_state from voucher.timeline where reason='holdexpiry'`)).rows).toEqual([{ previous_state: 'held', next_state: 'active' }]);
    } finally {
  ...
  it('preserves approved value and activation across a product revision and a partially failed issuance retry', async () => {
    ...
    try {
      await data.submit();
      expect(data.request).toHaveBeenCalledWith(
        expect.anything(),
  ...
  it('does not allocate approved credentials after their pool closes', async () => {
    ...
    try {
      ...
      await data.issue();
      expect(await data.vouchers()).toEqual([]);
      expect((await data.database.query(`select state,error_code,retryable from voucher.issueitem`)).rows).toEqual(
        Array.from({ length: 3 }, () => ({
          state: 'failed',
          error_code: 'VOUCHER_POOL_CLOSED',
          retryable: false,
        }))
      );
  ...
  it('does not issue already expired benefits even when approval arrives late', async () => {
    ...
    try {
      ...
      expect((await data.database.query(`select state,error_code,retryable from voucher.issueitem`)).rows).toEqual(
        Array.from({ length: 3 }, () => ({
          state: 'failed',
          error_code: 'VOUCHER_STATE_INVALID',
          retryable: false,
        }))
      );
    } finally {
...
function options() {
  return {
    ...
    trace: 'trace:test',
    operation: 'voucher.issueorders.submit',
...
async function fixture() {
  ...
  const addCredential = (number: number) =>
    database.query(
      `insert into voucher.credential(id,scope_id,pool_id,product_id,number_ciphertext,secret_ciphertext,number_fingerprint,secret_fingerprint,number_masked,key_version,state,version,created_at,updated_at)
  ...
  await addCredential(1);
  await addCredential(2);
  ...
  return {
    ...
    addCredential,
    submit: async () => {
      const expectedVersion = await readVersion('issueorder', 'order:one');
      return manager.write(options(), (context) =>
        repository.submit({
          input: { path: { orderid: 'order:one' } },
    ...
    approve: () =>
      manager.write(options(), async () =>
        database.exec(`update voucher.issueorder set state='approved',version=version+1 where id='order:one';
    ...
    retry: async () => {
      const expectedVersion = await readVersion('issuebatch', 'batch:one');
      return manager.write(options(), (context) =>
        repository.retry({
          input: { path: { batchid: 'batch:one' } },
          context: { transaction: context },
          scope: 'scope:test',
          actor: 'principal:maker',
          expectedVersion,
    ...
    activate: () =>
      manager.write(options(), (context) =>
        new PgVoucherRepository(new PgActivationRate(), {} as never, {} as never, { post }).activateSecret(
          {
            context: { transaction: context },
            scope: 'scope:test',
            actor: 'principal:consumer',
