...
export class RecoveryGateway implements RecoveryPort {
  ...
  async reset(input: Recovery, session: SessionRequest, signal?: AbortSignal): Promise<void> {
    const state = await this.bootstrap.read(session, signal);
    await this.sdk.passwordReset({ body: { challenge: input.challenge, code: input.code.trim(), newPassword: input.password } }, commandContext(this.environment, session.target, state.csrf, signal));
  }
}
