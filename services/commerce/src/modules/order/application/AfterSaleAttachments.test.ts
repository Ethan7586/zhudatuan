...
import { AfterSaleAttachments } from './AfterSaleAttachments';
...
describe('AfterSaleAttachments', () => {
  it('uploads verified bytes into the member-scoped private path', async () => {
    const create = vi.fn(async () => upload(png));
    await expect(new AfterSaleAttachments({ create }).verify(request([{ data: Buffer.from(png).toString('base64'), contentType: 'image/png', name: 'damage.png' }]))).resolves.toEqual([
  ...
  it('rejects spoofed MIME, invalid base64 and failed integrity checks', async () => {
    const create = vi.fn(async () => upload(png));
    await expect(new AfterSaleAttachments({ create }).verify(request([{ data: Buffer.from('javascript').toString('base64'), contentType: 'image/png', name: 'damage.png' }]))).rejects.toThrow('VALIDATION_FAILED');
    await expect(new AfterSaleAttachments({ create }).verify(request([{ data: '***', contentType: 'image/png', name: 'damage.png' }]))).rejects.toThrow('VALIDATION_FAILED');
    await expect(new AfterSaleAttachments({ create: vi.fn(async () => upload(png, 'b'.repeat(64))) }).verify(request([{ data: Buffer.from(png).toString('base64'), contentType: 'image/png', name: 'damage.png' }]))).rejects.toThrow(
      'VALIDATION_FAILED'
    );
  });
});
