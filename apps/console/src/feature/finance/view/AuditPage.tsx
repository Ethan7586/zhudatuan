...
function AuditSearch({ model }: Readonly<{ model: AuditViewModel }>) {
  return (
    ...
    >
      ...
      <button type="submit" disabled={model.draft.trim().length === 0}>查询证据链</button>
      {model.reference === undefined ? null : <button type="button" onClick={model.clear}>清除</button>}
    </form>
  );
}
  ...
  return (
    <div className="financeauditresult">
      <header className="financeauditsummary">
        <div><span>查询编号</span><strong>{chineseReference('业务', data.reference)}</strong></div>
        <div><span>关联事实</span><strong>{data.facts.length.toLocaleString('zh-CN')} 条</strong></div>
        <div><span>来源事件</span><strong>{data.events.length.toLocaleString('zh-CN')} 条</strong></div>
        <div><span>审计证据</span><strong>{data.records.length.toLocaleString('zh-CN')} 条</strong></div>
        <div><span>证据水位</span><strong>{formatDate(data.watermark)}</strong></div>
      </header>
...
function AuditFacts({ facts }: Readonly<{ facts: readonly FinanceAuditFact[] }>) {
  return (
    <section className="financeauditsection" aria-labelledby="financeauditfacts">
      <header><h2 id="financeauditfacts">业务事实链</h2><p>金额和状态来自各自权威业务表；页面不补算、不改写。</p></header>
      <ol className="financeaudittimeline">
        {facts.map((fact) => (
          <li key={`${fact.kind}:${fact.id}`}>
            <i aria-hidden="true" />
            <article>
              <header><div><span>{kindLabel(fact.kind)}</span><strong>{fact.label}</strong></div><time dateTime={fact.occurredAt ?? undefined}>{formatDate(fact.occurredAt)}</time></header>
              <dl>
                <div><dt>业务编号</dt><dd>{chineseReference(kindLabel(fact.kind), fact.businessReference)}</dd></div>
                <div><dt>状态</dt><dd>{fact.state === null ? '不适用' : chineseDomainLabel(fact.state)}</dd></div>
                <div><dt>金额</dt><dd>{fact.amountMinor === null || fact.currency === null ? '不适用' : formatMinor(fact.amountMinor, fact.currency)}</dd></div>
                <div><dt>版本</dt><dd>{fact.version === null ? '不可变记录' : `第 ${fact.version} 版`}</dd></div>
              </dl>
...
function AuditEvents({ events }: Readonly<{ events: readonly FinanceAuditEvent[] }>) {
  return (
    <section className="financeauditsection" aria-labelledby="financeauditevents">
      <header><h2 id="financeauditevents">来源事件</h2><p>展示事实写入后形成的事件信封和投递终态，不展示事件正文中的敏感字段。</p></header>
      {events.length === 0 ? <p className="financeauditempty">当前热存储中没有可关联的来源事件；业务事实仍然有效。</p> : (
        <ol className="financeauditcards">
          {events.map((event) => (
            <li key={event.id}>
              <header><strong>{eventLabel(event.type)}</strong><span className={`financeauditeventstate state-${event.state}`}>{eventState(event.state)}</span></header>
              <p>{formatDate(event.occurredAt)} · 第 {event.eventVersion} 版事件</p>
...
function AuditRecords({ records }: Readonly<{ records: readonly FinanceAuditEvidence[] }>) {
  return (
    <section className="financeauditsection" aria-labelledby="financeauditrecords">
      <header><h2 id="financeauditrecords">不可变审计证据</h2><p>命令与访问记录按证据时间倒序展示，哈希用于验证记录链完整性。</p></header>
      {records.length === 0 ? <p className="financeauditempty">当前热存储中没有关联审计记录；归档证据需由审计中心按保留策略读取。</p> : (
        <ol className="financeauditcards">
          {records.map((record) => (
            <li key={record.id}>
              <header><strong>{record.kind === 'command' ? '业务操作证据' : '数据访问证据'}</strong><span>{chineseDomainLabel(record.action)}</span></header>
              <p>{formatDate(record.occurredAt)} · {actorLabel(record.actorType)}</p>
              <TechnicalDetails title="查看审计技术信息" values={{ auditId: record.id, action: record.action, resourceType: record.resourceType, resourceId: record.resourceId, actorId: record.actorId, beforeHash: record.beforeHash, afterHash: record.afterHash, recordHash: record.recordHash, evidence: record.evidence }} trace={record.traceId} />
            </li>
          ))}
        </ol>
      )}
...
function TechnicalDetails({ title, values, trace }: Readonly<{ title: string; values: Readonly<Record<string, unknown>>; trace: string | null }>) {
  ...
  return (
    <details className="financeaudittechnical">
      ...
      {trace === null ? null : <div className="financeaudittrace"><code>{trace}</code><button type="button" onClick={copy}>{copied ? '已复制' : '复制追踪编号'}</button><span className="sr-only" aria-live="polite">{copied ? '追踪编号已复制' : ''}</span></div>}
...
function eventLabel(type: string): string {
  const labels: Readonly<Record<string, string>> = { 'finance.entry.posted': '账务分录已入账', 'finance.reconciliation.difference': '对账差异已生成', 'finance.settlement.approved': '结算单已复核', 'finance.withdrawal.paid': '提现已支付', 'finance.withdrawal.uncertain': '提现结果待核实', 'invoice.issued': '发票已开具', 'invoice.red.issued': '红字发票已开具' };
