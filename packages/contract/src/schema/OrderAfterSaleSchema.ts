...
const state = literal(['applied', 'reviewing', 'approved', 'returning', 'received', 'refunding', 'resolved', 'rejected']);
...
export const ORDER_AFTERSALE_BODY_SCHEMAS = {
  ...
  OrderAftersalesApplyInput: strictObject({
    ...
    reason: string(),
    description: string(),
    attachments: optional(array(strictObject({ objectId: string(), name: string(), contentType: literal(['image/jpeg', 'image/png', 'application/pdf']), sizeBytes: unsigned, sha256: string() }))),
  }),
