...
export class OwnershipCancelPreviewHandler implements OperationHandler<'access.ownership.transfers.cancel.preview', 'write'> {
  ...
  async execute(input: OperationInputFor<'access.ownership.transfers.cancel.preview'>, context: WriteHandlerContext<'access.ownership.transfers.cancel.preview'>): Promise<OperationReply<OperationOutputFor<'access.ownership.transfers.cancel.preview'>>> {
