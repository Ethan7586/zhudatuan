...
export function saveTemplateOperations(repositories: NotificationRepositoryFactory): OperationActions {
  return {
    'notification.templates.manage': async (request, database) => {
      ...
      const id = request.input.path.templateid!;
      ...
      const result = await repositories(database).saveTemplate({
        id: template.id,
        scope_id: template.scope,
        channel: template.channel,
        event_type: template.event,
        version: template.version,
        variable_schema: template.variables,
        provider_template: template.providerTemplate,
