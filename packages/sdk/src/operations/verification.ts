...
export function bindMembercodesIssue(client: OperationExecutor): OperationMethod<"verification.membercodes.issue"> { return bindOperation(client, defineOperation({ ...{"id":"verification.membercodes.issue","method":"POST","path":"/api/v1/verifications/member-codes","audience":"storefront","targets":["storefront","miniapp"],"responseMode":"json","idempotencyPolicy":"required","idempotent":false,"timeout":800,"errorUnion":["AUTHENTICATION_REQUIRED","AUTHORIZATION_DENIED","CAPABILITY_DENIED","CONTENT_TYPE_UNSUPPORTED","CONTRACT_VERSION_UNSUPPORTED","CSRF_TOKEN_INVALID","DEADLINE_EXCEEDED","IDEMPOTENCY_CONFLICT","IDEMPOTENCY_KEY_REQUIRED","INTERNAL_ERROR","MEMBERSHIP_SELECTION_REQUIRED","ORIGIN_REQUIRED","PERMISSION_DENIED","RATE_LIMITED","REQUEST_BODY_TOO_LARGE","REQUEST_JSON_INVALID","SCOPE_DENIED","STEPUP_REQUIRED","URL_SENSITIVE_DATA_FORBIDDEN","VALIDATION_FAILED"]}, input: exactOperationInputFrom(VERIFICATION_BODY_SCHEMAS.VerificationMemberCodesIssueInput, [] as const, true), output: exactOperationOutputFrom(VERIFICATION_OUTPUT_SCHEMAS.VerificationMemberCodesIssueOutput) })); }
