...
const fallbacks = manifests.flatMap(({ fallback }) => fallback ? [fallback] : []);
if (fallbacks.length !== 1) throw new Error(`STOREFRONT_ROUTE_FALLBACK_INVALID:${fallbacks.length}`);
...
export const RouteRegistry = Object.freeze({ all: () => manifests, routes: () => Object.freeze(manifests.flatMap(({ routes }) => routes)), fallback: () => fallbacks[0]! });
function isManifest(value: unknown): value is StorefrontManifest {
  if (!value || typeof value !== 'object') return false;
  const item = value as Partial<StorefrontManifest>;
  return typeof item.feature === 'string' && Array.isArray(item.routes);
}
