...
describe('storefront product-to-payment journey', () => {
  it('carries server references through product, cart, quote, order, and payment without calculating money locally', async () => {
    ...
    const cartPort = {
      put: vi.fn((_access: CartAccess, _input: CartChange) => {
        calls.push('cart');
    ...
    const checkoutPort = {
      quote: vi.fn((_session: StorefrontSession, _input: QuoteRequest, _key: string) => {
        calls.push('quote');
        return Promise.resolve(quote);
      }),
      commit: vi.fn((_session: StorefrontSession, _quoteId: string, _token: string, _scene: PaymentScene, _key: string) => {
        calls.push('order');
        return Promise.resolve({ orderId: payment.orderId, payment: { paymentId: payment.paymentId } });
      }),
    };
    const paymentPort = {
      read: vi.fn(() => {
