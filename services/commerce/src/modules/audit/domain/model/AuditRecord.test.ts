...
const write = {
  ...
  actor: 'principal:1',
  actorType: 'console',
  action: 'catalog.listings.publish',
...
describe('Audit integrity records', () => {
  ...
  it('includes sensitive read metadata in the same integrity chain', () => {
    ...
    const access = new AccessRecord(
      ...
      { scope: 'mall:1', actor: 'principal:1', actorType: 'console', resourceType: 'member', resource: 'member:1', fields: { projection: ['id'] }, purpose: 'member.profile.read', trace: 'trace:2' },
      { projection: ['id'] },
      command.recordHash,
