...
export class PgPayoutProcess {
  ...
  constructor(private readonly transactions: TransactionManager, private readonly payouts: PayoutGateway) {}
  ...
  async execute(input: Readonly<{ withdrawal: string; business: string }>, execution: FinanceJobExecution): Promise<void> {
    ...
    const payout = await this.payouts.submit({ withdrawal: id, inputHash: requiredHash(withdrawal.request_hash),
      destination: withdrawal.destination_ref, amountMinor: withdrawal.amount_minor, currency: withdrawal.currency });
  ...
  private async claim(id: string, execution: FinanceJobExecution): Promise<WithdrawalRow | null> {
    return this.transactions.write(options(execution), async (context) => {
      ...
      const selected = await database.query<WithdrawalRow>(
        ...
        from finance.withdrawal where id=$1 for update`, [id]);
      ...
      if (current.state === 'processing') {
        InputWatermark.restore({ hash: requiredHash(current.request_hash), count: 1,
          occurredAt: date(current.input_watermark) }).assert(hash, 1);
      ...
      const claimed = await database.query<WithdrawalRow>(
        ...
        returning id,scope_id,settlement_id,destination_ref,amount_minor::float8 amount_minor,currency,state,
          request_hash,input_watermark,provider,provider_reference,created_at,version::float8 version`, [id, hash]);
  ...
  private async complete(withdrawal: WithdrawalRow, payout: PayoutResult, execution: FinanceJobExecution): Promise<void> {
    ...
    await this.transactions.write(options(execution), async (context) => {
      ...
      await this.finance.post(context, {
        scopeId: row.scope_id, source: { module: 'finance', aggregate: 'withdrawal', aggregateId: id, event: 'finance.withdrawal.paid', eventId: id, leg: 'payout' }, currency: row.currency,
        description: 'Settlement payout', debit: { code: `settlement.payable.${row.partner_id}`, kind: 'liability' },
      ...
      const updated = await client.query(
        ...
        [id, withdrawal.request_hash, payout.provider, payout.reference, payoutHashResult(payout),
          JSON.stringify({ payoutState: payout.state, payoutReference: payout.reference })]);
      ...
      await new PgRuntimeWriter(client).append({
        id: event, type: 'finance.withdrawal.paid', aggregateType: 'withdrawal', aggregate: id, scope: row.scope_id,
        payload: { withdrawal: id, settlement: row.settlement_id, amountMinor: row.amount_minor, currency: row.currency,
          provider: payout.provider, providerReference: payout.reference }, trace: event,
      });
    });
  }
  ...
  private async save(id: string, hash: string, payout: PayoutResult, execution: FinanceJobExecution): Promise<void> {
    await this.transactions.write(options(execution), async (context) => {
      const result = await this.access.database(context).query(
        ...
        [id, hash, payout.provider, payout.reference, payout.state, payoutHashResult(payout),
          JSON.stringify({ payoutState: payout.state, payoutReference: payout.reference, payoutReason: payout.reason ?? null })]);
...
function payoutHash(withdrawal: Pick<WithdrawalRow, 'id' | 'scope_id' | 'settlement_id' | 'destination_ref' | 'amount_minor' | 'currency'>): string {
  return financeFingerprint(['payout-v1', withdrawal.id, withdrawal.scope_id, withdrawal.settlement_id,
    withdrawal.destination_ref, withdrawal.amount_minor, withdrawal.currency]);
}
...
interface WithdrawalRow {
  readonly id: string; readonly scope_id: string; readonly settlement_id: string; readonly destination_ref: string;
  readonly amount_minor: number; readonly currency: string; readonly state: string; readonly request_hash: string | null;
  readonly input_watermark: string | null; readonly provider: string | null; readonly provider_reference: string | null;
  readonly created_at: string; readonly version: number;
}
interface PayoutWithdrawal {
  readonly scope_id: string; readonly settlement_id: string; readonly partner_id: string; readonly amount_minor: number;
  readonly currency: string; readonly provider: string | null; readonly provider_reference: string | null;
}
