...
import { CART_PRICING_PORT, CATALOG_PRICING_PORT, CHECKOUT_PRICING_PORT, PROVIDER_PRICING_PORT, RUNTIME_PRICING_PORT } from './public/index';
...
export const PricingModule = defineModule(Manifest, {
  ...
  handlers: () => {
    const rules = new PgRuleRepository(new PgTransactionAccess());
    return [new RulesCreateHandler(rules), new RulesPublishHandler(rules)];
  },
  ports: (context) => {
