...
describe('NavigationTree', () => {
  it('accepts a deep link only when its complete Chinese breadcrumb can be reversed', () => {
    const child = node('child', '订单详情', 'root', '/orders/:orderId', [{ key: 'root', title: '订单中心' }, { key: 'child', title: '订单详情' }]);
...
function navigationTree(nodes: readonly NavigationNodeValue[], defaultKey: string, defaultRoute: string): NavigationTree {
  return new NavigationTree({ scope: { id: 'enterprise:one', kind: 'enterprise' }, target: 'console', version: 'version:one', etag: '"version:one"', generatedAt: '2026-09-04T00:00:00.000Z', catalogVersion: 'catalog:one', defaultKey, defaultRoute, nodes });
}
...
function node(key: string, title: string, parent: string | null, route: string, breadcrumbs: NavigationNodeValue['experience']['breadcrumbs'], children: readonly NavigationNodeValue[] = []): NavigationNodeValue {
  return { key, title, parent, order: 10, operation: 'order.orders.read', experience: { icon: 'order', routeKey: `${key}route`, route, component: 'order', placement: parent === null ? 'primary' : 'contextual', disabled: false, disabledReason: null, breadcrumbs }, children };
}
