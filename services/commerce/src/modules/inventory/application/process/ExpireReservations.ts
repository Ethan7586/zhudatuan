...
export class ExpireReservations {
  ...
  async execute(request: ReservationExpiryRequest): Promise<void> {
    ...
    while (more && batches < 20) {
      ...
      const result = await this.transactions.write({ tenant: request.scope, membership: '', scope: request.scope, actor: 'job:reservationexpiry',
        trace: request.trace, operation: 'job.inventory.reservationexpiry', workload: 'jobs', signal: request.signal, deadline: request.deadline },
