...
export class VoucherDeadletter implements JobDeadletter {
  ...
  private async issue(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, batchid: string, error: string): Promise<void> {
    await database.query(`with failed as (update voucher.issuebatch batch set state='failed' from voucher.program program
      ...
      owned as (select failed.*,pool.scope_id owner_scope from failed join voucher.cardpool pool on pool.id=failed.cardpool_id)
      ...
      from owned where owned.owner_scope<>owned.scope_id and allocation.cardpool_id=owned.cardpool_id and allocation.scope_id=owned.scope_id`, [batchid]);
    await database.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,available_at)
      ...
      on conflict(id) do nothing`, [`event:${randomUUID()}`, error, batchid]);
  }
  ...
  private async import(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, importid: string, error: string): Promise<void> {
    await database.query(`with failed as (update voucher.importjob set state='failed',updated_at=clock_timestamp() where id=$1
        returning cardpool_id,scope_id) update voucher.cardpool pool set status='disabled',version=version+1 from failed where pool.id=failed.cardpool_id`, [importid]);
    await database.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,available_at)
  ...
  private async status(database: Readonly<{ query(text: string, values?: readonly unknown[]): Promise<unknown> }>, batchid: string, error: string): Promise<void> {
    await database.query(`update voucher.statusitem set state='failed',error_code=$2,updated_at=clock_timestamp()
      where batch_id=$1 and state='queued'`, [batchid, error]);
    await database.query(`update voucher.statusbatch batch set state='failed',
      succeeded_count=(select count(*) from voucher.statusitem where batch_id=batch.id and state='succeeded'),
      failed_count=(select count(*) from voucher.statusitem where batch_id=batch.id and state='failed'),updated_at=clock_timestamp() where id=$1`, [batchid]);
    await database.query(`insert into runtime.outbox(id,event_type,event_version,aggregate_type,aggregate_id,scope_id,payload,trace_id,occurred_at,available_at)
      select $1,'voucher.status.failed',1,'statusbatch',batch.id,batch.scope_id,jsonb_build_object('batch',batch.id,'error',$2),$1,
        clock_timestamp(),clock_timestamp() from voucher.statusbatch batch where batch.id=$3`, [`event:${randomUUID()}`, error, batchid]);
  }
}
