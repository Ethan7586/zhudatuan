...
export class PgCatalogSku implements CatalogSku {
  ...
  async find(context: ReadTransactionContext, scope: string, reference: string): Promise<string | null> {
    ...
    const selected = await database.query<{
      ...
      where (sku.id=$1 or sku.code=$1) and (product.owner_partner_id=$2
        or exists(select 1 from catalog.listing listing where listing.sku_id=sku.id and listing.scope_id=$2)
        or exists(select 1 from catalog.sourcelisting source where source.sku_id=sku.id and source.scope_id=$2))
      order by (sku.id=$1) desc limit 1`,
      [reference, scope]
    );
