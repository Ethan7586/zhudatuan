...
import type { HandlerContext, PrepareContext, WriteHandlerContext } from '../../../../foundation/application/HandlerContext';
import type { DurableCommit, DurableOperationHandler, OperationReply } from '../../../../foundation/application/OperationHandler';
import type { TransactionMode } from '../../../../foundation/persistence/TransactionContext';

type Reply<TKey extends OperationId> = OperationReply<OperationOutputFor<TKey>>;
type Context<TKey extends OperationId, TMode extends TransactionMode> = TMode extends 'write'
  ? WriteHandlerContext<TKey>
  : HandlerContext<TKey>;

export abstract class PreparedOperation<TKey extends OperationId, TPrepared, TMode extends TransactionMode>
implements DurableOperationHandler<TKey, TPrepared, Reply<TKey>, TMode> {
  abstract readonly operation: TKey;
  abstract readonly mode: TMode;

  constructor(private readonly prepareValue: (
    input: OperationInputFor<TKey>,
    context: PrepareContext<TKey>,
  ) => Promise<TPrepared>) {}

  abstract commit(
    input: OperationInputFor<TKey>,
    prepared: TPrepared,
    context: Context<TKey, TMode>,
  ): Promise<DurableCommit<Reply<TKey>, OperationOutputFor<TKey>>>;
