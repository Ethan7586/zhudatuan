...
export class PoolPolicy {
  validate(value: CredentialPoolValue): void {
    if (!value.id || !value.scope || !value.product || !Number.isSafeInteger(value.capacity) || value.capacity <= 0 || !Number.isSafeInteger(value.generated) || value.generated < 0 || value.generated > value.capacity || value.version < 0) throw new DomainError('VOUCHER_CREDENTIAL_CONFLICT');
  }
