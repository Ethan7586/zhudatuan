...
export const pricingViewModel = defineSupplierViewModel({
  ...
  execute: async (client, context, _route, value, selectedKey, action, input) => {
    ...
    await client.catalog.listingsPriceSet(
      { path: { listingid: operatorText(item, 'id') }, body: { amountMinor: requiredMoneyMinor(input, 'amount') } },
...
function priceActions(item: OperatorRecord | undefined): readonly OperatorAction[] {
  ...
  return Object.freeze([Object.freeze({
    id: 'price', label: current === undefined ? '设置报价' : '更新报价', description: '输入含税人民币单价，服务端按价格版本防止覆盖。',
    confirmation: '请核对商品和报价；提交后平台将立即记录新版本。', tone: 'primary', requiresSelection: true, identityScope: true, expectedVersion: version,
...
function priceProjection(value: unknown) {
  return operatorCollection(value, operatorItems(value).map((item) => operatorRow({
    ...
    statusLabel: operatorText(item, 'sku_id') ? '可维护报价' : '等待商品映射', timestamp: operatorText(item, 'cursor_sort'),
  })));
}
