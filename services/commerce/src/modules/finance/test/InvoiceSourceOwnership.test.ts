...
describe('invoice source ownership', () => {
  it('verifies payment and order sources through their owner Ports before persisting the request', async () => {
    ...
    const query = vi.fn(async (sql: string) => {
      ...
      if (sql.includes('from finance.settlement settlement'))
        return result([
    ...
    const create = invoiceActions(
      () => {
        throw new Error('UNUSED_WORKFLOW');
      },
      { verified },
      { orders: paymentOrders } as unknown as FinancePaymentPort
    ).requestsCreate as FinanceAction;
  ...
  it('fails closed when a source owner cannot verify every order', async () => {
    const query = vi.fn(async (sql: string) => (sql.includes('from finance.settlement settlement') ? result([{ id: 'line:one', source_type: 'payment', source_id: 'payment:one', amount_minor: 100, tax_minor: 5 }]) : result([])));
    const create = invoiceActions(
      () => {
        throw new Error('UNUSED_WORKFLOW');
      },
      ...
      } as unknown as FinancePaymentPort
    ).requestsCreate as FinanceAction;

    await withWriteTransaction(query, async (transaction) => {
      await expect(create(invoiceRequest(transaction, ['line:one'], 100), new PgTransactionAccess().database(transaction))).rejects.toThrow('INVOICE_LINES_NOT_ELIGIBLE_OR_AMOUNT_MISMATCH');
    });
...
function invoiceRequest(transaction: FinanceRequest['transaction'], lines = ['line:one', 'line:two'], amountMinor = 100): FinanceRequest {
  return {
    ...
    input: {
      ...
      query: {},
      headers: {},
      body: { settlement: 'settlement:one', profile: 'profile:one', amountMinor, lines, reason: '业务开票', evidence: {} },
      rawBody: '{}',
