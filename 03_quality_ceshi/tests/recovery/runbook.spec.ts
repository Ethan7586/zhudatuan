...
import { JOB_CATALOG } from '../../../01_core_hexin/services/commerce/src/app/jobs';
...
test('all mandatory incident recovery runbooks contain business validation and evidence', () => {
  for (const name of incidents) {
    const text = readFileSync(join(root, `05_docs_ziliao/docs_wendang/runbooks_yunwei/${name}.md`), 'utf8').toLowerCase();
...
test('recovery configuration declares the required RPO, RTO and immutable evidence checks', () => {
  const telemetry = readFileSync(join(root, '02_platform_pingtai/config/telemetry.yml'), 'utf8');
  assert.match(telemetry, /rpoMinutes: 5/);
  assert.match(telemetry, /rtoMinutes: 30/);
  assert.match(readFileSync(join(root, '05_docs_ziliao/docs_wendang/runbooks_yunwei/databasefailover.md'), 'utf8'), /LSN|RPO/);
  assert.match(readFileSync(join(root, '05_docs_ziliao/docs_wendang/runbooks_yunwei/migrationrollback.md'), 'utf8'), /94 historical hashes/);
});
