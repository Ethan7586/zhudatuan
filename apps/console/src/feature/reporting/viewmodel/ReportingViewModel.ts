...
export function useReportingViewModel(context: ConsoleContext, dependencies: ReportingDependencies, requestStepup: () => void) {
  ...
  useEffect(() => {
    ...
    if (job?.state !== 'completed' || receipt?.reference === job.id) return;
    setReceipt(Object.freeze({ requestId: identity, reference: job.id, occurredAt: job.generatedAt ?? new Date().toISOString(), message: `安全导出已生成，共 ${job.recordCount} 条；下载链接将在服务端指定时间失效。` }));
