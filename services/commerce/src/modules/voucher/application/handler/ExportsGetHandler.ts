...
export class ExportsGetHandler implements DurableOperationHandler<'voucher.exports.get', null, Checkpoint, 'write'> {
  ...
  prepare(_input: OperationInputFor<'voucher.exports.get'>, _context: PrepareContext<'voucher.exports.get'>): Promise<null> {
  ...
  async commit(input: OperationInputFor<'voucher.exports.get'>, _prepared: null, context: CommitContext<'voucher.exports.get'>) {
    ...
    const download = record.state === 'completed' && downloadSeconds(record) >= 60 ? await this.exports.take(context.transaction, record.id, access.scope.id, 'voucher') : null;
  ...
  async finalize(_input: OperationInputFor<'voucher.exports.get'>, checkpoint: Checkpoint, context: FinalizeContext<'voucher.exports.get'>): Promise<Reply> {
    ...
    const access = requireSession(context.security);
    await this.transactions.read(
      { tenant: access.scope.id, scope: access.scope.id, membership: access.actor.membership, actor: access.actor.id, trace: context.traceId, operation: context.operation, deadline: context.deadline, signal: context.signal },
    ...
    if (!Number.isFinite(signedExpiry) || signedExpiry <= Date.now() || signedExpiry > Date.parse(checkpoint.download.record.expiresAt)) throw new DomainError('VOUCHER_EXPORT_NOT_READY');
  ...
  private assertOwner(work: RuntimeExportWork, context: PrepareContext<'voucher.exports.get'>): void {
    ...
    if (work.scope !== access.scope.id || work.authorization.actor !== access.actor.id || work.authorization.membership !== access.actor.membership) throw new DomainError('AUTHORIZATION_DENIED');
  }
