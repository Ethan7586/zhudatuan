...
export class PgOutboxCleanup implements OutboxCleanup {
  ...
  async plan(context: ReadTransactionContext, before: Date, limit: number): Promise<readonly string[]> {
    cleanupLimit(limit);
    const result = await this.transactions.database(context).query<{ id: string }>(`select id from runtime.outbox where published_at is not null and published_at<$1 order by published_at,id limit $2`, [retentionBoundary(before), limit]);
  ...
  async purge(context: WriteTransactionContext, ids: readonly string[], before: Date): Promise<number> {
    ...
    const result = await this.transactions.database(context).query(`delete from runtime.outbox where id=any($1::text[]) and published_at is not null and published_at<$2 returning id`, [planned, retentionBoundary(before)]);
