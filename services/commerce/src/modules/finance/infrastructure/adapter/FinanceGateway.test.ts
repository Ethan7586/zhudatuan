...
describe('finance provider gateways', () => {
  it('submits payout with the withdrawal idempotency key', async () => {
    ...
    await expect(gateway.submit({ withdrawal: 'withdrawal:1', destination: 'secret:destination:1', amountMinor: 100, currency: 'CNY' })).resolves.toEqual({ reference: 'provider:1', state: 'paid' });
