...
export async function createMiniappPaymentParameters(config: WechatPayConfig, appId: string, prepayId: string, options: { nowMs?: number; nonce?: string } = {}): Promise<MiniappPaymentParameters> {
  if (!/^wx[A-Za-z0-9]{16}$/.test(appId)) throw new Error('WECHAT_APP_ID_INVALID');
  ...
  const paySign = await signRsaSha256(config.merchantPrivateKeyPem, miniappSignatureMessage(appId, timeStamp, nonceStr, packageValue));
...
function pemToArrayBuffer(pem: string, label: 'PRIVATE KEY' | 'PUBLIC KEY'): ArrayBuffer {
  ...
  const end = `-----END ${label}-----`;
  if (!pem.startsWith(begin) || !pem.endsWith(end)) throw new Error('WECHAT_PAY_KEY_INVALID');
