...
export function manageRiskPolicyOperations(factory: RiskRepositoryFactory): OperationActions {
  return { 'risk.policies.manage': async (request, database) => {
    const access = requireAccess(request); const body = bodyRecord(request); const action = body.action ?? 'save';
    const repository = factory(database); const id = request.input.path.policyid!;
    if (action === 'activate') {
      const version = integerField(body, 'version', 1); const rollout = optionalPercent(body.rolloutPercent, 100);
      return { status: 200, body: await repository.activatePolicy({ id, scope: access.scope.id, version, rolloutPercent: rollout,
        actor: access.actor.id, trace: access.trace }) };
    }
    ...
    const saved = await repository.savePolicy({ id, scope: access.scope.id, name: textField(body, 'name'), rule: policy.rule,
      ruleHash: policy.hash, rolloutPercent: rollout, actor: access.actor.id });
