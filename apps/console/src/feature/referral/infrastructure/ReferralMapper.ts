...
export class ReferralMapper {
  ...
  private setting(item: ReturnType<typeof ReferralSettingSchema.parse>): ReferralSetting {
    return {
      ...
      recruitEnabled: item.recruitEnabled,
      ...
      settlementTrigger: item.settlementTrigger,
      rateBasisPoints: item.rateBasisPoints,
      minimumWithdrawalMinor: item.minimumWithdrawalMinor,
      monthlyWithdrawalLimit: item.monthlyWithdrawalLimit,
  ...
  private product(item: ReturnType<typeof ReferralProductPageSchema.parse>['items'][number]): ReferralProduct {
    return { id: item.id, productId: item.productId, enabled: item.enabled, rateBasisPoints: item.rateBasisPoints, rewardBasisPoints: item.rewardBasisPoints, version: item.version, updatedAt: item.updatedAt };
  }
  ...
  private binding(item: ReturnType<typeof ReferralBindingPageSchema.parse>['items'][number]): ReferralBinding {
    return { id: item.id, promoterId: item.promoterId, memberId: item.memberId, source: item.source, boundAt: item.boundAt, expiresAt: item.expiresAt, status: item.status, version: item.version };
  }
  ...
  private commission(item: ReturnType<typeof ReferralCommissionPageSchema.parse>['items'][number]): ReferralCommission {
    return {
      ...
      refundedBaseMinor: item.refundedBaseMinor,
      reversedMinor: item.reversedMinor,
      rateBasisPoints: item.rateBasisPoints,
      currency: item.currency,
      availableAt: item.availableAt,
      settlementJournalId: item.settlementJournalId,
      version: item.version,
    };
  }
