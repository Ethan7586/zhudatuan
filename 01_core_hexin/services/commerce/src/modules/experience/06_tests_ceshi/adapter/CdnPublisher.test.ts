...
class MemoryObjects implements ObjectStore {
  readonly values = new Map<string, { bytes: Uint8Array; object: StoredObject & { contentType: string } }>();
  creates = 0;
  ...
  async create(path: string, contentType: string): Promise<ObjectUpload> {
    ...
    return {
      append: async (part) => { bytes = new Uint8Array([...bytes, ...part]); },
      abort: async () => undefined,
      complete: async () => {
        const sha256 = createHash('sha256').update(bytes).digest('hex');
        const object = { reference: `object:${path}`, sha256, size: bytes.byteLength, scan: 'clean' as const, contentType };
        this.values.set(path, { bytes, object }); this.values.set(object.reference, { bytes, object });
        return object;
      },
    };
  }
  async find(path: string) { return this.values.get(path)?.object ?? null; }
  async inspect(reference: string) { const value = this.values.get(reference)?.object; if (!value) throw new Error('NOT_FOUND'); return value; }
  async read(reference: string) { const value = this.values.get(reference)?.bytes; if (!value) throw new Error('NOT_FOUND'); return value; }
  async authorize(reference: string) { return { url: `https://objects.test/${reference}`, expiresAt: new Date(Date.now()+60_000).toISOString() }; }
}
...
describe('content addressed experience publisher', () => {
  ...
  it('rejects a hash that does not describe the canonical payload', async () => {
    await expect(new CdnPublisher(new MemoryObjects()).publish(`experience/application:test/${'0'.repeat(64)}.json`, document, '0'.repeat(64), new AbortController().signal))
      .rejects.toThrow('EXPERIENCE_CONTENT_HASH_MISMATCH');
  });
});
