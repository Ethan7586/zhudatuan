...
export class WechatChannel implements DeliveryChannel {
  ...
  constructor(
    private readonly configuration: WechatDeliveryConfiguration,
    fetcher: typeof fetch = fetch
  ) {
    if (
      !/^[A-Za-z0-9_-]{6,64}$/.test(configuration.appId) ||
      configuration.appSecret.length < 16 ||
      !['developer', 'trial', 'formal'].includes(configuration.state) ||
      (configuration.page && !/^pages?\/[A-Za-z0-9/_-]{1,240}$/.test(configuration.page))
    ) {
  ...
  async send(request: Parameters<DeliveryChannel['send']>[0]) {
    ...
    const response = await this.http.send(
      `https://api.weixin.qq.com/cgi-bin/message/subscribe/send?access_token=${encodeURIComponent(token.value)}`,
      {
        method: 'POST',
        redirect: 'error',
        ...
        body: JSON.stringify({
          ...
          template_id: request.providerTemplate,
          ...(this.configuration.page ? { page: this.configuration.page } : {}),
          miniprogram_state: this.configuration.state,
          lang: 'zh_CN',
  ...
  private async accessToken(): Promise<AccessToken> {
    ...
    this.loading ??= this.loadToken().finally(() => {
      this.loading = null;
    });
...
function wechatValue(value: string | number | boolean): string {
  ...
  if (normalized.length > 20) throw new Error('WECHAT_VARIABLE_TOO_LONG');
  return normalized;
}
