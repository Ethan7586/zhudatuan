import { chineseDomainLabel, chineseProviderLabel, chineseReference, chineseSectionLabel, queryCondition, safeQueryError } from '@shop/presentation';
...
const columns: readonly DataColumn<OrderLine>[] = [
  ...
  { key: 'provider', label: '履约来源', render: (row) => (row.provider ? chineseProviderLabel(row.provider) : row.partner ? chineseReference('合作方', row.partner) : '—') },
];
...
export function Component() {
  ...
  return (
    <ResourcePanel
      ...
      description={`${chineseReference('内部订单', orderId)}的权威快照；当前接口协议不支持使用展示订单号反查。`}
      condition={state}
      ...
      {data === undefined ? (
        ...
        <div className="featurestack">
          ...
          <section className="capabilitynote" aria-labelledby="orderdetailboundary">
            ...
            <p>发货、退款与售后审批缺少完整的操作预览、二次验证、预期版本和操作绑定凭证时不执行。</p>
          </section>
