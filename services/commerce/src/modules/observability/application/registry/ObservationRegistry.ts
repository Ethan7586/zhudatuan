...
export class ObservationRegistry {
  constructor(
    ...
    unique(
      serviceLevels.map(({ definition }) => definition.id),
      'SERVICE_LEVEL_CATALOG_DUPLICATE'
    );
    unique(
      alerts.map(({ definition }) => definition.id),
      'ALERT_RULE_CATALOG_DUPLICATE'
    );
  ...
  catalog(): ObservabilityCatalog {
    const metricIds = new Set([...this.serviceLevels.map(({ definition }) => definition.indicator.metric), ...this.alerts.flatMap(({ definition }) => (definition.signal.startsWith('slo.') ? [] : [definition.signal]))]);
