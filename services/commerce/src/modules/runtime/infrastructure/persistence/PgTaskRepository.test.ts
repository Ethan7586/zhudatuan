...
describe('runtime import confirmation', () => {
  it('rejects confirmation when server preflight has row errors', async () => {
    ...
    await expect(repository.confirmImport({} as never, { id: 'import:one', scope: 'mall:one', actor: 'principal:one', expectedVersion: 2, previewHash: 'a'.repeat(64), reason: '确认商品导入' })).rejects.toMatchObject({
      code: 'VALIDATION_FAILED',
      details: { field: 'validationErrors' },
    });
  ...
  it('guards the confirming update against errors inserted after the task read', async () => {
    ...
    await repository.confirmImport({} as never, { id: 'import:one', scope: 'mall:one', actor: 'principal:one', expectedVersion: 2, previewHash: 'a'.repeat(64), reason: '确认商品导入' });
...
function task(validationErrors: number) {
  return {
    ...
    downloadAvailable: validationErrors > 0,
