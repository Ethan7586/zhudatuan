...
describe('payment webhook boundary', () => {
  it('rejects forged input before scope resolution and Inbox persistence', async () => {
    ...
    const handler = new WebhooksWechatHandler(
      {
        verifyNotification: vi.fn(async () => {
          throw new Error('PROVIDER_SIGNATURE_INVALID');
        }),
      } as never,
      scopes as never,
      webhooks as never
  ...
  it('acknowledges a verified duplicate without scheduling business work in the HTTP handler', async () => {
    const notification = {
      ...
      application: { scene: 'miniapp', applicationHash: 'b'.repeat(64) },
      occurredAt: '2026-09-05T00:00:00.000Z',
      evidence: {},
    } as const;
    const handler = new WebhooksWechatHandler({ verifyNotification: vi.fn(async () => notification) } as never, { resolve: vi.fn(async () => 'mall:one') } as never, { accept: vi.fn(async () => ({ replayed: true })) } as never);
