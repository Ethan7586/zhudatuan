...
describe('runtime configuration schema', () => {
  it('keeps the private CA trust path in every registration-only HTTPS client env', async () => {
    ...
    for (const file of ['identity-registration-api.env.example','identity-notification-jobs.env.example',
      ...
      const source = await readFile(new URL(`../../../../02_platform_pingtai/infrastructure/zhudatuan/aliyun/${file}`, import.meta.url), 'utf8');
  ...
  it('isolates strict registration migration settings from the generic migration environment', () => {
    const environment = {
      ...
      MIGRATION_DIRECTORY: '/opt/zhudatuan/current/02_platform_pingtai/database/supabase/migrations',
  ...
  it('accepts only the fail-closed registration API dependency allowlist', () => {
    const registration = {
      ...
      API_ALLOWED_ORIGINS: 'https://accounts.zhudatuan.com,https://console.zhudatuan.com,https://h5.zhudatuan.com,https://hbbtzn.com,https://mini.zhudatuan.com,https://zhudatuan.com',
