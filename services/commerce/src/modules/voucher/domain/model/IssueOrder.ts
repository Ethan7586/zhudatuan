...
export interface IssueOrderValue {
  readonly id: string;
  readonly scope: string;
  readonly customer: string;
  readonly product: string;
  readonly stockRequest: string;
  readonly quantity: number;
  readonly purpose: 'benefit' | 'order' | 'campaign' | 'manual';
  readonly delivery: 'account' | 'claim';
  readonly startsAt: Date;
  readonly expiresAt: Date;
  readonly recipientSnapshot: string;
  readonly reason: string;
  readonly requester: string;
  readonly approval: string | null;
  readonly state: IssueOrderState;
  readonly version: number;
}
export class IssueOrder {
  constructor(readonly value: IssueOrderValue) {
    new IssuePolicy().validateOrder(value);
  }
