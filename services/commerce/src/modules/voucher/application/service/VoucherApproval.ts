...
export class VoucherApproval {
  constructor(private readonly approval: ApprovalPort, private readonly read: ApprovalReadPort) {}
  ...
  request(context: WriteTransactionContext, input: Pick<ApprovalRequest, 'subject' | 'action' | 'constraints' | 'expiresAt' | 'amountMinor' | 'currency'>) {
    return this.approval.request(context, { ...input, scopeId: context.scope, requesterId: context.membership,
      evidenceHash: createHash('sha256').update(JSON.stringify(input.subject.snapshot)).digest('hex') });
  }
  ...
  async cancel(context: WriteTransactionContext, input: Readonly<{ scope: string; instance: string; reason: string }>): Promise<void> {
    ...
    await this.approval.cancel(context, { scopeId: input.scope, instanceId: current.id,
      requesterId: context.membership, expectedVersion: current.version, reason: input.reason });
  }
}
