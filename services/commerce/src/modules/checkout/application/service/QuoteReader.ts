...
export class QuoteReader {
  ...
  async read(transaction: ReadTransactionContext, membership: string, selection: CheckoutSelection, control: Readonly<{ expiresAt: number; signal: AbortSignal }>): Promise<CheckoutQuote> {
    ...
    const [lines, policies, purchases, tags, campaigns, priceRules, vouchers, benefits] = await allParallel(
      [
        ...
        () => this.policies(transaction, cart.mall_id),
        () => this.purchases(transaction, cart.member_id),
        () => this.tags(transaction, cart.member_id),
        () => this.campaigns(transaction, cart.mall_id),
        () => this.priceRules(transaction, cart.mall_id),
        () => this.vouchers(transaction, cart, selection),
        () => this.benefits(transaction, cart, selection),
      ] as const,
      { concurrency: 4, ...control }
    );
    ...
    const evidence = Object.freeze({
      ...
      vouchers: vouchers.map(({ id, version, program, remainingMinor }) => ({ id, version, program, remainingMinor })),
  ...
  private async cart(transaction: ReadTransactionContext, membership: string, selection: CheckoutSelection, control: Readonly<{ expiresAt: number; signal: AbortSignal }>): Promise<CartRow> {
    const owner = await this.dependencies.access.profile(transaction, membership);
    const cart = await this.dependencies.cart.current(transaction, owner.member, owner.organization);
    const [profile, address, invoice, experience] = await allParallel(
      [
        () => this.dependencies.qualification.profile(transaction, owner.member, owner.organization),
        () => this.dependencies.address.snapshot(transaction, selection.addressId, owner.member),
        () => this.dependencies.invoice.snapshot(transaction, selection.invoiceId, owner.member),
        () => this.dependencies.experience.published(transaction, cart.application),
      ] as const,
      { concurrency: 4, ...control }
    );
  ...
  private async lines(transaction: ReadTransactionContext, cart: CartRow, control: Readonly<{ expiresAt: number; signal: AbortSignal }>): Promise<readonly LineRow[]> {
    ...
    const [catalog, prices, stocks] = await allParallel(
      [
        () => this.dependencies.catalog.items(transaction, cart.mall_id, listings),
        () => this.dependencies.pricing.offers(transaction, cart.mall_id, skus),
        () => this.dependencies.inventory.availability(transaction, cart.mall_id, skus),
      ] as const,
      { concurrency: 4, ...control }
    );
  ...
  private async policies(context: ReadTransactionContext, scope: string): Promise<readonly PolicyRow[]> {
    return Object.freeze((await this.dependencies.qualification.policies(context, scope)).map((row) => Object.freeze({ ...row, rule_hash: row.hash, amount_minor: row.amountMinor })));
  }

  private async purchases(context: ReadTransactionContext, member: string): Promise<ReadonlyMap<string, PurchaseRow>> {
    const rows = await this.dependencies.orders.purchases(context, member);
    return new Map(
      rows.map((row) => [
        row.listing,
        Object.freeze({
  ...
  private async tags(context: ReadTransactionContext, member: string): Promise<ReadonlySet<string>> {
    return new Set(await this.dependencies.qualification.tags(context, member));
  }

  private async campaigns(context: ReadTransactionContext, scope: string): Promise<readonly CampaignRow[]> {
    return Object.freeze((await this.dependencies.marketing.campaigns(context, scope)).map((row) => Object.freeze({ id: row.id, version: row.version, rule: row.rule, remaining_budget: row.remainingBudget })));
  }

  private async priceRules(context: ReadTransactionContext, scope: string): Promise<readonly PriceRuleRow[]> {
    return this.dependencies.pricing.rules(context, scope);
  }

  private async vouchers(context: ReadTransactionContext, cart: CartRow, selection: CheckoutSelection): Promise<readonly QuoteVoucherChoice[]> {
    if (selection.voucherIds.length === 0) return [];
    const rows = await this.dependencies.voucher.preview(context, selection.voucherIds, cart.member_id, cart.mall_id);
    if (rows.length !== selection.voucherIds.length) throw new Error('VOUCHER_NOT_USABLE');
    return rows;
  }

  private async benefits(context: ReadTransactionContext, cart: CartRow, selection: CheckoutSelection): Promise<readonly BenefitChoice[]> {
    if (selection.benefits.length === 0) return [];
    const ids = selection.benefits.map(({ accountId }) => accountId);
    const rows = await this.dependencies.benefit.preview(context, cart.member_id, cart.mall_id, ids);
