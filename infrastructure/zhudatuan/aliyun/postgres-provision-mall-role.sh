...
: "${ZHUDATUAN_PROVISIONING_API_PASSWORD:?ZHUDATUAN_PROVISIONING_API_PASSWORD is required}"
...
select
  pg_catalog.set_config('zhudatuan.provisioning.expected_database',:'expected_database',true) is not null database,
  pg_catalog.set_config('zhudatuan.provisioning.expected_server_addr',:'expected_server_addr',true) is not null server_addr
\gset guard_

select length(:'provisioning_api_password')>=32 secrets_valid
\gset guard_
\if :guard_secrets_valid
\else
  \echo 'Mall provisioning role secret contract invalid'
  \quit 3
\endif

do $guard$
declare
  expected_addr constant inet := current_setting('zhudatuan.provisioning.expected_server_addr')::inet;
  authority constant oid := (select oid from pg_roles where rolname=current_user);
  role_oid constant oid := to_regrole('zhudatuanprovisioningapi');
begin
  if current_database()<>current_setting('zhudatuan.provisioning.expected_database')
    ...
    raise exception 'ZHUDATUAN_PROVISIONING_ROLE_DATABASE_IDENTITY_INVALID';
  end if;
  if authority is null or not (
    ...
    raise exception 'ZHUDATUAN_PROVISIONING_ROLE_AUTHORITY_INVALID';
  end if;
  if not exists(select 1 from deployment.boundary
    ...
    raise exception 'ZHUDATUAN_PROVISIONING_ROLE_DATABASE_BOUNDARY_MISSING';
  end if;
  if role_oid is not null and (
    ...
    raise exception 'ZHUDATUAN_PROVISIONING_ROLE_EXISTING_ROLE_UNSAFE';
  end if;
end
$guard$;

select format('create role zhudatuanprovisioningapi login password %L noinherit',:'provisioning_api_password')
where to_regrole('zhudatuanprovisioningapi') is null
\gexec
...
begin
  if not exists(select 1 from pg_roles where rolname='zhudatuanprovisioningapi'
    ...
    raise exception 'ZHUDATUAN_PROVISIONING_ROLE_FINAL_STATE_INVALID';
  end if;
end
$assert$;
