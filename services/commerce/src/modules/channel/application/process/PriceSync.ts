...
export async function synchronizePrice(session: SyncSession): Promise<void> {
  ...
  const batch = await session.provider('Price').pullPrice(await session.providerContext(), keys);
  ...
  await session.apply('channel.price.persist', async (context, applying) => {
    const catalog = required(session.dependencies.catalog, 'CHANNEL_CATALOG_PORT_MISSING');
    const pricing = required(session.dependencies.pricing, 'CHANNEL_PRICING_PORT_MISSING');
    ...
    for (const record of batch.records) {
      ...
      new ExternalMapping({
        ...
        internalType: 'sku',
        internalId: sku,
        sourceVersion: String(record.version ?? record.effectiveAt ?? 'current'),
        watermark: effective,
        version: 0,
      });
      await pricing.saveProviderPrice(context, {
        ...
        book,
        sku,
        amountMinor: integer(record.amountMinor, 'PROVIDER_PRICE_INVALID'),
