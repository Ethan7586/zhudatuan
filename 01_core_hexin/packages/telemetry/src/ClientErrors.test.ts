...
describe('ClientErrorBuffer', () => {
  it('deduplicates, redacts and keeps a stable fault code', () => {
    ...
    const buffer = new ClientErrorBuffer((record) => { written.push(record); });
    const input = { scope:mall, surface:'console', route:'/orders', message:'mobile 13800138000 mail a@b.com', stack:'Bearer abc.def',
      componentStack:null, traceId:'trace', actorId:'actor', membershipId:'membership' };
    const first = buffer.record(input); const second = buffer.record(input);
  ...
  it('filters records by the operator scope hierarchy', () => {
    ...
    buffer.record({ scope:mall, surface:'console', route:'/orders', message:'failure', stack:null, componentStack:null,
      traceId:'trace', actorId:'actor', membershipId:'membership' });
  ...
  it('expires old records and bounds memory', () => {
    let now = 0;
    const buffer = new ClientErrorBuffer(() => undefined, 1, 10, () => now);
    const input = { scope:mall, surface:'console', route:'/one', message:'one', stack:null, componentStack:null,
      traceId:'trace', actorId:'actor', membershipId:'membership' };
    buffer.record(input);
