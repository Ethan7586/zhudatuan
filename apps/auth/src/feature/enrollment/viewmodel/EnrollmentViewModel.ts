...
export function useEnrollmentViewModel(
  ...
  useEffect(() => {
    ...
    const confirmElement = confirmInput.current;
    return () => {
      clearSecretInput(passwordElement, password);
      clearSecretInput(confirmElement, confirm);
    };
  }, [confirm, password]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setNotice(challengeNotice(result.value));
    } finally {
      setBusy(undefined);
    }
  };
  ...
  const submit = async () => {
    ...
    if (challenge === undefined || challengeSubject !== subject) return setError('请先为当前邀请获取验证码。');
...
function initial(enrollment: EnrollmentState): EnrollmentForm {
  return { displayName: enrollment.employee?.displayName ?? '', mobile: '', code: '' };
}

function strong(value: string, policy: PasswordPolicy): boolean {
  return (
