...
export function DataTable<T extends RowData>({ caption, columns, rows, rowKey }: DataTableProps<T>) {
  const tableColumns = useMemo(() => {
    ...
    return helper.columns(
      columns.map((column) => helper.accessor((row) => row, { id: column.key, header: column.label, cell: ({ row }) => column.render(row.original) }))
    );
  }, [columns]);
      ...
      <table>
        ...
        <thead>
          {table.getHeaderGroups().map((group) => (
            <tr key={group.id}>
              {group.headers.map((header) => <th key={header.id} scope="col" data-column={header.column.id}>{header.isPlaceholder ? null : <table.FlexRender header={header} />}</th>)}
            </tr>
          ))}
        </thead>
        <tbody>
          {table.getRowModel().rows.map((row) => (
            <tr key={row.id}>{row.getAllCells().map((cell) => <td key={cell.id} data-column={cell.column.id}><table.FlexRender cell={cell} /></td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
