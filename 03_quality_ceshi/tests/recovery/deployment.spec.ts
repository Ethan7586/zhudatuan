...
test('deployment topology has no retired runtime and keeps migration before runtime apply', () => {
  const script = readFileSync(resolve(root, '02_platform_pingtai/infrastructure/aliyun/deploy.sh'), 'utf8');
  const topology = readFileSync(resolve(root, '02_platform_pingtai/infrastructure/aliyun/runtime.template.yml'), 'utf8');
