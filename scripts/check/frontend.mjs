...
const designTokens = JSON.parse(readFileSync(join(root, 'packages/design/src/tokens.json'), 'utf8'));
...
for (const application of ['auth', 'console', 'store', 'supplier', 'storefront']) {
  ...
  if (!/<Brand\b/.test(source)) findings.push(`CANONICAL_BRAND_NOT_CONSUMED:apps/${application}`);
}
const miniappBrand = readFileSync(join(clientRoot, 'miniapp/miniprogram/page/home/index.wxml'), 'utf8');
if (!miniappBrand.includes('/assets/brandmark.svg')) findings.push('CANONICAL_BRAND_NOT_CONSUMED:apps/miniapp');
...
if (miniappManifest.window?.navigationBarBackgroundColor !== designTokens.color.brand.primary
  ...
  || miniappManifest.tabBar?.backgroundColor !== designTokens.color.surface.base) findings.push('MINIAPP_MANIFEST_TOKEN_DRIFT:apps/miniapp/miniprogram/app.json');
if (miniappManifest.tabBar?.custom !== true || JSON.stringify(miniappManifest.tabBar?.list?.map((item) => item.text)) !== JSON.stringify(['首页', '分类', '翼码', '订单', '我的'])) {
  findings.push('MINIAPP_FROZEN_NAVIGATION_DRIFT:apps/miniapp/miniprogram/app.json');
}
