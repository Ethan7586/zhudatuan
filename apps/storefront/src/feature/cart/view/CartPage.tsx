              ...
              <div className="divide-y">
                {cart.map((item) => (
                  <article key={item.id} className="flex flex-wrap items-center gap-3 p-4">
                    <button type="button" onClick={() => viewmodel.toggleCartItemSelected(item.id)} aria-label={item.selected ? '取消选择商品' : '选择商品'} className="grid h-11 w-11 place-items-center">
                      {item.selected ? <CheckSquare className="text-brand" /> : <Square className="text-muted" />}
                    </button>
                    <ProductMedia source={item.product.images[0]} alt={item.product.title} className="h-20 w-20 rounded-xl object-cover" emptyClassName="grid h-20 w-20 place-items-center rounded-xl bg-subtle text-[10px] text-muted" />
                    <div className="min-w-[160px] flex-1">
                      ...
                      <span className="mt-2 flex items-center gap-1 text-[10px] text-success-strong">
                        <ShieldCheck size={12} />
                        {item.product.purchasable ? '库存与资格有效' : '商品状态已变化，请移除'}
                      </span>
                      <b className="mt-2 block text-danger">¥{formatMinor(item.product.priceWelfareMinor)}</b>
                    </div>
                    <div className="flex overflow-hidden rounded-xl border">
                      <button type="button" onClick={() => viewmodel.updateCartQuantity(item.id, item.quantity - 1)} aria-label={`减少${item.product.title}数量`} className="grid h-11 w-11 place-items-center">
                        <Minus size={14} />
                      </button>
                      <span className="grid min-w-11 place-items-center border-x font-bold">{item.quantity}</span>
                      <button type="button" onClick={() => viewmodel.updateCartQuantity(item.id, item.quantity + 1)} aria-label={`增加${item.product.title}数量`} className="grid h-11 w-11 place-items-center">
                        <Plus size={14} />
                      </button>
                    </div>
