  ...
  const actions: OperationActions = {
      ...
      'identity.challenges.create': operationLifecycle({
        prepare: async (request) => {
          ...
          return { body, id, code, purpose, destinationHash, inviteHash, legacyMobileToken, device, peer, envelope, recipient, mobileLookup };
        },
        execute: async (request, database, prepared) => {
          const { body, id, code, purpose, destinationHash, inviteHash, legacyMobileToken, device, peer, envelope, recipient, mobileLookup } = prepared;
      ...
      'identity.invitations.create': async (request, database) => {
        ...
        if (targetClient === 'operator' && (access.scope.kind !== 'tenant' || access.scope.id !== access.scope.tenant)) throw new Error('INVITATION_SCOPE_INVALID');
      ...
      'identity.mobile.challenge': operationLifecycle({
        ...
        execute: async (request, database, prepared) => {
          ...
          await consumeChallengeRate(database, [[destinationHash, 'phone_change'], [peer, 'network:phone_change'], [device, 'device:phone_change']]);
          const result = await database.query(
            `with challenge as (
