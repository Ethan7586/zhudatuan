...
function BootStatus() {
  return (
    <main className="authboot" role="status" aria-live="polite">
      <span className="authbootmark" aria-hidden="true">
        翼
      </span>
      <span>正在初始化安全登录…</span>
    </main>
...
class ChunkBoundary extends Component<Readonly<{ children: ReactNode }>, Readonly<{ failed: boolean }>> {
  ...
  override render() {
    ...
    return (
      <main className="authboot" role="alert">
        <span className="authbootmark" aria-hidden="true">
          翼
        </span>
        <strong>安全登录暂时未能载入</strong>
        <Button tone="primary" onPress={() => window.location.reload()}>
          重新载入
        </Button>
      </main>
    );
