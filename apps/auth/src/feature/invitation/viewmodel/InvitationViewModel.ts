...
export class InvitationViewModel {
  constructor(
    private readonly resolver: ResolveInvitation,
    private readonly enrollment: ReadEnrollment,
    private readonly completion: CompleteEnrollment
  ) {}
  resolve(input: Parameters<ResolveInvitation['execute']>[0]) {
    return this.resolver.execute(input);
  }
  read(id: string, signal?: AbortSignal) {
    return this.enrollment.execute(id, signal);
  }
  complete(input: Parameters<CompleteEnrollment['execute']>[0], signal?: AbortSignal) {
    return this.completion.execute(input, signal);
  }
}
