...
export class OrderProjectionPort extends OrderAfterSalePort {
  async recordPayment(
    ...
    await database.query(
      `insert into ordering.paymentread(order_id,payment_id,currency,captured_minor,refunded_minor,state,version,updated_at)
      values($1,$2,$3,$4,$5,$6,$7,clock_timestamp()) on conflict(order_id) do update set
      payment_id=excluded.payment_id,currency=excluded.currency,captured_minor=excluded.captured_minor,
      refunded_minor=excluded.refunded_minor,state=excluded.state,version=excluded.version,updated_at=excluded.updated_at`,
  ...
  async recordFulfillments(context: WriteTransactionContext, order: string, items: readonly OrderFulfillmentProjection[]): Promise<void> {
    ...
    await this.transactions.database(context).query(
      `insert into ordering.fulfillmentread(id,order_id,provider,partner_id,kind,state,external_reference,version,created_at,updated_at)
