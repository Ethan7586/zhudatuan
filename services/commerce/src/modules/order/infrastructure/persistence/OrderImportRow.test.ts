...
const source = {
  source: 'jdproduct', externalOrderNo: 'JD-20260905-1', mall: 'mall:one', member: 'member:one',
  orderedAt: '2026-09-04T08:00:00.000Z', currency: 'CNY', state: 'paid', totalMinor: '1900',
  paymentReference: 'wx:one', sku: 'sku:one', listing: 'listing:one', title: '中秋福利礼盒', quantity: '2',
  unitMinor: '1000', discountMinor: '100', productType: 'physical', provider: 'jdproduct',
};
...
describe('order import row', () => {
  it('creates an immutable monetary and product snapshot', () => {
    expect(orderImportValue(source, Date.parse('2026-09-05T00:00:00.000Z'))).toMatchObject({
      source: 'jdproduct', externalOrderNo: 'JD-20260905-1', totalMinor: 1900,
  ...
  it('supports a bounded multi-line snapshot', () => {
    const value = orderImportValue({ ...source, totalMinor: '2500', lines: JSON.stringify([
  ...
  it('keeps only a validated masked address snapshot and derives its integrity hash on the server', () => {
    ...
    expect(() => orderImportValue({ ...source, address: JSON.stringify({ recipientMasked: '王明', mobileMasked: '13812345678', addressMasked: '上海市', regionCode: '310000' }) }, Date.parse('2026-09-05T00:00:00.000Z'))).toThrow('ORDER_SNAPSHOT_INVALID');
  });
