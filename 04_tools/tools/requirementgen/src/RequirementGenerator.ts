...
for (const sheet of SHEETS) {
  ...
  for (let ordinal = 0; ordinal < sheet.rows.length; ordinal += 1) {
    ...
    requirements.push({
      ...
      uiRoute,
      tests: ['01_core_hexin/services/commerce/src/modules/ModuleCatalog.test.ts', '01_core_hexin/services/commerce/src/modules/DomainPolicy.test.ts',
        '03_quality_ceshi/tests/integration/registry.spec.ts', journeyTest],
...
const mvpRequirements = MVP_LABELS.map((label, index) => {
  ...
  return {
    ...
    moduleSources: [...new Set(operations.map(({ owner }) => '01_core_hexin/services/commerce/src/modules/' + owner))].sort(),
    unitTests: ['01_core_hexin/services/commerce/src/modules/ModuleCatalog.test.ts', '01_core_hexin/services/commerce/src/modules/DomainPolicy.test.ts'],
    contractTest: row === 23 ? '03_quality_ceshi/tests/contracts/providers.spec.ts' : '03_quality_ceshi/tests/integration/registry.spec.ts',
    journeyTest: '03_quality_ceshi/tests/journeys/mvp' + String(row).padStart(2, '0') + '_' + MVP_JOURNEYS[index] + '.spec.ts',
    dashboard: '05_docs_ziliao/docs_wendang/metrics/catalog.md',
    runbook: '05_docs_ziliao/docs_wendang/operations/' + MVP_RUNBOOKS[index] + '.md',
...
const providers = PROVIDERS.map((provider, index) => ({
  ...
  status: 'Designed',
  evidence: [],
  extension: provider.priority === 1 ? '01_core_hexin/extensions/providers/' + provider.id : null,
...
const outputs = new Map<string, string>([
  [resolve(root, '05_docs_ziliao/docs_wendang/requirements/mapping.json'), JSON.stringify({
  ...
  [resolve(root, '05_docs_ziliao/docs_wendang/requirements/requirements.yml'), stringify({
    source: authority.logicalSource,
    workbookSha256: workbookHash,
    generated: true,
  ...
  [resolve(root, '05_docs_ziliao/docs_wendang/requirements/mvp.yml'), stringify({
  ...
  [resolve(root, '05_docs_ziliao/docs_wendang/requirements/providers.yml'), stringify({
  ...
  [resolve(root, '05_docs_ziliao/docs_wendang/requirements/frontend.yml'), stringify({
    source: authority.logicalSource,
    workbookSha256: workbookHash,
    generated: true,
    count: authority.sheets.requirements,
  ...
  [resolve(root, '01_core_hexin/packages/contract/src/RequirementCatalog.generated.ts'), contractType(
