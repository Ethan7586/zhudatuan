...
export class PgInventoryImport implements BatchImportPort {
  constructor(private readonly pool: DatabasePool, private readonly catalog: CatalogSku) {}

  async find(id: string): Promise<ImportTarget | null> {
    const result = await this.pool.query<TargetRow>('select id,scope_id,object_ref,sha256,state from inventory.importjob where id=$1', [id]);
  ...
  async stage(target: ImportTarget, rows: readonly Readonly<Record<string, string>>[]): Promise<void> {
    await workerTransaction(this.pool, target.scope, async (client) => {
      await client.query("update inventory.importjob set state='validating',last_error=null,updated_at=clock_timestamp() where id=$1", [target.id]);
      await client.query('delete from inventory.importrow where job_id=$1', [target.id]);
      await client.query('delete from inventory.importerror where job_id=$1', [target.id]);
    });
    for (let offset = 0; offset < rows.length; offset += 500) {
      const batch = rows.slice(offset, offset + 500).map((payload, index) => ({ row_number: offset + index + 2, payload }));
      await workerTransaction(this.pool, target.scope, async (client) => {
        await client.query(`insert into inventory.importrow(job_id,scope_id,row_number,payload)
    ...
    await workerTransaction(this.pool, target.scope, async (client) => {
      await client.query(`update inventory.importjob set state='ready',total_count=$2,cursor_value=0,success_count=0,failure_count=0,
  ...
  async process(target: ImportTarget, signal: AbortSignal): Promise<boolean> {
    ...
    try {
      ...
      const job = await client.query<{ cursor_value: number; total_count: number }>(`select cursor_value,total_count from inventory.importjob
        where id=$1 and state in('ready','running') for update`, [target.id]);
      if (!job.rows[0]) { await client.query('commit'); return true; }
      const staged = await client.query<StagedRow>(`select row_number,payload from inventory.importrow where job_id=$1 and row_number>$2
      ...
      for (const row of staged.rows) {
        ...
        try { await importStock(client, this.catalog, target.scope, target.id, row.row_number, row.payload); successes += 1; }
        catch (cause) {
          await client.query('rollback to savepoint importrow'); failures += 1;
          await client.query(`insert into inventory.importerror(job_id,scope_id,row_number,reason_code,field,detail) values($1,$2,$3,$4,null,$5)
      ...
      await client.query(`update inventory.importjob set state=$2,cursor_value=$3,success_count=success_count+$4,failure_count=failure_count+$5,
  ...
  async failures(target: ImportTarget): Promise<readonly ImportFailure[]> {
    const result = await this.pool.query<{ row_number: number; reason_code: string; field: string | null; detail: string }>(
      'select row_number,reason_code,field,detail from inventory.importerror where job_id=$1 order by row_number,reason_code', [target.id]);
  ...
  async complete(target: ImportTarget, report: StoredObject): Promise<void> {
    await this.pool.query(`update inventory.importjob set state='completed',report_object_ref=$2,report_sha256=$3,report_size=$4,
  ...
  async reject(target: ImportTarget, code: string, detail: string): Promise<void> {
    await this.pool.query(`update inventory.importjob set state='failed',last_error=$2,
  ...
  async fault(target: ImportTarget, detail: string): Promise<void> {
    await this.pool.query('update inventory.importjob set last_error=$2,updated_at=clock_timestamp() where id=$1', [target.id, detail]);
  }
}
...
async function continuation(client: PoolClient, target: ImportTarget, cursor: number): Promise<void> {
  await client.query(`insert into runtime.job(id,kind,owner,scope_id,payload,state,priority,available_at,created_at,updated_at)
    values($1,'inventoryimport','inventory',$2,jsonb_build_object('import',$3),'queued',100,clock_timestamp(),clock_timestamp(),clock_timestamp()) on conflict(id) do nothing`,
