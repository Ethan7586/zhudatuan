...
describe('MembershipSelector', () => {
  it('binds a password selection to browser, device and PKCE and consumes it once', async () => {
    ...
    const started = await selector.begin(
      {} as never,
      {
        principal: 'principal-one',
        target: 'storefront',
    ...
    const completed = await selector.select({} as never, 'selection-id', 'membership-two', { peer: '127.0.0.1', agent: 'browser', device: 'device-one', trace: 'trace-one' });
    expect(completed.destination).toBe('https://zhudatuan.com/');
    expect(issue).toHaveBeenCalledWith({}, expect.objectContaining({ membership: 'membership-two', assurance: 1 }));
  });
});
