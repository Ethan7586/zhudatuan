...
function bindChallengesIssue(client: OperationExecutor): OperationMethod<"verification.challenges.issue"> { return bindOperation(client, defineOperation({ ...{"id":"verification.challenges.issue","method":"POST","path":"/api/v1/verifications/challenges","audience":"console","targets":["console","store"],"responseMode":"json","idempotencyPolicy":"required","idempotent":false,"timeout":800,"errorUnion":["AUTHENTICATION_REQUIRED","AUTHORIZATION_DENIED","CAPABILITY_DENIED","CONTENT_TYPE_UNSUPPORTED","CONTRACT_VERSION_UNSUPPORTED","CSRF_TOKEN_INVALID","DEADLINE_EXCEEDED","IDEMPOTENCY_CONFLICT","IDEMPOTENCY_KEY_REQUIRED","INTERNAL_ERROR","ORIGIN_REQUIRED","PERMISSION_DENIED","RATE_LIMITED","REQUEST_BODY_TOO_LARGE","REQUEST_JSON_INVALID","SCOPE_DENIED","STEPUP_REQUIRED","URL_SENSITIVE_DATA_FORBIDDEN","VALIDATION_FAILED","VOUCHER_NOT_REDEEMABLE"]}, input: exactOperationInput("VerificationChallengesIssueInput", [] as const, true), output: exactOperationOutput("VerificationChallengesIssueOutput") })); }
...
function bindChallengesVerify(client: OperationExecutor): OperationMethod<"verification.challenges.verify"> { return bindOperation(client, defineOperation({ ...{"id":"verification.challenges.verify","method":"POST","path":"/api/v1/verifications/challenges/{challengeid}/verification","audience":"console","targets":["console","store"],"responseMode":"json","idempotencyPolicy":"required","idempotent":false,"timeout":800,"errorUnion":["AUTHENTICATION_REQUIRED","AUTHORIZATION_DENIED","CAPABILITY_DENIED","CONTENT_TYPE_UNSUPPORTED","CONTRACT_VERSION_UNSUPPORTED","CSRF_TOKEN_INVALID","DEADLINE_EXCEEDED","IDEMPOTENCY_CONFLICT","IDEMPOTENCY_KEY_REQUIRED","INTERNAL_ERROR","ORIGIN_REQUIRED","PERMISSION_DENIED","RATE_LIMITED","REQUEST_BODY_TOO_LARGE","REQUEST_JSON_INVALID","SCOPE_DENIED","STEPUP_REQUIRED","URL_SENSITIVE_DATA_FORBIDDEN","VALIDATION_FAILED","VERIFICATION_DEVICE_DENIED","VERIFICATION_TOKEN_INVALID","VOUCHER_REDEMPTION_CONFLICT"]}, input: exactOperationInput("VerificationChallengesVerifyInput", ["challengeid"] as const, true), output: exactOperationOutput("VerificationChallengesVerifyOutput") })); }
