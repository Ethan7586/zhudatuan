...
export function ExperiencePage({ title, model }: Readonly<{ title: string; model: ApplicationViewModel }>) {
  ...
  return (
    <div className="commerceworkspace" data-mode={model.presentation.mode}>
      <ResourcePanel
        title={title}
        eyebrow={model.presentation.eyebrow}
        ...
        actions={
          <>
            ...
            <Button tone="primary" onPress={primary} isDisabled={primaryDisabled}>
              {model.presentation.primaryAction}
            </Button>
          </>
        }
      >
        <div className="commercecontent">
          <section className="commerceownership" role="note">
            ...
            <div>
              <strong>
                {model.presentation.ownership}：{model.scopeName}
              </strong>
              <p>{model.presentation.ownershipDetail}</p>
            </div>
          ...
          <section className="commerceboundarybanner" role="note">
            ...
            <div>
              <strong>{model.presentation.mode === 'management' ? '商城创建与初始草稿已启用' : '装修版本链已启用'}</strong>
              <p>{boundaryMessage(model.presentation.mode)}</p>
            </div>
          </section>
          <ol className="commerceflow" aria-label={`${model.presentation.title}业务闭环`}>
          ...
          <section className="commerceboard" aria-labelledby="commerceboardtitle">
            <nav className="commercetabs" aria-label="商城与应用状态视图">
              {model.views.map((candidate) => (
                <button key={candidate.key} type="button" aria-pressed={candidate.key === model.view} onClick={() => model.actions.selectView(candidate.key)}>
                  {candidate.label}
                </button>
              ))}
            </nav>
            <div className="commercefilterbar">
              ...
              <label className="commercesearch">
                ...
                <input type="search" value={model.search} onChange={(event) => model.actions.search(event.target.value)} placeholder="搜索名称、代码、商城或公开链接" />
              </label>
            </div>
            <p className="commercefiltermeta">
              当前页筛选 · 显示 {model.rows.length} / {model.data?.items.length ?? 0} 条 · 不推断未返回的商城总量
            </p>
            {model.data !== undefined && model.data.items.length === 0 ? (
              <section className="commerceempty" role="status">
                <strong>当前范围暂无商城应用</strong>
                <p>{model.presentation.mode === 'management' ? '点击创建商城，原子建立应用、初始装修草稿和商城商品池绑定。' : '当前商城暂无应用，请先从集团范围创建。'}</p>
              </section>
            ) : null}
            {model.data !== undefined && model.data.items.length > 0 && model.rows.length === 0 ? (
              <section className="commerceempty" role="status">
            ...
            {model.rows.length === 0 ? null : (
              <ExperienceTable
                rows={model.rows}
                mode={model.presentation.mode}
                permissions={model.permissions}
                onOpen={model.actions.openRecord}
                onManage={(record) => model.action.actions.open({ kind: 'manage', record })}
                onEntry={model.entry.actions.open}
                onCopy={(record) => model.action.actions.open({ kind: 'copy', record })}
                onDesign={(record) => model.action.actions.open({ kind: 'design', record })}
              />
            )}
            <footer className="commercepagination">
              <span>服务端返回 {model.data?.count ?? 0} 条 · 游标分页</span>
              <Button onPress={model.actions.nextPage} isDisabled={model.data?.nextCursor === undefined}>
                下一页
              </Button>
            </footer>
          </section>
      ...
      <ExperienceActionDialog application={model.action} creation={model.creation} version={model.version} />
    </div>
  );
}
